...
static unsigned _hardware_concurrency(
  void)
{
  int count;
...
static unsigned _threadpool_count_serial(
...
static unsigned _threadpool_count_parallel(
  struct threadpool *self)
{
  assert(_has_pthread);
  return self->count;
...
static void _add_next_thread(
  struct _parallel_startup_type *self)
{
...
static void *_thread_free_and_unlock(
  struct threadpool *self,
  void *thread)
{
...
static void *_thread_destroy_and_unlock(
  struct threadpool *self,
  void *thread)
{
  self->thread_destroy(thread);
...
static void _unlock_and_destroy(
  struct threadpool *self)
{
  pthread_t *threads;
...
static enum _io_thread_status _status_load(
...
static enum _io_thread_status _status_exchange(
  enum _io_thread_status *obj,
  enum _io_thread_status desired)
{
  enum _io_thread_status result;
  _lock();
  result= *obj;
...
void *io_thread_create(
  struct io_thread *self,
  void *parent,
