...
const char *_async_netdb_strerror(
  int errcode)
{
  ...
  if (_has_threads >= 0)
    {
      ...
      if (thread_malloc((void **) &self, dpy, sizeof(struct async_name_from_addr)))
        return NULL;

      _async_name_from_addr_set_param(&self->param, addr, addrlen);

      result= io_thread_create(&self->io, self, _async_name_from_addr_thread, dpy, ASYNC_NETDB_STACK);
      if (! result)
        thread_free(self);
      return result;
    }
...
#if ASYNC_NETDB_USE_GAI
void async_name_from_addr_cancel(
  async_name_from_addr_t self)
{
  if (_has_threads >= 0)
    {
...
int async_name_from_addr_finish(
  async_name_from_addr_t self_raw,
  char **host,
  int *errno_error)
{
#if ASYNC_NETDB_USE_GAI
    ...
    void *self_raw)
{
  ...
  self->gai_error= getaddrinfo(_async_addr_from_name_hostname(self), NULL, NULL, &self->res);
  self->errno_error= errno;
...
{
  async_addr_from_name_t self;
  if (thread_malloc((void **) &self, dpy, sizeof(struct async_addr_from_name) + strlen(hostname) + 1))
    return NULL;
  ...
  if (_has_threads >= 0)
    {
      async_addr_from_name_t result=
        io_thread_create(&self->io, self, _async_addr_from_name_thread, dpy, ASYNC_NETDB_STACK);
...
#if ASYNC_NETDB_USE_GAI
void async_addr_from_name_cancel(
  async_addr_from_name_t self)
{
  if (_has_threads >= 0)
    {
...
int async_addr_from_name_finish(
  async_addr_from_name_t self,
  void *addr,
  socklen_t *addrlen,
  int *errno_error)
