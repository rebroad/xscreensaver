    ...
    unsigned int height)
{
  ...
  if (! image)
  ...
  else
    {
       ...
       */
      int error= aligned_malloc((void **) &image->data, get_cache_line_size(), image->height * image->bytes_per_line);
      if (error)
        {
          print_error(error);
          XDestroyImage(image);
    ...
    unsigned int height)
{
  ...
  CATCH_X_ERROR(dpy);
  image= XShmCreateImage(dpy, visual, depth, format, NULL, shm_info, width, height);
  UNCATCH_X_ERROR(dpy);
  if (shm_got_x_error)
    return create_fallback(dpy, visual, depth, format, shm_info, width, height);
...
#ifdef DEBUG
      fprintf(stderr, "%s: shmat(%d, 0, 0) ==> %d\n", progname, shm_info->shmid, (int) image->data);
#endif
  ...
  if (shm_info->shmid != -1)
    {
       ...
       */
      return XShmPutImage(dpy, d, gc, image, src_x, src_y, dest_x, dest_y, width, height, False);
    }
#endif /* HAVE_XSHM_EXTENSION */
...
#endif /* HAVE_XSHM_EXTENSION */
  return XGetSubImage(dpy, d, x, y, image->width, image->height, plane_mask, image->format, image, 0, 0) != NULL;
}
  ...
  XShmSegmentInfo *shm_info)
{
  ...
  if (status != 0)
    {
      char buf [ 1024 ];
      sprintf(buf, "%s: shmdt(0x%lx) failed", progname, (unsigned long) shm_info->shmaddr);
      perror(buf);
    }
#ifdef DEBUG
  else
