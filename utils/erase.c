    ...
    eraser_state *st)
{
  ...
  for (i= st->nlines * st->prev_ratio;
    ...
    i++)
    if (st->horiz_p)
      XDrawLine(st->dpy, st->window, st->bg_gc, 0, st->lines [ i ], st->width, st->lines [ i ]);
    else
      XDrawLine(st->dpy, st->window, st->bg_gc, st->lines [ i ], 0, st->lines [ i ], st->height);
      ...
      for (i= 0; i < st->nlines / 4; i++)
        {
          ...
          st->lines [ i * 4 + 3 ]= st->height + st->width - i * 2 - (st->width % 2 ? 0 : 1);
        }
    }
    ...
    eraser_state *st)
{
  ...
  XFillArc(st->dpy, st->window, st->bg_gc, (st->width / 2) - rad, (st->height / 2) - rad, rad * 2, rad * 2, (st->start + oth) % max, th - oth);
}
    ...
    eraser_state *st)
{
  ...
  for (i= 0; i < 3; i++)
    {
      int off= i * max / 3;
      XFillArc(st->dpy, st->window, st->bg_gc, (st->width / 2) - rad, (st->height / 2) - rad, rad * 2, rad * 2, (st->start + off + oth) % max, th - oth);
      XFillArc(st->dpy, st->window, st->bg_gc, (st->width / 2) - rad, (st->height / 2) - rad, rad * 2, rad * 2, (st->start + off - oth) % max, oth - th);
    }
}
      ...
      for (x= 0; x < st->width; x+= 256)
        {
          ...
          XDrawPoints(st->dpy, st->window, st->bg_gc, points, npoints + need0, CoordModeOrigin);
          *rnd= r;
          rnd++;
        }
    }
    ...
    eraser_state *st)
{
  ...
  for (i= steps * st->prev_ratio;
    ...
    i++)
    {
      ...
      XFillPolygon(st->dpy, st->window, st->bg_gc, points, 3, Convex, CoordModeOrigin);
    }
}
    ...
    eraser_state *st)
{
  ...
  for (i= st->nlines * st->prev_ratio;
    ...
    i++)
    {
      ...
      XFillRectangle(st->dpy, st->window, st->bg_gc, st->width * x / st->cols, st->height * y / rows, size + 1, size + 1);
    }
  ...
  for (y= 0; y < st->height; y+= h)
    {
      if (st->width <= step)
        ...
        {
          XCopyArea(st->dpy, st->window, st->window, st->fg_gc, 0, y, st->width - step, h, step, y);
          XFillRectangle(st->dpy, st->window, st->bg_gc, 0, y, step, h);
        }
      else
        {
          XCopyArea(st->dpy, st->window, st->window, st->fg_gc, step, y, st->width - step, h, 0, y);
          XFillRectangle(st->dpy, st->window, st->bg_gc, st->width - step, y, step, h);
        }
  ...
  if (st->ratio < mode1) /* squeeze from the sides */
    {
      ...
      /* pull from left */
      XCopyArea(st->dpy, st->window, st->window, st->fg_gc, 0, 0, max - step, st->height, step, 0);
      XFillRectangle(st->dpy, st->window, st->bg_gc, 0, 0, max * ratio, st->height);

      /* pull from right */
      XCopyArea(st->dpy, st->window, st->window, st->fg_gc, max + step, 0, max - step, st->height, max, 0);
      XFillRectangle(st->dpy, st->window, st->bg_gc, max + max * (1 - ratio), 0, max, st->height);

      /* expand white from center */
      XFillRectangle(st->dpy, st->window, st->fg_gc, max - (radius * ratio), 0, radius * ratio * 2, st->height);
    }
  else if (st->ratio < mode2) /* squeeze from the top/bottom */
    {
      ...
      /* fill middle */
      XFillRectangle(st->dpy, st->window, st->fg_gc, st->width / 2 - radius, max * ratio, radius * 2, st->height * (1 - ratio));

      /* fill left and right */
      XFillRectangle(st->dpy, st->window, st->bg_gc, 0, 0, st->width / 2 - radius, st->height);
      XFillRectangle(st->dpy, st->window, st->bg_gc, st->width / 2 + radius, 0, st->width / 2, st->height);

      /* fill top and bottom */
      XFillRectangle(st->dpy, st->window, st->bg_gc, 0, 0, st->width, max * ratio);
      XFillRectangle(st->dpy, st->window, st->bg_gc, 0, st->height - (max * ratio), st->width, max);

      /* cap top */
      XFillArc(st->dpy, st->window, st->fg_gc, st->width / 2 - radius, max * ratio - radius, radius * 2, radius * 2, 0, 180 * 64);

      /* cap bottom */
      XFillArc(st->dpy, st->window, st->fg_gc, st->width / 2 - radius, st->height - (max * ratio + radius), radius * 2, radius * 2, 180 * 64, 180 * 64);
    }
  else if (st->ratio < mode3) /* starburst */
    {
      ...
      XFillRectangle(st->dpy, st->window, st->bg_gc, 0, 0, st->width, st->height);
