          ...
          if (in [ 1 ] == '#' && in [ 2 ] == 'x')
            { // &#x41;
              ...
              while ((*in >= '0' && *in <= '9') ||
                ...
                (*in >= 'a' && *in <= 'f'))
                {
                  i= (i * 16) + (*in >= 'a' ? *in - 'a' + 16 : *in >= 'A' ? *in - 'A' + 16 :
                                                                            *in - '0');
                  in++;
                }
    ...
    const char *d)
{
  ...
  if (al > bl && al > cl && al > dl)
    ret= a2;
  else if (bl > al && bl > cl && bl > dl)
    ret= b2;
  else if (cl > al && cl > bl && cl > dl)
    ret= c2;
  else
    ret= d2;
