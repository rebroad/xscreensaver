    ...
    Bool verbose_p)
{
  ...
  pixmap= XCreatePixmap(dpy, root_window, win_xgwa.width, win_xgwa.height, win_xgwa.depth);
  ...
  if (external_p)
  ...
  else
    {
      ...
      XCopyArea(dpy, root_window, pixmap, gc, win.x, win.y, win.x2 - win.x, win.y2 - win.y, 0, 0);
...

  {
    ...
    if (verbose_p || ! pixmap || *late)
      fprintf(stderr,
        "%s: %s screenshot 0x%lx %dx%d" " for window 0x%lx %dx%d+%d+%d%s\n",
        blurb(),
...
{
  screenshot_init(dpy);
  XChangeProperty(dpy, window, XA_SCREENSAVER_SCREENSHOT, XA_PIXMAP, 32, PropModeReplace, (unsigned char *) &pixmap, 1);
}


/* Returns a screenshot pixmap from a property on the window. */
static Pixmap
  screenshot_load_prop(
    Display *dpy,
    Window window)
{
  Atom type;
  ...
  screenshot_init(dpy);
  status= XGetWindowProperty(dpy, window, XA_SCREENSAVER_SCREENSHOT, 0, 2, False, XA_PIXMAP, &type, &format, &nitems, &bytesafter, &ret);
  if (status == Success &&
    type == XA_PIXMAP &&
    nitems == 1 &&
    bytesafter == 0)
    ...
    Bool verbose_p)
{
  ...
  p2= XCreatePixmap(dpy, window, win_xgwa.width, win_xgwa.height, win_xgwa.depth);
  ...
  XCopyArea(dpy, screenshot, p2, gc, from.x, from.y, from.w, from.h, to.x, to.y);
  XFreeGC(dpy, gc);

  if (verbose_p)
    fprintf(stderr,
      "%s: loaded screenshot 0x%lx"
      " for window 0x%lx %dx%d+%d+%d" " from saved 0x%lx %dx%d\n",
      blurb(),
      (unsigned long) p2,
      (unsigned long) window,
      win_xgwa.width,
