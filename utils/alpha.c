...
{
  while (nplanes > 1 &&
    ! XAllocColorCells(dpy, cmap, False, plane_masks, nplanes, base_pixel_ret, 1))
    nplanes--;
...
static void
  initialize_transparency_colormap(
    Display *dpy,
    Colormap cmap,
    int nplanes,
  ...
  if (! cmap) /* A TrueColor visual, or similar. */
    {
      ...
      vi_out= XGetVisualInfo(dpy, VisualScreenMask | VisualIDMask, &vi_in, &i);
      if (! vi_out) abort();
  ...
  else /* A PseudoColor visual, or similar. */
    {
      ...
      *plane_masks= (unsigned long *) malloc(sizeof(unsigned long) * nplanes);
      ...
      nplanes= allocate_color_planes(dpy, cmap, nplanes, *plane_masks, base_pixelP);
      *nplanesP= nplanes;
      ...
      initialize_transparency_colormap(dpy, cmap, nplanes, *base_pixelP, *plane_masks, colors, additive_p);
