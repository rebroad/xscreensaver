    ...
    Bool verbose_p)
{
  ...
  else if (! strcmp(v, "monochrome"))
    {
      vclass= MONO_VISUAL;
    }
  else if (! strcmp(v, "gray"))
    {
      vclass= GRAY_VISUAL;
    }
  else if (! strcmp(v, "grey"))
    {
      vclass= GRAY_VISUAL;
    }
  else if (! strcmp(v, "color"))
    {
      vclass= COLOR_VISUAL;
    }
  else if (! strcmp(v, "gl"))
  ...
  else if (! strcmp(v, "staticgray"))
    {
      vclass= StaticGray;
    }
  else if (! strcmp(v, "staticcolor"))
    {
      vclass= StaticColor;
    }
  else if (! strcmp(v, "truecolor"))
    {
      vclass= TrueColor;
    }
  else if (! strcmp(v, "grayscale"))
    {
      vclass= GrayScale;
    }
  else if (! strcmp(v, "greyscale"))
    {
      vclass= GrayScale;
    }
  else if (! strcmp(v, "pseudocolor"))
    {
      vclass= PseudoColor;
    }
  else if (! strcmp(v, "directcolor"))
    {
      vclass= DirectColor;
    }
  else if (1 == sscanf(v, " %lu %c", &id, &c))
    {
      vclass= SPECIFIC_VISUAL;
    }
  else if (1 == sscanf(v, " 0x%lx %c", &id, &c))
    {
      vclass= SPECIFIC_VISUAL;
    }
  else
    ...
    Screen *screen)
{
  ...
  vi_out= XGetVisualInfo(dpy, (VisualDepthMask | VisualScreenMask), &vi_in, &out_count);
  if (vi_out)
    {
    ...
    Screen *screen)
{
  ...
  vi_out= XGetVisualInfo(dpy, (VisualClassMask | VisualScreenMask | VisualDepthMask), &vi_in, &out_count);
  if (out_count > 0)
    ...
    int id)
{
  ...
  vi_in.screen= screen_number(screen);
  vi_in.visualid= id;
  return XGetVisualInfo(dpy, VisualScreenMask | VisualIDMask, &vi_in, &out_count);
}
  ...
  Bool private_cmap_p)
{
  ...
  fprintf(f, "0x%02x (%s depth: %2d, cmap: %s)\n", (unsigned int) vi_out->visualid, (vi_out->class == StaticGray ? "StaticGray, " : vi_out->class == StaticColor ? "StaticColor," :
    ...
    Visual *old_visual)
{
   ...
   */
  vi_out= XGetVisualInfo(dpy, (VisualScreenMask | VisualClassMask | VisualDepthMask), &vi_in, &out_count);
  ...
  if (! result)
    {
      vi_out= XGetVisualInfo(dpy, (VisualScreenMask | VisualClassMask), &vi_in, &out_count);
      if (vi_out && out_count > 0)
        result= vi_out [ 0 ].visual;
      if (vi_out) XFree(vi_out);
      vi_out= 0;
