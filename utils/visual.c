...
Visual * get_visual (Screen *screen, const char *string, Bool prefer_writable_cells,
	    Bool verbose_p) {
  ...
  if (vclass == DEFAULT_VISUAL) result = DefaultVisualOfScreen (screen);
    ...
    if (class != TrueColor && class != PseudoColor &&
          class != DirectColor && class != StaticColor)
      result = 0;
    if (result && visual_depth(screen, result) <= 1) result = 0;

    /* Else, find the best non-default color visual */
    if (!result) result = pick_best_visual (screen, prefer_writable_cells, True);

    if (!result && verbose_p) fprintf (stderr, "%s: no color visuals.\n", progname);
    ...
    if (visual) result = visual;
    else if (verbose_p) fprintf (stderr, "%s: no visual of class %s.\n", progname, v);
  }
...
Visual * get_visual_resource (Screen *screen, char *name, char *class,
  ...
  if (v) return v;
  else return DefaultVisualOfScreen (screen);
}
...
#undef TRY_CLASS
  ...
  if (!color_only || visual_depth(screen, visual) > 1) return visual;
  else return 0;
}

static Visual * pick_mono_visual (Screen *screen) {
...
static Visual * pick_best_gl_visual (Screen *screen) {
  ...
  vi_out = XGetVisualInfo (dpy, (VisualClassMask | VisualScreenMask |
                                 VisualDepthMask),
			   &vi_in, &out_count);
  if (out_count > 0) result = vi_out[0].visual;

  if (vi_out) XFree ((char *) vi_out);
...
static XVisualInfo * visual_info_id (Screen *screen, int id) {
  Display *dpy = DisplayOfScreen (screen);
  XVisualInfo vi_in;
  int out_count;
  vi_in.screen = screen_number (screen);
...
static XVisualInfo * visual_info (Screen *screen, Visual *visual) {
  XVisualInfo *vi_out = visual_info_id (screen, XVisualIDFromVisual (visual));
  if (! vi_out) abort ();
  return vi_out;
}

Visual * id_to_visual (Screen *screen, int id) {
...
int visual_pixmap_depth (Screen *screen, Visual *visual) {
  ...
  for (i = 0; i < pfvc; i++) if (pfv[i].depth == vdepth) {
...
describe_visual (FILE *f, Screen *screen, Visual *visual, Bool private_cmap_p) {
  char n[10];
  XVisualInfo *vi_out = visual_info (screen, visual);
  if (private_cmap_p) sprintf(n, "%3d", vi_out->colormap_size);
  else strcpy(n, "default");
...
int screen_number (Screen *screen) {
  ...
  for (i = 0; i < ScreenCount (dpy); i++)
    if (ScreenOfDisplay (dpy, i) == screen) return i;
  abort ();
  return 0;
}
...
Visual * find_similar_visual(Screen *screen, Visual *old_visual) {
  ...
  vi_out = XGetVisualInfo (dpy, (VisualScreenMask | VisualClassMask |
				 VisualDepthMask), &vi_in, &out_count);
  if (vi_out && out_count > 0) result = vi_out[0].visual;
  ...
  if (!result) result = DefaultVisualOfScreen (screen);
