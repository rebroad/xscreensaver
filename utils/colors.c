    ...
    Bool got_writable)
{
  ...
  if (wanted_writable && ! got_writable)
    ...
    fprintf(stderr, "%s: wanted %d%s colors; got %d.\n", progname, wanted_colors, (got_writable ? " writable" : ""), got_colors);
}
...
void make_color_ramp(
  ...
  int h1,
  double s1,
  double v1, /* 0-360, 0-1.0, 0-1.0 */
  int h2,
  double s2,
  double v2, /* 0-360, 0-1.0, 0-1.0 */
  XColor *colors,
  int *ncolorsP,
  Bool closed_p,
  Bool allocate_p,
  ...
  for (i= 0; i < ncolors; i++)
    {
      ...
      hsv_to_rgb((int) (h1 + (i * dh)), (s1 + (i * ds)), (v1 + (i * dv)), &colors [ i ].red, &colors [ i ].green, &colors [ i ].blue);
    }
...
WARN:
  ...
  if (verbose_p &&
    ...
    complain(wanted, ncolors, wanted_writable, (wanted_writable && writable_pP && *writable_pP));
}
    ...
    Bool *writable_pP)
{
  ...
  if (npoints == 0)
    ...
    {
      make_color_ramp(screen, visual, cmap, h [ 0 ], s [ 0 ], v [ 0 ], h [ 1 ], s [ 1 ], v [ 1 ], colors, ncolorsP, True, /* closed_p */
        allocate_p,
        writable_pP);
      return;
    }
      ...
      for (j= 0; j < ncolors [ i ]; j++, k++)
        {
          ...
          hsv_to_rgb((int)
            ...
            &colors [ k ].blue);
#ifdef DEBUG
          fprintf(stderr, "point %d+%d: %.2f %.2f %.2f  %04X %04X %04X\n", i, j, hh, (s [ i ] + (j * ds [ i ])), (v [ i ] + (j * dv [ i ])), colors [ k ].red, colors [ k ].green, colors [ k ].blue);
#endif /* DEBUG */
        }
    }
...
RETRY_NON_WRITABLE:
  make_color_ramp(screen, visual, cmap, 0, S, V, 359, S, V, colors, &ncolors, False, allocate_p, writable_pP);
      ...
      if (bright_p)
        {
          ...
          double V= ((double) (random() % 34) + 66) / 100.0; /* range 66%-100% */
          hsv_to_rgb(H, S, V, &colors [ i ].red, &colors [ i ].green, &colors [ i ].blue);
        }
      else
        {
