  ...
  if (! egl_display)
    {
       ...
       */
      egl_display= eglGetPlatformDisplay(EGL_PLATFORM_X11_KHR, (void *) dpy, NULL);
    }
    ...
    Screen *screen)
{
  ...
  if (! eglGetConfigAttrib(egl_display, egl_config, EGL_NATIVE_VISUAL_ID, &vid))
    {
...
#define GLX_TRANSPARENT_ALPHA_VALUE_EXT GLX_TRANSPARENT_ALPHA_VALUE
    ...
    if (! glXGetConfig(dpy, vi_out, GLX_VISUAL_CAVEAT_EXT, &value) &&
      ...
      fprintf(f, "    GLX rating:        %s\n", (value == GLX_NONE_EXT ? "none" : value == GLX_SLOW_VISUAL_EXT ? "slow" :
    ...
    if (! glXGetConfig(dpy, vi_out, GLX_TRANSPARENT_TYPE_EXT, &value) &&
      value != GLX_NONE_EXT)
      {
        if (value == GLX_NONE_EXT)
          ...
          {
            if (! glXGetConfig(dpy, vi_out, GLX_TRANSPARENT_INDEX_VALUE_EXT, &value))
...
Bool validate_gl_visual(
  FILE *out,
  Screen *screen,
  const char *window_desc,
  Visual *visual)
{
#ifdef HAVE_GL
#ifndef HAVE_EGL
 ...
 */
#ifdef HAVE_EGL
egl_data *openGL_context_for_window(
  Screen *screen,
  Window window)
#else
GLXContext openGL_context_for_window(
  Screen *screen,
  Window window)
#endif
{
...
#ifdef HAVE_EGL
  {
    ...
    const EGLint ctxattr1 []= {
      ...
      1, /* Request an OpenGL ES 1.1 context. */
      EGL_CONTEXT_MINOR_VERSION,
      1,
#else
      EGL_CONTEXT_MAJOR_VERSION,
      1, /* Request an OpenGL 1.3 context. */
      EGL_CONTEXT_MINOR_VERSION,
      3,
#endif
      EGL_NONE};
    ...
    const EGLint ctxattr3 []= {
      EGL_CONTEXT_MAJOR_VERSION,
      3, /* Request an OpenGL ES 3.0 context. */
      EGL_CONTEXT_MINOR_VERSION,
      0,
      EGL_NONE};
    ...
    if (! d->egl_surface)
      {
        fprintf(stderr,
          "%s: eglCreatePlatformWindowSurface failed:" " window 0x%lx visual 0x%x\n",
          progname,
          window,
          vid);
        abort();
    ...
    d->egl_context= eglCreateContext(d->egl_display, d->egl_config, EGL_NO_CONTEXT, ctxattr);
    ...
    if (! d->egl_context && ctxattr != ctxattr1)
      {
        ...
        d->egl_context= eglCreateContext(d->egl_display, d->egl_config, EGL_NO_CONTEXT, ctxattr1);
      }
#endif /* HAVE_GLES3 */
            ...
            if (nconfs)
              {
                ...
                ac= 0;
                av [ ac++ ]= GLX_CONTEXT_MAJOR_VERSION_ARB;
                av [ ac++ ]= 3;
                av [ ac++ ]= GLX_CONTEXT_MINOR_VERSION_ARB;
                av [ ac++ ]= 2;
