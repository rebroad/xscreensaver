    ...
    struct overlay_data **data_ret)
{
  ...
  *data_ret= 0;
  result= XGetWindowProperty(dpy, window, XA_SERVER_OVERLAY_VISUALS, 0, (65536 / sizeof(long)), False, XA_SERVER_OVERLAY_VISUALS, &actual_type, &actual_format, &nitems, &bytes_after, &data);
  if (result != Success ||
      ...
      for (i= 0; i < n_visuals; i++)
        {
          ...
          if (data [ i ].transparency == 1 ||
            data [ i ].transparency == 2)
            {
              ...
              vi_out= XGetVisualInfo(DisplayOfScreen(screen), VisualIDMask, &vi_in, &out_count);
              if (vi_out)
                {
