...
open_browser() {
    ...
    echo -e "${BLUE}🌐 Opening browser (new window)...${NC}"
...
EMCC_ARGS=(
    ...
    -s EXPORTED_FUNCTIONS=['_main','_init_hextrail','_draw_hextrail','_reshape_hextrail_wrapper','_free_hextrail','_set_speed','_set_thickness','_set_spin','_set_wander','_stop_rendering','_start_rendering','_handle_mouse_down','_handle_mouse_drag','_handle_mouse_up','_handle_mouse_wheel','_handle_keypress','_set_debug_level','_track_memory_allocation','_track_memory_free']
...
if [ "$MATRIX_DEBUG" = true ]; then
    EMCC_ARGS=(-DMATRIX_DEBUG "$REPO_ROOT/web/matrix_debug.c" "${EMCC_ARGS[@]}")
fi
