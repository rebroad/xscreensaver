      ...
      for (y= fp->image->height - 1; y >= 0; y--)
        {
          ...
          if (fp->image->depth > 1)
            XSetForeground(display, MI_GC(mi), XGetPixel(fp->image, x, y));
            ...
            XSetForeground(display, MI_GC(mi), MI_PIXEL(mi, (y + x + fp->sidx + fp->startcolor) % MI_NPIXELS(mi)));

          if (fp->cache == MI_WINDOW(mi))
          ...
          if (fp->pointsize <= 1)
            ...
            XFillRectangle(display, fp->cache, MI_GC(mi), xp, yp, fp->pointsize, fp->pointsize);
          else
            XFillArc(display, fp->cache, MI_GC(mi), xp, yp, fp->pointsize, fp->pointsize, 0, 360 * 64);
        }
    }
}
  ...
  if (bitmap_name &&
    *bitmap_name &&
    ! ! strcmp(bitmap_name, "(default)"))
    {
      ...
      bitmap= file_to_pixmap(dpy, MI_WINDOW(mi), bitmap_name, &width, &height, 0);
      if (bitmap)
        {
          if (fp->image) XDestroyImage(fp->image);
          fp->image= XGetImage(dpy, bitmap, 0, 0, width, height, ~0L, ZPixmap);
...
#if defined(_AIX)
              sprintf(text, "%s\n%s %s.%s", uts.nodename, uts.sysname, uts.version, uts.release);
...
#undef GRAB
                ...
                if (pbv)
                  sprintf(text, "%s\n%s\n%s", uts.nodename, pn, puvv /*, uts.machine*/);
                else
                  sprintf(text, "%s\n%s %s", uts.nodename, uts.sysname, uts.release);
                if (pbv) free(pbv);
                if (pn) free(pn);
                if (puvv) free(puvv);
              }
      ...
      while ((line= strtok(token, "\r\n")))
        {
          ...
          token= 0;
          XTextExtents(font, line, strlen(line), &direction, &ascent, &descent, &o2);
      ...
      while ((line= strtok(token, "\r\n")))
        {
          ...
          xoff= ((overall.lbearing + overall.rbearing) -
                  (o2.lbearing + o2.rbearing)) /
            2;

          XDrawString(dpy, bitmap, gc, overall.lbearing + margin + xoff, ((font->ascent * (lines + 1)) + (font->descent * lines) + margin), line, strlen(line));
          lines++;
        }
      free(text2);
      XFreeFont(dpy, font);
  ...
  if (! fp->image)
    {
      XImage *im= image_data_to_ximage(dpy, MI_VISUAL(mi), bob_png, sizeof(bob_png));
      int x, y;

      fp->image= XCreateImage(dpy, MI_VISUAL(mi), 1, XYBitmap, 0, 0, im->width, im->height, 8, 0);
    ...
    ModeInfo *mi)
{
  ...
  if (fp->image) XDestroyImage(fp->image);
  fp->image=
    XCreateImage(MI_DISPLAY(mi), MI_VISUAL(mi), 1, XYBitmap, 0, /* dpth, fmt, offset */
    ...
    ModeInfo *mi)
{
  ...
  if (! fp->initialized)
    {
      ...
      else if (! (fp->cache= XCreatePixmap(display, MI_WINDOW(mi), MAXW(fp), MAXH(fp), MI_WIN_DEPTH(mi))))
#ifdef STANDALONE
        exit(-1);
#else  /* !STANDALONE */
  ...
  XSetForeground(display, MI_GC(mi), MI_WIN_BLACK_PIXEL(mi));
  XFillRectangle(display, fp->cache, MI_GC(mi), 0, 0, MAXW(fp), MAXH(fp));
    ...
    ModeInfo *mi)
{
  ...
  if (fp->cache == window)
  ...
  else if (fp->width <= MAXW(fp) || fp->height <= MAXH(fp))
    {
      ...
      XCopyArea(display, fp->cache, window, MI_GC(mi), 0, 0, MINW(fp), MINH(fp), fp->x_flag, fp->y_flag);
    }
  else
    {
      ...
      XCopyArea(display, fp->cache, window, MI_GC(mi), 0, 0, MAXW(fp), MAXH(fp), fp->x_flag, fp->y_flag);
    }
