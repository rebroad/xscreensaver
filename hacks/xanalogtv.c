    ...
    analogtv_input *input)
{
  ...
  */
  ...
  for (col= 0; col < 7; col++)
    {
      analogtv_draw_solid_rel_lcp(input, col * (1.0 / 7.0), (col + 1) * (1.0 / 7.0), 0.00, 0.68, top_cb_table [ col ][ 0 ], top_cb_table [ col ][ 1 ], top_cb_table [ col ][ 2 ]);

      analogtv_draw_solid_rel_lcp(input, col * (1.0 / 7.0), (col + 1) * (1.0 / 7.0), 0.68, 0.75, mid_cb_table [ col ][ 0 ], mid_cb_table [ col ][ 1 ], mid_cb_table [ col ][ 2 ]);
    }
  ...
  if (st->logo)
    {
      ...
      analogtv_load_ximage(st->tv, input, st->logo, st->logo_mask, (st->tv->xgwa.width - w2) / 2, st->tv->xgwa.height * 0.28, w2, h2);
    }
  ...
  void *closure)
{
  ...
  */
  ...
  /* Load this image into the next channel. */
  {
    ...
    XImage *image= XGetImage(st->dpy, pixmap, 0, 0, width, height, ~0L, ZPixmap);
...
static void load_station_images(
  struct state *st)
{
  int i;
  ...
  for (i= 0; i < MAX_STATIONS; i++)
    {
      ...
      if (! st->no_colorbars_p &&
        ...
        ((random() % 5) == 0))
        {
          ...
          j= random() % count;
          analogtv_setup_sync(input, 1, 0);
          analogtv_load_ximage(st->tv, input, st->test_patterns [ j ], 0, 0, 0, 0, 0);
          analogtv_setup_teletext(input);
        }
#endif
    ...
    Window window)
{
  ...
  /* if (!st->colorbars_only_p) */
  {
    int w, h;
    Pixmap mask= 0;
    Pixmap p= image_data_to_pixmap(dpy, window, logo_180_png, sizeof(logo_180_png), &w, &h, &mask);
...
#ifdef USE_TEST_PATTERNS
  {
    ...
    int w, h;
    Pixmap p;
    p= image_data_to_pixmap(dpy, window, testcard_rca_png, sizeof(testcard_rca_png), &w, &h, 0);
    st->test_patterns [ i++ ]= XGetImage(dpy, p, 0, 0, w, h, ~0L, ZPixmap);
    XFreePixmap(dpy, p);

    p= image_data_to_pixmap(dpy, window, testcard_pm5544_png, sizeof(testcard_pm5544_png), &w, &h, 0);
    st->test_patterns [ i++ ]= XGetImage(dpy, p, 0, 0, w, h, ~0L, ZPixmap);
    XFreePixmap(dpy, p);

    p= image_data_to_pixmap(dpy, window, testcard_bbcf_png, sizeof(testcard_bbcf_png), &w, &h, 0);
    st->test_patterns [ i++ ]= XGetImage(dpy, p, 0, 0, w, h, ~0L, ZPixmap);
    XFreePixmap(dpy, p);
  }
