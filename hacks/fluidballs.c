...
static void
  draw_fps_string(
    b_state *state)
{
  XFillRectangle(state->dpy, state->b, state->erase_gc, 0, state->xgwa.height - state->font_height * 3 - 20, state->xgwa.width, state->font_height * 3 + 20);
  XftDrawStringUtf8(state->xftdraw, &state->xft_fg, state->font, 10, state->xgwa.height - state->font_height * 2 - state->font_baseline - 10, (FcChar8 *) state->fps_str, strlen(state->fps_str));
}
...
static void
  window_origin(
    ...
    int *y)
{
  XTranslateCoordinates(dpy, window, RootWindow(dpy, DefaultScreen(dpy)), 0, 0, x, y, &window);
}
  ...
  if (state->dbuf && (state->ba))
      ...
      if (oxmax != state->xmax || oymax != state->ymax)
        {
          ...
          state->ba= XCreatePixmap(state->dpy, state->window, state->xgwa.width, state->xgwa.height, state->xgwa.depth);
          XFillRectangle(state->dpy, state->ba, state->erase_gc, 0, 0, state->xgwa.width, state->xgwa.height);
          state->b= state->ba;
        }
    }
  else
    {
      ...
      if (oxmin != state->xmin || oymin != state->ymin)
        ...
        XFillRectangle(state->dpy, state->b, state->erase_gc, 0, state->xgwa.height - state->font_height * 3, state->xgwa.width, state->font_height * 3);
    }
}
    ...
    int *y)
{
  ...
  unsigned int mask;
  if (XQueryPointer(state->dpy, state->window, &root1, &child1, &root_x, &root_y, &mouse_x, &mouse_y, &mask))
    {
      *x= root_x;
      *y= root_y;
    }
    ...
    Window window)
{
  ...
  state->draw_gc= XCreateGC(state->dpy, state->b, GCForeground | GCBackground, &gcv);

  gcv.foreground= get_pixel_resource(state->dpy, state->xgwa.colormap, "mouseForeground", "MouseForeground");
  state->draw_gc2= XCreateGC(state->dpy, state->b, GCForeground | GCBackground, &gcv);

  gcv.foreground= gcv.background;
  state->erase_gc= XCreateGC(state->dpy, state->b, GCForeground | GCBackground, &gcv);
   ...
   */
  {
    float r= (state->random_sizes_p ? state->max_radius * 0.7 : state->max_radius);
  ...
  state->shake_threshold= get_float_resource(dpy, "shakeThreshold", "ShakeThreshold");
  ...
  if (state->fps_p)
    {
      ...
      state->font=
        load_xft_font_retry(dpy, screen_number(state->xgwa.screen), fontname);
      if (! state->font) abort();
      s= get_string_resource(state->dpy, "textColor", "Foreground");
      if (! s) s= strdup("white");
      XftColorAllocName(state->dpy, state->xgwa.visual, state->xgwa.colormap, s, &state->xft_fg);
      free(s);
      state->xftdraw= XftDrawCreate(state->dpy, state->window, state->xgwa.visual, state->xgwa.colormap);
  ...
  for (i= 1; i <= state->count; i++)
    {
      ...
      state->r [ i ]= (state->random_sizes_p ? ((0.2 + frand(0.8)) * state->max_radius) : state->max_radius);
...
#ifdef HAVE_MOBILE /* Always obey real-world gravity */
    {
      float a= fabs(fabs(state->accx) > fabs(state->accy) ? state->accx : state->accy);
  ...
  for (a= 1; a <= state->count; a++)
    {
      ...
      if (! state->dbeclear_p || ! state->backb)
#endif /* HAVE_DOUBLE_BUFFER_EXTENSION */
        {
          ...
          {
            gc= state->erase_gc;
            XFillArc(state->dpy, state->b, gc, x1a, y1a, x2a - x1a, y2a - y1a, 0, 360 * 64);
          }
        }
#endif /* !HAVE_JWXYZ */
...
#endif /* HAVE_DOUBLE_BUFFER_EXTENSION */
    if (state->dbuf)
      XCopyArea(state->dpy, state->b, state->window, state->erase_gc, 0, 0, state->xgwa.width, state->xgwa.height, 0, 0);
    ...
    XEvent *event)
{
  ...
  if (event->xany.type == ButtonPress)
    {
      int i, rx, ry;
      XTranslateCoordinates(dpy, window, RootWindow(dpy, DefaultScreen(dpy)), event->xbutton.x, event->xbutton.y, &rx, &ry, &window);
