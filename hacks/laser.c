...
ModStruct laser_description=
  {"laser", "init_laser", "draw_laser", (char *) NULL, "refresh_laser", "init_laser", "free_laser", &laser_opts, 20000, -10, 200, 1, 64, 1.0, "", "Shows spinning lasers", 0, NULL};
  ...
  if (lp->stippledGC == None)
    {
      ...
      if ((lp->stippledGC= XCreateGC(display, MI_WINDOW(mi), GCForeground | GCBackground, &gcv)) == None)
        {
          free_laser(mi);
          return;
        }
  ...
  for (i= 0; i < lp->ln; i++)
    {
      ...
      if (lp->sw >= lp->lw)
        {
          ...
          XDrawLine(display, MI_WINDOW(mi), lp->stippledGC, lp->cx, lp->cy, l->sx [ lp->so ], l->sy [ lp->so ]);
        }
      if (l->dir)
        {
          switch (l->bn)
      ...
      XDrawLine(display, MI_WINDOW(mi), lp->stippledGC, lp->cx, lp->cy, l->bx, l->by);
