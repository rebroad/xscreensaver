...
ModStruct slip_description=
  {"slip", "init_slip", "draw_slip", (char *) NULL, "init_slip", "init_slip", (char *) NULL, &slip_opts, 50000, 35, 50, 1, 64, 1.0, "", "Shows slipping blits", 0, NULL};
    ...
    void *closure)
{
  ...
  XCopyArea(dpy, d, w, mi->gc, 0, 0, sp->width, sp->height, 0, 0);
  XFreePixmap(dpy, d);
  ...
  for (i= 0; i < n; i++)
    {
      ...
      XFillRectangle(display, MI_WINDOW(mi), gc, halfrandom(sp, MAX(sp->width - ww, 1)), halfrandom(sp, MAX(sp->height - ww, 1)), ww, ww);
    }
  sp->first_time= 0;
  ...
  if (! sp->image_loading_p &&
    (1 || halfrandom(sp, 2) == 0))
    {
      /* Load it into a pixmap so that the "Loading" message and checkerboard
         ...
         in progress... */
      Pixmap p= XCreatePixmap(MI_DISPLAY(mi), MI_WINDOW(mi), sp->width, sp->height, mi->xgwa.depth);
  ...
  while (timer--)
    {
      ...
      switch (sp->mode)
        {
          case 0 : /* rotor */
            ...
            if (sp->backwards)
              { /* jwz: go the other way sometimes */
                dx= -dx;
                dy= -dy;
              }
            break;
        ...
        }
      {
         ...
         */
        XCopyArea(display, window, window, gc, xi, yi, sp->blit_width, sp->blit_height, qx, qy);
        switch (sp->mode)
          {
            case 0 :
              /* wrap */
              ...
              if (qx > wrap)
                XCopyArea(display, window, window, gc, qx, qy, sp->blit_width, sp->blit_height, qx - wrap, qy);
              if (qx < 2 * sp->blit_width)
                XCopyArea(display, window, window, gc, qx, qy, sp->blit_width, sp->blit_height, qx + wrap, qy);
              wrap= sp->height - (2 * sp->blit_height);
              if (qy > wrap)
                XCopyArea(display, window, window, gc, qx, qy, sp->blit_width, sp->blit_height, qx, qy - wrap);
