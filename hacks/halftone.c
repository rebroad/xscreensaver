...
static void update_dot_attributes(
  halftone_screen *halftone,
  XWindowAttributes *attrs)
{
    ...
    Window window)
{
  ...
  make_smooth_colormap(attrs.screen, attrs.visual, attrs.colormap, halftone->colors, &halftone->ncolors, True, 0, False);
  halftone->color0= 0;
  ...
  int size)
{
  ...
  XFillArc(dpy, window, gc, start_x, start_y, width, height, angle1, angle2);
}

static void repaint_halftone(
  halftone_screen *halftone)
{
  int x, y;
  /*
  ...
  */
  ...
  XSetForeground(halftone->dpy, halftone->buffer_gc, halftone->colors [ halftone->color0 ].pixel);
  XFillRectangle(halftone->dpy, halftone->buffer, halftone->buffer_gc, 0, 0, halftone->buffer_width, halftone->buffer_height);

  /* Draw dots on buffer */
  XSetForeground(halftone->dpy, halftone->buffer_gc, halftone->colors [ halftone->color1 ].pixel);
  ...
  for (x= 0; x < halftone->dots_width; x++)
    for (y= 0; y < halftone->dots_height; y++)
      fill_circle(halftone->dpy, halftone->buffer, halftone->buffer_gc, x_offset + x * halftone->spacing, y_offset + y * halftone->spacing, halftone->max_dot_size * halftone->dots [ x + y * halftone->dots_width ]);
...
static double calculate_gravity(
  halftone_screen *halftone,
  int x,
  int y)
{
