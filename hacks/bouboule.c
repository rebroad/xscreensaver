  ...
  if (MI_WIN_IS_INSTALL(mi) && MI_WIN_IS_USE3D(mi))
    {
      ...
      XFillRectangle(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), 0, 0, sp->width, sp->height);
    }
  else
    {
  ...
  if (sp->hasbeenchecked == -2)
    {
      ...
      XFillRectangle(display, window, gc, x_1, y_1, x_2, y_2);
    }
  else if (sp->hasbeenchecked == -1)
    {
      /* Erasing is done with XFillArcs */
      XFillArcs(display, window, gc, sp->oldxarc, sp->NbStars);
      if (MI_WIN_IS_USE3D(mi))
        XFillArcs(display, window, gc, sp->oldxarcleft, sp->NbStars);
    }
  else
    {
      long usec;
