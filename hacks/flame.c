    ...
    Window window)
{
  ...
  if (mono_p)
  ...
  else
    {
      ...
      make_smooth_colormap(xgwa.screen, xgwa.visual, xgwa.colormap, st->colors, &st->ncolors, True, 0, True);
      if (st->ncolors <= 2)
      ...
      if (x > -1.0 && x < 1.0 && y > -1.0 && y < 1.0)
        {
          ...
          if (st->num_points >= POINT_BUFFER_SIZE)
            {
              XFillRectangles(st->dpy, win, st->gc, st->points, st->num_points);
              st->num_points= 0;
            }
        }
    }
