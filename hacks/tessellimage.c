    ...
    struct state *st)
{
  ...
  img2= XCreateImage(st->dpy, st->xgwa.visual, st->img->depth, ZPixmap, 0, NULL, st->img->width, st->img->height, 8, 0);
  if (! img2) abort();
    ...
    struct state *st)
{
  ...
  st->delta= XCreateImage(st->dpy, st->xgwa.visual, d, ZPixmap, 0, NULL, w, h, 32, 0);
  st->delta->data= (char *)
      ...
      for (x= 0; x < st->delta->width; x++)
        {
          ...
          for (i= 1; i < countof(pixels); i++)
            distance+= pixel_distance(st->xgwa.screen, st->xgwa.visual, pixels [ 0 ], pixels [ i ]);
  ...
  for (i= 0; i < np; i++)
    {
      ...
      out [ i ].p= (n > 0 ? (XPoint *) calloc(out [ i ].npoints + 1, sizeof(*out [ i ].p)) : 0);
    ...
    struct state *st)
{
  ...
  if (ticked_p && st->cache [ st->thresh ])
  ...
  else if (ticked_p)
    {
      ...
      if (st->output)
        XFreePixmap(st->dpy, st->output);
      st->output= XCreatePixmap(st->dpy, st->window, st->xgwa.width, st->xgwa.height, st->xgwa.depth);
      XFillRectangle(st->dpy, st->output, st->pgc, 0, 0, st->xgwa.width, st->xgwa.height);
                  ...
                  if (polys [ i ].npoints >= 3)
                    {
                      ...
                      XFillPolygon(st->dpy, st->output, st->pgc, polys [ i ].p, polys [ i ].npoints, Convex, CoordModeOrigin);
                      ...
                      if (st->outline_p && ! small_cell_p(&polys [ i ]))
                        {
                          ...
                          XDrawLines(st->dpy, st->output, st->pgc, polys [ i ].p, polys [ i ].npoints, CoordModeOrigin);
            ...
            for (i= 0; i < ntri; i++)
              {
                XPoint xp [ 3 ];
                unsigned long color;
                xp [ 0 ].x= p [ v [ i ].p1 ].x * wscale;
                xp [ 0 ].y= p [ v [ i ].p1 ].y * wscale;
                xp [ 1 ].x= p [ v [ i ].p2 ].x * wscale;
                xp [ 1 ].y= p [ v [ i ].p2 ].y * wscale;
                xp [ 2 ].x= p [ v [ i ].p3 ].x * wscale;
                xp [ 2 ].y= p [ v [ i ].p3 ].y * wscale;
                ...
                XSetForeground(st->dpy, st->pgc, color);
                XFillPolygon(st->dpy, st->output, st->pgc, xp, countof(xp), Convex, CoordModeOrigin);
                ...
                if (st->outline_p && ! small_triangle_p(xp))
                  { /* Border the triangle with a color that is darker */
                    ...
                    XAllocColor(st->dpy, st->xgwa.colormap, &bd);
                    XSetForeground(st->dpy, st->pgc, bd.pixel);
                    XDrawLines(st->dpy, st->output, st->pgc, xp, countof(xp), CoordModeOrigin);
                    XFreeColors(st->dpy, st->xgwa.colormap, &bd.pixel, 1, 0);
                  }
              }
            break;
    ...
    struct state *st)
{
  ...
  dimg= XCreateImage(st->dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, 0, NULL, w, h, 8, 0);
  if (! dimg) abort();
    ...
    void *closure)
{
  ...
  if (st->img_loader) /* still loading */
    {
      st->img_loader= load_image_async_simple(st->img_loader, 0, 0, 0, 0, &st->geom);
      if (! st->img_loader)
        { /* just finished */
          analyze(st);
  ...
  if (! st->img_loader &&
    st->start_time + st->duration < double_time())
    {
      ...
      st->img_loader= load_image_async_simple(0, st->xgwa.screen, st->window, st->image, 0, &st->geom);
      goto DONE;
    }
