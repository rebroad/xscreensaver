    ...
    struct state *st)
{
  ...
  return (zeit->tm_sec + 60 * (zeit->tm_min + 60 * (zeit->tm_hour)) + time1.tv_usec * 1.0E-6);
}
    ...
    int setcol)
{
  ...
  if (st->fastdraw && d < st->fastch)
    {
#ifdef FASTCOPY
      XCopyArea(st->dpy, st->fastmask, buf, st->andgc, sum1ton(d) - (d + 1) / 2, 1, d, d, (int) (st->kugeln [ i ].x1) - d / 2, (int) (st->kugeln [ i ].y1) - d / 2);
      XCopyArea(st->dpy, st->fastcircles, buf, st->orgc, sum1ton(d) - (d + 1) / 2, 1, d, d, (int) (st->kugeln [ i ].x1) - d / 2, (int) (st->kugeln [ i ].y1) - d / 2);
#else
      XPutImage(st->dpy, buf, st->andgc, st->fastmask [ d - 1 ], 0, 0, (int) (st->kugeln [ i ].x1) - d / 2, (int) (st->kugeln [ i ].y1) - d / 2, d, d);
      XPutImage(st->dpy, buf, st->orgc, st->fastcircles [ d - 1 ], 0, 0, (int) (st->kugeln [ i ].x1) - d / 2, (int) (st->kugeln [ i ].y1) - d / 2, d, d);
#endif
    }
  else
#endif
...
#else /* Use XFillArc */
        XFillArc(st->dpy, buf, st->gc, (int) (st->kugeln [ i ].x1 + (st->kugeln [ i ].r1 + ra) / 2), (int) (st->kugeln [ i ].y1 + (st->kugeln [ i ].r1 + ra) / 2), (int) -(2 * ra + 1), (int) -(2 * ra + 1), 0, 360 * 64);
#endif
        }
    }
}
  ...
  for (i= 0; i < st->maxk; i++)
    {
      ...
      if ((x1 > -2000.0) && (x1 < st->scrnWidth + 2000.0) && (y1 > -2000.0) && (y1 < st->scrnHeight + 2000.0))
        {
    ...
    Window window)
{
  ...
  XQueryPointer(st->dpy, st->window, &st->junk_win, &st->junk_win, &st->junk, &st->junk, &st->px, &st->py, &st->kb);
    ...
    void *closure)
{
  ...
  XQueryPointer(st->dpy, st->window, &st->junk_win, &st->in_win, &st->junk, &st->junk, &st->px, &st->py, &st->kb);
