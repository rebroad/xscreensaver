...
#endif
    {
      hsv_to_rgb(random() % 360, frand(1.0), frand(0.5) + 0.5, &qix->lines [ 0 ].color.red, &qix->lines [ 0 ].color.green, &qix->lines [ 0 ].color.blue);
  ...
  if (st->transparent_p)
    {
      ...
      allocate_alpha_colors(xgwa.screen, xgwa.visual, st->cmap, &nplanes, st->additive_p, &plane_masks, &base_pixel);
      ...
      st->gcs [ 1 ]= (st->xor_p ? st->gcs [ 0 ] : (GC *) malloc(st->count * sizeof(GC)));
      ...
      for (i= 0; i < st->count; i++)
        {
          ...
          if (st->xor_p)
            {
              gcv.function= GXxor;
              st->gcs [ 0 ][ i ]= XCreateGC(st->dpy, st->window, GCForeground | GCFunction | GCPlaneMask, &gcv);
            }
          else
            {
              st->gcs [ 0 ][ i ]= XCreateGC(st->dpy, st->window, GCForeground | GCPlaneMask, &gcv);
              gcv.foreground= 0;
              st->gcs [ 1 ][ i ]= XCreateGC(st->dpy, st->window, GCForeground | GCPlaneMask, &gcv);
#ifdef HAVE_JWXYZ
    ...
    struct qix *qix)
{
  ...
  if (qline->dead || ! prev)
  ...
  else if (st->solid_p)
    {
      ...
      XFillPolygon(st->dpy, st->window, (st->transparent_p && st->gcs [ 1 ] ? st->gcs [ 1 ][ qix->id ] : st->erase_gc), points, 4, Complex, CoordModeOrigin);
    }
  else
    {
      ...
      XDrawLines(st->dpy, st->window, (st->transparent_p && st->gcs [ 1 ] ? st->gcs [ 1 ][ qix->id ] : st->erase_gc), points, qix->npoly + 1, CoordModeOrigin);
    }
...
#define wiggle(point, delta, max)						\
  ...
  if (st->max_size)
    {
      ...
      if (qline->p [ 0 ].x - qline->p [ 1 ].x > st->max_size)
        qline->p [ 0 ].x= qline->p [ 1 ].x + st->max_size - (st->random_p ? random() % st->max_spread : 0);
      else if (qline->p [ 1 ].x - qline->p [ 0 ].x > st->max_size)
        qline->p [ 1 ].x= qline->p [ 0 ].x + st->max_size - (st->random_p ? random() % st->max_spread : 0);
      if (qline->p [ 0 ].y - qline->p [ 1 ].y > st->max_size)
        qline->p [ 0 ].y= qline->p [ 1 ].y + st->max_size - (st->random_p ? random() % st->max_spread : 0);
      else if (qline->p [ 1 ].y - qline->p [ 0 ].y > st->max_size)
        qline->p [ 1 ].y= qline->p [ 0 ].y + st->max_size - (st->random_p ? random() % st->max_spread : 0);
    }
  ...
  if (! mono_p && ! st->transparent_p)
#endif
    {
      ...
      int h;
      double s, v;
      rgb_to_hsv(qline->color.red, qline->color.green, qline->color.blue, &h, &s, &v);
      h= (h + st->color_shift) % 360;
      hsv_to_rgb(h, s, v, &qline->color.red, &qline->color.green, &qline->color.blue);
  ...
  if (! st->solid_p)
    {
      ...
      points [ qix->npoly ]= points [ 0 ];
      XDrawLines(st->dpy, st->window, (st->transparent_p && st->gcs [ 0 ] ? st->gcs [ 0 ][ qix->id ] : st->draw_gc), points, qix->npoly + 1, CoordModeOrigin);
    }
  else if (! prev_qline->dead)
    {
      ...
      XFillPolygon(st->dpy, st->window, (st->transparent_p && st->gcs [ 0 ] ? st->gcs [ 0 ][ qix->id ] : st->draw_gc), points, 4, Complex, CoordModeOrigin);
    }
    ...
    struct qix *qix)
{
  ...
  free_qline(st, &qix->lines [ qix->fp ], &qix->lines [ (qix->fp + 1) % qix->nlines ], qix);
