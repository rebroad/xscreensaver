...
ModStruct fiberlamp_description=
  {"fiberlamp", "init_fiberlamp", "draw_fiberlamp", (char *) NULL, "draw_fiberlamp", "change_fiberlamp", "free_fiberlamp", &fiberlamp_opts, 1000, 500, 10000, 0, 64, 1.0, "", "Shows a Fiber Optic Lamp", 0, NULL};
  ...
  if (fl->dbufp)
    {
      ...
      XFillRectangle(MI_DISPLAY(mi), fl->buffer, MI_GC(mi), 0, 0, MI_WIDTH(mi), MI_HEIGHT(mi));
    }
}
    ...
    ModeInfo *mi)
{
  ...
  if (fl->dbufp)
    {
      fl->buffer= XCreatePixmap(MI_DISPLAY(mi), MI_WINDOW(mi), MI_WIDTH(mi), MI_HEIGHT(mi), MI_DEPTH(mi));
      if (fl->buffer == None)
        {
          free_fiberlamp(mi);
          return;
    ...
    ModeInfo *mi)
{
  ...
  XTranslateCoordinates(MI_DISPLAY(mi), MI_WINDOW(mi), RootWindow(MI_DISPLAY(mi), 0 /*#### MI_SCREEN(mi)*/), cx, cy, &x, &y, &unused);
  sort_fibers(fl);
  ...
  for (f= 0; f < fl->nfibers; f++)
    {
...

      {
        ...
        XDrawLines(MI_DISPLAY(mi), fl->buffer, MI_GC(mi), fs->draw, NODES - tiplen, CoordModeOrigin);

        XSetForeground(MI_DISPLAY(mi), MI_GC(mi), tipcolor);
        XDrawLines(MI_DISPLAY(mi), fl->buffer, MI_GC(mi), fs->draw + NODES - 1 - tiplen, tiplen, CoordModeOrigin);
      }
    }

  /* Update the screen from the double-buffer */
  if (fl->dbufp)
    XCopyArea(MI_DISPLAY(mi), fl->buffer, MI_WINDOW(mi), MI_GC(mi), 0, 0, MI_WIDTH(mi), MI_HEIGHT(mi), 0, 0);
