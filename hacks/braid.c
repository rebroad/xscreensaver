...
ModStruct braid_description=
  {"braid", "init_braid", "draw_braid", (char *) NULL, "refresh_braid", "init_braid", (char *) NULL, &braid_opts, 1000, 15, 100, 1, 64, 1.0, "", "Shows random braids and knots", 0, NULL};
    ...
    ModeInfo *mi)
{
  ...
  MI_IS_DRAWN(mi)= True;
  XSetLineAttributes(display, MI_GC(mi), braid->linewidth, LineSolid, (braid->linewidth <= 3 ? CapButt : CapRound), JoinMiter);
              ...
              if (ABS(braid->braidword [ i ]) - 1 == s)
                {
                  ...
                  if (MI_NPIXELS(mi) > 2)
                    {
                      color_use= color + SPINRATE * braid->components [ applywordbackto(braid, s, i) ] +
                  ...
                  if (braid->braidword [ i ] > 0 ||
                    (FABSF(t - theta / 2.0) > theta / 7.0))
                    {
                      ...
                      XDrawLine(display, window, MI_GC(mi), (int) (x_1), (int) (y_1), (int) (x_2), (int) (y_2));
                    }
#ifdef COLORCOMP
                  if (MI_NPIXELS(mi) > 2)
                    {
                      color_use= color + SPINRATE * braid->components [ applywordbackto(braid, s + 1, i) ] +
                        (psi + t) / 2.0 / M_PI * (float) MI_NPIXELS(mi);
                      while (((int) color_use) >= MI_NPIXELS(mi))
                        color_use-= (float) MI_NPIXELS(mi);
                      while (((int) color_use) < 0)
