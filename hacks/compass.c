...
struct disc
{
    ...
    int limit;
    GC gc;
    void (*draw)(struct state *, Drawable, struct disc *, int x, int y, int radius);
};
    ...
    int radius)
{
  ...
  XDrawLine(st->dpy, d, disc->gc, (int) (x + radius2 * cos(th)), (int) (y + radius2 * sin(th)), (int) (x + -radius * cos(th)), (int) (y + -radius * sin(th)));
    ...
    Window window)
{
  ...
  st->discs [ 0 ]->gc= XCreateGC(st->dpy, st->b, GCForeground | GCLineWidth | GCJoinStyle, &gcv);
  init_spin(st->discs [ 0 ]);

  gcv.foreground= get_pixel_resource(st->dpy, st->xgwa.colormap, "arrow2Foreground", "Foreground");
  ...
  gcv.foreground= get_pixel_resource(st->dpy, st->xgwa.colormap, "arrow1Foreground", "Foreground");
  gcv.line_width= MAX(4, (st->size / 30));
  st->discs [ 2 ]->draw= draw_thin_arrow;
  st->discs [ 2 ]->gc= XCreateGC(st->dpy, st->b, GCForeground | GCLineWidth, &gcv);
  init_spin(st->discs [ 2 ]);

  gcv.foreground= get_pixel_resource(st->dpy, st->xgwa.colormap, "pointerForeground", "Foreground");
    ...
    void *closure)
{
  ...
  if (st->backb)
    ...
    if (st->dbuf)
      {
        XCopyArea(st->dpy, st->b, st->window, st->erase_gc, 0, 0, st->size2, st->size2, st->xgwa.width / 2 - st->x, st->xgwa.height / 2 - st->y);
