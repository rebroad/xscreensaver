      ...
      switch (i)
        {
          ...
          case 5 :
            colors [ i ].red= 0x2000;
            colors [ i ].blue= 0xCFFF;
            break; /* blue */
          case 6 :
            colors [ i ].red= 0xDFFF; /* yellow */
            colors [ i ].green= 0xCFFF;
            break;
        }
    }
    ...
    int depth)
{
  ...
  else if ((st->goldenRatio || st->mondrian) ? (w > h) : (random() & 1))
  ...
  if (st->mondrian)
    {
      ...
      make_mondrian_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, True, 0, True);
    ...
    {
      make_random_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, False, True, 0, True);
    }
  ...
  if (st->mondrian)
    {
      ...
      if (st->line_width != st->old_line_width)
        {
          XSetLineAttributes(dpy, st->fgc, st->line_width, LineSolid, CapButt, JoinBevel);
