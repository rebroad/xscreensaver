  ...
  if ((tp->j == 0) && (tp->i == 0))
    {
...
#else
      {
        ...
        XFillRectangle(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), x, y, x2, y2);
      }
#endif
    }
    ...
    ModeInfo *mi)
{
  ...
  for (i= 0; i < 2 * tp->size + 1; i++)
    tp->xpos [ i ]= (short) ((((double) i) / ((double) (2 * tp->size)) * (RIGHT - LEFT) + LEFT) * dim) + (tp->width - dim) / 2;

  for (i= 0; i < (tp->size + 1); i++)
    tp->ypos [ i ]= (short) ((((double) i) / ((double) tp->size) * (BOTTOM - TOP) + TOP) * dim) + (tp->height - dim) / 2;
