      ...
      if (st->be_wormy)
        {
          ...
          rotation_bias+= (st->terrain [ view_pos ][ TERRAIN_BREADTH / 4 + 2 ] -
                            st->terrain [ view_pos ][ TERRAIN_BREADTH / 4 - 2 ]) /
            8;
        }
      else
        {
      ...
      for (j= 0; j < TERRAIN_BREADTH; j++)
        {
          ...
          r= rotation_bias + (int) (st->vertigo * zcc);
          MODULO(r, ROTS);
    ...
    struct state *st)
{
  ...
  for (i= 0; i < TERRAIN_LENGTH; i++)
    {
      in= st->nearest + i;
      MODULO(in, TERRAIN_BREADTH);
      ip= st->nearest - i;
      MODULO(ip, TERRAIN_BREADTH);
      for (j= 0; j < TERRAIN_BREADTH; j++)
        {
          t= st->terrain [ ip ][ j ];
  ...
  for (w= diff / 2, l= TERRAIN_BREADTH / 4;
    ...
    w/= 2, l/= 2)
    {
      ...
      for (i= start + w - 1; i < end; i+= (w * 2))
        {
          ip= i - w;
          MODULO(ip, TERRAIN_LENGTH);
          in= i + w;
          MODULO(in, TERRAIN_LENGTH);
          for (j= l - 1; j < TERRAIN_BREADTH; j+= (l * 2))
            {
              jp= j - 1;
              MODULO(jp, TERRAIN_BREADTH);
              jn= j + 1;
              MODULO(jn, TERRAIN_BREADTH);
              st->terrain [ i ][ j ]=
                generate_terrain_value(st, st->terrain [ ip ][ jp ], st->terrain [ in ][ jp ], st->terrain [ ip ][ jn ], st->terrain [ in ][ jn ], w);
            }
        }
          ...
          for (j= l - 1; j < TERRAIN_BREADTH; j+= (l * 2))
            {
              ...
              MODULO(jn, TERRAIN_BREADTH);
              st->terrain [ i ][ j ]=
                generate_terrain_value(st, st->terrain [ ip ][ j ], st->terrain [ in ][ j ], st->terrain [ i ][ jp ], st->terrain [ i ][ jn ], w);
            }
        }
    ...
    struct state *st)
{
  ...
  if (st->step >= 0.0)
    {
      start= st->nearest;
      end= st->nearest + (int) floor(st->step);
    }
  else
    {
      end= st->nearest;
      start= st->nearest + (int) floor(st->step);
    }
    ...
    double time_per_frame)
{
  ...
  if (st->spin_bonus > 10)
    st->spin_bonus-= (int) (st->step * 13.7);
  else if (st->spin_bonus < -10)
    st->spin_bonus+= (int) (st->step * 11.3);

  if (st->backwards_bonus > 1)
    {
      st->backwards_bonus--;
    }
    ...
    int end)
{
  ...
  for (i= start; i <= end; i++)
    {
      ii= i;
      if (ii >= TERRAIN_LENGTH) ii-= TERRAIN_LENGTH;
      st->bonuses [ ii ]= 0;
    }
  if (random() % 4 == 0)
    {
    ...
    struct state *st)
{
  ...
  end= st->nearest - passed - 1;
  MODULO(end, TERRAIN_LENGTH);
  start= end - TERRAIN_LENGTH / 4 + 1;
  MODULO(start, TERRAIN_LENGTH);
  ...
  switch (random() % 64)
    {
      ...
      case 1 :
        ...
        generate_smooth(st, start, start + TERRAIN_LENGTH / 8 + (random() % TERRAIN_LENGTH / 8));
        break;
      case 2 :
        generate_smooth(st, start, end);
        generate_terrain(st, start, end, 4);
        break;
      case 3 :
        generate_smooth(st, start, end);
        generate_terrain(st, start, end, 2);
        break;
      default :
        generate_terrain(st, start, end, 1);
    }
    ...
    int i)
{
  ...
  for (j= 0; j < TERRAIN_BREADTH; j+= dt)
    {
      t2= t + dt;
      if (t2 >= TERRAIN_LENGTH) t2-= TERRAIN_LENGTH;
      j2= j + dt;
      if (j2 >= TERRAIN_BREADTH) j2-= TERRAIN_BREADTH;
      points [ 0 ].x= st->xvals [ t ][ j ];
      points [ 0 ].y= st->yvals [ t ][ j ];
      points [ 1 ].x= st->xvals [ t2 ][ j ];
      points [ 1 ].y= st->yvals [ t2 ][ j ];
      points [ 2 ].x= st->xvals [ t2 ][ j2 ];
      points [ 2 ].y= st->yvals [ t2 ][ j2 ];
      points [ 3 ].x= st->xvals [ t ][ j2 ];
      points [ 3 ].y= st->yvals [ t ][ j2 ];
  ...
  for (j= 0; j < TERRAIN_BREADTH; j+= dt * 2)
    {
      ...
      if (j2 >= TERRAIN_BREADTH) j2-= TERRAIN_BREADTH;
      j3= j + dt + dt;
      if (j3 >= TERRAIN_BREADTH) j3-= TERRAIN_BREADTH;
      points [ 0 ].x= st->xvals [ t ][ j ];
      points [ 0 ].y= st->yvals [ t ][ j ];
      points [ 1 ].x= st->xvals [ t2 ][ j ];
      points [ 1 ].y= st->yvals [ t2 ][ j ];
      points [ 2 ].x= st->xvals [ t2 ][ j2 ];
      points [ 2 ].y= st->yvals [ t2 ][ j2 ];
      points [ 3 ].x= st->xvals [ t2 ][ j3 ];
      points [ 3 ].y= st->yvals [ t2 ][ j3 ];
      points [ 4 ].x= st->xvals [ t ][ j3 ];
      points [ 4 ].y= st->yvals [ t ][ j3 ];
    ...
    int t)
{
  ...
  XFillPolygon(st->dpy, d, gc, erase_points, TERRAIN_BREADTH / 2, Complex, CoordModeOrigin);
}
    ...
    int t)
{
  ...
  XFillRectangle(st->dpy, st->stars_mask, st->stars_erase_gc, 0, 0, st->width, st->height);
  ...
  for (i= 0; i < lim; i++)
    {
      a= RAND(w);
      b= RAND(h);
      XDrawLine(st->dpy, st->stars_mask, st->stars_gc, st->minx [ t ] + a - l1, st->miny [ t ] + b, st->minx [ t ] + a + l1, st->miny [ t ] + b);
      XDrawLine(st->dpy, st->stars_mask, st->stars_gc, st->minx [ t ] + a, st->miny [ t ] + b - l1, st->minx [ t ] + a, st->miny [ t ] + b + l1);
    }
  for (i= 0; i < lim; i++)
    {
      a= RAND(w);
      b= RAND(h);
      XDrawLine(st->dpy, st->stars_mask, st->stars_gc, st->minx [ t ] + a - l2, st->miny [ t ] + b, st->minx [ t ] + a + l2, st->miny [ t ] + b);
      XDrawLine(st->dpy, st->stars_mask, st->stars_gc, st->minx [ t ] + a, st->miny [ t ] + b - l2, st->minx [ t ] + a, st->miny [ t ] + b + l2);
    }
}
  ...
  if (st->crosshair_flag)
    {
      ...
      XFillRectangle(st->dpy, d, gc, st->width / 2 + (st->xoffset) - 8, st->height / 2 + (st->yoffset * 2) - 1, 16, 3);
      XFillRectangle(st->dpy, d, gc, st->width / 2 + (st->xoffset) - 1, st->height / 2 + (st->yoffset * 2) - 8, 3, 16);
    }
}
    ...
    double v2)
{
  ...
  make_color_ramp(st->screen, st->visual, st->cmap, h1, s1, v1, h2, s2, v2, colors, ncolors, False, True, False);
    ...
    struct state *st)
{
  ...
  if (st->psychedelic_flag)
    {
      ...
      speedmine_color_ramp(st, st->ground_gcs, st->ground_colors, &st->ncolors, 0.0, 0.8, 0.0, 0.9);
  ...
  st->ncolors= MAX_COLORS;
  speedmine_color_ramp(st, st->wall_gcs, st->wall_colors, &st->ncolors, s1, s2, 0.0, 0.9);
  st->nr_wall_colors= st->ncolors;

  st->ncolors= MAX_COLORS;
  speedmine_color_ramp(st, st->bonus_gcs, st->bonus_colors, &st->ncolors, 0.6, 0.9, 0.4, 1.0);
    ...
    struct state *st)
{
  ...
  st->ncolors= MAX_COLORS;
  speedmine_color_ramp(st, st->wall_gcs, st->wall_colors, &st->ncolors, 0.0, 0.6, 0.0, 0.9);
  st->nr_wall_colors= st->ncolors;
    ...
    struct state *st)
{
  ...
  make_color_ramp(st->screen, st->visual, st->cmap, h1, s1, v1, h2, s2, v2, st->ground_colors, &st->ncolors, False, True, False);
  st->nr_ground_colors= st->ncolors;
