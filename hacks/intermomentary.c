...
static struct field *init_field(
  void)
{
      ...
      for (j= -2; j < 3; j++)
        {
          ...
          XFillRectangle(st->dpy, drawable, fgc, px + i, py + j, st->pscale, st->pscale);
    ...
    float r)
{
  ...
  if (cv > 0.0006921)
  ...
  else
    {
      ...
      XSetForeground(st->dpy, fgc, get_pixel(st, c));
      XFillRectangle(st->dpy, drawable, fgc, px, py, st->pscale, st->pscale);
      XSetForeground(st->dpy, fgc, f->fgcolor);
    }
}
    ...
    int dnum)
{
          ...
          if (d > fabsf(f->discs [ n ].r - di->r))
            {
              ...
              XSetForeground(st->dpy, fgc, get_pixel(st, c));
              XFillRectangle(st->dpy, drawable, fgc, p3ax, p3ay, st->pscale, st->pscale);

              /* XPutPixel(f->off_map, p3bx, p3by, f->fgcolor); */
              c= trans_point(st, p3bx, p3by, 255, 0.75, f);
              XSetForeground(st->dpy, fgc, get_pixel(st, c));
              XFillRectangle(st->dpy, drawable, fgc, p3bx, p3by, st->pscale, st->pscale);
              XSetForeground(st->dpy, fgc, f->fgcolor);
            }
        }
    }
  ...
  for (m= 0; m < di->numr; m++)
    moverender_rider(st, drawable, fgc, f, &(di->pxRiders [ m ]), di->x, di->y, di->r);
}
    ...
    Window window)
{
...

  {
    ...
    rgb_to_hsv(bgc.red, bgc.green, bgc.blue, &bgh, &bgs, &bgv);
      ...
      bgv = fgv / 10.0;
#endif
    make_color_ramp(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, bgh, bgs, bgv, fgh, fgs, fgv, st->colors, &st->ncolors, False, /* closed */
      True,
      False);
  }
    ...
    void *closure)
{
  ...
  if (st->f->off_map != window)
    XCopyArea(dpy, st->f->off_map, window, st->copygc, 0, 0, st->f->width, st->f->height, 0, 0);
