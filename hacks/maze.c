...
static void alt_mask_out_rect(struct state *st, char *corners, int x, int y, int w, int h);
...
static void
  create_maze(
    struct state *st) /* create a maze layout given the initialized maze */
{
  int i, newdoor= 0;
  ...
  do {
      ...
      while ((newdoor= choose_door(st)) == -1)
        { /* pick a door */
          if (backup(st) == -1)
...
static int
  choose_door(
    struct state *st) /* pick a new path */
{
  int candidates [ 3 ];
  int num_candidates;
...
static int
  backup(
    struct state *st) /* back up a move */
{
  st->sqnum--;
  if (st->sqnum >= 0)
    {
  ...
  for (i= 0; i < st->maze_size_x; i++)
    {
      if (st->maze [ i ][ 0 ] & WALL_TOP)
        XDrawLine(st->dpy, st->window, st->gc, border_x + st->grid_width * i, border_y, border_x + st->grid_width * (i + 1) - 1, border_y);
      if ((st->maze [ i ][ st->maze_size_y - 1 ] & WALL_BOTTOM))
        XDrawLine(st->dpy, st->window, st->gc, border_x + st->grid_width * i, border_y + st->grid_height * (st->maze_size_y) - 1, border_x + st->grid_width * (i + 1) - 1, border_y + st->grid_height * (st->maze_size_y) - 1);
    }
  for (j= 0; j < st->maze_size_y; j++)
    {
      if (st->maze [ st->maze_size_x - 1 ][ j ] & WALL_RIGHT)
        XDrawLine(st->dpy, st->window, st->gc, border_x + st->grid_width * st->maze_size_x - 1, border_y + st->grid_height * j, border_x + st->grid_width * st->maze_size_x - 1, border_y + st->grid_height * (j + 1) - 1);
      if (st->maze [ 0 ][ j ] & WALL_LEFT)
        XDrawLine(st->dpy, st->window, st->gc, border_x, border_y + st->grid_height * j, border_x, border_y + st->grid_height * (j + 1) - 1);
    }
  ...
  if (st->logo_x != -1)
    {
      ...
      XFillRectangle(st->dpy, st->window, st->ugc, border_x + 3 + st->grid_width * st->logo_x, border_y + 3 + st->grid_height * st->logo_y, ww, hh);

      XSetClipOrigin(st->dpy, st->logo_gc, lx, ly);
      if (d == 1)
        XCopyPlane(st->dpy, st->logo_map, st->window, st->logo_gc, 0, 0, w, h, lx, ly, 1);
      else
        XCopyArea(st->dpy, st->logo_map, st->window, st->logo_gc, 0, 0, w, h, lx, ly);
    }
...
static void
  draw_wall(
    ...
    int j,
    int dir,
    GC with_gc) /* draw a single wall */
{
  switch (dir)
    {
      case 0 :
        XDrawLine(st->dpy, st->window, with_gc, border_x + st->grid_width * i, border_y + st->grid_height * j, border_x + st->grid_width * (i + 1), border_y + st->grid_height * j);
        break;
      case 1 :
        XDrawLine(st->dpy, st->window, with_gc, border_x + st->grid_width * (i + 1), border_y + st->grid_height * j, border_x + st->grid_width * (i + 1), border_y + st->grid_height * (j + 1));
        break;
      case 2 :
        XDrawLine(st->dpy, st->window, with_gc, border_x + st->grid_width * i, border_y + st->grid_height * (j + 1), border_x + st->grid_width * (i + 1), border_y + st->grid_height * (j + 1));
        break;
      case 3 :
        XDrawLine(st->dpy, st->window, with_gc, border_x + st->grid_width * i, border_y + st->grid_height * j, border_x + st->grid_width * i, border_y + st->grid_height * (j + 1));
        break;
    }
    ...
    GC with_gc)
{
  switch (dir)
    {
      case WALL_TOP :
        XFillRectangle(st->dpy, st->window, with_gc, border_x + st->bw + (st->bw == 0 ? 1 : 0) + st->grid_width * i, border_y - st->bw - (st->bw == 0 ? 1 : 0) + st->grid_height * j, st->grid_width - (st->bw + st->bw + (st->bw == 0 ? 1 : 0)), st->grid_height);
        break;
      case WALL_RIGHT :
        XFillRectangle(st->dpy, st->window, with_gc, border_x + st->bw + (st->bw == 0 ? 1 : 0) + st->grid_width * i, border_y + st->bw + (st->bw == 0 ? 1 : 0) + st->grid_height * j, st->grid_width, st->grid_height - (st->bw + st->bw + (st->bw == 0 ? 1 : 0)));
        break;
      case WALL_BOTTOM :
        XFillRectangle(st->dpy, st->window, with_gc, border_x + st->bw + (st->bw == 0 ? 1 : 0) + st->grid_width * i, border_y + st->bw + (st->bw == 0 ? 1 : 0) + st->grid_height * j, st->grid_width - (st->bw + st->bw + (st->bw == 0 ? 1 : 0)), st->grid_height);
        break;
      ...
      for (xx= 0; xx < st->maze_size_x; xx++)
        {
          for (yy= 0; yy < st->maze_size_y; yy++)
            {
              if (! (st->maze [ xx ][ yy ] & (SOLVER_VISIT | NOT_DEAD)) && ((xx && (st->maze [ xx - 1 ][ yy ] & NOT_DEAD)) || (yy && (st->maze [ xx ][ yy - 1 ] & NOT_DEAD))))
                {
                  flipped= 1;
      ...
      for (xx= st->maze_size_x - 1; xx >= 0; xx--)
        {
          for (yy= st->maze_size_y - 1; yy >= 0; yy--)
            {
              if (! (st->maze [ xx ][ yy ] & (SOLVER_VISIT | NOT_DEAD)) && ((xx != st->maze_size_x - 1 && (st->maze [ xx + 1 ][ yy ] & NOT_DEAD)) || (yy != st->maze_size_y - 1 && (st->maze [ xx ][ yy + 1 ] & NOT_DEAD))))
                {
                  flipped= 1;
                  st->maze [ xx ][ yy ]|= NOT_DEAD;
                }
              ...
              if ((xx < st->logo_x || xx > st->logo_x + st->logo_width / st->grid_width) ||
                (yy < st->logo_y || yy > st->logo_y + st->logo_height / st->grid_height))
                {
                  ...
                  if ((st->maze [ xx ][ yy ] & WALL_ANY) == WALL_ANY)
                    {
                      XFillRectangle(st->dpy, st->window, st->ugc, border_x + st->bw + st->grid_width * xx, border_y + st->bw + st->grid_height * yy, st->grid_width - (st->bw + st->bw), st->grid_height - (st->bw + st->bw));
                    }
                  else
                    {
    ...
    Window window_arg)
{
...

  {
    ...
    if (xgwa.width > 2500 || xgwa.height > 2500)
      st->logo_map= image_data_to_pixmap(st->dpy, st->window, logo_360_png, sizeof(logo_360_png), &st->logo_width, &st->logo_height, &logo_mask);
    else if (xgwa.width > 900 || xgwa.height > 900)
      st->logo_map= image_data_to_pixmap(st->dpy, st->window, logo_180_png, sizeof(logo_180_png), &st->logo_width, &st->logo_height, &logo_mask);
    else
      st->logo_map= image_data_to_pixmap(st->dpy, st->window, logo_50_png, sizeof(logo_50_png), &st->logo_width, &st->logo_height, &logo_mask);
    if (logo_mask)
      {
