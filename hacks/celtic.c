...
static void edge_del(
  void *e) /* not Edge * because the function is passed to array_del */
{
  free(e);
}
...
static Edge graph_next_edge_around(
  Graph g,
  Node n,
  Edge e,
  Direction direction)
      ...
      for (p= 0; p < nbp; p++)
        {
          ...
          /* link along orbit */
          graph_add_edge(g, edge_new(grid [ 1 + o * nbp + p ], grid [ 1 + o * nbp + (p + 1) % nbp ]));
        }
    }
  ...
  for (row= 0; row < nbrow; row++)
    {
      for (col= 0; col < nbcol; col++)
        {
          if (col != nbcol - 1)
            graph_add_edge(g, edge_new(grid [ row + col * nbrow ], grid [ row + (col + 1) * nbrow ]));
          if (row != nbrow - 1)
            graph_add_edge(g, edge_new(grid [ row + col * nbrow ], grid [ row + 1 + col * nbrow ]));
          if (col != nbcol - 1 && row != nbrow - 1)
            {
              graph_add_edge(g, edge_new(grid [ row + col * nbrow ], grid [ row + 1 + (col + 1) * nbrow ]));
              graph_add_edge(g, edge_new(grid [ row + 1 + col * nbrow ], grid [ row + (col + 1) * nbrow ]));
            }
        }
    }
...
static Graph make_triangle_graph(
  struct state *st,
  int xmin,
  int ymin,
  int width,
...
static Graph make_kennicott_graph(
  struct state *st,
  int xmin,
  int ymin,
  int width,
          ...
          if (row != nbrow - 1)
            {
              ...
              graph_add_edge(g, edge_new(grid [ 5 * (row + col * nbrow) + 4 ], grid [ 5 * (row + 1 + col * nbrow) + 2 ]));
            }
        }
    }
  free(grid);
...
static void spline_add_segment(
  Spline s,
  double x1,
  double y1,
  double x2,
...
static EdgeCouple edge_couple_new(
  int nb_edges)
{
  int i;
...
static void pattern_edge_couple_set(
  Pattern p,
  Edge e,
  Direction d,
  int value)
...
static void pattern_draw_spline_direction(
  Pattern p,
  Spline s,
  Node node,
  Edge edge1,
...
static int pattern_next_unfilled_couple(
  Pattern p,
  Edge *e,
  Direction *d)
{
...
static void pattern_make_curves(
  Pattern p)
{
  ...
  while (pattern_next_unfilled_couple(p, &first_edge, &first_direction))
    {
      ...
      do {
          ...
          pattern_draw_spline_direction(p, s, current_node, current_edge, next_edge, current_direction);
              ...
              if (t2 <= 1.0)
                {
                  ...
                  /* draw shadow line */
                  XDrawLine(st->dpy, st->window, st->shadow_gc, (int) rint(x3), (int) rint(y3), (int) rint(x4), (int) rint(y4));
                }
              /* draw line segment */
...
#else
      make_smooth_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, True, 0, True);
#endif
      if (st->ncolors < 2)
        {
          goto MONO;
        ...
        {
          st->colors [ 0 ].pixel= get_pixel_resource(st->dpy, st->xgwa.colormap, "foreground", "Foreground");
          st->colors [ 1 ].pixel= get_pixel_resource(st->dpy, st->xgwa.colormap, "background", "Background");
        }
    }
  ...
  if (st->pattern == NULL)
    {
      ...
      switch (st->params.type)
        {
          case tgrid :
            st->graph= make_grid_graph(st, st->params.margin, st->params.margin, st->xgwa.width - 2 * st->params.margin, st->xgwa.height - 2 * st->params.margin, st->params.edge_size);
            break;
          case kennicott :
            st->graph= make_kennicott_graph(st, st->params.margin, st->params.margin, st->xgwa.width - 2 * st->params.margin, st->xgwa.height - 2 * st->params.margin, st->params.edge_size, st->params.cluster_size);
            break;
          case triangle :
            st->graph= make_triangle_graph(st, st->params.margin, st->params.margin, st->xgwa.width - 2 * st->params.margin, st->xgwa.height - 2 * st->params.margin, st->params.edge_size);
            break;
          case polar :
            st->graph= make_polar_graph(st, st->params.margin, st->params.margin, st->xgwa.width - 2 * st->params.margin, st->xgwa.height - 2 * st->params.margin, st->params.nb_nodes_per_orbit, st->params.nb_orbits);
            break;
          default :
            st->graph= make_grid_graph(st, st->params.margin, st->params.margin, st->xgwa.width - 2 * st->params.margin, st->xgwa.height - 2 * st->params.margin, st->params.edge_size);
            break;
