    ...
    Window window)
{
  ...
  else if (! strcmp(mode_str, "balls"))
    {
      st->mode= ball_mode;
    }
  else if (! strcmp(mode_str, "lines"))
    {
      st->mode= line_mode;
    }
  else if (! strcmp(mode_str, "polygons"))
    {
      st->mode= polygon_mode;
    }
  else if (! strcmp(mode_str, "tails"))
    {
      st->mode= tail_mode;
    }
  else if (! strcmp(mode_str, "splines"))
    {
      st->mode= spline_mode;
    }
  else if (! strcmp(mode_str, "filled-splines"))
    {
      st->mode= spline_filled_mode;
    }
  else
    {
      fprintf(stderr,
  ...
  if (! graph_mode_str)
    {
      st->graph_mode= graph_none;
    }
  else if (! strcmp(graph_mode_str, "x"))
    {
      st->graph_mode= graph_x;
    }
  else if (! strcmp(graph_mode_str, "y"))
    {
      st->graph_mode= graph_y;
    }
  else if (! strcmp(graph_mode_str, "both"))
    {
      st->graph_mode= graph_both;
    }
  else if (! strcmp(graph_mode_str, "speed"))
    {
      st->graph_mode= graph_speed;
    }
  else if (! strcmp(graph_mode_str, "none"))
    {
      st->graph_mode= graph_none;
    }
  else
      ...
      switch (st->mode)
        {
            ...
            if (st->glow_p)
              {
                ...
                make_color_ramp(xgwa.screen, xgwa.visual, cmap, H, S1, V, H, S2, V, st->colors, &st->ncolors, False, True, False);
              }
            else
              {
                st->ncolors= st->npoints;
                st->colors= (XColor *) malloc(sizeof(*st->colors) * (st->ncolors + 1));
                make_random_colormap(xgwa.screen, xgwa.visual, cmap, st->colors, &st->ncolors, True, True, False, True);
              }
            break;
          case line_mode :
          case polygon_mode :
          ...
          case tail_mode :
            st->colors= (XColor *) malloc(sizeof(*st->colors) * (st->ncolors + 1));
            make_smooth_colormap(xgwa.screen, xgwa.visual, cmap, st->colors, &st->ncolors, True, False, True);
            break;
          default :
            abort();
        }
  ...
  gcv.line_width= (st->mode == tail_mode ? (st->global_size ? st->global_size : (MAX_SIZE * 2 / 3)) : 1);
    ...
    int *y)
{
  ...
  unsigned int mask;
  if (XQueryPointer(dpy, win, &root1, &child1, &root_x, &root_y, &mouse_x, &mouse_y, &mask))
    {
      *x= mouse_x;
      *y= mouse_y;
    }
          ...
          if (st->cbounce_p) /* with correct bouncing */
            {
              ...
              while (bounce_allowed && ((st->balls [ i ].x >= (st->xlim - st->balls [ i ].size)) || (st->balls [ i ].y >= (st->ylim - st->balls [ i ].size)) || (st->balls [ i ].x <= 0) || (st->balls [ i ].y <= 0)))
                {
                  bounce_allowed--;
          ...
          if (st->mode == ball_mode)
            {
              ...
              XSetForeground(dpy, st->draw_gc, (i == st->mouse_ball ? st->mouse_pixel : st->colors [ st->balls [ i ].pixel_index ].pixel));
            }
        }

      if (st->mode == ball_mode)
        {
          XFillArc(dpy, window, st->erase_gc, (int) old_x, (int) old_y, size, size, 0, 360 * 64);
          XFillArc(dpy, window, st->draw_gc, (int) new_x, (int) new_y, size, size, 0, 360 * 64);
        }
      else
        {
  ...
  switch (st->mode)
    {
      ...
      case line_mode :
        if (st->segments > 0)
          XDrawLines(dpy, window, st->erase_gc, st->point_stack + st->point_stack_fp, st->npoints + 1, CoordModeOrigin);
        XDrawLines(dpy, window, st->draw_gc, st->point_stack + last_point_stack_fp, st->npoints + 1, CoordModeOrigin);
        break;
      case polygon_mode :
        if (st->segments > 0)
          XFillPolygon(dpy, window, st->erase_gc, st->point_stack + st->point_stack_fp, st->npoints + 1, (st->npoints == 3 ? Convex : Complex), CoordModeOrigin);
        XFillPolygon(dpy, window, st->draw_gc, st->point_stack + last_point_stack_fp, st->npoints + 1, (st->npoints == 3 ? Convex : Complex), CoordModeOrigin);
        break;
      case tail_mode :
        {
          for (i= 0; i < st->npoints; i++)
            {
              ...
              if (st->no_erase_yet == 1)
                {
                  if (st->total_ticks >= st->segments)
                    {
                      st->no_erase_yet= 0;
                      XDrawLine(dpy, window, st->erase_gc, st->point_stack [ index ].x + radius, st->point_stack [ index ].y + radius, st->point_stack [ next_index ].x + radius, st->point_stack [ next_index ].y + radius);
                    }
                }
              else
                {
              ...
              if (st->point_stack [ next_index ].x == 0 &&
                ...
                continue;
              XDrawLine(dpy, window, st->draw_gc, st->point_stack [ index ].x + radius, st->point_stack [ index ].y + radius, st->point_stack [ next_index ].x + radius, st->point_stack [ next_index ].y + radius);
            }
        }
        break;
      case spline_mode :
      case spline_filled_mode :
        {
          ...
          if (st->segments > 0)
            {
              ...
              if (st->mode == spline_filled_mode)
                XFillPolygon(dpy, window, st->erase_gc, st->spl->points, st->spl->n_points, (st->spl->n_points == 3 ? Convex : Complex), CoordModeOrigin);
              else
                XDrawLines(dpy, window, st->erase_gc, st->spl->points, st->spl->n_points, CoordModeOrigin);
            }
          for (i= 0; i < st->npoints; i++)
            {
          ...
          compute_closed_spline(st->spl);
          if (st->mode == spline_filled_mode)
            XFillPolygon(dpy, window, st->draw_gc, st->spl->points, st->spl->n_points, (st->spl->n_points == 3 ? Convex : Complex), CoordModeOrigin);
          else
            XDrawLines(dpy, window, st->draw_gc, st->spl->points, st->spl->n_points, CoordModeOrigin);
        }
        break;
      default :
        abort();
