    ...
    Window window)
{
  ...
  st->buffer= 0;
#else
  st->buffer= XCreatePixmap(st->dpy, st->window, st->sizex, st->sizey, st->depth);
#endif
  ...
  st->img_loader= load_image_async_simple(0, xgwa.screen, st->window, st->pm, 0, &st->geom);
  ...
  XFillArc(st->dpy, clip_pm, clip_gc, st->radius, st->radius, st->radius * 2, st->radius * 2, 0, 360 * 64);
    ...
    struct state *st)
{
  ...
  if (! st->img_loader &&
    ...
    {
      st->img_loader= load_image_async_simple(0, st->screen, st->window, st->pm, 0, &st->geom);
      return;
    }
...
#define nrnd(x) (random() % (x))
  ...
  /* find new x,y */
  st->x= st->geom.x +
    ((1 + sin(((double) now) / X_PERIOD * 2. * M_PI)) / 2.0) * (st->geom.width - st->s / 2) - st->s / 4;
  st->y= st->geom.y +
    ((1 + sin(((double) now) / Y_PERIOD * 2. * M_PI)) / 2.0) * (st->geom.height - st->s / 2) - st->s / 4;
