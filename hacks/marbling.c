...
static INLINE v_hi IMUL_HI(
  v_hi a,
  v_hi b)
{
  return IMUL_HI_OP(a, b);
}
static INLINE v_uhi MUL_HI(
  v_uhi a,
  v_uhi b)
{
  return (v_uhi) MUL_HI_OP((v_hi) a, (v_hi) b);
}
    ...
    v_uhi t)
{
  ...
  v_hi it= (v_hi) t;
  v_hi iret=
    vqdmulhq_s16(it2, 10 * it - vqdmulhq_s16(it2, (int16_t) (15 * F) - it * 6)) << (noise_work_bits - noise_in_bits + 1);
  return (v_uhi) iret;
#else
  v_uhi t2= t * t;
  return MUL_HI(t2, 10 * t - MUL_HI(t2, (uint16_t) (15 * F) - t * 6)) << (noise_work_bits - noise_in_bits + 1);
#endif
}
    ...
    v_uhi z)
{
  ...
  return (v_uhi) SCALE(LERP(w, LERP(v, LERP(u, c0, c1), /* AND ADD BLENDED */
    ...
    struct state *st)
{
  ...
  make_smooth_colormap(xgwa.screen, xgwa.visual, st->cmap, st->colors, &st->ncolors, True, 0, False);
}
    ...
    struct state *st)
{
  ...
  st->h= st->h / g;
  st->image= create_xshm_image(st->dpy, xgwa.visual, xgwa.depth, ZPixmap, &st->shm_info, ((st->w * g * bpp + align) & ~align) / bpp, st->h * g);
}
    ...
    void *closure)
{
  ...
  put_xshm_image(st->dpy, st->window, st->gc, st->image, 0, 0, 0, 0, st->image->width, st->image->height, &st->shm_info);
    ...
    void *closure)
{
  ...
  destroy_xshm_image(st->dpy, st->image, &st->shm_info);
  free_colors(DefaultScreenOfDisplay(st->dpy), st->cmap, st->colors, st->ncolors);
