    ...
    GC fgc)
{
  switch (mode)
    {
      case spin_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) spin_explanation, strlen(spin_explanation));
        break;
      case funky_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) funky_explanation, strlen(funky_explanation));
        break;
      case circle_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) circle_explanation, strlen(circle_explanation));
        break;
      case linear_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) linear_explanation, strlen(linear_explanation));
        break;
      case test_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) test_explanation, strlen(test_explanation));
        break;
      case fun_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) fun_explanation, strlen(fun_explanation));
        break;
      case innie_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) innie_explanation, strlen(innie_explanation));
        break;
      case lissajous_mode :
        XDrawString(st->dpy, st->window, st->fgc, 50, offset, (char *) lissajous_explanation, strlen(linear_explanation));
    }
}
    ...
    Window window)
{
...

  {
    ...
    make_uniform_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, True, &writable_p, True);
  }
  ...
  if (! st->xmode_str)
    st->xmode= spin_mode;
  else if (! strcmp(st->xmode_str, "spin"))
    st->xmode= spin_mode;
  else if (! strcmp(st->xmode_str, "funky"))
    st->xmode= funky_mode;
  else if (! strcmp(st->xmode_str, "circle"))
    st->xmode= circle_mode;
  else if (! strcmp(st->xmode_str, "linear"))
    st->xmode= linear_mode;
  else if (! strcmp(st->xmode_str, "test"))
    st->xmode= test_mode;
  else if (! strcmp(st->xmode_str, "fun"))
    st->xmode= fun_mode;
  else if (! strcmp(st->xmode_str, "innie"))
    st->xmode= innie_mode;
  else if (! strcmp(st->xmode_str, "lissajous"))
    st->xmode= lissajous_mode;
  else
    st->xmode= random() % (int) lissajous_mode;
  if (! st->ymode_str)
    st->ymode= spin_mode;
  else if (! strcmp(st->ymode_str, "spin"))
    st->ymode= spin_mode;
  else if (! strcmp(st->ymode_str, "funky"))
    st->ymode= funky_mode;
  else if (! strcmp(st->ymode_str, "circle"))
    st->ymode= circle_mode;
  else if (! strcmp(st->ymode_str, "linear"))
    st->ymode= linear_mode;
  else if (! strcmp(st->ymode_str, "test"))
    st->ymode= test_mode;
  else if (! strcmp(st->ymode_str, "fun"))
    st->ymode= fun_mode;
  else if (! strcmp(st->ymode_str, "innie"))
    st->ymode= innie_mode;
  else if (! strcmp(st->ymode_str, "lissajous"))
    st->ymode= lissajous_mode;
  else
    ...
    void *closure)
{
  ...
  if (st->explaining == 1)
    {
      ...
      draw_explain_string(st, st->xmode, st->info->half_height - 100, st->dpy, st->window, st->fgc);
      st->explaining++;
      return 3000000;
    }
  ...
  if (st->backb)
    ...
    if (st->dbuf)
      {
        XCopyArea(st->dpy, st->b, st->window, st->bgc, 0, 0, st->xgwa.width, st->xgwa.height, 0, 0);
      }
