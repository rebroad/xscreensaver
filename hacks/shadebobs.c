  ...
  SShadeBob *pShadeBob)
{
  ...
  */
  if (pShadeBob->nAngle + 0.5 >= st->iDegreeCount)
    pShadeBob->nAngle-= st->iDegreeCount;
  else if (pShadeBob->nAngle < -0.5)
    pShadeBob->nAngle+= st->iDegreeCount;
  ...
  if (pShadeBob->nPosX >= st->iWinWidth)
    pShadeBob->nPosX-= st->iWinWidth;
  else if (pShadeBob->nPosX < 0)
    pShadeBob->nPosX+= st->iWinWidth;

  if (pShadeBob->nPosY >= st->iWinHeight)
    pShadeBob->nPosY-= st->iWinHeight;
  else if (pShadeBob->nPosY < 0)
    pShadeBob->nPosY+= st->iWinHeight;
}
  ...
  SShadeBob *pShadeBob)
{
  ...
  XPutImage(st->dpy, st->window, st->gc, st->pImage, pShadeBob->nPosX, pShadeBob->nPosY, pShadeBob->nPosX, pShadeBob->nPosY, st->iBobDiameter, st->iBobDiameter);
}
...
static unsigned long *SetPalette(
  struct state *st)
{
  ...
  struct state *st)
{
  ...
  st->pImage= XCreateImage(st->dpy, XWinAttribs.visual, XWinAttribs.depth, ZPixmap, 0, NULL, XWinAttribs.width, XWinAttribs.height, 8 /*BitmapPad( st->dpy )*/, 0);
  ...
  if (st->iDegreeCount == 0)
    st->iDegreeCount= (XWinAttribs.width / 6) + 400;
  else if (st->iDegreeCount < 90)
    st->iDegreeCount= 90;
  else if (st->iDegreeCount > 5400)
    st->iDegreeCount= 5400;
