    ...
    int h)
{
  ...
  scale= (fromw > fromh ? (float) fromw / w : (float) fromh / h);
              ...
              for (yy= yy1; yy < yy2; yy++)
                {
                  ...
                  unsigned long sp= ((xx > in->width || yy > in->height) ? 0 : XGetPixel(in, xx, yy));
                  if (colors)
                    {
...
static void slideshow_controller(
  apple2_sim_t *sim,
  int *stepno,
  ...
  switch (*stepno)
    {
      ...
      case 10 :
        {
          ...
          p= XCreatePixmap(sim->dpy, sim->window, xgwa.width, xgwa.height, xgwa.depth);
      ...
      case 40 :
        ...
        for (i= 0; i < 6 && mine->render_img_lineno < 192; i++)
          a2_display_image_loading(st, mine->render_img, mine->render_img_lineno++);
    ...
    struct terminal_controller_data *mine)
{
  ...
  if (t < 0) t= 0;
  sprintf(buf, "%.30s %d:%02d:%02d.%1d", COUNTDOWN_BANNER, (int) (t / 60 / 60), (int) (t / 60) % 60, (int) t % 60, (int) (t * 10) % 10);
  x= (w - strlen(buf)) / 2;
  y= 0;
  s= buf;
  while (*s && x < w)
...
static void
  a2_vt100_printc(
    apple2_sim_t *sim,
    struct terminal_controller_data *state,
    unsigned char c)
{
  apple2_state_t *st= sim->st;
  int w= SCREEN_COLS;
      ...
      for (x= 0; x < w; x++)
        {
          ...
          if (flag & TTY_SYMBOLS)
            ...
            {
              a2_ascii_printc(st, ascii, 0, flag & TTY_BLINK, inv_p, False);
            }
        }
    }
...
static void (*const controllers [])(apple2_sim_t *sim, int *stepno, double *next_actiontime)= {
