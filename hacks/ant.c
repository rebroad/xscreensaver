  ...
  if (ap->neighbors == 6)
    {
      ...
      if (ap->xs == 1 && ap->ys == 1)
        XDrawPoint(MI_DISPLAY(mi), MI_WINDOW(mi), gc, ap->shape.hexagon [ 0 ].x, ap->shape.hexagon [ 0 ].y);
      else
        XFillPolygon(MI_DISPLAY(mi), MI_WINDOW(mi), gc, ap->shape.hexagon, 6, Convex, CoordModePrevious);
    }
  else if (ap->neighbors == 4 || ap->neighbors == 8)
    {
      XFillRectangle(MI_DISPLAY(mi), MI_WINDOW(mi), gc, ap->xb + ap->xs * col, ap->yb + ap->ys * row, ap->xs - (ap->xs > 3), ap->ys - (ap->ys > 3));
    }
  else
    {                              /* TRI */
      ...
      if (ap->xs <= 3 || ap->ys <= 3)
        {
          XDrawPoint(MI_DISPLAY(mi), MI_WINDOW(mi), gc, ((orient) ? -1 : 1) + ap->shape.triangle [ orient ][ 0 ].x, ap->shape.triangle [ orient ][ 0 ].y);
        }
      else
        {
          if (orient)
            ...
            ap->shape.triangle [ orient ][ 0 ].x-= (ap->xs / 2 - 1);
          XFillPolygon(MI_DISPLAY(mi), MI_WINDOW(mi), gc, ap->shape.triangle [ orient ], 3, Convex, CoordModePrevious);
        }
    }
}
    ...
    int truchetstate)
{
  ...
  if (ap->neighbors == 6)
          ...
          for (side= 0; side < 6; side++)
            {
              ...
              if (truchetstate == side % 2)
                XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), hex.x, hex.y, ap->xs, ap->ys, ((570 - (side * 60) + fudge) % 360) * 64, (120 - 2 * fudge) * 64);
            }
        }
      else
        {
              ...
              if (truchetstate == side % 3)
                {
                  ...
                  XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), hex2.x, hex2.y, (int) ((double) ap->xs * 1.5), (int) ((double) ap->ys * 1.5), ((555 - (side * 60)) % 360) * 64, 90 * 64);
                }
            }
        }
    }
  else if (ap->neighbors == 4)
    {
      if (truchetstate)
        {
          XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), ap->xb + ap->xs * col - ap->xs / 2 + 1, ap->yb + ap->ys * row + ap->ys / 2 - 1, ap->xs - 2, ap->ys - 2, 0 * 64, 90 * 64);
          XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), ap->xb + ap->xs * col + ap->xs / 2 - 1, ap->yb + ap->ys * row - ap->ys / 2 + 1, ap->xs - 2, ap->ys - 2, -90 * 64, -90 * 64);
        }
      else
        {
          XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), ap->xb + ap->xs * col - ap->xs / 2 + 1, ap->yb + ap->ys * row - ap->ys / 2 + 1, ap->xs - 2, ap->ys - 2, 0 * 64, -90 * 64);
          ...
          if (truchetstate == side)
            {
              ...
              XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), (int) (tri.x - ap->xs * fudge2 / 2), (int) (tri.y - 3 * ap->ys * fudge2 / 4), (unsigned int) (ap->xs * fudge2), (unsigned int) (3 * ap->ys * fudge2 / 2), (ang + fudge) * 64, (60 - 2 * fudge) * 64);
            }
        }
    }
}
    ...
    unsigned char color)
{
  ...
  if (! color)
  ...
  else if (MI_NPIXELS(mi) <= 2)
    {
      ...
      XChangeGC(MI_DISPLAY(mi), ap->stippledGC, GCStipple | GCForeground | GCBackground, &gcv);
      gc= ap->stippledGC;
#endif /* !DO_STIPPLE */
    }
  else
    {
      XSetForeground(MI_DISPLAY(mi), MI_GC(mi), MI_PIXEL(mi, ap->colors [ color - 1 ]));
      gc= MI_GC(mi);
    }
      ...
      if (ap->neighbors == 6)
      ...
      else if (ap->neighbors == 4 || ap->neighbors == 8)
        {
          ...
          switch (direction)
            {
              case 0 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * (col + 1) - 3, ap->yb + ap->ys * row + ap->ys / 2 - 2);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * (col + 1) - 3, ap->yb + ap->ys * row + ap->ys / 2);
                break;
              case 45 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * (col + 1) - 4, ap->yb + ap->ys * row + 1);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * (col + 1) - 3, ap->yb + ap->ys * row + 2);
                break;
              case 90 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + ap->xs / 2 - 2, ap->yb + ap->ys * row + 1);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + ap->xs / 2, ap->yb + ap->ys * row + 1);
                break;
              case 135 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + 2, ap->yb + ap->ys * row + 1);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + 1, ap->yb + ap->ys * row + 2);
                break;
              case 180 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + 1, ap->yb + ap->ys * row + ap->ys / 2 - 2);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + 1, ap->yb + ap->ys * row + ap->ys / 2);
                break;
              case 225 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + 2, ap->yb + ap->ys * (row + 1) - 3);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + 1, ap->yb + ap->ys * (row + 1) - 4);
                break;
              case 270 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + ap->xs / 2 - 2, ap->yb + ap->ys * (row + 1) - 3);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * col + ap->xs / 2, ap->yb + ap->ys * (row + 1) - 3);
                break;
              case 315 :
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * (col + 1) - 4, ap->yb + ap->ys * (row + 1) - 3);
                XDrawPoint(display, window, MI_GC(mi), ap->xb + ap->xs * (col + 1) - 3, ap->yb + ap->ys * (row + 1) - 4);
                break;
              default :
      ...
      if (ap->stippledGC == None)
        {
          ...
          if ((ap->stippledGC= XCreateGC(display, window, GCFillStyle, &gcv)) == None)
            {
              free_ant(mi);
              return;
            }
  ...
  if (ap->neighbors == 6)
    {
      ...
      if (size < -MINSIZE)
        {
          ap->ys= NRAND(MIN(-size, MAX(MINSIZE, MIN(ap->width, ap->height) / MINGRIDSIZE)) - MINSIZE + 1) + MINSIZE;
        ...
        {
          ap->ys= MIN(size, MAX(MINSIZE, MIN(ap->width, ap->height) / MINGRIDSIZE));
        }
      ap->xs= ap->ys;
      ...
      for (i= 0; i < REDRAWSTEP; i++)
        {
          if (ap->tape [ ap->redrawpos ] ||
            ...
            {
              drawcell(mi, ap->redrawpos % ap->ncols, ap->redrawpos / ap->ncols, ap->tape [ ap->redrawpos ]);
              if (ap->truchet)
                drawtruchet(mi, ap->redrawpos % ap->ncols, ap->redrawpos / ap->ncols, ap->tape [ ap->redrawpos ], ap->truchet_state [ ap->redrawpos ] - 1);
            }
