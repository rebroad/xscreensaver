...
static argtype vars []=
  {
    {&tail_len, "eulertail", "EulerTail", (char *) DEF_EULERTAIL, t_Int},
    {&power, "eulerpower", "EulerPower", "1", t_Float},
};
...
ModStruct euler2d_description= {
  ...
  1.0,
  "",
  "Simulates 2D incompressible invisid fluid.",
  0,
  NULL};
    ...
    euler2dstruct *sp)
{
  ...
  for (j= (sp->hide_vortex ? sp->Nvortex : 0); j < sp->N; j++)
    {
      if (! sp->dead [ j ])
        {
          p1= 0;
...
#define nr_rotates 18 /* how many rotations to try to fill as much of screen as possible - must be even number */
  ...
  if (sp->width > 2560 || sp->height > 2560) /* Retina displays */
    XSetLineAttributes(MI_DISPLAY(mi), MI_GC(mi), 3, LineSolid, CapRound, JoinRound);
  ...
  for (i= sp->Nvortex; i < sp->N; i++)
    {
      ...
      while (variable_boundary &&
        calc_mod_dp2(sp->x [ 2 * i + 0 ], sp->x [ 2 * i + 1 ], sp->p_coef) < positive_rand(4));
    }
  ...
  if (sp->count)
    {
      ...
      if (MI_NPIXELS(mi) > 2)
        ...
        { /* render mono */
          XSetForeground(display, gc, MI_WHITE_PIXEL(mi));
          XDrawSegments(display, window, gc, sp->csegs, sp->cnsegs);
        }
      ...
      if (variable_boundary)
        XDrawSegments(display, window, gc, sp->boundary, n_bound_p);
      else
        XDrawArc(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), sp->width / 2 - (int) sp->radius - 1, sp->height / 2 - (int) sp->radius - 1, (int) (2 * sp->radius) + 2, (int) (2 * sp->radius) + 2, 0, 64 * 360);
