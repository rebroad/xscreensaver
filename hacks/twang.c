    ...
    XWindowAttributes *xwa)
{
  ...
  st->pm= XCreatePixmap(st->dpy, st->window, xwa->width, xwa->height, xwa->depth);

  st->start_time= time((time_t *) 0);
  st->img_loader= load_image_async_simple(0, xwa->screen, st->window, st->pm, 0, 0);
}
    ...
    struct state *st)
{
  ...
  st->sourceImage= XGetImage(st->dpy, st->pm, 0, 0, st->windowWidth, st->windowHeight, ~0L, ZPixmap);

  if (st->workImage) destroy_xshm_image(st->dpy, st->workImage, &st->shmInfo);

  st->workImage= create_xshm_image(st->dpy, xwa.visual, xwa.depth, ZPixmap, &st->shmInfo, st->windowWidth, st->windowHeight);
}
  ...
  struct state *st)
{
  ...
  gcv.foreground= get_pixel_resource(st->dpy, xgwa.colormap, "borderColor", "BorderColor");
  st->borderPixel= gcv.foreground;
  st->borderGC= XCreateGC(st->dpy, st->window, GCForeground | GCLineWidth, &gcv);
...
static void randomizeAllAngularVelocities(
  struct state *st)
{
  int c;
  int r;
...
static void randomizeAllZoomularVelocities(
  struct state *st)
{
  int c;
  int r;
...
static void randomizeAllVelocities(
  struct state *st)
{
...
static void randomizeAllAngularOrientations(
  struct state *st)
{
  int c;
  int r;
...
static void randomizeAllZoomularOrientations(
  struct state *st)
{
  int c;
  int r;
...
static void randomizeAllOrientations(
  struct state *st)
{
...
static void randomizeEverything(
  struct state *st)
{
...
static void randomizeOneColumn(
  struct state *st)
{
  int c= RAND_FLOAT_01 * st->columns;
  int r;
      ...
      for (c= 0; c < st->columns; c++)
        {
          ...
          if (va > MAX_VANGLE)
            va= MAX_VANGLE;
          else if (va < -MAX_VANGLE)
            va= -MAX_VANGLE;
          t->vAngle= va;

          if (vz > MAX_VZOOM)
            vz= MAX_VZOOM;
          else if (vz < -MAX_VZOOM)
            vz= -MAX_VZOOM;
          t->vZoom= vz;
        }
    }
 ...
 * closures*!) */
static int sortTilesComparator(
  const void *v1,
  const void *v2)
{
  Tile *t1= *(Tile **) v1;
  ...
  struct state *st)
{
  ...
  memset(st->workImage->data, 0, st->workImage->bytes_per_line * st->workImage->height);
  ...
  put_xshm_image(st->dpy, st->window, st->backgroundGC, st->workImage, 0, 0, 0, 0, st->windowWidth, st->windowHeight, &st->shmInfo);
}
