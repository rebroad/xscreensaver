...
static void
  DisplayImage(
    ...
    int h)
{
  put_xshm_image(st->dpy, st->window, st->gc, st->buffer_map, x, y, x, y, w, h, &st->shm_info);
}
    ...
    struct state *st)
{
  ...
  if (st->height && st->orig_map->data)
    memcpy(st->buffer_map->data, st->orig_map->data, st->height * st->buffer_map->bytes_per_line);
    ...
    void *closure)
{
  ...
  if (st->img_loader) /* still loading */
    {
      ...
      if (! st->img_loader)
        { /* just finished */
          ...
          st->orig_map= XGetImage(st->dpy, st->pm, 0, 0, st->width, st->height, ~0L, ZPixmap);
          init_hack(st);
        }
      return st->delay;
    }
  ...
  if (! st->img_loader &&
    st->start_time + st->duration < time((time_t *) 0))
    {
      ...
      st->pm= XCreatePixmap(st->dpy, st->window, xgwa.width, xgwa.height, xgwa.depth);
  ...
  for (i= 0; i < st->num_zoom; i++)
    DisplayImage(st, st->zoom_box [ i ]->x, st->zoom_box [ i ]->y, st->zoom_box [ i ]->w, st->zoom_box [ i ]->h);
    ...
    struct state *st)
{
  ...
  st->buffer_map= create_xshm_image(st->dpy, xgwa.visual, depth, ZPixmap, &st->shm_info, st->width, st->height);
}
