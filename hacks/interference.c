...
static double fast_inv_table(
  unsigned x)
{
  return x < (FAST_SQRT_CUTOFF >> FAST_SQRT_DISCARD_BITS1) ?
    (x << FAST_SQRT_DISCARD_BITS1) :
    ((x - (FAST_SQRT_CUTOFF >> FAST_SQRT_DISCARD_BITS1)) << FAST_SQRT_DISCARD_BITS2) + FAST_SQRT_CUTOFF;
}
...
static void abort_no_mem(
  void)
{
  abort_on_error(ENOMEM);
}

static void check_no_mem(
  Display *dpy,
  struct inter_context *c,
  void *ptr)
{
...
static void inter_thread_destroy(
  void *self_raw)
{
  struct inter_thread *self= (struct inter_thread *) self_raw;
#ifdef USE_XIMAGE
  ...
  const XWindowAttributes *xgwa)
{
  ...
  c->ximage= create_xshm_image(dpy, xgwa->visual, xgwa->depth, ZPixmap, &c->shm_info, wbits / c->bits_per_pixel, h);
  ...
  struct inter_context *c)
{
  ...
  if (! mono)
    {
      ...
      make_color_loop(c->screen, xgwa.visual, c->cmap, H [ 0 ], S [ 0 ], V [ 0 ], H [ 1 ], S [ 1 ], V [ 1 ], H [ 2 ], S [ 2 ], V [ 2 ], c->pal, &(c->colors), True, False);
      if (c->colors < 2)
  ...
  if (mono)
    { /* DON'T else this with the previous if! */
      c->colors= 2;
  ...
  for (i= 0; i < c->radius; i++)
    {
      ...
      max=
        ...
        ((float) radius);
      c->wave_height [ i ]=
        (unsigned) ((max + max * cos(fi / (50.0 * scale))) / 2.0);
    }
...
#ifdef USE_XIMAGE
  put_xshm_image(c->dpy, c->win, c->copy_gc, c->ximage, 0, 0, 0, 0, c->ximage->width, c->ximage->height, &c->shm_info);
...
#endif /* HAVE_DOUBLE_BUFFER_EXTENSION */
    if (c->pix_buf)
      XCopyArea(c->dpy, c->pix_buf, c->win, c->copy_gc, 0, 0, c->w, c->h, 0, 0);
