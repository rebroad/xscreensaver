    ...
    Window window)
{
  ...
  st->img_loader= load_image_async_simple(0, xgwa.screen, st->window, st->pm, 0, 0);
  ...
  if (st->lenses)
    st->s= MAX((nblocksx - 1) * st->lensoffsetx + st->pixwidth,
             (nblocksy - 1) * st->lensoffsety + st->pixheight) *
      2;
  else
    ...
    void *closure)
{
  ...
  if (! st->img_loader &&
    ...
    {
      st->img_loader= load_image_async_simple(0, st->screen, st->window, st->pm, 0, 0);
      return st->delay;
    }
...
#define nrnd(x) (random() % (x))
  ...
  /* find new x,y */
  st->tlx= ((1. + sin(((double) now) / X_PERIOD * 2. * M_PI)) / 2.0) * (st->sizex - st->s / 2) /* -s/4 */ + MINX;
  st->tly= ((1. + sin(((double) now) / Y_PERIOD * 2. * M_PI)) / 2.0) * (st->sizey - st->s / 2) /* -s/4 */ + MINY;

  if (st->lenses)
    {
      for (x= i= 0; x < st->sizex; x+= (st->pixwidth + st->pixspacex), ++i)
        for (y= j= 0; y < st->sizey; y+= (st->pixheight + st->pixspacey), ++j)
          XCopyArea(st->dpy, st->pm /* src */, st->window /* dest */, st->window_gc, st->tlx + i * st->lensoffsetx /* src_x */, st->tly + j * st->lensoffsety /* src_y */, st->pixwidth, st->pixheight, x /* dest_x */, y /* dest_y */);
    }
  else
    {
      for (x= i= 0; x < st->sizex; x+= (st->pixwidth + st->pixspacex), ++i)
          ...
          for (y= j= 0; y < st->sizey; y+= (st->pixheight + st->pixspacey), ++j)
            {
              ...
              XFillRectangle(st->dpy, st->window, st->window_gc, i * (st->pixwidth + st->pixspacex), j * (st->pixheight + st->pixspacey), st->pixwidth, st->pixheight);
            }
        }
    }
