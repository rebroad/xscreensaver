...
{
    int dx, dy;
} dirvecs [ DIRVECS ]= {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
    ...
    ghoststruct *g)
{
  ...
  for (i= 0; i < DIRVECS; i++)
    {
      /* remove opposite */
      if (g->lastbox != NOWHERE && i == (g->lastbox + 2) % DIRVECS && (g->aistate != goingout))
        {
          posdirs [ i ]= 0;
          continue;
        }
      ...
      if ((posdirs [ i ]=
              pacman_check_pos(pp, g->row + dirvecs [ i ].dy, g->col + dirvecs [ i ].dx, can_go_in)) == True)
        nrdirs++;
    }
          ...
          if (pacman_check_dot(pp, x, y) == 1)
            {
              ...
              if (dist > top)
                top= dist;
              *vx+= (dx * ((long) LEVWIDTH * (long) LEVHEIGHT)) / dist;
              *vy+= (dy * ((long) LEVWIDTH * (long) LEVHEIGHT)) / dist;
            }
        }
    }
}
  ...
  for (g= 0; g < pp->nghosts; g++)
    {
      if (pp->ghosts [ g ].dead == True ||
        ...
        continue;
      dx= pp->ghosts [ g ].col + /*dirvecs[pp->ghosts[g].lastbox].dx */ -p->col;
      dy= pp->ghosts [ g ].row + /*dirvecs[pp->ghosts[g].lastbox].dy */ -p->row;
      dist= dx * dx + dy * dy;
      if (dist < closest)
        {
          closest= dist;
  ...
  for (i= 0; i < DIRVECS; i++)
    {
      ...
      if (p->justate == 0 &&
        ...
        posdirs [ i ]= 0;
                   ...
                   pacman_check_pos(pp, p->row + dirvecs [ i ].dy, p->col + dirvecs [ i ].dx, 0)) == 1)
        nrdirs++;
    }
  p->state_change= 0;
  ...
  for (i= 0; i < DIRVECS; i++)
    {
      ...
      if (pacman_check_dot(pp, p->col + dirvecs [ i ].dx, p->row + dirvecs [ i ].dy) == 1)
        {
          if (dotfound == 0)
            {
              highest= score;
    ...
    pacmanstruct *p)
{
  ...
  (void) XQueryPointer(MI_DISPLAY(mi), MI_WINDOW(mi), &r, &c, &dx, &dy, &cx, &cy, &m);
