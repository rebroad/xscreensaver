    ...
    struct state *st)
{
  ...
  st->xim= create_xshm_image(st->dpy, st->visual, st->depth, ZPixmap, &st->shminfo, st->width, st->height);
  if (! st->xim)
    {
...
static void
  DisplayImage(
    struct state *st)
{
  put_xshm_image(st->dpy, st->window, st->gc, st->xim, 0, (st->top - 1) << 1, 0, (st->top - 1) << 1, st->width, st->height - ((st->top - 1) << 1), &st->shminfo);
}
  ...
  if ((xx >= 0) &&
  ...
  else
    {
      ...
      if (! warned)
        {
          fprintf(stderr,
            "%s: st->window is %dx%d; image must be " "smaller than %dx%d (not %dx%d).\n",
            progname,
            st->width,
            st->height,
            st->fwidth,
...
{
  int x, y;
  XImage *out= XCreateImage(dpy, visual, image->depth, ZPixmap, 0, 0, image->width * 2, image->height * 2, 8, 0);
    ...
    struct state *st)
{
  ...
  if (! bitmap_name ||
    ...
    image= image_data_to_ximage(st->dpy, st->visual, bob_png, sizeof(bob_png));
  else
      ...
      for (x= 0; x < image->width; x++)
        {
          ...
          unsigned long gray= (a == 0 ? 0xFF : ((((agbr >> 16) & 0xFF) + ((agbr >> 8) & 0xFF) + ((agbr >> 0) & 0xFF)) / 3));
    ...
    void *closure)
{
  ...
  if (st->theim)
    FlamePasteData(st, st->theim, (st->fwidth - st->theimx) / 2, st->fheight - st->theimy - st->baseline, st->theimx, st->theimy);
