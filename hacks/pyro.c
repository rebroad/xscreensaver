    ...
    int g)
{
  ...
  if (! mono_p)
    {
      hsv_to_rgb(random() % 360, 1.0, 1.0, &p->color.red, &p->color.green, &p->color.blue);
    ...
    const void *b)
{
  ...
  if (pa->color.pixel == pb->color.pixel)
    return 0;
  else if (pa->color.pixel < pb->color.pixel)
    return -1;
  else
    return 1;
}
      ...
      if (old_size > 0)
        {
          if (old_size == 1)
            ...
            XFillRectangle(st->dpy, st->window, st->erase_gc, old_x, old_y, old_size, old_size);
        }
