    ...
    XErrorEvent *error)
{
  if (error->error_code == BadWindow || error->error_code == BadDrawable)
    {
      fprintf(stderr, "%s: target %s 0x%lx unexpectedly deleted\n", blurb(), (error->error_code == BadWindow ? "window" : "pixmap"), (unsigned long) error->resourceid);
    }
  else
    {
      fprintf(stderr, "\nX error in %s:\n", blurb());
    ...
    Window window)
{
  ...
  if (parent == root)
  ...
  else
    {
      ...
      */
      if (XGetWindowProperty(dpy, window, XInternAtom(dpy, "WM_STATE", True), 0, 0, False, AnyPropertyType, &type, &format, &nitems, &bytesafter, (unsigned char **) &data) == Success && type != None)
        return True;
    }
...
static void
  compute_image_scaling(
    ...
    int dest_h,
    Bool verbose_p,
    int *scaled_from_x_ret,
    int *scaled_from_y_ret,
    int *scaled_to_x_ret,
    int *scaled_to_y_ret,
    int *scaled_w_ret,
    int *scaled_h_ret)
  ...
  if (! exact_fit_p) /* scale the image up or down */
    {
      ...
      if (dest_w > dest_h * 5 || dest_h > dest_w * 5)
        {
          double r2= (dest_w > dest_h ? dest_w / (double) dest_h : dest_h / (double) dest_w);
          r*= r2;
          if (verbose_p)
    ...
    int logo_size)
{
  ...
  int npixels;
  Pixmap logo= xscreensaver_logo(screen, visual, drawable, cmap, BlackPixelOfScreen(screen), &pixels, &npixels, &mask, logo_size);
    ...
    int new_height)
{
  ...
  XImage *ximage2= XCreateImage(dpy, visual, depth, ZPixmap, 0, 0, new_width, new_height, 8, 0);
  ximage2->data= (char *) calloc(ximage2->height, ximage2->bytes_per_line);

  if (! ximage2->data)
    {
      fprintf(stderr, "%s: out of memory scaling %dx%d image to %dx%d\n", blurb(), ximage->width, ximage->height, ximage2->width, ximage2->height);
    ...
    XRectangle *geom_ret)
{
  ...
  /* Find the size of the Drawable. */
  {
    ...
    int x, y;
    unsigned int bw;
    XGetGeometry(dpy, drawable, &root, &x, &y, &win_width, &win_height, &bw, &win_depth);
  }
  ...
  if (! pb)
  ...
  else
    {
      ...
      compute_image_scaling(w, h, win_width, win_height, verbose_p, &srcx, &srcy, &destx, &desty, &w2, &h2);
      if (w != w2 || h != h2)
        {
          GdkPixbuf *pb2= gdk_pixbuf_scale_simple(pb, w2, h2, GDK_INTERP_BILINEAR);
          if (pb2)
            {
              g_object_unref(pb);
              pb= pb2;
      ...
      gdk_pixbuf_xlib_render_to_drawable_alpha(pb, drawable, srcx, srcy, destx, desty, w, h, GDK_PIXBUF_ALPHA_FULL, 127, XLIB_RGB_DITHER_NORMAL, 0, 0);
#else  /* !HAVE_GDK_PIXBUF_XLIB */
      {
        ...
        image= XCreateImage(dpy, xgwa.visual, xgwa.depth, ZPixmap, 0, 0, w, h, 8, 0);
    ...
    Bool verbose_p)
{
  ...
  ximage= XCreateImage(dpy, visual, depth, ZPixmap, 0, 0, cinfo.output_width, cinfo.output_height, 8, 0);
  if (ximage)
    ximage->data= (char *) calloc(ximage->height, ximage->bytes_per_line);

  if (ximage && ximage->data)
    scanbuf= (*cinfo.mem->alloc_sarray)((j_common_ptr) &cinfo, JPOOL_IMAGE, cinfo.rec_outbuf_height * cinfo.output_width * cinfo.output_components, 1);
    ...
    XRectangle *geom_ret)
{
  ...
  /* Read the file...
   */
  ximage= read_jpeg_ximage(screen, visual, drawable, cmap, filename, verbose_p);
  if (! ximage) return False;

  /* Scale it, if necessary...
   */
  compute_image_scaling(ximage->width, ximage->height, win_width, win_height, verbose_p, &srcx, &srcy, &destx, &desty, &w2, &h2);
  ...
  if (ximage->width != win_width || ximage->height != win_height)
    XFillRectangle(dpy, drawable, gc, 0, 0, win_width, win_height);
  XPutImage(dpy, drawable, gc, ximage, srcx, srcy, destx, desty, ximage->width, ximage->height);
  XFreeGC(dpy, gc);
    ...
    XRectangle *geom_ret)
{
  ...
  if (read_file_jpeglib(screen, window, drawable, filename, verbose_p, geom_ret))
    return True;
      ...
      case GRAB_DESK :
        {
          ...
          sprintf(outfile, "%s/xscreensaver.%08x.png", tmpdir, random() % 0xFFFFFFFF);
    ...
    XRectangle *geom_ret)
{
  ...
  status= display_file(screen, window, drawable, filename, verbose_p, geom_ret);
...
static void
  copy_default_colormap_contents(
    Screen *screen,
    Colormap to_cmap,
    Visual *to_visual,
    Bool verbose_p)
    ...
    Bool verbose_p)
{
  ...
  if (verbose_p)
    {
      fprintf(stderr,
        "%s: window 0x%0lX" " root: %d saver: %d map: %d wait: %.1f\n",
        blurb(),
  ...
  if (getenv("WAYLAND_DISPLAY") || getenv("WAYLAND_SOCKET"))
...

    {
      ...
      screenshot= XCreatePixmap(dpy, window, xgwa.width, xgwa.height, xgwa.depth);
      if (! screenshot)
        {
          unlink(filename);
          return None;
...
#if defined(HAVE_GDK_PIXBUF)
      if (! read_file_gdk(screen, window, screenshot, filename, verbose_p, &geom))
        {
          unlink(filename);
...
#elif defined(HAVE_JPEGLIB)
    if (! read_file_jpeglib(screen, window, screenshot, filename, verbose_p, &geom))
      {
        unlink(filename);
        XFreePixmap(dpy, screenshot);
        return None;
    ...
    abort();
      ...
      if (verbose_p)
        fprintf(stderr, "%s: screenshot %dx%d+%d+%d via \"%s\"\n", blurb(), xgwa.width, xgwa.height, xgwa.x, xgwa.y, GETIMAGE_SCREEN_PROGRAM);
    }
#endif /* USE_EXTERNAL_SCREEN_GRABBER */
#ifndef HAVE_MACOS_X11
  else
    {
      ...
      if (! root_p && xgwa.visual->class != TrueColor)
        copy_default_colormap_contents(screen, xgwa.colormap, xgwa.visual, verbose_p);

      gcv.function= GXcopy;
      gcv.subwindow_mode= IncludeInferiors;
      gc= XCreateGC(dpy, window, GCFunction | GCSubwindowMode, &gcv);
      window_root_offset(dpy, window, &xgwa.x, &xgwa.y);
      XCopyArea(dpy, real_root, screenshot, gc, xgwa.x, xgwa.y, xgwa.width, xgwa.height, 0, 0);
      XFreeGC(dpy, gc);
    ...
    XRectangle *geom_ret)
{
  ...
  compute_image_scaling(srcw, srch, dstw, dsth, verbose_p, &srcx, &srcy, &dstx, &dsty, &w2, &h2);
...
COLORBARS:
          ...
          XGetWindowAttributes(dpy, window, &xgwa);
          XGetGeometry(dpy, drawable, &root, &xx, &yy, &w, &h, &bw, &d);
          colorbars(screen, xgwa.visual, drawable, xgwa.colormap, (h >= 600 ? 2 : h >= 300 ? 1 :
                                                                                             0));
      ...
      case GRAB_DESK :
        if (! display_desktop(screen, window, drawable, cache_p, verbose_p, &geom))
          goto COLORBARS;
        file_prop= "desktop";
        break;
      ...
      case GRAB_FILE :
        ...
        if (! display_file(screen, window, drawable, (absfile ? absfile : file), verbose_p, &geom))
          goto COLORBARS;
        file_prop= file;
...

  {
    ...
    if (xattr_url)
      {
        XChangeProperty(dpy, window, a, XA_STRING, 8, PropModeReplace, (unsigned char *) xattr_url, strlen(xattr_url));
      }
    else if (file_prop && *file_prop)
      {
        ...
        if (dot) *dot= 0;
        XChangeProperty(dpy, window, a, XA_STRING, 8, PropModeReplace, (unsigned char *) f2, strlen(f2));
        free(f2);
      }
    else
      {
    ...
    if (xattr_title)
      XChangeProperty(dpy, window, a, XA_STRING, 8, PropModeReplace, (unsigned char *) xattr_title, strlen(xattr_title));
    else
      XDeleteProperty(dpy, window, a);
    ...
    if (geom.width > 0)
      {
        ...
        XChangeProperty(dpy, window, a, XA_STRING, 8, PropModeReplace, (unsigned char *) gstr, strlen(gstr));
      }
    else
      {
        XDeleteProperty(dpy, window, a);
  ...
  for (i= 1; i < argc; i++)
    {
      ...
      if (! strcmp(argv [ i ], "-v") ||
      ...
      else if (! strcmp(argv [ i ], "-no-desktop"))
        {
          grab_desktop_p= False;
        }
        ...
        {
          cache_desktop_p= False;
        }
      else if (! strcmp(argv [ i ], "-video"))
        {
          grab_video_p= True;
        }
      else if (! strcmp(argv [ i ], "-no-video"))
        {
          grab_video_p= False;
        }
      ...
      else if (! strcmp(argv [ i ], "-no-images"))
        {
          random_image_p= False;
        }
...
LOSE:
  ...
  get_image(screen, window, drawable, verbose_p, grab_desktop_p, cache_desktop_p, grab_video_p, random_image_p, image_directory, file);
  XSync(dpy, False);
  exit(0);
}
