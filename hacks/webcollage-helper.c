    ...
    int h)
{
  ...
  if (bevel_size < 20 && small_size > 40)
    bevel_size= 20;
  else if (bevel_size < 10 && small_size > 20)
    bevel_size= 10;
    ...
    int h)
{
  ...
  if (from_scale != 1.0)
    {
      ...
      GdkPixbuf *new_pb= gdk_pixbuf_scale_simple(paste_pb, new_w, new_h, GDK_INTERP_HYPER);
  ...
  if (bevel_pct > 0)
    bevel_image(&paste_pb, bevel_pct, from_x, from_y, w, h);
  ...
  if (opacity == 1.0 && bevel_pct == 0)
  ...
  else
    {
      ...
      if (w > 0 && h > 0)
        gdk_pixbuf_composite(paste_pb, base_pb, to_x, to_y, w, h, to_x - from_x, to_y - from_y, 1.0, 1.0, GDK_INTERP_HYPER, opacity * 255);
    }
    ...
    const char *file)
{
  ...
  pb= gdk_pixbuf_new_from_data(bytes, GDK_COLORSPACE_RGB, FALSE, 8, /* alpha, sample size */
    w,
    h,
    w * 3, /* rowstride */
    NULL,
  ...
  if (argc == 11 || argc == 12)
    {
      ...
      s= argv [ i++ ];
      if (1 != sscanf(s, " %d %c", &from_x, &dummy)) usage();
      s= argv [ i++ ];
      if (1 != sscanf(s, " %d %c", &from_y, &dummy)) usage();
      s= argv [ i++ ];
      if (1 != sscanf(s, " %d %c", &to_x, &dummy)) usage();
      s= argv [ i++ ];
      if (1 != sscanf(s, " %d %c", &to_y, &dummy)) usage();
      s= argv [ i++ ];
      if (1 != sscanf(s, " %d %c", &w, &dummy)) usage();
      s= argv [ i ];
      if (1 != sscanf(s, " %d %c", &h, &dummy)) usage();
      ...
      paste(paste_file, base_file, from_scale, opacity, bevel_pct, from_x, from_y, to_x, to_y, w, h);
    }
