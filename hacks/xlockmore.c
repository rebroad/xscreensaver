...
#undef ya_rand_init
  ...
  {
    char *args []= {"-count", "-cycles", "-delay", "-ncolors", "-size", "-font", "-wireframe", "-use3d", "-useSHM"};
        ...
        for (k= 0; k < i - 1; k++)
          {
            ...
            if (! strcasecmp(old, new))
              fprintf(stderr,
                "%s: duplicate resource \"%s\": " "set in both DEFAULTS and vars[]\n",
                progname,
                old);
            free(oold);
          }
      ...
      switch (xlockmore_opts->vars [ i ].type)
        {
          case t_String :
            {
              char *c= get_string_resource(dpy, xlockmore_opts->vars [ i ].name, xlockmore_opts->vars [ i ].classname);
          ...
          case t_Float :
            {
              float f= get_float_resource(dpy, xlockmore_opts->vars [ i ].name, xlockmore_opts->vars [ i ].classname);
          ...
          case t_Int :
            {
              int ii= get_integer_resource(dpy, xlockmore_opts->vars [ i ].name, xlockmore_opts->vars [ i ].classname);
              if (ii != *var_i)
                {
                  xlockmore_free_screens(mi);
                  *var_i= ii;
          ...
          case t_Bool :
            {
              Bool b= get_boolean_resource(dpy, xlockmore_opts->vars [ i ].name, xlockmore_opts->vars [ i ].classname);
              if (b != *var_b)
                {
                  xlockmore_free_screens(mi);
                  *var_b= b;
      ...
      switch (mi->xlmft->desired_color_scheme)
        {
          case color_scheme_uniform :
            make_uniform_colormap(mi->xgwa.screen, mi->xgwa.visual, mi->xgwa.colormap, mi->colors, &mi->npixels, True, &mi->writable_p, True);
            break;
          case color_scheme_smooth :
            make_smooth_colormap(mi->xgwa.screen, mi->xgwa.visual, mi->xgwa.colormap, mi->colors, &mi->npixels, True, &mi->writable_p, True);
            break;
          case color_scheme_bright :
          case color_scheme_default :
            make_random_colormap(mi->xgwa.screen, mi->xgwa.visual, mi->xgwa.colormap, mi->colors, &mi->npixels, (mi->xlmft->desired_color_scheme == color_scheme_bright), True, &mi->writable_p, True);
            break;
          default :
            abort();
        }
  ...
  void **state_array)
{
  ...
  /* Find the appropriate state object, clear it, and we're done. */
  {
    ...
    memset((char *) (*xlmft->state_array) + mi->screen_number * state_size, 0, state_size);
  }
}


Bool xlockmore_no_events(
  ModeInfo *mi,
  XEvent *event)
{
  return False;
