    ...
    int dst_y)
{
  if (op == GXclear)
    ...
    {
      XImage *srci= XGetImage(dpy, src, src_x, src_y, width, height, ~0L, ZPixmap);
      XImage *dsti= XGetImage(dpy, dst, dst_x, dst_y, width, height, ~0L, ZPixmap);
      unsigned long *out= (unsigned long *) dsti->data;
      unsigned long *in= (unsigned long *) srci->data;
      unsigned long *end= (in + (height * srci->bytes_per_line / sizeof(unsigned long)));
      switch (op)
        {
          case GXor :
            while (in < end) *out++|= *in++;
    ...
    void *closure)
{
  ...
  if (! st->img_loader &&
    ...
    st->start_time + st->duration < time((time_t *) 0))
    {
      ...
      st->img_loader= load_image_async_simple(0, st->xgwa.screen, st->window, st->bitmap, 0, 0);
    }
    ...
    Window w_arg)
{
  ...
  st->fg= get_pixel_resource(st->dpy, st->xgwa.colormap, "foreground", "Foreground");
  st->bg= get_pixel_resource(st->dpy, st->xgwa.colormap, "background", "Background");
  ...
  if (! strcasecmp(bitmap_name, "(builtin)") ||
    ...
    {
      Pixmap mask= 0;
      Pixmap pixmap= image_data_to_pixmap(st->dpy, st->window, som_png, sizeof(som_png), &st->width, &st->height, &mask);
      XGCValues gcv;
      GC gc;
      gcv.foreground= st->bg;
      gc= XCreateGC(st->dpy, st->window, GCForeground, &gcv);
      st->bitmap= XCreatePixmap(st->dpy, st->window, st->xgwa.width, st->xgwa.height, st->xgwa.depth);
      XFillRectangle(st->dpy, st->bitmap, gc, 0, 0, st->width, st->height);
      XSetClipMask(st->dpy, gc, mask);
      XCopyArea(st->dpy, pixmap, st->bitmap, gc, 0, 0, st->width, st->height, 0, 0);
      XFreeGC(st->dpy, gc);
    ...
    {
      st->bitmap= file_to_pixmap(st->dpy, st->window, bitmap_name, &st->width, &st->height, 0);
...
#define make_gc(op) \
  ...
  XCopyArea(st->dpy, st->bitmap, st->self, st->gc, 0, 0, st->width, st->height, (st->size - st->width) >> 1, (st->size - st->height) >> 1);
    ...
    Pixmap pixmap)
{
  ...
  if (st->xgwa.depth != 1)
    XCopyArea(st->dpy, pixmap, st->window, st->gc, 0, 0, st->size, st->size, (st->xgwa.width - st->size) >> 1, (st->xgwa.height - st->size) >> 1);
  else
    XCopyPlane(st->dpy, pixmap, st->window, st->gc, 0, 0, st->size, st->size, (st->xgwa.width - st->size) >> 1, (st->xgwa.height - st->size) >> 1, 1);
  /*
