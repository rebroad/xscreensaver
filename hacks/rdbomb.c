    ...
    struct state *st)
{
  ...
  make_smooth_colormap(st->xgwa.screen, st->visual, st->cmap, st->colors, &st->ncolors, True, 0, True);
  if (st->ncolors <= 2)
    {
      mono_p= True;
      st->ncolors= 2;
    ...
    Window win)
{
  ...
  */
  ...
  st->image= create_xshm_image(st->dpy, st->xgwa.visual, vdepth, ZPixmap, &st->shm_info, st->width, st->height);
  st->pd= st->image->data;
  ...
  for (ii= 0; ii < chunk; ii++)
    {
      ...
      if (ii == chunk - 1)
        { /* Only need to putimage on the final frame */
          for (i= 0; i < st->array_width; i+= st->width)
            for (j= 0; j < st->array_height; j+= st->height)
              put_xshm_image(st->dpy, st->window, st->gc, st->image, 0, 0, i + st->array_x, j + st->array_y, st->width, st->height, &st->shm_info);
        }
