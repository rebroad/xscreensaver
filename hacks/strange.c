...
ModStruct strange_description=
  {"strange", "init_strange", "draw_strange", (char *) NULL, "init_strange", "init_strange", "free_strange", &strange_opts, 10000, 1, 1, 1, 64, 1.0, "", "Shows strange attractors", 0, NULL};
#endif
...
#if defined(__BIGGEST_ALIGNMENT__) && (defined(__GNUC__) && (__GNUC__ == 4 && __GNUC_MINOR__ >= 7 || __GNUC__ >= 5) || defined(__clang__))
    ...
    unsigned id)
{
  ...
  T->accMap [ 0 ]= NULL;
  if (aligned_malloc((void **) &T->accMap [ 0 ], __BIGGEST_ALIGNMENT__, A->alignedWidth * A->Height * sizeof(PIXEL0)))
    {
      thread_destroy(T);
      return ENOMEM;
    }
  ...
  if (aligned_malloc((void **) &T->bloomRows, __BIGGEST_ALIGNMENT__, A->alignedWidth * (pointSize + 2) * sizeof(*T->bloomRows)))
    {
      thread_destroy(T);
      return ENOMEM;
    }
  if (aligned_malloc((void **) &T->colorRow, __BIGGEST_ALIGNMENT__, A->alignedWidth * sizeof(*T->colorRow)))
    {
      thread_destroy(T);
      return ENOMEM;
    }
  if (A->blurFac)
    {
      if (aligned_malloc((void **) &T->motionBlur, __BIGGEST_ALIGNMENT__, A->alignedWidth * (T->y2 - T->y1) * sizeof(*T->motionBlur)))
        {
          thread_destroy(T);
          return ENOMEM;
        }
    ...
    void *Self_Raw)
{
  ...
  unsigned long colorScale=
    (double) A->Width * A->Height * (1 << COLOR_BITS) * brightness * A->colorFac * (zoom * zoom) / (0.9 * 0.9) / 640.0 / 480.0 / (pointSize * pointSize) * 800000.0 / (float) A->Max_Pt * (float) A->numCols / 256;
#ifdef VARY_SPEED_TO_AVOID_BOREDOM
  unsigned pixelCount= 0;
#endif
  PIXEL0 *motionBlurRow= T->motionBlur;
  void *outRow= (char *) A->accImage->data + A->accImage->bytes_per_line * T->y1;
...
#define HIGH_COLOR(c) ((65535-(c))*(li-FULLBLUE)/(256-FULLBLUE)+(c))
  li= MINBLUE + (255.0 - MINBLUE) * log(1.0 + ACC_GAMMA * (float) i / n) / log(1.0 + ACC_GAMMA);
  if (li < FULLBLUE)
    {
  ...
  if (useAccumulator)
    {
      ...
      put_xshm_image(display, A->dbuf != None ? A->dbuf : window, A->dbuf != None ? A->dbuf_gc : gc, A->accImage, 0, 0, 0, 0, A->accImage->width, A->accImage->height, &A->shmInfo);
...
#undef A
#endif
    Attractor->dbuf= XCreatePixmap(display, MI_WINDOW(mi), Attractor->Width, Attractor->Height,
  ...
  if (Attractor->dbuf != None)
    {
      ...
      if ((Attractor->dbuf_gc= XCreateGC(display, Attractor->dbuf,
        ...
        {
          XFillRectangle(display, Attractor->dbuf, Attractor->dbuf_gc, 0, 0, Attractor->Width, Attractor->Height);
  ...
  if (useAccumulator)
    {
      ...
      if (A->visualClass == TrueColor)
        {
          ...
          visual_rgb_masks(screen, MI_VISUAL(mi), &A->rMask, &A->gMask, &A->bMask);
      ...
      if (A->accImage)
        destroy_xshm_image(display, A->accImage, &A->shmInfo);
      A->accImage= create_xshm_image(display, MI_VISUAL(mi), MI_DEPTH(mi), ZPixmap, &A->shmInfo, ((A->alignedWidth * bpp + threadAlign1) & ~threadAlign1) / bpp, A->Height);
