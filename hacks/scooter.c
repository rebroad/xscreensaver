...
ModStruct scooter_description=
  {"scooter", "init_scooter", "draw_scooter", "release_scooter", "refresh_scooter", "change_scooter", (char *) NULL, &scooter_opts, 20000, 24, 5, 100, 64, 1.0, "", "Shows a journey through space tunnel and stars", 0, NULL};
/*
...
/* comment this to color the doors from xlock's palette (created with saturation value) */
  ...
  xcol.red= sp->begincolor.r + ((sp->endcolor.r - sp->begincolor.r) * sp->colorcount / sp->colorsteps);
  xcol.green= sp->begincolor.g + ((sp->endcolor.g - sp->begincolor.g) * sp->colorcount / sp->colorsteps);
  xcol.blue= sp->begincolor.b + ((sp->endcolor.b - sp->begincolor.b) * sp->colorcount / sp->colorsteps);
  xcol.pixel= 0;
  ...
  Rect *rect)
{
  ...
  if (((new1.x >= rect->lefttop.x) && (new1.x <= rect->rightbottom.x)) || ((new1.y >= rect->lefttop.y) && (new1.y <= rect->rightbottom.y)) || ((new2.x >= rect->lefttop.x) && (new2.x <= rect->rightbottom.x)) || ((new2.y >= rect->lefttop.y) && (new2.y <= rect->rightbottom.y)))
    return 1;
  ...
  ModeInfo *mi)
{
  ...
  XSetLineAttributes(display, gc, 2 * sp->pscale, LineSolid, CapNotLast, JoinRound);
  ...
  for (i= sp->spectator_zelement; i < ztotal; i++)
    {
      ...
      XFillRectangle(display, window, gc, p.x, p.y, st->pscale, st->pscale);
    }
#endif
      ...
      for (j= 0; j < 4; j++)
        {
          ...
          lines [ j ].x= midx + (int) (sp->doors [ i ].coords [ j ].x * proj / SPACE_XY_FACTOR);
          lines [ j ].y= midy - (int) (sp->doors [ i ].coords [ j ].y * proj / SPACE_XY_FACTOR);
        }
      if (j < 4) continue;
  ...
  for (i= 0; i < sp->starcount; i++)
    {
      ...
      lefttop.x= midx + (int) ((coords.x - sp->stars [ i ].width / 2) * proj / SPACE_XY_FACTOR);
      lefttop.y= midy - (int) ((coords.y + sp->stars [ i ].height / 2) * proj / SPACE_XY_FACTOR);
      if (lefttop.x < 0)
        lefttop.x= 0;
      ...
      rightbottom.x= midx + (int) ((coords.x + sp->stars [ i ].width / 2) * proj / SPACE_XY_FACTOR);
      rightbottom.y= midy - (int) ((coords.y - sp->stars [ i ].height / 2) * proj / SPACE_XY_FACTOR);
      if (rightbottom.x < 0)
        continue;
...
#else
      XFillArc(display, window, gc, lefttop.x, lefttop.y, rightbottom.x - lefttop.x, rightbottom.y - lefttop.y, 0, 360 * 64);
#endif
    }
}

ENTRYPOINT void draw_scooter(
  ModeInfo *mi)
{
  scooterstruct *sp;
