...
static int
  gcd(
    int m,
    int n) /* Greatest Common Divisor (also Greates common factor). */
{
  int r;
    ...
    Window window)
{
  ...
  if (st->maxlines < MINLINES)
    st->maxlines= MINLINES;
  else if (st->maxlines > MAXLINES)
    ...
    void *closure)
{
  ...
  XFillPolygon(st->dpy, st->window, st->gc, st->points, numpoints, Complex, CoordModeOrigin);
