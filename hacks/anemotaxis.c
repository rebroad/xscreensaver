    ...
    Window win)
{
  ...
  make_random_colormap(wa.screen, wa.visual, wa.colormap, st->colors, &st->ncolors, True, True, 0, True);
      ...
      for (j= 0; j < st->source [ i ]->n; j++)
        {
          ...
          int size= (st->scrWidth > 2560 || st->scrHeight > 2560 ? 8 : 4); /* Retina displays */
...
static void animate_anemotaxis(
  struct state *st,
  Drawable curr_window)
{
  int i, j;
  ...
  if (st->max_searcher == 0)
    { /* kill some sources when searchers don't do that */
      for (i= 0; i < st->max_src; i++)
        {
          if (st->source [ i ] != NULL && RND(st->max_dist * st->max_src) == 0)
            {
