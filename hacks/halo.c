...
{
  int i;
  st->count= (st->global_count ? st->global_count : (3 + (random() % max(1, (min(st->width, st->height) / 50))) + (random() % max(1, (min(st->width, st->height) / 50)))));
    ...
    Window window)
{
  ...
  else if (! strcmp(mode_str, "seuss"))
    {
      cmode= seuss_mode;
    }
  else if (! strcmp(mode_str, "ramp"))
    {
      cmode= ramp_mode;
    }
  ...
  if (mono_p)
    ...
    make_uniform_colormap(xgwa.screen, xgwa.visual, st->cmap, st->colors, &st->ncolors, True, 0, True);
  else
    make_smooth_colormap(xgwa.screen, xgwa.visual, st->cmap, st->colors, &st->ncolors, True, 0, True);
  ...
  if (cmode == seuss_mode)
    {
      ...
      gcv.background= 0;
      gcv.function= GXxor;
      st->merge_gc= XCreateGC(st->dpy, st->pixmap, GCForeground | GCBackground | GCFunction, &gcv);
    }
  else
    {
      gcv.foreground= st->fg_pixel;
      gcv.background= st->bg_pixel;
      gcv.function= GXcopy;
      st->merge_gc= XCreateGC(st->dpy, st->window, GCForeground | GCBackground | GCFunction, &gcv);
    }
    ...
    void *closure)
{
  ...
  for (i= 0; i < st->count; i++)
    {
      ...
      if (radius > 0 &&
        ...
        XFillArc(st->dpy,
          ...
          st->circles [ i ].x - radius,
          st->circles [ i ].y - radius,
          radius * 2,
          radius * 2,
          0,
          360 * 64);
  ...
  if (st->buffer && (st->anim_p ? (done || (! st->done_once && (st->iterations & 1))) : (st->iterations & 1)))
    {
