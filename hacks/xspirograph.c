  ...
  /* see below about alwaysfinish */
  {
    ...
    tmpy= ymid + (                    /* of the figure.       */
      ...
      (d * sin           /* used to play with    */
        (                /* when he was a kid.  */
          (              /*            * * * * */
            ((radius1 * st->theta) - delta) / radius2) *
          M_PI / 180));
    ...
    if (st->theta != 1)
      XDrawLine(st->dpy, st->window, st->draw_gc, st->x1, st->y1, st->x2, st->y2);
    ...
    struct state *st)
{
  if (mono_p)
    ...
    {
      hsv_to_rgb(random() % 360, frand(1.0), frand(0.5) + 0.5, &st->color.red, &st->color.green, &st->color.blue);
      if ((st->got_color= XAllocColor(st->dpy, st->xgwa.colormap, &st->color)))
  ...
  switch (st->drawstate)
    {
      ...
      case DRAW :
        ...
        for (i= 0; i < 1000; i++)
          {
            if (go(st, st->radius1, (flip_p ? st->radius2 : -st->radius2), st->distance))
              {
                st->drawstate= NEW_LAYER;
                break;
              }
