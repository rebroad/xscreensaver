    ...
    Window window)
{
  ...
  st->pix= XCreatePixmap(dpy, window, st->w > st->h ? st->w : st->h, st->w > st->h ? st->w : st->h, st->xgwa.depth);
  st->gc= XCreateGC(dpy, st->pix, 0, &gcv);

  st->xftdraw= XftDrawCreate(dpy, st->pix, st->xgwa.visual, st->xgwa.colormap);
  ...
  st->bg= get_pixel_resource(dpy, st->xgwa.colormap, "textBackground", "Background");
  st->text_color= get_pixel_resource(dpy, st->xgwa.colormap, "textColor", "Foreground");
  st->ring_color= get_pixel_resource(dpy, st->xgwa.colormap, "ringColor", "Foreground");
  st->trace_color= get_pixel_resource(dpy, st->xgwa.colormap, "traceColor", "Foreground");

  s= get_string_resource(dpy, "textColor", "Foreground");
  if (! s) s= strdup("white");
  XftColorAllocName(dpy, st->xgwa.visual, st->xgwa.colormap, s, &st->xft_text_color_1);
  free(s);

  s= get_string_resource(dpy, "textBackground", "Background");
  if (! s) s= strdup("black");
  XftColorAllocName(dpy, st->xgwa.visual, st->xgwa.colormap, s, &st->xft_text_color_2);
  if (s) free(s);
    ...
    void *closure)
{
      ...
      if (st->value >= blurbs [ i ].t && st->value <= blurbs [ i ].t + 1 / 15.0)
        {
          ...
          xftfont= (blurbs [ i ].f == 1 ? st->font2 :
              blurbs [ i ].f == 2       ? st->font :
                                          st->font3);

          XSetForeground(dpy, st->gc, blurbs [ i ].k == 3 ? st->bg : st->text_color);
          XFillRectangle(dpy, st->pix, st->gc, 0, 0, st->w, st->h);
          XSetForeground(dpy, st->gc, blurbs [ i ].k == 3 ? st->text_color : st->bg);
          ...
          XftTextExtentsUtf8(dpy, xftfont, (FcChar8 *) blurbs [ i ].s [ 0 ], strlen(blurbs [ i ].s [ 0 ]), &extents);
          /* lbearing = -extents.x; */
          ...
          for (j= 0; j < countof(blurbs [ i ].s); j++)
            {
              if (blurbs [ i ].s [ j ])
                XftDrawStringUtf8(st->xftdraw, xftcolor, xftfont, x, y, (FcChar8 *) blurbs [ i ].s [ j ], strlen(blurbs [ i ].s [ j ]));

              y+= line_height * 1.5;

              if (blurbs [ i ].s [ j ])
                {
                  XftTextExtentsUtf8(dpy, xftfont, (FcChar8 *) blurbs [ i ].s [ 0 ], strlen(blurbs [ i ].s [ j ]), &extents);
                  /* lbearing = -extents.x; */
                  /* rbearing = extents.width - extents.x; */
                  /* ascent   = extents.y; */
                  /* descent  = extents.height - extents.y; */
          ...
          if (blurbs [ i ].k == 2) /* Rotate clockwise and flip */
            {
              ...
              XImage *img1= XGetImage(dpy, st->pix, (st->w - wh) / 2, (st->h - wh) / 2, wh, wh, ~0L, ZPixmap);
              XImage *img2= XCreateImage(dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, 0, 0, wh, wh, 32, 0);
              ...
              XSetForeground(dpy, st->gc, blurbs [ i ].k == 3 ? st->bg : st->text_color);
              XFillRectangle(dpy, st->pix, st->gc, 0, 0, st->w, st->h);
              XPutImage(dpy, st->pix, st->gc, img2, 0, 0, (st->w - wh) / 2, (st->h - wh) / 2, wh, wh);
              XDestroyImage(img1);
              XDestroyImage(img2);
            }
          else if (blurbs [ i ].k == 1) /* Flip vertically */
            {
              XImage *img1= XGetImage(dpy, st->pix, 0, 0, st->w, st->h, ~0L, ZPixmap);
              XImage *img2= XCreateImage(dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, 0, 0, st->w, st->h, 32, 0);
              img2->data= malloc(img2->bytes_per_line * img2->height);
              for (y= 0; y < img2->height; y++)
                for (x= 0; x < img2->width; x++)
                  XPutPixel(img2, x, img2->height - y - 1, XGetPixel(img1, x, y));
  ...
  if (r > 1 / 30.0) /* Sweep line and background */
    {
      ...
      XFillArc(dpy, st->pix, st->gc, -st->w, -st->h, st->w * 3, st->h * 3, 90 * 64, 90 * 64 - ((r + 0.25) * 360 * 64));
  ...
  if ((st->value >= 7.75 && st->value <= 7.85) ||
    (st->value >= 4.00 && st->value <= 4.25))
    {
      ...
      x= st->w * 0.1;
      y= st->h * 0.1 + ascent;
      XftDrawStringUtf8(st->xftdraw, xftcolor, xftfont, x, y, (FcChar8 *) s, strlen(s));
  ...
  if (r > 1 / 30.0) /* Two rings around number */
    {
      ...
      XSetForeground(dpy, st->gc, st->ring_color);
      XSetLineAttributes(dpy, st->gc, st->w * 0.025, LineSolid, CapRound, JoinRound);
