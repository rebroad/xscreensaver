    ...
    struct state *st)
{
  ...
  make_random_colormap(st->xgwa.screen, st->xgwa.visual, st->mycmap, &st->mycolors [ 1 ], &ncolors, True, True, 0, True);
    ...
    if (oldcol != 0)
      {
        ...
        if (oldcol == 1 && LP->realbounce && (vertwall || horiwall))
          if (vertwall)
            ...
            LP->deg= -LP->deg;
        else if ((oldcol != LP->col && LP->realbounce) || (oldcol == LP->col && LP->selfbounce))
            ...
            if (vertwall && xi + 1 <= xmax)
              {
                ...
                for (yy= yi - yi % st->boxh;
                  yy <= yi - yi % st->boxh + st->boxh && yy <= ymax;
                  yy++)
            ...
            if (horiwall && yi + 1 <= ymax)
              {
                ...
                for (xx= xi - xi % st->boxw;
                  xx <= xi - xi % st->boxw + st->boxw && xx <= xmax;
                  xx++)
    ...
    Window w)
{
  ...
  XSetWindowBackground(st->dpy, st->window, BlackPixel(st->dpy, DefaultScreen(st->dpy)));
  {
                ...
                case 'N' :
                  {
                    ...
                    while (! (st->ch == '\15' || st->ch == '#' || st->threads == thrmax));
                    if (st->threads == 0)
                      st->threads= othreads;
                    st->reset_p= 1;
                  }
		...
		((bankc <= st->bnkt) ? ( \
			(LDP = &st->thread[st->bank[bankc - 1] - 1],	1) \
                    ...
                    case 'S' :
                      ...
                      while (! (st->ch == '\15' || st->ch == '#' || st->thread [ st->bank [ 0 ] - 1 ].tslen == 50));
                      {
                        forallinbank(L)
                        {
                          int seqSum= 0, c;
