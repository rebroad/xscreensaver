  ...
  int src)
{
  ...
  if (! mono_p)
    {
      hsv_to_rgb(m->jenis, 1.0, 1.0, &m->color.red, &m->color.green, &m->color.blue);
    ...
    Window window)
{
  ...
  st->font= load_xft_font_retry(st->dpy, screen_number(xgwa.screen), levelfont);
  if (! st->font) abort();

  st->scoreFont= load_xft_font_retry(st->dpy, screen_number(xgwa.screen), scorefont);
  ...
  s= "#FF8811";
  XftColorAllocName(st->dpy, xgwa.visual, xgwa.colormap, s, &st->xft_level_fg);
  ...
  int ylim)
{
  ...
  XftTextExtentsUtf8(st->dpy, st->scoreFont, (FcChar8 *) buf, strlen(buf), &overall);
  width= overall.xOff;
  height= font_height(st->scoreFont);
  XSetForeground(st->dpy, st->draw_gc, st->scoreColor.pixel);
  XFillRectangle(st->dpy, st->window, st->erase_gc, xlim - width - 6, ylim - height - 2, width + 6, height + 2);
  XftDrawStringUtf8(st->xftdraw, &st->xft_fg, st->scoreFont, xlim - width - 2, ylim - 2, (FcChar8 *) buf, strlen(buf));

  sprintf(buf, "%ld", st->highscore);
  XftTextExtentsUtf8(st->dpy, st->scoreFont, (FcChar8 *) buf, strlen(buf), &overall);
  width= overall.xOff;
  XFillRectangle(st->dpy, st->window, st->erase_gc, 4, ylim - height - 2, width + 4, height + 2);
  XftDrawStringUtf8(st->xftdraw, &st->xft_fg, st->scoreFont, 4, ylim - 2, (FcChar8 *) buf, strlen(buf));
}
  ...
  XColor col)
{
  ...
  XFillRectangle(st->dpy, st->window, st->draw_gc, x - 30 * st->pscale, y - 40 * st->pscale, 60 * st->pscale, 40 * st->pscale);
  XFillRectangle(st->dpy, st->window, st->draw_gc, x - 20 * st->pscale, y - 50 * st->pscale, 10 * st->pscale, 10 * st->pscale);
  XFillRectangle(st->dpy, st->window, st->draw_gc, x + 10 * st->pscale, y - 50 * st->pscale, 10 * st->pscale, 10 * st->pscale);
}
  ...
  for (i= 0; i < kMaxMissiles; i++)
    {
      ...
      XDrawLine(st->dpy, st->window, st->draw_gc, old_x, old_y, m->x, m->y);
  ...
  for (i= 0; i < kMaxLasers; i++)
    {
      ...
      if (m->oldx != -1)
        {
          ...
          XDrawLine(st->dpy, st->window, st->erase_gc, m->oldx2, m->oldy2, m->oldx, m->oldy);
        }
      ...
      XSetForeground(st->dpy, st->draw_gc, m->color.pixel);
      XDrawLine(st->dpy, st->window, st->draw_gc, m->x, m->y, x, y);
      ...
      if (m->alive == 0)
        {
          /* we just died */
          XDrawLine(st->dpy, st->window, st->erase_gc, m->x, m->y, x, y);
          Explode(st, m->x, m->y, kBoomRad, m->color, 1);
        }
    }
}

static void LoopBooms(
  struct state *st,
  int xlim,
  int ylim)
{
          ...
          if (m->outgoing)
            {
              ...
              XSetForeground(st->dpy, st->draw_gc, m->color.pixel);
              XDrawArc(st->dpy, st->window, st->draw_gc, m->x - m->rad * st->pscale, m->y - m->rad * st->pscale, m->rad * 2 * st->pscale, m->rad * 2 * st->pscale, 0, 360 * 64);
            }
          else
            {
              XSetLineAttributes(st->dpy, st->erase_gc, 1 * st->pscale, 0, 0, 0);
              XDrawArc(st->dpy, st->window, st->erase_gc, m->x - m->rad * st->pscale, m->y - m->rad * st->pscale, m->rad * 2 * st->pscale, m->rad * 2 * st->pscale, 0, 360 * 64);
              m->rad--;
              if (m->rad <= 0)
                m->alive= 0;
            }
  ...
  int ylim)
{
  ...
  /* print out screen */
  XFillRectangle(st->dpy, st->window, st->erase_gc, 0, 0, xlim, ylim);
  if (st->bround)
    sprintf(buf, "Bonus Round Over");
  else if (sumlive || freecity)
    sprintf(buf, "Level %d Cleared", st->level);
  else
    sprintf(buf, "GAME OVER");
  if (st->level > 0)
    {
      XftTextExtentsUtf8(st->dpy, st->font, (FcChar8 *) buf, strlen(buf), &overall);
      width= overall.xOff;
      XftDrawStringUtf8(st->xftdraw, &st->xft_level_fg, st->font, xlim / 2 - width / 2, ylim / 2 - font_height(st->font) / 2, (FcChar8 *) buf, strlen(buf));
      XSync(st->dpy, False);
      usleep(1000000);
    }
      ...
      if (sumlive || freecity)
        {
          int sumwidth;
          /* draw live cities */
          XFillRectangle(st->dpy, st->window, st->erase_gc, 0, ylim - 100 * st->pscale, xlim, 100 * st->pscale);
          ...
          XftDrawStringUtf8(st->xftdraw, &st->xft_level_fg, st->font, xlim / 2 - sumwidth / 2 + 40 + 60, ylim * 0.7, (FcChar8 *) buf, strlen(buf));
          for (i= 0; i < kNumCities; i++)
            {
              if (liv [ i ])
                {
  ...
  if (freecity && sumlive < 5)
    {
      ...
      XftTextExtentsUtf8(st->dpy, st->font, (FcChar8 *) buf, strlen(buf), &overall);
      width= overall.xOff;
      XftDrawStringUtf8(st->xftdraw, &st->xft_level_fg, st->font, xlim / 2 - width / 2, ylim / 4, (FcChar8 *) buf, strlen(buf));
      DrawCities(st, xlim, ylim);
      XSync(st->dpy, False);
      usleep(1000000);
    }

  XFillRectangle(st->dpy, st->window, st->erase_gc, 0, 0, xlim, ylim - 100 * st->pscale);
