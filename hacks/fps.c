  ...
  if (st->this_frame_end.tv_sec != st->prev_frame_end.tv_sec)
    {
      double uprev_frame_end= (st->prev_frame_end.tv_sec +
        ((double) st->prev_frame_end.tv_usec * 0.000001));
      double uthis_frame_end= (st->this_frame_end.tv_sec +
        ((double) st->this_frame_end.tv_usec * 0.000001));
      ...
      sprintf(st->string, (polys ? "FPS:   %.1f \nLoad:  %.1f%% " : "FPS:  %.1f \nLoad: %.1f%% "), fps, load);
      ...
      if (polys > 0)
        {
          ...
          strcat(st->string, "\nPolys: ");
          if (polys >= 1000000)
            sprintf(st->string + strlen(st->string), "%lu,%03lu,%03lu%s ", (polys / 1000000), ((polys / 1000) % 1000), (polys % 1000), s);
          else if (polys >= 1000)
            sprintf(st->string + strlen(st->string), "%lu,%03lu%s ", (polys / 1000), (polys % 1000), s);
          else
      ...
      if (depth >= 0.0)
        {
          ...
          if (ldepth >= 1000000000)
            ...
            {
              sprintf(st->string + strlen(st->string), "%lu,%03lu,%03lu%s ", (ldepth / 1000000), ((ldepth / 1000) % 1000), (ldepth % 1000), s);
            }
          else if (ldepth >= 1000)
            {
              sprintf(st->string + strlen(st->string), "%lu,%03lu%s ", (ldepth / 1000), (ldepth % 1000), s);
            }
          else if (*s)
            {
  ...
  if (st->clear_p)
    {
      ...
      XFillRectangle(st->dpy, st->window, st->erase_gc, x - st->font->descent, y - lh, w + 2 * st->font->descent, h + 2 * st->font->descent);
