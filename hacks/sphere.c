...
ModStruct sphere_description=
  {"sphere", "init_sphere", "draw_sphere", (char *) NULL, "refresh_sphere", "init_sphere", "free_sphere", &sphere_opts, 5000, 1, 20, 0, 64, 1.0, "", "Shows a bunch of shaded spheres", 0, NULL};
    ...
    ModeInfo *mi)
{
  ...
  if (sp->dirx)
    XDrawLine(display, MI_WINDOW(mi), gc, sp->x0 + sp->x, sp->y0 + miny, sp->x0 + sp->x, sp->y0 + maxy);
  if (sp->diry)
    XDrawLine(display, MI_WINDOW(mi), gc, sp->x0 + minx, sp->y0 + sp->y, sp->x0 + maxx, sp->y0 + sp->y);
