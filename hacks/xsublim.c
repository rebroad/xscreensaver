...
static XrmOptionDescRec options []=
  {
    {"-" XSUBLIM_ARG_FONT, "." XSUBLIM_ARG_FONT, XrmoptionSepArg, 0},
    {"-" XSUBLIM_ARG_DELAYSHOW, "." XSUBLIM_ARG_DELAYSHOW, XrmoptionSepArg, 0},
    {"-" XSUBLIM_ARG_DELAYWORD, "." XSUBLIM_ARG_DELAYWORD, XrmoptionSepArg, 0},
    {"-" XSUBLIM_ARG_DELAYPHRASEMIN, "." XSUBLIM_ARG_DELAYPHRASEMIN, XrmoptionSepArg, 0},
    {"-" XSUBLIM_ARG_DELAYPHRASEMAX, "." XSUBLIM_ARG_DELAYPHRASEMAX, XrmoptionSepArg, 0},
    {"-" XSUBLIM_ARG_RANDOM, "." XSUBLIM_ARG_RANDOM, XrmoptionNoArg, "true"},
    {"-no-" XSUBLIM_ARG_RANDOM, "." XSUBLIM_ARG_RANDOM, XrmoptionNoArg, "false"},
    {"-" XSUBLIM_ARG_FILE, "." XSUBLIM_ARG_FILE, XrmoptionSepArg, 0},
#if ! defined(VMS)
    {"-" XSUBLIM_ARG_PROGRAM, "." XSUBLIM_ARG_PROGRAM, XrmoptionSepArg, 0},
#endif
    {"-" XSUBLIM_ARG_SCREENSAVER, "." XSUBLIM_ARG_SCREENSAVER, XrmoptionNoArg, "true"},
    {"-no-" XSUBLIM_ARG_SCREENSAVER, "." XSUBLIM_ARG_SCREENSAVER, XrmoptionNoArg, "false"},
    {"-" XSUBLIM_ARG_OUTLINE, "." XSUBLIM_ARG_OUTLINE, XrmoptionNoArg, "true"},
    {"-no-" XSUBLIM_ARG_OUTLINE, "." XSUBLIM_ARG_OUTLINE, XrmoptionNoArg, "false"},
    {"-" XSUBLIM_ARG_CENTER, "." XSUBLIM_ARG_CENTER, XrmoptionNoArg, "true"},
    {"-no-" XSUBLIM_ARG_CENTER, "." XSUBLIM_ARG_CENTER, XrmoptionNoArg, "false"},
    {NULL, NULL, 0, 0}};
...
static Window xsublim_Ss_GetWindow(
  Display* ss_Display)
{
...
#undef ya_rand_init
  ...
  app_App= XtAppInitialize(&app, progclass, options, arg_Count, &argc, argv, defaults, 0, 0);
  ...
  arg_FlagScreensaver= get_boolean_resource(dpy, XSUBLIM_ARG_SCREENSAVER, "Boolean");
  ...
  arg_DelayPhraseMin= get_integer_resource(dpy, XSUBLIM_ARG_DELAYPHRASEMIN, "Integer");
  arg_DelayPhraseMax= get_integer_resource(dpy, XSUBLIM_ARG_DELAYPHRASEMAX, "Integer");
  ...
  if (arg_Source != NULL)
    ...
    {
      arg_Source= get_string_resource(dpy, XSUBLIM_ARG_PROGRAM, "Executable");
      if (arg_Source != NULL)
        {
          ...
          if (arg_Text != NULL)
            {
              if (fread(arg_Text, 1, XSUBLIM_PROGRAM_SIZE, exe_Fs) <= 0)
                {
                  fprintf(stderr,
  ...
  gc_GcFore= XCreateGC(dpy, win_Root, (GCFont | GCForeground | GCBackground | GCSubwindowMode), &gc_ValFore);
  ...
  gc_GcBack= XCreateGC(dpy, win_Root, (GCFont | GCForeground | GCBackground | GCSubwindowMode), &gc_ValBack);
  ...
  while (Xsublim_Sig_Last == -1)
    {
      ...
      strncpy(text_Text, text_List [ text_Item ], XSUBLIM_TEXT_LENGTH);
      ...
      while (((text_Word= strtok(text_Phrase, " \t")) != NULL) &&
        (Xsublim_Sig_Last == -1))
        {
          ...
          text_Width= XTextWidth(font_Font, text_Word, text_Length) + XSUBLIM_TEXT_OUTLINE * 2;
          ...
          if (arg_FlagCenter == FALSE)
            {
              ...
              text_Y= random() % (attr_Win.height - text_Height);
            }
          else
            {
          ...
          image_Image= XGetImage(dpy, win_Root, image_X, image_Y, image_Width, image_Height, ~0L, ZPixmap);
              ...
              if (Xsublim_Sh_Status == 0)
                {
                  ...
                  if (arg_FlagOutline != FALSE)
                    {
                      for (text_OutlineIndex= 0;
                        text_Outline [ text_OutlineIndex ].outline_X != 0;
                        text_OutlineIndex++)
                        {
                          /* Y'know, eight
                             character tabs and
                          ...
                          XDrawString(
                            ...
                            win_Root,
                            gc_GcBack,
                            text_X + text_Outline [ text_OutlineIndex ].outline_X * XSUBLIM_TEXT_OUTLINE,
                            text_Y +
                              (font_Font->ascent) +
                              text_Outline [ text_OutlineIndex ].outline_Y *
                  ...
                  /* Draw the word */
                  XDrawString(dpy, win_Root, gc_GcFore, text_X, text_Y + (font_Font->ascent), text_Word, text_Length);
                }
              if (Xsublim_Sh_Status == 0)
                {
                  /* Wait a bit */
                  ...
                  XPutImage(dpy, win_Root, gc_GcFore, image_Image, 0, 0, image_X, image_Y, image_Width, image_Height);
                }
      ...
      if (Xsublim_Sig_Last == -1)
        usleep(random() % (arg_DelayPhraseMax - arg_DelayPhraseMin + 1) + arg_DelayPhraseMin);
    }
