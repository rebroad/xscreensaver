...
struct state
{
    ...
    int cs1 [ LAYERS ], cs2 [ LAYERS ], cs3 [ LAYERS ];
    int cs4 [ LAYERS ];
    ...
    Bool closed_p)
{
  ...
  make_color_ramp(screen, visual, cmap, h1, s1, v1, h2, s2, v2, colors, ncolorsP, False, True, 0);
}
      ...
      for (c1= 0; c1 < st->ncolors; c1++)
        {
          ...
          make_color_ramp_rgb(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, basecol [ col [ c1 ] ][ 0 ], basecol [ col [ c1 ] ][ 1 ], basecol [ col [ c1 ] ][ 2 ], 0xFFFF, 0xFFFF, 0xFFFF, st->colors, &st->shades, False);
        }
      return;
    }
  /* not 3d */
  ...
  switch (st->cmap)
    {
      ...
      case 3 :
        ...
        if (st->cmap > 1)
          r2= g2= b2= 0xFFFF;
        make_color_ramp_rgb(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, r1, g1, b1, r2, g2, b2, st->colors, &st->ncolors, random() % 2);
        break;
      ...
      case 7 :
        ...
        make_color_loop(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, h1, s1, v1, h2, s2, v2, h3, s3, v3, st->colors, &st->ncolors, True, False);
        break;
      case 8 : /* random smooth */
      case 9 :
        st->ncolors= (random() % 4) * 6 + 12;
        make_smooth_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, True, False, True);
        break;
      case 10 : /* rainbow */
        st->ncolors= (random() % 4) * 6 + 12;
        make_uniform_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, True, False, True);
        break;
      ...
      case 14 :
        st->ncolors= 7;
        make_color_ramp_rgb(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, r1, g1, b1, 0xFFFF, 0xFFFF, 0xFFFF, tmp_col1, &st->ncolors, False);
        make_color_ramp_rgb(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, r2, g2, b2, 0xFFFF, 0xFFFF, 0xFFFF, tmp_col2, &st->ncolors, False);
        if (st->cmap < 13)
          {
            for (c1= 0; c1 <= 4; c1++)
              {
          ...
          {
            make_color_ramp_rgb(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, r3, g3, b3, 0xFFFF, 0xFFFF, 0xFFFF, tmp_col3, &st->ncolors, False);
            for (c1= 0; c1 <= 4; c1++)
              {
      ...
      default : /* random */
        st->ncolors= (random() % 4) * 6 + 12;
        make_random_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, False, True, False, True);
        break;
    }
    ...
    struct state *st)
{
  ...
  st->d3d= ((st->tile == TILE_FLAT) || (st->tile == TILE_THIN) ||
    ...
    ((st->ii == 1) && (! st->newcols)) ? D3D_TILED :
                                         (random() % 5) % 4;
  /* st->d3d=D3D_BLOCK; st->lwid=2; */
  st->outline= (st->tile == TILE_OUTLINE)         ? 1 :
    ((st->tile != TILE_RANDOM) || (random() % 5)) ? 0 :
                                                    1;
  st->round= (st->d3d == D3D_NEON)                              ? 1 :
    ((st->d3d == D3D_BLOCK) || (st->outline) || (random() % 6)) ? 0 :
                                                                  1;
  ...
  st->layers= (random() % 2) ? 2 : (random() % 2) ? 1 :
    (random() % 2)                                ? 3 :
                                                    4;
    ...
    struct state *st)
{
      ...
      if (dir == DIR_NONE)
        {
          ...
          while ((dir == DIR_NONE) ||
            ...
            dir= random() % 4;
          bz= (dir == DIR_UP) ? z - st->gridx : (dir == DIR_DOWN) ? z + st->gridx :
            (dir == DIR_LEFT)                                     ? z - 1 :
                                                                    z + 1;
          bl= st->grid [ bz ].line;
        }
  ...
  st->li++;
  st->dline [ st->li ].len= (lt == LINE_FORCE) ? 1 : (lt == LINE_BRIN) ? st->olen + 1 :
    (! st->forcemax)                                                   ? st->olen :
                                                                         1 + random() % st->olen;
  st->dline [ st->li ].x= x;
  if (dir == DIR_LEFT)
    ...
    int adj)
{
  ...
  if (! st->round)
    ...
    { /* horizontal */
      a= (h - 1) / 2;
      for (b= 0; b <= a; b++)
        XFillRectangle(st->display, st->window, st->fgc, x + b, y + a - b, w - b * 2, h - ((a - b) * 2));
    }
  else
    { /* vertical */
      a= (w - 1) / 2;
      for (b= 0; b <= a; b++)
        XFillRectangle(st->display, st->window, st->fgc, x + a - b, y + b, w - ((a - b) * 2), h - b * 2);
    }
}
    ...
    int y3)
{
  ...
  XFillPolygon(st->display, st->window, st->fgc, points, 3, Convex, CoordModeOrigin);
}
    ...
    int y4)
{
  ...
  points [ 3 ].y= y4;
  XSetForeground(st->display, st->fgc, st->colors [ color ].pixel);
  XFillPolygon(st->display, st->window, st->fgc, points, 4, Convex, CoordModeOrigin);
}
      ...
      switch (d)
        {
          ...
          case 15 :
            h= ((d == 1) || (d == 5)) ? lr : nl;
            XSetForeground(st->display, st->fgc, st->colors [ color ].pixel);
            XFillRectangle(st->display, st->window, st->fgc, x, y, m2, h);
            XSetForeground(st->display, st->fgc, st->colors [ color + 3 ].pixel);
            XFillRectangle(st->display, st->window, st->fgc, x + m2, y, m1, h);
            break;
          case 4 :
          case 8 : /* horizontal */
          case 9 :
          ...
          case 14 :
            w= (d == 4) ? lr : nl;
            XSetForeground(st->display, st->fgc, st->colors [ color + 1 ].pixel);
            XFillRectangle(st->display, st->window, st->fgc, x, y, w, m2);
            XSetForeground(st->display, st->fgc, st->colors [ color + 2 ].pixel);
            XFillRectangle(st->display, st->window, st->fgc, x, y + m2, w, m1);
            break;
        }
      /* draw angles */
      switch (d)
      ...
      switch (st->d3d)
        {
          case D3D_NEON :
            ...
            for (sh= 0; sh < st->lwid / 2; sh++)
              {
                XSetForeground(st->display, st->fgc, st->colors [ color + sh ].pixel);
                di= st->di;
                while (di > 0)
                  {
          ...
          case D3D_BLOCK :
            ...
            for (sh= 0; sh < st->lwid / 2; sh++)
              {
                XSetForeground(st->display, st->fgc, st->colors [ color + (st->lwid / 2) - sh - 1 ].pixel);
                di= st->di;
                while (di > 0)
                  {
                    _XFillRectangle(st, di, sh);
  ...
  for (st->di= st->bi; st->di < _min(st->eli + 1, st->bi + st->elpu); st->di++)
    {
      if (st->eline [ st->di ].hv)
        XFillRectangle(st->display, st->window, st->bgc, st->eline [ st->di ].x * st->elwid, st->eline [ st->di ].y * st->elwid, (st->eline [ st->di ].len + 1) * st->elwid, st->elwid);
      else
        XFillRectangle(st->display, st->window, st->bgc, st->eline [ st->di ].x * st->elwid, st->eline [ st->di ].y * st->elwid, st->elwid, (st->eline [ st->di ].len + 1) * st->elwid);
      if (st->di == st->eli) /* clear just in case */
        XFillRectangle(st->display, st->window, st->bgc, 0, 0, st->xgwa.width, st->xgwa.height);
    }
  if (st->di > st->eli)
    {
      st->bi= 1;
    ...
    Window window)
{
  ...
  if (tile && ! strcmp(tile, "random"))
    {
      st->tile= TILE_RANDOM;
    }
  else if (tile && ! strcmp(tile, "flat"))
    {
      st->tile= TILE_FLAT;
    }
  else if (tile && ! strcmp(tile, "thin"))
    {
      st->tile= TILE_THIN;
    }
  else if (tile && ! strcmp(tile, "outline"))
    {
      st->tile= TILE_OUTLINE;
    }
  else if (tile && ! strcmp(tile, "block"))
    {
      st->tile= TILE_BLOCK;
    }
  else if (tile && ! strcmp(tile, "neon"))
    {
      st->tile= TILE_NEON;
    }
  else if (tile && ! strcmp(tile, "tiled"))
    {
      st->tile= TILE_TILED;
    }
  else
  ...
  gcv.foreground= get_pixel_resource(display, st->xgwa.colormap, "foreground", "Foreground");
  st->fgc= XCreateGC(display, window, GCForeground, &gcv);
  gcv.foreground= get_pixel_resource(display, st->xgwa.colormap, "background", "Background");
