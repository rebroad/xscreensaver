...
static const struct tiles def_tiles [ TILES_COUNT ]= {
  ...
  {"     " "  `  " "##`##" "  `  " "     ", {GO_UP, GO_DOWN, 0, 0}, 2, (unsigned) (1 << 1 | 1 << 7 | 1 << 9 | 1 << 10)},
  {"   ##" "##`##" "##`` " "#### " "#### ", {GO_UP, GO_RIGHT, 0, 0}, 2, (unsigned) (1 << 2 | 1 << 6 | 1 << 7 | 1 << 10)},
  {"#### " "#### " "##`` " "##`##" "   ##", {GO_RIGHT, GO_DOWN, 0, 0}, 2, (unsigned) (1 << 3 | 1 << 7 | 1 << 8 | 1 << 10)},
  {"  ###" "  ###" " ``##" "##`  " "##   ", {GO_LEFT, GO_DOWN, 0, 0}, 2, (unsigned) (1 << 4 | 1 << 8 | 1 << 9 | 1 << 10)},
  {"##   " "##`##" " ``##" " ####" " ####", {GO_LEFT, GO_UP, 0, 0}, 2, (unsigned) (1 << 5 | 1 << 6 | 1 << 9 | 1 << 10)},
  ...
  while (ndirs > 0)
    {
      ...
      switch (curdir)
        {
          case GO_UP :
            if (y < 1 || (ret= creatlevelblock(pp, level, x, y - 1)) == 0)
              return 0;
            break;
          case GO_RIGHT :
            if (x > LEVWIDTH - 2 || (ret= creatlevelblock(pp, level, x + 1, y)) == 0)
              return 0;
            break;
          case GO_DOWN :
            if (y > LEVHEIGHT - 2 || (ret= creatlevelblock(pp, level, x, y + 1)) == 0)
              return 0;
            break;
          case GO_LEFT :
            if (x < 1 || (ret= creatlevelblock(pp, level, x - 1, y)) == 0)
              return 0;
        }
      if (ret != -1)
        inc+= (unsigned) ret;
  ...
  while (tried != 0x00)
    {
      ...
      if (tryset(level, x, y, pp->tiles [ tilenr ].block) != 0)
        {
          if ((ret= nextstep(pp, level, x, y, pp->tiles [ tilenr ].dirvec, pp->tiles [ tilenr ].ndirs)) != 0)
            return ret + 1;
...
static void
  top_left(
    lev_t *level,
    unsigned int *passed_x,
    unsigned int *passed_y)
{
  int x, y;
  for (y= 0; y < LEVHEIGHT; y++)
    {
  ...
  if (NRAND(2) == 0)
    {
      ...
      do {
          ...
          if ((ret= nextstep(pp, level, LEVWIDTH / 2 - 1, LEVHEIGHT / 2 - JAILHEIGHT / 2 - 3, dirvec, 1)) == 0)
            {
