  ...
  if (! mono_p)
    {
      ...
      make_smooth_colormap(pxgwa->screen, pxgwa->visual, st->cmap, st->colors, &st->ncolors, True, /* allocate */
        False,                                                                                     /* not writable */
  ...
  int y_min)
{
  ...
  if (st->width > st->height * 5 || /* window has weird aspect */
    ...
    {
      Circle *p;
      double r= (st->width > st->height ? st->width / (double) st->height : st->height / (double) st->width);
      for (p= pb->epicycles; p; p= p->pchild)
        p->radius*= r;
    }
}
  ...
  if (st->restart)
    {
      ...
      precalculate_figure(st->pb0, st->xtime, st->timestep_coarse, &st->xmax, &st->ymax, &st->xmin, &st->ymin);
