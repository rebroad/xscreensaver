...
MONO:
  ...
  if (mono_p)
  ...
  else
    {
      ...
      make_color_ramp(xgwa.screen, xgwa.visual, xgwa.colormap, fgh, fgs, fgv, bgh, bgs, bgv, st->colors, &st->ncolors, True, True, False);
  ...
  if (st->draw_y == 0)
    {
      ...
      st->draw_image= create_xshm_image(st->dpy, st->xgwa.visual, st->depth, ZPixmap, &st->shm_info, /* depth, format, shm_info */
        st->xgwa.width,
        chunk_size); /* w, h */
    }
