...
static void char_to_pixmap(p_state *state, p_char *pc, unsigned long c, Bool invert_p, Bool symbol_p);
...
static unsigned short scale_color_channel(
  unsigned short ch1,
  unsigned short ch2)
{
...
#error BUILTIN_FONT is required unless HAVE_XFT.
  ...
  font= state->font;
  state->xftdraw= XftDrawCreate(dpy, window, state->xgwa.visual, state->xgwa.colormap);
...

  {
    ...
    unsigned long fg= get_pixel_resource(state->dpy, state->xgwa.colormap, "foreground", "Foreground");
    unsigned long bg= get_pixel_resource(state->dpy, state->xgwa.colormap, "background", "Background");
    ...
    make_color_ramp(state->xgwa.screen, state->xgwa.visual, state->xgwa.colormap, h1, s1, v1, h2, s2, v2, colors, &ncolors, False, True, False);
    ...
    state->gcs [ BLANK ]= XCreateGC(state->dpy, state->window, flags, &state->gcv);

    state->gcv.foreground= flare;
    state->gcs [ FLARE ]= XCreateGC(state->dpy, state->window, flags, &state->gcv);

    state->gcv.foreground= fg;
    state->gcs [ NORMAL ]= XCreateGC(state->dpy, state->window, flags, &state->gcv);

    for (i= 0; i < ncolors; i++)
      {
        state->gcv.foreground= colors [ i ].pixel;
        state->gcs [ STATE_MAX + i ]= XCreateGC(state->dpy, state->window, flags, &state->gcv);
      }
    ...
    Bool symbol_p)
{
  ...
  if (! font)
    {
      ...
      XWindowAttributes xgwa;
      Pixmap m= 0;
      Pixmap p= image_data_to_pixmap(state->dpy, state->window, _6x10font_png, sizeof(_6x10font_png), &pix_w, &pix_h, &m);
      ...
      im2= XCreateImage(state->dpy, xgwa.visual, 1, XYBitmap, 0, 0, pix_w, pix_h, 8, 0);
      im2->data= malloc(im2->bytes_per_line * im2->height);

      /* Convert deep image to 1 bit */
      for (y= 0; y < pix_h; y++)
        for (x= 0; x < pix_w; x++)
          XPutPixel(im2, x, y, (XGetPixel(mm, x, y) ? (XGetPixel(im, x, y) == black) : 0));

      XDestroyImage(im);
      XDestroyImage(mm);
      im= 0;

      p2= XCreatePixmap(state->dpy, state->window, im2->width, im2->height, im2->depth);
      gcv.foreground= 1;
      gcv.background= 0;
    ...
    {
      XGlyphInfo overall;
      XftTextExtentsUtf8(state->dpy, state->font, (FcChar8 *) "N", 1, &overall);
  ...
  p= XCreatePixmap(state->dpy, state->window, (safe_width * 256), height, 1);
  ...
  state->gc0= XCreateGC(state->dpy, p, (GCForeground | GCBackground), &state->gcv);

  state->gcv.foreground= 1;
  state->gc1= XCreateGC(state->dpy, p, (GCForeground | GCBackground | GCCapStyle | GCLineWidth), &state->gcv);
...
#ifdef FUZZY_BORDER
  {
    ...
    if (state->gcv.line_width < 1)
      state->gcv.line_width= 1;
    state->gc2= XCreateGC(state->dpy, p, (GCForeground | GCBackground | GCCapStyle | GCLineWidth), &state->gcv);
  }
#endif /* FUZZY_BORDER */
  ...
  if (p2)
    {
      XCopyPlane(state->dpy, p2, p, state->gc1, 0, 0, FONT6x10_WIDTH, FONT6x10_HEIGHT, 0, 0, 1);
...
#endif /* BUILTIN_FONT */
    {
      ...
      Pixmap pm_mono= XCreatePixmap(state->dpy, state->window, (safe_width * 256), height, 1);
#else  /* !USE_XFT_BITMAP */
    Pixmap pm_color= XCreatePixmap(state->dpy, state->window, (safe_width * 256), height, state->xgwa.depth);
    XImage *xim_color;
    int x, y;
      ...
      XFillRectangle(state->dpy, pm_mono, text_gc, 0, 0, (safe_width * 256), height);
    ...
    XFillRectangle(state->dpy, pm_color, text_gc, 0, 0, (safe_width * 256), height);
    xftdraw= XftDrawCreate(state->dpy, pm_color, state->xgwa.visual, state->xgwa.colormap);
    xft_fg.pixel= ~0L;
    xft_fg.color.red= xft_fg.color.green= xft_fg.color.blue= ~0L;
      ...
      for (i= 0; i < 256; i++)
        {
          if (! symbol_p)
            {
              XftDrawStringUtf8(xftdraw, &xft_fg, state->font, i * safe_width, font->ascent, (FcChar8 *) (string + i), 1);
            }
          else
            {
              ...
              XftDrawStringUtf8(xftdraw, &xft_fg, state->font, i * safe_width, font->ascent, (FcChar8 *) ss, L);
            }
        }

#ifdef USE_XFT_BITMAP
      /* Retrieve a 1-bit XImage. */
      xim_mono= XGetImage(state->dpy, pm_mono, 0, 0, i * safe_width, font->ascent, 1, XYPixmap);
      if (! xim_mono) abort();
    ...
    xim_color= XGetImage(state->dpy, pm_color, 0, 0, i * safe_width, font->ascent, ~0L, ZPixmap);

    /* Convert it to a mono XImage. */
    xim_mono= XCreateImage(state->dpy, state->xgwa.visual, 1, XYPixmap, 0, 0, i * safe_width, font->ascent, 8, 0);
    xim_mono->data= (char *)
    ...
    xim_color= 0;
      ...
      XPutImage(state->dpy, p, text_gc, xim_mono, 0, 0, 0, 0, (safe_width * 256), height);
      XFreeGC(state->dpy, text_gc);
  ...
  {
    XImage *im= XGetImage(state->dpy, p, 0, 0, (safe_width * 256), height, ~0L, XYPixmap);
    if (symbol_p)
          ...
          if (pix)
            {
              ...
              x2--;
              XDrawLine(state->dpy, p, gc, (x1 - from) * state->scale + xoff, y * state->scale, (x2 - from) * state->scale + xoff, y * state->scale);
#ifdef FUZZY_BORDER
              XDrawLine(state->dpy, p2, gc2, (x1 - from) * state->scale + xoff, y * state->scale, (x2 - from) * state->scale + xoff, y * state->scale);
#endif /* FUZZY_BORDER */
              x1= x2;
              pc->blank_p= False;
            }
  ...
  if (get_boolean_resource(state->dpy, "cache", "Cache"))
    {
      ...
      int st;
      pc->cache= XCreatePixmap(state->dpy, state->window, width * state->ticks, height, state->xgwa.depth);
      for (st= 0; st < state->ticks; st++)
        {
          int tx= st * width;
          int ty= 0;
          GC gc1= state->gcs [ st ];
          GC gc2= ((st + 2) < state->ticks ? state->gcs [ st + 2 ] : 0);
          GC gc3= (gc2 ? gc2 : gc1);
          if (gc3)
            XCopyPlane(state->dpy, pc->pixmap, pc->cache, gc3, 0, 0, width, height, tx, ty, 1L);
          if (gc2)
            {
              XSetClipMask(state->dpy, gc1, pc->pixmap2);
              XSetClipOrigin(state->dpy, gc1, tx, ty);
              XFillRectangle(state->dpy, pc->cache, gc1, tx, ty, width, height);
    ...
    XtIntervalId *id)
{
  ...
  state->cursor_timer= XtAppAddTimeOut(app, state->cursor_blink, cursor_on_timer, closure);
}
    ...
    XtIntervalId *id)
{
  ...
  state->cursor_timer= XtAppAddTimeOut(app, 2 * state->cursor_blink, cursor_off_timer, closure);
}
      ...
      for (x= 0; x < tty->width; x++)
        {
          ...
          pc= (cell->symbol_p ? (inv_p ? state->sichars [ cell->c ] : state->schars [ cell->c ]) : (inv_p ? state->ichars [ cell->c ] : state->chars [ cell->c ]));
      ...
      for (x= 0; x < state->grid_width; x++)
        {
          ...
          if (! c) c= ' ';
          pc= (sym_p ? (inv_p ? state->sichars [ c ] : state->schars [ c ]) : (inv_p ? state->ichars [ c ] : state->chars [ c ]));
          ...
          if (pc->blank_p || (cell->state == BLANK && ! cursor_p))
            {
              XFillRectangle(state->dpy, state->window, state->gcs [ BLANK ], tx, ty, width, height);
            }
#ifdef FUZZY_BORDER
          else if (pc->cache)
            {
              int src_x= st * width;
              int src_y= 0;
              XCopyArea(state->dpy, pc->cache, state->window, state->gcs [ BLANK ], src_x, src_y, width, height, tx, ty);
            }
#endif /* FUZZY_BORDER */
          else
            {
              ...
              GC gc3= (gc2 ? gc2 : gc1);
              if (gc3)
                XCopyPlane(state->dpy, pc->pixmap, state->window, gc3, 0, 0, width, height, tx, ty, 1L);
              if (gc2)
                {
                  XSetClipMask(state->dpy, gc1, pc->pixmap2);
                  XSetClipOrigin(state->dpy, gc1, tx, ty);
                  XFillRectangle(state->dpy, state->window, gc1, tx, ty, width, height);
                  XSetClipMask(state->dpy, gc1, None);
                }
#else  /* !FUZZY_BORDER */
