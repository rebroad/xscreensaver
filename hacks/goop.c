    ...
    Bool fill_p)
{
...
#ifdef DEBUG
  {
    int i;
    for (i= 0; i < b->npoints; i++)
      XDrawLine(dpy, drawable, gc, b->x / SCALE, b->y / SCALE, b->spline->control_x [ i ], b->spline->control_y [ i ]);
  }
#else
  if (fill_p)
    XFillPolygon(dpy, drawable, gc, b->spline->points, b->spline->n_points, Nonconvex, CoordModeOrigin);
  else
#endif
  XDrawLines(dpy, drawable, gc, b->spline->points, b->spline->n_points, CoordModeOrigin);
}
  ...
  for (i= 0; i < layer->nblobs; i++)
    {
      ...
      layer->blobs [ i ]= make_blob(dpy, width, height, (j ? random() % j : 0) + blob_min);
    }
    ...
    long depth)
{
  ...
  if (goop->mode == transparent)
    {
      ...
      allocate_alpha_colors(screen, visual, cmap, &nplanes, goop->additive_p, &plane_masks, &base_pixel);
      if (nplanes > 1)
        {
...

  {
    ...
    for (i= 0; i < goop->nlayers; i++)
      goop->layers [ i ]= make_layer(dpy, window, width, height, (nblobs > 0 ? nblobs : lblobs [ i ]));
  }
  ...
  goop->pixmap= XCreatePixmap(dpy, window, width, height, (goop->mode == xor ? 1L : depth));
    ...
    Window window)
{
  ...
  return make_goop(xgwa.screen, xgwa.visual, window, xgwa.colormap, xgwa.width, xgwa.height, xgwa.depth);
}
  ...
  switch (goop->mode)
    {
      ...
      case transparent :
        ...
        XFillRectangle(dpy, goop->pixmap, goop->pixmap_gc, 0, 0, goop->width, goop->height);
        ...
        if (! goop->cmap_p && ! goop->additive_p)
          {
            ...
            for (i= 0; i < goop->nlayers; i++)
              for (j= 0; j < goop->layers [ i ]->nblobs; j++)
                draw_blob(dpy, goop->pixmap, goop->pixmap_gc, goop->layers [ i ]->blobs [ j ], True);
        ...
        for (i= 0; i < goop->nlayers; i++)
          {
            ...
            draw_layer_blobs(dpy, goop->pixmap, goop->pixmap_gc, goop->layers [ i ], goop->width, goop->height, True);
          }
        XCopyArea(dpy, goop->pixmap, window, goop->window_gc, 0, 0, goop->width, goop->height, 0, 0);
        break;
#endif /* !HAVE_JWXYZ */
      ...
      case xor :
        ...
        for (i= 0; i < goop->nlayers; i++)
          draw_layer_blobs(dpy, goop->pixmap, goop->pixmap_gc, goop->layers [ i ], goop->width, goop->height, (goop->mode != outline));
        XCopyPlane(dpy, goop->pixmap, window, goop->window_gc, 0, 0, goop->width, goop->height, 0, 0, 1L);
        break;
