    ...
    int *n_colors)
{
  ...
  *colors= (XColor *) calloc(sizeof(XColor), *n_colors);
  ...
  if (! strcmp(color_scheme, "random"))
    make_random_colormap(st->wattr.screen, st->wattr.visual, st->wattr.colormap, *colors, n_colors, True, True, &writable, True);
  else if (! strcmp(color_scheme, "smooth"))
    make_smooth_colormap(st->wattr.screen, st->wattr.visual, st->wattr.colormap, *colors, n_colors, True, &writable, True);
  else
    make_uniform_colormap(st->wattr.screen, st->wattr.visual, st->wattr.colormap, *colors, n_colors, True, &writable, True);
    ...
    int pos)
{
  ...
  XDrawLine(st->dpy, st->window, gc, st->history [ pos ].x * cell_size + half, st->history [ pos ].y * cell_size + half, st->history [ old_pos ].x * cell_size + half, st->history [ old_pos ].y * cell_size + half);
}
    ...
    Window window)
{
  ...
  model_h= st->settings.cell_size ?
    st->wattr.height / st->settings.cell_size :
    1;
  ...
  if (! st->history)
    {
      fprintf(stderr,
        "critical: " "couldn't allocate trail history of %d cells\n",
        st->settings.trail);
      abort();
    }
  ...
  st->gcv.foreground= get_pixel_resource(st->dpy, st->wattr.colormap, "background", "Background");
