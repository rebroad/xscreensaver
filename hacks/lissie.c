...
ModStruct lissie_description=
  {"lissie", "init_lissie", "draw_lissie", (char *) NULL, "refresh_lissie", "init_lissie", "free_lissie", &lissie_opts, 10000, 1, 2000, -200, 64, 0.6, "", "Shows lissajous worms", 0, NULL};
    ...
    lissiestruct *lissie)
{
  ...
  if (size < -MINSIZE)
    lissie->ri= NRAND(MIN(-size, MAX(MINSIZE, MIN(lp->width, lp->height) / 4)) - MINSIZE + 1) + MINSIZE;
  ...
  lissie->rx= INTRAND(lp->width / 4,
                MIN(lp->width - lissie->xi, lissie->xi)) -
    2 * lissie->ri;
  lissie->ry= INTRAND(lp->height / 4,
                MIN(lp->height - lissie->yi, lissie->yi)) -
    2 * lissie->ri;
