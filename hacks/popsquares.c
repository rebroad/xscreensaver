    ...
    unsigned int h)
{
  ...
  if (st->xgwa.width < 100 || st->xgwa.height < 100) /* tiny window */
    {
      int ss= (st->xgwa.width < st->xgwa.height ? st->xgwa.width : st->xgwa.height);
      s= ss / 15;
      if (s < 1) s= 1;
    }
    ...
    Window window)
{
  ...
  make_color_ramp(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, h1, s1, v1, h2, s2, v2, st->colors, &st->ncolors, /* would this be considered a value-result argument? */
    True,
    True,
    False);
  if (st->ncolors < 2)
    ...
    void *closure)
{
      ...
      for (x= 0; x < st->gw; x++)
        {
          ...
          XFillRectangle(st->dpy, st->b, st->gc, s->x, s->y, st->border ? s->w - st->border : s->w, st->border ? s->h - st->border : s->h);
          s->color++;
  ...
  if (st->backb)
    ...
    if (st->dbuf)
      {
        XCopyArea(st->dpy, st->b, st->window, st->gc, 0, 0, st->xgwa.width, st->xgwa.height, 0, 0);
