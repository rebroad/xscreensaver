...
#define BitmapPad(dpy) 8
  XImage *image= XCreateImage(dpy, visual, depth, format, 0, NULL, width, height, BitmapPad(dpy), 0);
  int error= thread_malloc((void **) &image->data, dpy, image->height * image->bytes_per_line);
  if (error)
    {
      XDestroyImage(image);
      image= NULL;
...
Bool put_xshm_image(
  ...
  XShmSegmentInfo *shm_info)
{
  return XPutImage(dpy, d, gc, image, src_x, src_y, dest_x, dest_y, width, height);
}
  ...
  if (st->logo)
    {
      double aspect= (double)
                       st->output_frame->width /
        st->output_frame->height;
      double scale= (aspect > 1 ? 0.35 : 0.6);
      int w2= st->tv->xgwa.width * scale;
      int h2= st->tv->xgwa.height * scale * aspect;
      analogtv_load_ximage(st->tv, input, st->logo, st->logo_mask, (st->tv->xgwa.width - w2) / 2, st->tv->xgwa.height * 0.20, w2, h2);
    }
    ...
    int new_height)
{
  ...
  if (! ximage2->data)
    {
      fprintf(stderr, "%s: out of memory scaling %dx%d image to %dx%d\n", progname, ximage->width, ximage->height, ximage2->width, ximage2->height);
    ...
    Bool powerp)
{
    ...
    for (i= 0; i < nfiles; i++)
      {
        ...
        ximages [ i ]= ximage;
        if (verbose_p > 1)
          fprintf(stderr, "%s: loaded %s %dx%d\n", progname, infiles [ i ], ximage->width, ximage->height);
        flip_ximage(ximage);
  ...
  st->output_frame= XCreateImage(dpy, 0, ximages [ 0 ]->depth, ximages [ 0 ]->format, 0, NULL, output_w, output_h, ximages [ 0 ]->bitmap_pad, 0);
  ...
  if (logofile)
    {
      ...
      st->logo_mask= XCreateImage(dpy, 0, st->logo->depth, st->logo->format, 0, NULL, st->logo->width, st->logo->height, st->logo->bitmap_pad, 0);
  ...
  ffst= ffmpeg_out_init(outfile, audiofile, st->output_frame->width, st->output_frame->height, 4, True);
  ...
  if (slideshow)
    {
      ...
      base_image= ximage;
      if (verbose_p > 1)
        fprintf(stderr, "%s: initializing for %s %dx%d in %d channels\n", progname, infiles [ n ], ximage->width, ximage->height, MAX_STATIONS);
      ...
      for (i= 0; i < MAX_STATIONS; i++)
        {
          ...
          if (i == 1)
            { /* station 0 is the unadulterated image.
          ...
          if (now > (verbose_p == 1 ? lastlog : lastlog + 10))
            {
              ...
              for (ii= 0; ii < cols * ratio; ii++)
                dots [ ii ]= '.';
              dots [ ii ]= 0;
              fprintf(stderr, "%s%s: %s %2d%%, %d:%02d:%02d ETA%s", (verbose_p == 1 ? "\r" : ""), progname, dots, pct, (remaining / 60 / 60), (remaining / 60) % 60, remaining % 60, (verbose_p == 1 ? "" : "\n"));
              lastlog= now;
            }
        }
    }
  ...
  if (verbose_p > 1)
    {
      ...
      for (i= 0; i < N_CHANNELS; i++)
        fprintf(stderr, "%s:   %2d: %3d%%\n", progname, i + 1, stats [ i ] * 100 / channel_changes);
    }
  ...
  for (i= 1; i < argc; i++)
    {
      ...
      else if (! strcmp(argv [ i ], "-vvvvv"))
        {
          verbose_p+= 5;
        }
      else if (! strcmp(argv [ i ], "-vvvvvv"))
        {
          verbose_p+= 6;
        }
      else if (! strcmp(argv [ i ], "-vvvvvvv"))
        {
          verbose_p+= 7;
        }
      else if (! strcmp(argv [ i ], "-vvvvvvvv"))
        {
          verbose_p+= 8;
        }
      else if (! strcmp(argv [ i ], "-vvvvvvvvv"))
        {
          verbose_p+= 9;
        }
      else if (! strcmp(argv [ i ], "-vvvvvvvvvv"))
        {
          verbose_p+= 10;
        }
...
#undef ya_rand_init
  ya_rand_init(0);
  analogtv_convert(infiles, outfile, audio, logo, w, h, duration, slideshow, powerp);
  exit(0);
}
