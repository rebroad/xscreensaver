...
static inline void fringe_add_intersection(
  fringe *f,
  double x,
  double y)
{
...
static int event_cut(
  ...
  event *e)
{
  return st->event_cut_y == e->y ? 0 : st->event_cut_y < e->y ? -1 :
                                                                1;
}

static void event_insert(
  struct state *st,
  event **eq,
  event *e)
{
...
static void circle_start_event(
  struct state *st,
  event **eq,
  circle *c)
{
...
static void circle_finish_event(
  struct state *st,
  event **eq,
  circle *c)
{
...
static int fringe_start_cut(
  struct state *st,
  fringe *f)
{
  double x= fringe_x(f, st->fringe_start_cut_y);
  return st->fringe_start_cut_x == x ? 0 : st->fringe_start_cut_x < x ? -1 :
                                                                        1;
}

static fringe *fringe_start(
  struct state *st,
  event **eq,
  fringe *f,
  double x,
  ...
  fringe *f)
{
  ...
  return st->fringe_double_cut_x == x ? 0 : st->fringe_double_cut_x < x ? -1 :
                                                                          1;
}

static int fringe_double_splay(
  struct state *st,
  fringe *f,
  double x,
  double y,
...
static void adjust_circle_visibility(
  circle *c)
{
  int i, j, n, a;
  int *in;
...
MONO:
    ...
    {
      make_color_loop(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, 0, 1, 1, 120, 1, 1, 240, 1, 1, st->colors, &st->ncolors, True, False);
      if (st->ncolors < 2)
        goto MONO;
    }
