    ...
    struct state *st)
{
   ...
   */
  XImage *image=
    XCreateImage(st->dpy, st->xgwa.visual, 1, XYBitmap, 0, /* depth, format, offset */
  ...
  if (! mono_p)
    {
      ...
      make_smooth_colormap(st->xgwa.screen, st->xgwa.visual, st->cmap, st->colors, &st->ncolors, True, 0, False);
      if (st->ncolors <= 2)
        mono_p= 1;
    }
  ...
  if (mono_p)
    {
      ...
      unsigned long fg_pixel=
        get_pixel_resource(st->dpy, st->xgwa.colormap, "foreground", "Foreground");
      unsigned long bg_pixel=
        get_pixel_resource(st->dpy, st->xgwa.colormap, "background", "Background");
      if (! st->colors)
        {
          st->ncolors= 50;
      ...
      for (y= 0; y < st->ymax; y+= st->ystep)
        {
          ...
          pixel= set(st, x, y1, st->iteration, ((int) CELL(x, y) + (int) CELL(x, y2) + 1) / 2);
          ...
          pixel= set(st, x1, y, st->iteration, ((int) CELL(x, y) + (int) CELL(x2, y) + 1) / 2);
          if (! mono_p &&
            (pixel != qpixels [ 0 ] || pixel != qpixels [ 1 ] ||
              pixel != qpixels [ 2 ] || pixel != qpixels [ 3 ]))
            draw(st, x1, y, pixel, st->ynextStep);

          pixel= set(st, x1, y1, st->iteration, ((int) CELL(x, y) + (int) CELL(x, y2) + (int) CELL(x2, y) + (int) CELL(x2, y2) + 2) / 4);
          if (! mono_p &&
            (pixel != qpixels [ 0 ] || pixel != qpixels [ 1 ] ||
              pixel != qpixels [ 2 ] || pixel != qpixels [ 3 ]))
