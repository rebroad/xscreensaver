...
static inline unsigned char *cell_at(
  struct field *f,
  unsigned int x,
  unsigned int y)
{
...
static inline unsigned char *new_cell_at(
  struct field *f,
  unsigned int x,
  unsigned int y)
{
  ...
  for (y= 1; y < f->height - 1; y++)
    {
      ...
      XDrawPoints(st->dpy, st->window, st->fgc, st->fg_points, fg_count, CoordModeOrigin);
      XDrawPoints(st->dpy, st->window, st->bgc, st->bg_points, bg_count, CoordModeOrigin);
    }
}
    ...
    struct field *f)
{
  ...
  memcpy(f->cells, f->new_cells, f->width * f->height * sizeof(unsigned char));
  return count;
}
    ...
    Window window)
{
  ...
  st->density= (get_integer_resource(st->dpy, "initialDensity", "Integer") % 100 * 256) / 100;
  ...
  if (st->cycle_colors)
    {
      ...
      make_smooth_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, st->colors, &st->ncolors, True, &tmp, True);
    }
    ...
    void *closure)
{
  ...
  if (st->field->height != st->xgwa.height / (1 << st->field->cell_size) + 2 ||
    st->field->width != st->xgwa.width / (1 << st->field->cell_size) + 2)
    {
      ...
      resize_field(st->field, st->xgwa.width / (1 << st->field->cell_size) + 2, st->xgwa.height / (1 << st->field->cell_size) + 2);
