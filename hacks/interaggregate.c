...
static inline unsigned long rgb2point(
  int depth,
  int r,
  int g,
  int b)
...
static inline unsigned long trans_point(
  int x1,
  int y1,
  unsigned long myc,
  double a,
  ...
  for (i= 0; i <= 10; ++i)
    {
      ...
      drawPoint(drawx, drawy, painter->color, intensity, dpy, window, fgc, f);
  ...
  for (i= 0; i < f->num_circles; ++i)
    {
      ...
      if (circle->path_type == LINEAR)
		...
		circle->y = -circle->radius;
          ...
          if (circle->x < 0)
            circle->x+= f->width;
          else if (circle->x >= f->width)
            circle->x-= f->width;

          if (circle->y < 0)
            circle->y+= f->height;
          else if (circle->y >= f->height)
            circle->y-= f->height;
#endif
        }
              ...
              if ((fabs(dx) < (c1->radius + c2->radius)) &&
                ...
                (d > fabs(c1->radius - c2->radius)))
                {
                  ...
                  for (s= 0; s < c1->num_painters; ++s)
                    paint(c1->painters + s, int1x, int1y, int2x, int2y, dpy, window, fgc, f);
                }
            }
        }
      else /* f->draw_centers */
    ...
    Window window)
{
  ...
  st->gcv.background= get_pixel_resource(st->dpy, st->xgwa.colormap, "background", "Background");
    ...
    void *closure)
{
  ...
  tdiff= (tm2.tv_sec - tm1.tv_sec) + (tm2.tv_usec - tm1.tv_usec) * 0.00001;

  if (tdiff > 1)
    {
      fprintf(stderr, "fps: %d %f %f\n", frames, tdiff, frames / tdiff);

      fprintf(stderr, "intersections: %d %d %f\n", f->intersection_count, f->possible_intersections, ((double) f->intersection_count) / f->possible_intersections);
