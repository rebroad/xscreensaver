    ...
    Window window)
{
  ...
  if (st->scrWidth > 2560 || st->scrHeight > 2560)
    { /* Retina displays */
      st->width*= 4;
      st->finpoints*= 2;
      st->withdraw*= 2;
  ...
  make_smooth_colormap(wa.screen, wa.visual, st->cmap, st->colors, &st->ncolors, True, 0, True);
  ...
  for (i= 0; i < st->arms; i++)
    {
      ...
      if (! withdrawall)
        ...
        {
          st->aCurr->growth= -st->aCurr->numpt;
        }
    ...
    double cost)
{
  ...
  for (q= 0; q < numpt - 1; q++)
    {
      ...
      XDrawLine(st->dpy, curr_window, st->gcDraw, mx2 + cx * cost - cz * sint, cy, mx2 + nx * cost - nz * sint, ny);
      st->vCurr++;
      st->vNext++;
  ...
  XSetLineAttributes(st->dpy, st->gcDraw, st->width * 3, LineSolid, CapRound, JoinBevel);
  XDrawLine(st->dpy, curr_window, st->gcDraw, st->mx / 2 + cx * cost - cz * sint, cy, st->mx / 2 + nx * cost - nz * sint, ny);
  XSetLineAttributes(st->dpy, st->gcDraw, st->width, LineSolid, CapRound, JoinBevel);
}
    ...
    void *closure)
{
  ...
  if (st->backb)
    ...
    if (st->dbuf)
      {
        XCopyArea(st->dpy, st->b, window, st->gcClear, 0, 0, st->scrWidth, st->scrHeight, 0, 0);
