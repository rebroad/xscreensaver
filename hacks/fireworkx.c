...
static inline unsigned char addbs(
  unsigned char c,
  unsigned int i)
{
  i+= c;
  ...
  for (y= st->height / 2; y; y--)
    {
      for (x= st->width / 2; x; x--, v+= 4)
        {
          r= rgb [ 0 ] * st->light_map [ v ] + rgb [ 4 ] * st->light_map [ v + 1 ] + rgb [ 8 ] * st->light_map [ v + 2 ] + rgb [ 12 ] * st->light_map [ v + 3 ];
          g= rgb [ 1 ] * st->light_map [ v ] + rgb [ 5 ] * st->light_map [ v + 1 ] + rgb [ 9 ] * st->light_map [ v + 2 ] + rgb [ 13 ] * st->light_map [ v + 3 ];
          b= rgb [ 2 ] * st->light_map [ v ] + rgb [ 6 ] * st->light_map [ v + 1 ] + rgb [ 10 ] * st->light_map [ v + 2 ] + rgb [ 14 ] * st->light_map [ v + 3 ];
  ...
  struct state *st)
{
  ...
  st->xim= XCreateImage(st->dpy, xwa.visual, xwa.depth, ZPixmap, 0, 0, st->width, st->height, 8, 0);
  if (! st->xim) return;
  ...
  if (st->depth == 8)
    {
      ...
      writable= False;
      make_smooth_colormap(xwa.screen, vi, cmap, st->colors, &st->ncolors, False, &writable, True);
    }
