...
static void analogtv_ntsc_to_yiq(const analogtv *it, int lineno, const float *signal, int start, int end, struct analogtv_yiq_s *it_yiq);
    ...
    analogtv *it)
{
  ...
  it->image= create_xshm_image(it->dpy, it->xgwa.visual, it->xgwa.depth, ZPixmap, &it->shm_info, width / bits_per_pixel, it->useheight);
...
static int analogtv_thread_create(
  void *thread_raw,
...
static void analogtv_thread_destroy(
  void *thread_raw)
{
}
    ...
    Window window)
{
  ...
  if (thread_malloc((void **) &it->rx_signal, dpy, sizeof(it->rx_signal [ 0 ]) * rx_signal_len))
    goto fail;
  ...
  visual_rgb_masks(it->xgwa.screen, it->xgwa.visual, &it->red_mask, &it->green_mask, &it->blue_mask);
  ...
  gcv.background= get_pixel_resource(it->dpy, it->colormap, "background", "Background");
    ...
    struct analogtv_yiq_s *it_yiq)
{
...

  {
    ...
    double cb_i= (it->line_cb_phase [ lineno ][ (2 + phasecorr) & 3 ] -
                   it->line_cb_phase [ lineno ][ (0 + phasecorr) & 3 ]) /
      16.0;
    double cb_q= (it->line_cb_phase [ lineno ][ (3 + phasecorr) & 3 ] -
                   it->line_cb_phase [ lineno ][ (1 + phasecorr) & 3 ]) /
      16.0;
  ...
  for (i= start, yiq= it_yiq + start, sp= signal + start;
    ...
    i++, dp--, yiq++, sp++)
    {
      ...
      dp [ 8 ]= (+1.0f * (dp [ 6 ] + dp [ 0 ]) + 4.0f * (dp [ 5 ] + dp [ 1 ]) + 7.0f * (dp [ 4 ] + dp [ 2 ]) + 8.0f * (dp [ 3 ]) - 0.0176648f * dp [ 12 ] - 0.4860288f * dp [ 10 ]);
      ...
      for (i= start, yiq= it_yiq + start, sp= signal + start;
        ...
        i++, dp--, yiq++, sp++)
        {
          ...
          */
          ...
          yiq->i= dp [ 8 ]= (dp [ 5 ] + dp [ 0 ] + 3.0f * (dp [ 4 ] + dp [ 1 ]) + 4.0f * (dp [ 3 ] + dp [ 2 ]) - 0.3333333333f * dp [ 10 ]);

          dp [ 16 ]= sig * multiq2 [ (i + 3) & 3 ] * 0.0833333333333f;
          yiq->q= dp [ 24 ]= (dp [ 16 + 5 ] + dp [ 16 + 0 ] + 3.0f * (dp [ 16 + 4 ] + dp [ 16 + 1 ]) + 4.0f * (dp [ 16 + 3 ] + dp [ 16 + 2 ]) - 0.3333333333f * dp [ 24 + 2 ]);
        }
    }
  else
    {
...
static void analogtv_init_signal(
  const analogtv *it,
  double noiselevel,
  unsigned start,
  unsigned end)
...
static void analogtv_add_signal(
  const analogtv *it,
  const analogtv_reception *rec,
  unsigned start,
  unsigned end,
  int ec)
{
...
static void analogtv_thread_add_signals(
  void *thread_raw)
{
  ...
  while (start != thread->signal_end)
    {
      ...
      for (i= 0; i != it->rec_count; ++i)
        analogtv_add_signal(it, it->recs [ i ], start, end, ! i ? it->channel_change_cycles : 0);
              ...
              for (x= 0, rpf= rgbf; rpf != rgbf_end; x++, rpf+= 3)
                {
                  ...
                  if (ntscbi >= ANALOGTV_CV_MAX) ntscbi= ANALOGTV_CV_MAX - 1;
                  for (j= 0; j < xrepl; j++)
                    XPutPixel(it->image, x * xrepl + j, y, it->red_values [ ntscri ] | it->green_values [ ntscgi ] | it->blue_values [ ntscbi ]);
                }
            }
        }
    }
...
static void analogtv_thread_draw_lines(
  void *thread_raw)
{
  const analogtv_thread *thread= (analogtv_thread *) thread_raw;
  const analogtv *it= thread->it;
  ...
  for (lineno= ANALOGTV_TOP + thread->thread_id;
    ...
    lineno+= it->threads.count)
    {
...

      {
        ...
        if (slineno < 16)
          shiftthisrow= it->horiz_desync * (expf(-0.17f * slineno) * (0.7f + cosf(slineno * 0.6f)));
        else
          shiftthisrow= 0.0f;
        ...
        squishright_i= (int) ((middle + viswidth * (0.25f + 0.25f * puramp(it, 2.0f, 0.0f, 1.1f) - it->squish_control)) * 65536.0f);
      ...
      if (it->use_cmap)
          ...
          for (y= ytop; y < ybot; y++)
            {
              ...
              float levelmult_y= levelmult * it->contrast_control * puramp(it, 1.0f, 0.0f, 1.0f) / (0.5f + 0.5f * it->puheight) * 0.070f;
              ...
              while (i < scanend_i && x < it->usewidth)
                {
                  ...
                  float interpy= (yiq [ pati ].y * invpixfrac + yiq [ pati + 1 ].y * pixfrac) * levelmult_y;
                  float interpi= (yiq [ pati ].i * invpixfrac + yiq [ pati + 1 ].i * pixfrac) * levelmult_iq;
                  float interpq= (yiq [ pati ].q * invpixfrac + yiq [ pati + 1 ].q * pixfrac) * levelmult_iq;
                  ...
                  for (j= 0; j < it->xrepl; j++)
                    {
                      XPutPixel(it->image, x, y, it->colors [ cmi ]);
                      x++;
                    }
                  if (i >= squishright_i)
      ...
      else
        {
          ...
          pixbright= it->contrast_control * puramp(it, 1.0f, 0.0f, 1.0f) / (0.5f + 0.5f * it->puheight) * 1024.0f / 100.0f;
          pixmultinc= pixrate;
          i= scanstart_i;
          rrp= rgb_start;
  ...
  unsigned rec_count)
{
  ...
  for (i= 0; i != rec_count; ++i)
    {
      ...
      it->rx_signal_level=
        sqrt(it->rx_signal_level * it->rx_signal_level +
          (level * level * (1.0 + 4.0 * (rec->ghostfir [ 0 ] + rec->ghostfir [ 1 ] + rec->ghostfir [ 2 ] + rec->ghostfir [ 3 ]))));

      /* duplicate the first line into the Nth line to ease wraparound computation */
      memcpy(inp->signal [ ANALOGTV_V ], inp->signal [ 0 ], ANALOGTV_H * sizeof(inp->signal [ 0 ][ 0 ]));
    }
      ...
      */

      {
        ...
        ncl= 0.95f * it->crtload [ lineno - 1 ] +
          0.05f * (baseload + (totsignal - 30000) / 100000.0f + (slineno > 184 ? (slineno - 184) * (lineno - 184) * 0.001f * it->squeezebottom : 0.0f));
  ...
  */
  ...
  if (overall_top > 0)
    XClearArea(it->dpy, it->window, it->screen_xo, it->screen_yo, it->usewidth, overall_top, 0);
  if (it->useheight > overall_bot)
    XClearArea(it->dpy, it->window, it->screen_xo, it->screen_yo + overall_bot, it->usewidth, it->useheight - overall_bot, 0);

  if (overall_bot > overall_top)
    put_xshm_image(it->dpy, it->window, it->gc, it->image, 0, overall_top, it->screen_xo, it->screen_yo + overall_top, it->usewidth, overall_bot - overall_top, &it->shm_info);
  ...
  if (0)
    {
      ...
      fps= 1.0 / ((tv.tv_sec - it->last_display_time.tv_sec) + 0.000001 * (tv.tv_usec - it->last_display_time.tv_usec));
      sprintf(buf, "FPS=%0.1f", fps);
      XDrawString(it->dpy, it->window, it->gc, 50, it->useheight * 2 / 3, buf, strlen(buf));
      ...
      for (x= 0; x < x_length; x++)
        {
          ...
          fyy [ 4 ]= fyy [ 5 ];
          fyy [ 5 ]= fyy [ 6 ];
          fyy [ 6 ]= (fyx [ 0 ] + fyx [ 6 ]) + 4 * (fyx [ 1 ] + fyx [ 5 ]) + 7 * (fyx [ 2 ] + fyx [ 4 ]) + 8 * fyx [ 3 ] + ((-151 * fyy [ 2 ] + 8115 * fyy [ 3 ] - 38312 * fyy [ 4 ] + 36586 * fyy [ 5 ]) >> 16);
          filty= fyy [ 6 ];
          ...
          fiy [ 1 ]= fiy [ 2 ];
          fiy [ 2 ]= fiy [ 3 ];
          fiy [ 3 ]= (fix [ 0 ] + fix [ 3 ]) + 3 * (fix [ 1 ] + fix [ 2 ]) + ((16559 * fiy [ 0 ] - 72008 * fiy [ 1 ] + 109682 * fiy [ 2 ]) >> 16);
          filti= fiy [ 3 ];
          ...
          fqy [ 1 ]= fqy [ 2 ];
          fqy [ 2 ]= fqy [ 3 ];
          fqy [ 3 ]= (fqx [ 0 ] + fqx [ 3 ]) + 3 * (fqx [ 1 ] + fqx [ 2 ]) + ((2612 * fqy [ 0 ] - 9007 * fqy [ 1 ] + 10453 * fqy [ 2 ]) >> 12);
          filtq= fqy [ 3 ];
...
void analogtv_reception_update(
  ...
  if (rec->multipath > 0.0)
    {
      ...
      if (random() % 20 == 0)
        rec->ghostfir2 [ random() % (ANALOGTV_GHOSTFIR_LEN) ]= rec->multipath * (frand(0.08) - 0.04);
      for (i= 0; i < ANALOGTV_GHOSTFIR_LEN; i++)
    ...
    {
      for (i= 0; i < ANALOGTV_GHOSTFIR_LEN; i++)
        rec->ghostfir [ i ]= (i >= ANALOGTV_GHOSTFIR_LEN / 2) ? ((i & 1) ? +0.04 : -0.08) / ANALOGTV_GHOSTFIR_LEN : 0.0;
    }
}
  ...
  if (fontname && ! strcmp(fontname, "6x10"))
    {
      ...
      XWindowAttributes xgwa;
      Pixmap m= 0;
      Pixmap p= image_data_to_pixmap(dpy, window, _6x10font_png, sizeof(_6x10font_png), &pix_w, &pix_h, &m);
      ...
      XGetWindowAttributes(dpy, window, &xgwa);
      f->text_im= XCreateImage(dpy, xgwa.visual, 1, XYBitmap, 0, 0, pix_w, pix_h, 8, 0);
      f->text_im->data= malloc(f->text_im->bytes_per_line * f->text_im->height);

      /* Convert deep image to 1 bit */
      for (y= 0; y < pix_h; y++)
        for (x= 0; x < pix_w; x++)
          XPutPixel(f->text_im, x, y, (XGetPixel(mm, x, y) ? XGetPixel(im, x, y) == black : 0));
      XDestroyImage(im);
      XDestroyImage(mm);
    }
  else if (fontname)
    {
      ...
      xim= XGetImage(dpy, text_pm, 0, 0, 256 * f->char_w, f->char_h, xgwa.depth, ZPixmap);
      f->text_im= XCreateImage(dpy, xgwa.visual, 1, XYPixmap, 0, 0, 256 * f->char_w, f->char_h, 8, 0);
...
void analogtv_draw_solid_rel_lcp(
  analogtv_input *input,
  double left,
  double right,
  double top,
...
void analogtv_draw_string_centered(
  analogtv_input *input,
  analogtv_font *f,
  char *s,
  int x,
