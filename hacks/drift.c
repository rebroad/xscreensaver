...
ModStruct drift_description=
  {"drift", "init_drift", "draw_drift", (char *) NULL, "refresh_drift", "init_drift", "free_drift", &drift_opts, 10000, 30, 1, 1, 64, 1.0, "", "Shows cosmic drifting flame fractals", 0, NULL};
...
#define VARIATION_LEN 14
  ...
  dp->major_variation= ((dp->major_variation >= VARIATION_LEN >> 1) &&
    ...
    (dp->major_variation + 1) >> 1 :
    dp->major_variation >> 1;
      ...
      if (++dp->ncpoints [ c ] == MAXBATCH2)
        {
          ...
          XDrawPoints(display, d, gc, &(dp->cpts [ c * MAXBATCH2 ]), dp->ncpoints [ c ], CoordModeOrigin);
          dp->ncpoints [ c ]= 0;
        }
    }
}
          ...
          if (dp->ncpoints [ i ])
            {
              ...
              XDrawPoints(display, d, gc, &(dp->cpts [ i * MAXBATCH2 ]), dp->ncpoints [ i ], CoordModeOrigin);
              dp->ncpoints [ i ]= 0;
            }
        }
    }
