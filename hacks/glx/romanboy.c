...
ModStruct romanboy_description=
  {"romanboy", "init_romanboy", "draw_romanboy", NULL, "draw_romanboy", "change_romanboy", "free_romanboy", &romanboy_opts, 25000, 1, 1, 1, 1.0, 4, "", "Rotate a 3d immersion of the real projective plane in 3d or walk on it", 0, NULL};
...
static void setup_roman_boy_color_texture(
  ModeInfo *mi,
  double umin,
  double umax,
  double vmin,
...
/* Compute the current walk frame, i.e., the coordinate system of the
   ...
   projective plane. */
static void compute_walk_frame(
  romanboystruct *pp,
  int g,
  float d,
  float radius,
                  ...
                  if (pp->colors != COLORS_ONESIDED && pp->colors != COLORS_TWOSIDED)
                    {
                      ...
                      glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, &pp->col [ 4 * o ]);
                    }
  ...
  int numv)
{
  ...
  if (! pp->buffers_initialized)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 2 * (numu + 1) * (numv + 1) * sizeof(GLfloat), pp->uv, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, pp->vertex_t_buffer);
      glBufferData(GL_ARRAY_BUFFER, 2 * (numu + 1) * (numv + 1) * sizeof(GLfloat), pp->tex, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);
      ...
      if (! pp->change_colors &&
        pp->colors != COLORS_ONESIDED && pp->colors != COLORS_TWOSIDED)
        {
          ...
          glBufferData(GL_ARRAY_BUFFER, 4 * (numu + 1) * (numv + 1) * sizeof(GLfloat), pp->col, GL_STATIC_DRAW);
          glBindBuffer(GL_ARRAY_BUFFER, 0);
        }
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, pp->ni * sizeof(GLuint), pp->indices, GL_STATIC_DRAW);
  ...
  if (pp->projection == DISP_PERSPECTIVE || pp->view == VIEW_WALK)
    ...
    glsl_Orthographic(p_mat, -pp->aspect, pp->aspect, -1.0f, 1.0f, 0.1f, 10.0f);
  else
    glsl_Orthographic(p_mat, -1.0f, 1.0f, -1.0f / pp->aspect, 1.0f / pp->aspect, 0.1f, 10.0f);
  ...
  if (pp->colors != COLORS_ONESIDED && pp->colors != COLORS_TWOSIDED)
    {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, pp->color_buffer);
      if (pp->change_colors)
        glBufferData(GL_ARRAY_BUFFER, 4 * (numu + 1) * (numv + 1) * sizeof(GLfloat), pp->col, GL_STREAM_DRAW);
      ...
      for (i= 0; i < pp->ne; i++)
        {
          ...
          glDrawElements(GL_TRIANGLE_STRIP, pp->nt, GL_UNSIGNED_INT, (const GLvoid *) index_offset);
        }
    }
  else /* pp->display_mode == DISP_WIREFRAME */
    {
      glLineWidth(1.0f);
      index_offset= 0;
      glDrawElements(GL_LINES, pp->ni, GL_UNSIGNED_INT, (const void *) index_offset);
    }
  ...
  ModeInfo *mi)
{
  ...
  if (! glsl_CompileAndLinkShaders(2, vertex_shader_source, 2, fragment_shader_source, &pp->shader_program))
    return;
...
#ifdef HAVE_GLSL
  if (pp->use_shaders)
    mi->polygon_count= roman_boy_pf(mi, 0.0, 2.0 * M_PI, 0.0, 2.0 * M_PI, pp->g * NUMU, NUMV);
  else
#endif /* HAVE_GLSL */
    mi->polygon_count= roman_boy_ff(mi, 0.0, 2.0 * M_PI, 0.0, 2.0 * M_PI, pp->g * NUMU, NUMV);
}


ENTRYPOINT void reshape_romanboy(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT Bool romanboy_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  romanboystruct *pp= &romanboy [ MI_SCREEN(mi) ];
  ...
  if (event->xany.type == ButtonPress && event->xbutton.button == Button1)
    {
      ...
      gltrackball_start(pp->trackball, event->xbutton.x, event->xbutton.y, MI_WIDTH(mi), MI_HEIGHT(mi));
      return True;
    }
    ...
    {
      gltrackball_track(pp->trackball, event->xmotion.x, event->xmotion.y, MI_WIDTH(mi), MI_HEIGHT(mi));
      return True;
    }
...
#ifndef STANDALONE
ENTRYPOINT void change_romanboy(
  ModeInfo *mi)
{
  romanboystruct *pp= &romanboy [ MI_SCREEN(mi) ];
