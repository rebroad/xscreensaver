...
static argtype vars []= {
  ...
  {&ceilingTexture, "ceilingTexture", "Ceiling Texture", "ceiling-tiles", t_String},
...
enum cellTypes
{
  ...
  FINISH,
  GL_3D_TEXT,
  INVERTER_TETRAHEDRON,
  INVERTER_OCTAHEDRON,
  INVERTER_DODECAHEDRON,
  INVERTER_ICOSAHEDRON,
  WALL_GL_REDBOOK
};
...
static void addCells(Tuple cellToAdd, Tuple currentWall, maze_configuration* maze);
...
static void drawWall(Tuple startCoordinates, Tuple endCoordinates, maze_configuration* maze);
...
{
  XImage* image= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), png_data, data_size);
  char buf [ 1024 ];
  clear_gl_error();
    ...
    char* filename)
{
  ...
  sprintf(buf, "texture: %.100s (%dx%d)", filename, image->width, image->height);
  check_gl_error(buf);
  return True;
}
    ...
    ModeInfo* mi)
{
  ...
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, mag);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, mag);
  if (! ceilingTexture || ! *ceilingTexture || ! strcmp(ceilingTexture, "ceiling-tiles"))
    {
DEFAULT_CEILING_TEXTURE:
  ...
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, mag);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, mag);
  if (! floorTexture || ! *floorTexture || ! strcmp(floorTexture, "wood-floor"))
    {
  ...
  while (maze->wallListSize > 0)
    {
      ...
      if (isEven(currentWall.row))
        {
          if (maze->mazeGrid [ currentWall.row - 1 ][ currentWall.column ] == CELL && maze->mazeGrid [ currentWall.row + 1 ][ currentWall.column ] == CELL_UNVISITED)
            {
              cellToAdd.row= currentWall.row + 1;
              cellToAdd.column= currentWall.column;
              addCells(cellToAdd, currentWall, maze);
            }
          else if (maze->mazeGrid [ currentWall.row + 1 ][ currentWall.column ] == CELL && maze->mazeGrid [ currentWall.row - 1 ][ currentWall.column ] == CELL_UNVISITED)
            {
              cellToAdd.row= currentWall.row - 1;
              cellToAdd.column= currentWall.column;
              addCells(cellToAdd, currentWall, maze);
      ...
      else if (maze->mazeGrid [ currentWall.row ][ currentWall.column - 1 ] == CELL && maze->mazeGrid [ currentWall.row ][ currentWall.column + 1 ] == CELL_UNVISITED)
        {
...
static unsigned char
  isRemovableWall(
    ...
    maze_configuration* maze)
{
  if (maze->mazeGrid [ coordinates.row ][ coordinates.column ] == WALL && coordinates.row > 0 && coordinates.row < maze->numRows - 1 && coordinates.column > 0 && coordinates.column < maze->numColumns - 1)
    return 1;
  else
    return 0;
}
    ...
    maze_configuration* maze)
{
  ...
  if (maze->mazeGrid [ maze->startPosition.row ][ maze->startPosition.column + 1 ] != WALL && maze->mazeGrid [ maze->startPosition.row ][ maze->startPosition.column + 1 ] != WALL_GL_REDBOOK)
    {
  ...
  for (i= 0; i < numRats; i++)
    {
      ...
      if (maze->mazeGrid [ (int) (rat->position.z * 2) ]
                         [ (int) (rat->position.x * 2) + 1 ] != WALL &&
        maze->mazeGrid [ (int) (rat->position.z * 2) ]
                       [ (int) (rat->position.x * 2) + 1 ] != WALL_GL_REDBOOK)
        rat->rotation= EAST;
      else if (maze->mazeGrid [ (int) (rat->position.z * 2) - 1 ]
                              [ (int) (rat->position.x * 2) ] != WALL &&
        maze->mazeGrid [ (int) (rat->position.z * 2) - 1 ]
  ...
  for (i= 0; i < numGlRedbooks; i++)
    {
      while (! (((isOdd(temp.row) && isEven(temp.column)) || (isEven(temp.row) && isOdd(temp.column))) && maze->mazeGrid [ temp.row ][ temp.column ] == WALL))
        {
    ...
    unsigned char type)
{
  ...
  while (! (maze->mazeGrid [ position.row ][ position.column ] == CELL && isOdd(position.row) && isOdd(position.column)))
    {
    ...
    ModeInfo* mi)
{
  ...
  if (((maze->numRows - 1) + (maze->numColumns - 1) + ((maze->numRows / 2 - 1) * (maze->numColumns / 2 - 1))) < maze->numGlRedbooks)
    maze->numGlRedbooks= (maze->numRows - 1) + (maze->numColumns - 1) + ((maze->numRows / 2 - 1) * (maze->numColumns / 2 - 1));
#endif
  ...
  maze->wallList= calloc(((maze->numColumns - 2) / 2) * ((maze->numRows - 2) / 2 + 1) + ((maze->numColumns - 2) / 2 + 1) * ((maze->numRows - 2) / 2), sizeof(Tuple));
    ...
    ModeInfo* mi)
{
  ...
  glTranslatef(-1 * maze->camera.position.x, -0.5, -1 * maze->camera.position.z);
...
static void
  refreshRemainingDistanceToTravel(
    ModeInfo* mi)
{
  unsigned i;
  maze_configuration* maze= &mazes [ MI_SCREEN(mi) ];
  maze->camera.remainingDistanceToTravel= speed * 1.6 * (MI_DELAY(mi) / 1000000.0);
  for (i= 0; i < numRats; i++)
    ...
    maze_configuration* maze)
{
  ...
  if (! rat->isCamera && (maze->wallHeight < 1 || (rat->position.x == 0 && rat->position.z == 0)))
    return;
    ...
    maze_configuration* maze)
{
  ...
  GLfloat tangentVectorDirection, previousRotation= rat->rotation;

  if (rat->state == TURNING_LEFT)
    {
      tangentVectorDirection= rat->rotation * (M_PI / 180) + M_PI;
      rotatingAround.x= roundToNearestHalf(rat->position.x + 0.5 * cos(tangentVectorDirection));
      rotatingAround.z= roundToNearestHalf(rat->position.z + 0.5 * sin(tangentVectorDirection));

      rat->rotation-= DEF_ANGULAR_CONVERSION_FACTOR * rat->remainingDistanceToTravel;

      if (previousRotation > WEST && rat->rotation <= WEST)
        {
          rat->rotation= WEST;
          rat->remainingDistanceToTravel-= (M_PI / 180) * fabs(previousRotation - rat->rotation);
        }
  ...
  if (rat->rotation == NORTH || rat->rotation == EAST || rat->rotation == SOUTH || rat->rotation == WEST)
    {
    ...
    maze_configuration* maze)
{
  ...
  rat->rotation-= 1.5 * DEF_ANGULAR_CONVERSION_FACTOR * rat->remainingDistanceToTravel;

  if (previousRotation > rat->desiredRotation && rat->rotation <= rat->desiredRotation)
    {
    ...
    maze_configuration* maze)
{
  ...
  maze->camera.inversion+= 1.5 * DEF_ANGULAR_CONVERSION_FACTOR * maze->camera.remainingDistanceToTravel;
  if (previousInversion < 180 && maze->camera.inversion >= 180)
    {
      maze->camera.inversion= 180;
      maze->camera.remainingDistanceToTravel-= (M_PI / 180) * fabs(previousInversion - maze->camera.inversion);
      shouldChangeState= 1;
    }
    ...
    maze_configuration* maze)
{
  ...
  else if (rat->isCamera && inFrontOfRat >= INVERTER_TETRAHEDRON && inFrontOfRat <= INVERTER_ICOSAHEDRON)
    {
      rat->state= INVERTING;
    }
  ...
  Tuple coordinates)
{
  ...
  if (maze->wallHeight < 1 ||
    type < INVERTER_TETRAHEDRON || type > INVERTER_ICOSAHEDRON || (coordinates.row == 0 && coordinates.column == 0))
    return;
  ...
  for (i= 0; i < maze->numRows; i++)
    {
      for (j= 0; j < maze->numColumns; j++)
        {
          if (maze->mazeGrid [ i ][ j ] == WALL || maze->mazeGrid [ i ][ j ] == WALL_GL_REDBOOK)
            {
              if (maze->mazeGrid [ i ][ j ] == WALL)
                {
                  glBindTexture(GL_TEXTURE_2D, maze->wallTexture);
                  if (dropAcid || dropAcidWalls)
                    glColor3f(maze->acidColor.red, maze->acidColor.green, maze->acidColor.blue);
#ifdef USE_FLOATING_IMAGES
                }
              else
                {
...
static void
  drawWall(
    Tuple startCoordinates,
    Tuple endCoordinates,
    maze_configuration* maze)
{
      ...
      if (j >= INVERTER_TETRAHEDRON && j <= INVERTER_ICOSAHEDRON)
        {
          ...
          glTranslatef(maze->inverterPosition [ i ].column / 2.0,
            maze->inverterPosition [ i ].row / 2.0,
            0);
