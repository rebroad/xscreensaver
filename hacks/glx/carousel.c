    ...
    image_frame *frame)
{
  ...
  if (debug_p && ! wire && frame->current.w != 0)
    fprintf(stderr, "%s:  dropped %4d x %-4d  %4d x %-4d  \"%s\"\n", progname, frame->current.geom.width, frame->current.geom.height, frame->current.tw, frame->current.th, (frame->current.title ? frame->current.title : "(null)"));
  ...
  if (wire)
  ...
  else
    {
      ...
      if (w > h * 5)
        { /* tiny window: use 16:9 boxes */
          h= w * 9 / 16;
        }

      load_texture_async(mi->xgwa.screen, mi->window, *ss->glx_context, w, h, mipmap_p, frame->loading.texid, image_loaded_cb, frame);
    }
}
    ...
    void *closure)
{
  ...
  if (ss->anisotropic >= 1.0)
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, ss->anisotropic);
  ...
  */
  ...
  if (debug_p)
    fprintf(stderr, "%s:   loaded %4d x %-4d  %4d x %-4d  \"%s\"\n", progname, frame->loading.geom.width, frame->loading.geom.height, frame->loading.tw, frame->loading.th, (frame->loading.title ? frame->loading.title : "(null)"));
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, ss->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &ss->button_down_p))
    {
      return True;
    }
    ...
    int n)
{
  ...
  if (n == 0)
    ...
    sprintf(text, "Loading images...  (%d%%)", (int) (n * 100 / MI_COUNT(mi)));
    ...
    ModeInfo *mi)
{
...

  {
    ...
    ss->rot= make_rotator(spin_speed, spin_speed, spin_speed, spin_accel, wander_speed, True);
    ...
    Bool body_p)
{
  ...
  */
  ...
  if (frame->mode == OUT || frame->mode == IN)
    {
      GLfloat t= (frame->mode == OUT ? frame->mode_tick / (fade_ticks / speed) : (((fade_ticks / speed) - frame->mode_tick + 1) / (fade_ticks / speed)));
      t= 1 - t;
  ...
  if (body_p) /* Draw the image quad. */
    ...
    { /* Draw a title under the image. */
      ...
      /* Wrap long lines */
      title= xft_word_wrap(MI_DISPLAY(mi), texfont_xft(ss->texfont), title, 1 / scale);
      token= title;
    ...
    ModeInfo *mi)
{
  ...
  if (ss->mode != NORMAL)
    {
      GLfloat s= (ss->mode == OUT ? ss->mode_tick / (fade_ticks / speed) : (((fade_ticks / speed) - ss->mode_tick + 1) / (fade_ticks / speed)));
      glScalef(s, s, s);
    }
