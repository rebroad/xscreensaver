...
ModStruct screenflip_description=
  {"screenflip", "init_screenflip", "draw_screenflip", NULL, "draw_screenflip", "init_screenflip", NULL, &screenflip_opts, 1000, 1, 2, 1, 4, 1.0, "", "Screenflips", 0, NULL};
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, c->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &c->button_down_p))
    {
      return True;
    }
  ...
  int wire)
{
  ...
  glColor4f(c->show_colors [ 0 ], c->show_colors [ 1 ], c->show_colors [ 2 ], c->show_colors [ 3 ]);
  ...
  int wire)
{
  ...
  */
  ...
  if (! c->button_down_p && ! c->fadetime && (c->rot >= 360 || c->rot <= -360) && ! (random() % 7))
    { /* rotate  change */
...
ENTRYPOINT void reshape_screenflip(
  ModeInfo *mi,
  int width,
  int height)
{
    ...
    void *closure)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (c->mipmap_p ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
  ...
  ModeInfo *modeinfo)
{
  ...
  c->mipmap_p= True;
  load_texture_async(modeinfo->xgwa.screen, modeinfo->window, *c->glx_context, 0, 0, c->mipmap_p, c->texid, image_loaded_cb, c);
}
...
ENTRYPOINT void draw_screenflip(
  ModeInfo *mi)
{
  Screenflip *c= &screenflip [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_screenflip(
  ModeInfo *mi)
{
  Screenflip *c= &screenflip [ MI_SCREEN(mi) ];
