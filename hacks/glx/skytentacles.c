  ...
  for (i= 0; i < t->nsegments; i++)
    {
      ...
      t->segments [ i ].rot= make_rotator(spin_speed, spin_speed, spin_speed, spin_accel, wander_speed, True);
    }
  ...
  for (i= 0; i < t->nsegments; i++)
    {
      ...
      */
      ...
      if (i > 0)
        {
          ...
          glBegin(wire ? GL_LINES : smooth_p ? GL_QUAD_STRIP :
                                               GL_QUADS);
          for (j= 0; j <= arg_slices; j++)
            {
              int j0= j % arg_slices;
              ...
              if (! front_p || outline_p)
                ...
                j >= arg_slices - indented_points / 2)
                {
                  ...
                  glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, t->stripe_color);
                }
              else
                {
                  glColor4fv(t->tentacle_color);
              ...
              */
              if (cel_p)
                {
                  t0= dot(light [ 0 ], light [ 1 ], light [ 2 ], onorm [ j0 ].x, onorm [ j0 ].y, onorm [ j0 ].z);
                  t1= dot(light [ 0 ], light [ 1 ], light [ 2 ], norm [ j0 ].x, norm [ j0 ].y, norm [ j0 ].z);
                  if (t0 < 0) t0= 0;
                  if (t1 < 0) t1= 0;
                }
           ...
           */
          {
            ...
            if (outline_p)
              {
                glColor4fv(tc->outline_color);
                glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, tc->outline_color);
              }
            else if (front_p)
              {
      ...
      if (! outline_p && (i == 0 || i == t->nsegments - 1))
        {
          int j;
          GLfloat ctrz= ctr.z + ((i == 0 ? -1 : 1) * t->segments [ i ].thickness / 4);
          if (front_p)
            {
              glColor4fv(t->tentacle_color);
              glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, t->tentacle_color);
...
ENTRYPOINT Bool
  tentacles_handle_event(
    ModeInfo *mi,
    XEvent *event)
{
  tentacles_configuration *tc= &tcs [ MI_SCREEN(mi) ];

  if (gltrackball_event_handler(event, tc->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &tc->button_down_p))
    {
      return True;
    }
    ...
    ModeInfo *mi)
{
  ...
  if (tc->tentacle_color [ 0 ] + tc->tentacle_color [ 1 ] + tc->tentacle_color [ 2 ] < 0.4)
      ...
      if (cel_p)
        {
           ...
           */
          int w= 16;
          tc->texture= XCreateImage(MI_DISPLAY(mi), MI_VISUAL(mi), 32, ZPixmap, 0, 0, w, 1, 32, 0);
        ...
        {
          tc->texture= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), scales_png, sizeof(scales_png));
        }
  ...
  if (texture_p)
    {
      ...
      clear_gl_error();
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tc->texture->width, tc->texture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, tc->texture->data);
    ...
    {
      clear_gl_error();
      glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, tc->texture->width, 0, GL_RGBA, GL_UNSIGNED_BYTE, tc->texture->data);
      check_gl_error("texture");
  ...
  compute_unit_torus(mi, 0.5, MAX(5, arg_slices / 6), MAX(9, arg_slices / 3));
}
