...
#else
  gluPerspective(100, 1 / h / 4, 1.0, 20 * grid_depth * 1.5 * (1 + grid_spacing));
#endif
          ...
          switch (random() % 2)
            {
              case 0 :
                ...
                glVertex3f(a, c - w, 0);
                polys++;
                glVertex3f(b + w, c + w, 0);
                polys++;
                glVertex3f(b, c - w, 0);
                polys++;
                glVertex3f(c + w, b + w, 0);
                polys++;
                glVertex3f(c - w, b, 0);
                polys++;
                glVertex3f(c + w, a, 0);
    ...
    Bool which)
{
  ...
  GLfloat sy= (which ? height * 0.8 : sx * 4); /* Tweaked to match gluPerspective */

  GLfloat lines_in_tex= ((bp->text [ n ].metrics.ascent +
                           bp->text [ n ].metrics.descent) /
    (GLfloat) (bp->text [ n ].ascent + bp->text [ n ].descent));
  ...
  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, which ? bp->tower_color2 : bp->tower_color);
        ...
        if (! wire)
          {
            ...
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, bp->bg_color);
            ...
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, bp->edge_color);
    ...
    ModeInfo *mi)
{
  ...
  texture_string_metrics(font, bp->billboard_text, &metrics, &ascent, &descent);
  ...
  for (i= 0; i < countof(bp->text); i++)
    {
      ...
      texture_string_metrics(bp->text [ i ].font_data, " ", &bp->text [ i ].metrics, &bp->text [ i ].ascent, &bp->text [ i ].descent);
      bp->text [ i ].em_width= bp->text [ i ].metrics.width;
      string_to_texture(bp->text [ i ].font_data, bp->text [ i ].text, &bp->text [ i ].metrics, &bp->text [ i ].width, &bp->text [ i ].height);
    }
  ...
  for (i= 0; i < bp->ntowers; i++)
    {
      tower *t= &bp->towers [ i ];
      glPushMatrix();
      glTranslatef(t->x, t->y - 1, -grid_height * ease(EASE_IN_OUT_SINE, 1 - t->h));
