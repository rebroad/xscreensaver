...
ModStruct projectiveplane_description=
  {"projectiveplane", "init_projectiveplane", "draw_projectiveplane", NULL, "draw_projectiveplane", "change_projectiveplane", NULL, &projectiveplane_opts, 25000, 1, 1, 1, 1.0, 4, "", "Rotate a 4d embedding of the real projective plane in 4d or walk on it", 0, NULL};
...
static void setup_projective_plane(
  ModeInfo *mi,
  double umin,
  double umax,
  double vmin,
...
static int projective_plane_ff(
  ModeInfo *mi,
  double umin,
  double umax,
  double vmin,
      ...
      for (j= 0; j <= NUMU; j++)
        {
          ...
          t= 1.0 / sqrt(pp->pn [ o ][ 0 ] * pp->pn [ o ][ 0 ] + pp->pn [ o ][ 1 ] * pp->pn [ o ][ 1 ] + pp->pn [ o ][ 2 ] * pp->pn [ o ][ 2 ]);
          pp->pn [ o ][ 0 ]*= t;
          pp->pn [ o ][ 1 ]*= t;
          pp->pn [ o ][ 2 ]*= t;
        }
...
static int projective_plane_pf(
  ModeInfo *mi,
  double umin,
  double umax,
  double vmin,
  double vmax)
{
  ...
  if (pp->projection_3d == DISP_3D_PERSPECTIVE ||
    ...
    glsl_Orthographic(p_mat, -0.6f * pp->aspect, 0.6f * pp->aspect, -0.6f, 0.6f, 0.1f, 10.0f);
  else
    glsl_Orthographic(p_mat, -0.6f, 0.6f, -0.6f / pp->aspect, 0.6f / pp->aspect, 0.1f, 10.0f);
...
static void display_projectiveplane(
  ModeInfo *mi)
{
  projectiveplanestruct *pp= &projectiveplane [ MI_SCREEN(mi) ];
...
ENTRYPOINT void reshape_projectiveplane(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT Bool projectiveplane_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  projectiveplanestruct *pp= &projectiveplane [ MI_SCREEN(mi) ];
...
ENTRYPOINT void init_projectiveplane(
  ModeInfo *mi)
{
  projectiveplanestruct *pp;
 ...
 *-----------------------------------------------------------------------------
 */
ENTRYPOINT void draw_projectiveplane(
  ModeInfo *mi)
{
...
#ifndef STANDALONE
ENTRYPOINT void change_projectiveplane(
  ModeInfo *mi)
{
  projectiveplanestruct *pp= &projectiveplane [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_projectiveplane(
  ModeInfo *mi)
{
  projectiveplanestruct *pp= &projectiveplane [ MI_SCREEN(mi) ];
