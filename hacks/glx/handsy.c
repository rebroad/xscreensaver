...
extern const struct gllist
  *handsy_model_finger_distal,
  *handsy_model_finger_intermediate,
...
static const struct gllist *const *all_objs []= {
  &handsy_model_finger_distal,
  &handsy_model_finger_intermediate,
  &handsy_model_finger_proximal,
  &handsy_model_finger_metacarpal,
  &handsy_model_thumb_distal,
  &handsy_model_thumb_proximal,
  &handsy_model_thumb_metacarpal,
  &handsy_model_palm,
    ...
    ModeInfo *mi)
{
...

  {
    ...
    bp->rot= make_rotator(bp->spinx ? spin_speed : 0,
      ...
      False);
    bp->rot2= (face_front_p ? make_rotator(0, 0, 0, 0, tilt_speed, True) : 0);
      ...
      for (i= 0; i < bp->nhands; i++)
        {
              ...
              for (k= 0; k < nbones; k++)
                bp->hands [ i ].current.joint [ j ][ k ]=
                  constrain_joint(bp->hands [ i ].from.joint [ j ][ k ] +
                      bp->pair [ sinister ].tick * (bp->hands [ i ].to.joint [ j ][ k ] - bp->hands [ i ].from.joint [ j ][ k ]),
                    bp->geom->fingers [ j ].bones [ k ].min,
                    bp->geom->fingers [ j ].bones [ k ].max);
              bp->hands [ i ].current.base [ j ]=
                constrain_joint(bp->hands [ i ].from.base [ j ] +
                    bp->pair [ sinister ].tick * (bp->hands [ i ].to.base [ j ] - bp->hands [ i ].from.base [ j ]),
                  bp->geom->fingers [ j ].base.min,
                  bp->geom->fingers [ j ].base.max);
            }
          j= 0;
          bp->hands [ i ].current.wrist [ j ]=
            constrain_joint(bp->hands [ i ].from.wrist [ j ] +
                bp->pair [ sinister ].tick * (bp->hands [ i ].to.wrist [ j ] - bp->hands [ i ].from.wrist [ j ]),
              bp->geom->palm.min,
              bp->geom->palm.max);
          j= 1;
          bp->hands [ i ].current.wrist [ j ]=
          ...
          for (j= 0; j < 3; j++)
            bp->hands [ i ].current.pos [ j ]=
              constrain_joint(bp->hands [ i ].from.pos [ j ] +
                  bp->pair [ sinister ].tick * (bp->hands [ i ].to.pos [ j ] - bp->hands [ i ].from.pos [ j ]),
                -999,
                999);
    ...
    ModeInfo *mi)
{
...

  {
    ...
    glPushMatrix();
    glRotatef((bp->spiny ? y : bp->spinx ? x :
                                           z) *
        90,
      0,
      0,
