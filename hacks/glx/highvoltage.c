    ...
    int height)
{
...

  {
    ...
    glFrustum(fw1, fw2, -fh * 0.6, fh, /* Move horizon closer to bottom of window */
      near,
      far);
#endif
  }
  ...
  glLoadIdentity();
  gluLookAt(0, 0, 30, 0, 3, 0, /* Look up */
    0,
    1,
    0);
    ...
    int cable_type)
{
  ...
  if (wire)
    ...
    {
      GLfloat diam= 0.25 * (cable_type == 0 ? 1 : cable_type == 1 ? 0.3 :
                                                                    0.1);
      GLfloat cap= diam / 2;
      int faces= 4;
      ...
      for (i= 0; i < list->points * 3; i+= 6)
        {
          ...
          polys+= tube(p [ i + 0 ], p [ i + 1 ], p [ i + 2 ], p [ i + 3 ], p [ i + 4 ], p [ i + 5 ], diam, cap, faces, False, False, wire);
              ...
              for (j= 0; j < ndiscs; j++)
                {
                  ...
                  polys+= tube(x1, y1, z1, x2, y2, z2, diam * disc_width, 0, 6, True, True, wire);
                }
            }
        }
    }
          ...
          if (j > 0)
            {
              if (wire)
                ...
                {
                  polys+= tube(prev.x, prev.y, prev.z, cur.x, cur.y, cur.z, diam, 0, 6, True, False, wire);
                }
            }
