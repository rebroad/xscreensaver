    ...
    void *closure)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (tp->mipmap_p ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
    ...
    texture *texture)
{
  ...
  gp->mipmap_p= True;
  load_texture_async(mi->xgwa.screen, mi->window, *gp->glx_context, 0, 0, gp->mipmap_p, texture->id, image_loaded_cb, texture);
    ...
    texture *texture)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, my_data);
...
BUILTIN:
  ...
  else if (strcmp(image_str, "GENERATE") == 0)
    {
#ifdef DEBUG
 ...
 */
ENTRYPOINT void reshape_gleidescope(
  ModeInfo *mi,
  int width,
  int height)
{
