    ...
    ModeInfo *mi)
{
  ...
  glBegin(GL_LINES);
  glVertex3f(-s, -s, -s);
  glVertex3f(s, s, s); /* corner spikes */
  glVertex3f(-s, -s, s);
  glVertex3f(s, s, -s);
  glVertex3f(-s, s, -s);
  glVertex3f(s, -s, s);
    ...
    ModeInfo *mi)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bp->tex_w, bp->tex_h, 0, GL_RGBA, GL_UNSIGNED_BYTE, bp->tex_data);
    ...
    ModeInfo *mi)
{
  ...
  glBindTexture(GL_TEXTURE_2D, bp->texture);
  glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, 0, 0, bp->tex_w, bp->tex_h, 0);
