    ...
    ModeInfo *mi)
{
  ...
  if (tb->dropSpeed > 9)
    {
      tb->dropSpeed= 9;
    } /* 10+ produces blocks that can pass through each other */
    ...
    ModeInfo *mi)
{
  ...
  if (! follow)
    {
      if (tb->highest > tb->eyeLine)
        {
          tb->eyeLine+= ((tb->highest - tb->eyeLine) / 100);
        }                                                                                                   /* creates a smooth camera transition */
...
static double quadrantCorrection(
  double angle,
  int cx,
  int cy,
  int x,
      ...
      for (c= 0; c < x; c++)
        {
          ...
          for (i= 0; i < y; i++)
            {
              tb->carpet_polys+= tube(0, 0, -0.1, 0, 0, 0.26, cylSize, 0, tb->resolution, True, True, wire);
      ...
      for (c= 0; c < 2; c++)
        {
          for (i= 0; i < 4; i++)
            {
              tb->block_polys+= tube(0, 0, 0, 0, 0, 0.25, cylSize, 0, tb->resolution, True, True, wire);
      ...
      if (! wire)
        {
          for (c= 0; c < 3; c++)
            {
              tb->block_polys+= tube(0, 0, 0.1, 0, 0, 1.4, uddSize, 0, tb->resolution, True, True, wire);
...
ENTRYPOINT Bool
  topBlock_handle_event(
    ModeInfo *mi,
    XEvent *event)
{
  topBlockSTATE *tb= &tbs [ MI_SCREEN(mi) ];

  if (gltrackball_event_handler(event, tb->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &tb->button_down_p))
    {
      return True;
    }
