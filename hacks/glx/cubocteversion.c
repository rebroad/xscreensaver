...
ModStruct cubocteversion_description=
  {"cubocteversion", "init_cubocteversion", "draw_cubocteversion", NULL, "draw_cubocteversion", "change_cubocteversion", "free_cubocteversion", &cubocteversion_opts, 25000, 1, 1, 1, 1.0, 4, "", "Show a cuboctahedron eversion", 0, NULL};
...
static Bool construct_intersection(
  double p1 [ 3 ],
  double q1 [ 3 ],
  double r1 [ 3 ],
  double p2 [ 3 ],
...
static Bool tri_tri_intersection_3d(
  double p1 [ 3 ],
  double q1 [ 3 ],
  double r1 [ 3 ],
  double p2 [ 3 ],
...

{
  ...
  if (dp1 > 0.0)
    {
      if (dq1 > 0.0)
        return tri_tri_inter_3d(r1, p1, q1, p2, r2, q2, dp2, dr2, dq2, N1, N2, source, target);
      else if (dr1 > 0.0)
        return tri_tri_inter_3d(q1, r1, p1, p2, r2, q2, dp2, dr2, dq2, N1, N2, source, target);
      else
        return tri_tri_inter_3d(p1, q1, r1, p2, q2, r2, dp2, dq2, dr2, N1, N2, source, target);
    }
  else if (dp1 < 0.0)
    {
      if (dq1 < 0.0)
        return tri_tri_inter_3d(r1, p1, q1, p2, q2, r2, dp2, dq2, dr2, N1, N2, source, target);
      else if (dr1 < 0.0)
        return tri_tri_inter_3d(q1, r1, p1, p2, q2, r2, dp2, dq2, dr2, N1, N2, source, target);
      else
        return tri_tri_inter_3d(p1, q1, r1, p2, r2, q2, dp2, dr2, dq2, N1, N2, source, target);
    }
  else
    {
      if (dq1 < 0.0)
          ...
          for (j= i + 1; j < NUM_FACE; j++)
            {
              ...
              intersects= tri_tri_intersection_3d(p1 [ 0 ], p1 [ 1 ], p1 [ 2 ], p2 [ 0 ], p2 [ 1 ], p2 [ 2 ], s1, s2);
              if (intersects)
                {
...
static void gen_vertex_edge_tube(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  ...
  for (i= 0; i < NUM_VERT; i++)
    {
      l= 3 * i;
      gen_sphere(ce->tv [ l + 0 ], ce->tv [ l + 1 ], ce->tv [ l + 2 ], RADIUS_TUBE, NUMU, NUMV, ce->tube_vertex, ce->tube_normal, ce->tube_colorf, ce->tube_colorb, ce->tube_index, ce->tube_tri_offset, ce->tube_tri_num, &num_p, &num_t, &num_i);
    }
  for (i= 0; i < NUM_EDGE; i++)
    {
      l= 6 * i;
      ...
      gen_cylinder(cx, cy, cz, ax, ay, az, len, RADIUS_TUBE, NUMU, NUMV, ce->tube_vertex, ce->tube_normal, ce->tube_colorf, ce->tube_colorb, ce->tube_index, ce->tube_tri_offset, ce->tube_tri_num, &num_p, &num_t, &num_i);
    }
  ...
  if (ce->use_shaders)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 3 * num_p * sizeof(GLfloat), ce->tube_vertex, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->tube_normal_buffer);
      glBufferData(GL_ARRAY_BUFFER, 3 * num_p * sizeof(GLfloat), ce->tube_normal, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->tube_colorf_buffer);
      glBufferData(GL_ARRAY_BUFFER, 4 * num_p * sizeof(GLfloat), ce->tube_colorf, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->tube_colorb_buffer);
      glBufferData(GL_ARRAY_BUFFER, 4 * num_p * sizeof(GLfloat), ce->tube_colorb, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ce->tube_index_buffer);
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, num_t * sizeof(GLuint), ce->tube_index, GL_STREAM_DRAW);
...
static void gen_self_intersection_tube(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  GLsizei num_p, num_t, num_i;
  ...
  for (i= 0; i < ce->num_sv; i++)
    {
      l= 3 * i;
      gen_sphere(ce->sv [ l + 0 ], ce->sv [ l + 1 ], ce->sv [ l + 2 ], RADIUS_SELF, NUMU, NUMV, ce->self_vertex, ce->self_normal, ce->self_colorf, ce->self_colorb, ce->self_index, ce->self_tri_offset, ce->self_tri_num, &num_p, &num_t, &num_i);
    }
  for (i= 0; i < ce->num_se; i++)
    {
      ...
      len= 0.5f * sqrtf(ax * ax + ay * ay + az * az);
      gen_cylinder(cx, cy, cz, ax, ay, az, len, RADIUS_SELF, NUMU, NUMV, ce->self_vertex, ce->self_normal, ce->self_colorf, ce->self_colorb, ce->self_index, ce->self_tri_offset, ce->self_tri_num, &num_p, &num_t, &num_i);
    }
  ...
  if (ce->use_shaders)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 3 * num_p * sizeof(GLfloat), ce->self_vertex, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->self_normal_buffer);
      glBufferData(GL_ARRAY_BUFFER, 3 * num_p * sizeof(GLfloat), ce->self_normal, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->self_colorf_buffer);
      glBufferData(GL_ARRAY_BUFFER, 4 * num_p * sizeof(GLfloat), ce->self_colorf, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->self_colorb_buffer);
      glBufferData(GL_ARRAY_BUFFER, 4 * num_p * sizeof(GLfloat), ce->self_colorb, GL_STREAM_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ce->self_index_buffer);
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, num_t * sizeof(GLuint), ce->self_index, GL_STREAM_DRAW);
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    }
#endif
}
  ...
  if (ce->use_shaders)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 4 * 3 * NUM_FACE * sizeof(GLfloat), ce->tcf, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_colorb_buffer);
      glBufferData(GL_ARRAY_BUFFER, 4 * 3 * NUM_FACE * sizeof(GLfloat), ce->tcb, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
#endif
}
  ...
  if (ce->use_shaders)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 3 * 3 * NUM_FACE * sizeof(GLfloat), ce->tex, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
}
...
static int cuboctahedron_eversion_ff(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  ...
  if (ce->projection == DISP_ORTHOGRAPHIC)
    glOrtho(-ce->fov_ortho * ce->aspect, ce->fov_ortho * ce->aspect, -ce->fov_ortho, ce->fov_ortho, z_min, z_max);
  else
      ...
      for (i= 0; i < ce->tube_tri_strips; i++)
        {
          ...
          for (k= ce->tube_tri_offset [ i ];
            k < ce->tube_tri_offset [ i ] + ce->tube_tri_num [ i ];
            k++)
            {
              l= 3 * ce->tube_index [ k ];
      ...
      for (i= 0; i < ce->self_tri_strips; i++)
        {
          ...
          for (k= ce->self_tri_offset [ i ];
            k < ce->self_tri_offset [ i ] + ce->self_tri_num [ i ];
            k++)
            {
              l= 3 * ce->self_index [ k ];
...
#else
          glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, mat_diff_magenta);
#endif
        }
      else /* ce->display_mode == DISP_TRANSPARENT */
        {
...
#else
          glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, mat_diff_magenta_trans);
#endif
        }
    }
...
/* Draw the cuboctahedron eversion using OpenGL's programmable
   functionality. */
static int cuboctahedron_eversion_pf(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  ...
  glBufferData(GL_ARRAY_BUFFER, 3 * 3 * NUM_FACE * sizeof(GLfloat), ce->tt, GL_STREAM_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);

  glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_normal_buffer);
  glBufferData(GL_ARRAY_BUFFER, 3 * 3 * NUM_FACE * sizeof(GLfloat), ce->tn, GL_STREAM_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);
  ...
  glsl_Identity(p_mat);
  if (ce->projection == DISP_ORTHOGRAPHIC)
    glsl_Orthographic(p_mat, -ce->fov_ortho * ce->aspect, ce->fov_ortho * ce->aspect, -ce->fov_ortho, ce->fov_ortho, z_min, z_max);
  else
  ...
  if (ce->display_mode == DISP_SURFACE ||
          ...
          for (i= 0; i < ce->tube_tri_strips; i++)
            {
              ...
              glDrawElements(GL_TRIANGLE_STRIP, ce->tube_tri_num [ i ], GL_UNSIGNED_INT, (const GLvoid *) index_offset);
              polys+= ce->tube_tri_num [ i ] - 2;
            }
          ...
          for (i= 0; i < ce->self_tri_strips; i++)
            {
              ...
              glDrawElements(GL_TRIANGLE_STRIP, ce->self_tri_num [ i ], GL_UNSIGNED_INT, (const GLvoid *) index_offset);
              polys+= ce->self_tri_num [ i ] - 2;
            }
      ...
      for (pass= 1; pass <= MAX_PASSES; pass++)
        {
          ...
          glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_pos_buffer);
          glVertexAttribPointer(ce->dp_peel_pos_index, 3, GL_FLOAT, GL_FALSE, 0, 0);

          glEnableVertexAttribArray(ce->dp_peel_normal_index);
          glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_normal_buffer);
          glVertexAttribPointer(ce->dp_peel_normal_index, 3, GL_FLOAT, GL_FALSE, 0, 0);

          glEnableVertexAttribArray(ce->dp_peel_colorf_index);
          glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_colorf_buffer);
          glVertexAttribPointer(ce->dp_peel_colorf_index, 4, GL_FLOAT, GL_FALSE, 0, 0);

          glEnableVertexAttribArray(ce->dp_peel_colorb_index);
          glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_colorb_buffer);
          glVertexAttribPointer(ce->dp_peel_colorb_index, 4, GL_FLOAT, GL_FALSE, 0, 0);

          glEnableVertexAttribArray(ce->dp_peel_vertex_tex_index);
          glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_tex_coord_buffer);
          glVertexAttribPointer(ce->dp_peel_vertex_tex_index, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  else /* ce->transparency == TRANSPARENCY_APPROXIMATE */
    {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, ce->cuboct_tex_coord_buffer);
      glVertexAttribPointer(ce->wa_init_vertex_tex_index, 3, GL_FLOAT, GL_FALSE, 0, 0);
...
static void delete_dual_peeling_render_targets(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
...
static void init_dual_peeling_render_targets(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  int i;
  ...
  if (ce->use_shaders && ce->transparency == TRANSPARENCY_CORRECT)
    {
      ...
      for (i= 0; i < 2; i++)
        {
          ...
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, ce->WindW, ce->WindH, 0, GL_RG, GL_FLOAT, 0);
          ...
          glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
          glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
          glTexImage2D(GL_TEXTURE_2D, 0, rgba_int, ce->WindW, ce->WindH, 0, GL_RGBA, float_format, 0);
          ...
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ce->dp_depth_tex [ i ], 0);
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, ce->dp_front_blender_tex [ i ], 0);
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, ce->dp_back_temp_tex [ i ], 0);
          status= glCheckFramebufferStatus(GL_FRAMEBUFFER);
          if (status != GL_FRAMEBUFFER_COMPLETE)
            all_fbo_complete= False;

          glBindFramebuffer(GL_FRAMEBUFFER, ce->dp_color_fbo [ i ]);
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ce->dp_front_blender_tex [ i ], 0);
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, ce->dp_back_temp_tex [ i ], 0);
          status= glCheckFramebufferStatus(GL_FRAMEBUFFER);
          if (status != GL_FRAMEBUFFER_COMPLETE)
            all_fbo_complete= False;
        }
      ...
      glBindFramebuffer(GL_FRAMEBUFFER, ce->dp_back_blender_fbo);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ce->dp_back_blender_tex, 0);
      status= glCheckFramebufferStatus(GL_FRAMEBUFFER);
      if (status != GL_FRAMEBUFFER_COMPLETE)
        all_fbo_complete= False;
      ...
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, ce->WindW, ce->WindH, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0);
      ...
      glBindFramebuffer(GL_FRAMEBUFFER, ce->dp_opaque_fbo);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, ce->dp_opaque_depth_tex, 0);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ce->dp_opaque_color_tex, 0);
      status= glCheckFramebufferStatus(GL_FRAMEBUFFER);
      if (status != GL_FRAMEBUFFER_COMPLETE)
        all_fbo_complete= False;
...
static void delete_weighted_average_render_targets(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
...
static void init_weighted_average_render_targets(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  GLenum status;
  ...
  if (ce->use_shaders && ce->transparency == TRANSPARENCY_APPROXIMATE)
    {
      ...
      glBindFramebuffer(GL_FRAMEBUFFER, ce->wa_opaque_fbo);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, ce->wa_opaque_depth_tex, 0);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ce->wa_opaque_color_tex, 0);
      status= glCheckFramebufferStatus(GL_FRAMEBUFFER);
      if (status != GL_FRAMEBUFFER_COMPLETE)
        all_fbo_complete= False;
      ...
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
      glTexImage2D(GL_TEXTURE_2D, 0, r_int, ce->WindW, ce->WindH, 0, GL_RED, float_format, 0);

      glGenFramebuffers(1, &ce->wa_accumulation_fbo);
      glBindFramebuffer(GL_FRAMEBUFFER, ce->wa_accumulation_fbo);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, ce->wa_opaque_depth_tex, 0);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ce->wa_color_tex, 0);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, ce->wa_num_tex, 0);
      status= glCheckFramebufferStatus(GL_FRAMEBUFFER);
      if (status != GL_FRAMEBUFFER_COMPLETE)
        all_fbo_complete= False;
  ...
  ModeInfo *mi)
{
  ...
  if (! glsl_CompileAndLinkShaders(2, poly_vertex_shader_source, 2, poly_fragment_shader_source, &ce->poly_shader_program))
    return;
  ...
  if (! glsl_CompileAndLinkShaders(2, dp_init_vertex_shader_source, 2, dp_init_fragment_shader_source, &ce->dp_init_shader_program))
    {
      glDeleteProgram(ce->poly_shader_program);
      return;
    }
  ...
  if (! glsl_CompileAndLinkShaders(2, dp_peel_vertex_shader_source, 2, dp_peel_fragment_shader_source, &ce->dp_peel_shader_program))
    {
      glDeleteProgram(ce->poly_shader_program);
      glDeleteProgram(ce->dp_init_shader_program);
      return;
  ...
  if (! glsl_CompileAndLinkShaders(2, dp_blnd_vertex_shader_source, 2, dp_blnd_fragment_shader_source, &ce->dp_blnd_shader_program))
    {
      glDeleteProgram(ce->poly_shader_program);
      glDeleteProgram(ce->dp_init_shader_program);
      glDeleteProgram(ce->dp_peel_shader_program);
  ...
  if (! glsl_CompileAndLinkShaders(2, dp_finl_vertex_shader_source, 2, dp_finl_fragment_shader_source, &ce->dp_finl_shader_program))
    {
      glDeleteProgram(ce->poly_shader_program);
      glDeleteProgram(ce->dp_init_shader_program);
      glDeleteProgram(ce->dp_peel_shader_program);
  ...
  if (! glsl_CompileAndLinkShaders(2, wa_init_vertex_shader_source, 2, wa_init_fragment_shader_source, &ce->wa_init_shader_program))
    return;
  ...
  if (! glsl_CompileAndLinkShaders(2, wa_finl_vertex_shader_source, 2, wa_finl_fragment_shader_source, &ce->wa_finl_shader_program))
    {
      glDeleteProgram(ce->poly_shader_program);
      glDeleteProgram(ce->dp_init_shader_program);
      glDeleteProgram(ce->dp_peel_shader_program);
...
static void display_cubocteversion(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  float t;
...
ENTRYPOINT void reshape_cubocteversion(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT Bool cubocteversion_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
  ...
  if (event->xany.type == ButtonPress && event->xbutton.button == Button1)
    {
      ...
      gltrackball_start(ce->trackball, event->xbutton.x, event->xbutton.y, MI_WIDTH(mi), MI_HEIGHT(mi));
      return True;
    }
    ...
    {
      gltrackball_track(ce->trackball, event->xmotion.x, event->xmotion.y, MI_WIDTH(mi), MI_HEIGHT(mi));
      return True;
    }
...
ENTRYPOINT void init_cubocteversion(
  ModeInfo *mi)
{
 ...
 *-----------------------------------------------------------------------------
 */
ENTRYPOINT void draw_cubocteversion(
  ModeInfo *mi)
{
...
#ifndef STANDALONE
ENTRYPOINT void change_cubocteversion(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_cubocteversion(
  ModeInfo *mi)
{
  cubocteversionstruct *ce= &cubocteversion [ MI_SCREEN(mi) ];
