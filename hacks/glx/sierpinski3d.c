...
ModStruct gasket_description=
  {"gasket", "init_gasket", "draw_gasket", NULL, "draw_gasket", "init_gasket", "free_gasket", &gasket_opts, 1000, 1, 2, 1, 4, 1.0, "", "Shows GL's Sierpinski gasket", 0, NULL};
      ...
      if (which == 0)
        {
          ...
          triangle(outer [ 0 ].x, outer [ 0 ].y, outer [ 0 ].z, outer [ 1 ].x, outer [ 1 ].y, outer [ 1 ].z, outer [ 2 ].x, outer [ 2 ].y, outer [ 2 ].z, wireframe_p);
        }
      else if (which == 1)
        {
          glNormal3f(normals [ 1 ].x, normals [ 1 ].y, normals [ 1 ].z);
          triangle(outer [ 0 ].x, outer [ 0 ].y, outer [ 0 ].z, outer [ 3 ].x, outer [ 3 ].y, outer [ 3 ].z, outer [ 1 ].x, outer [ 1 ].y, outer [ 1 ].z, wireframe_p);
        }
      else if (which == 2)
        {
          glNormal3f(normals [ 2 ].x, normals [ 2 ].y, normals [ 2 ].z);
          triangle(outer [ 0 ].x, outer [ 0 ].y, outer [ 0 ].z, outer [ 2 ].x, outer [ 2 ].y, outer [ 2 ].z, outer [ 3 ].x, outer [ 3 ].y, outer [ 3 ].z, wireframe_p);
        }
      else
        {
          glNormal3f(normals [ 3 ].x, normals [ 3 ].y, normals [ 3 ].z);
          triangle(outer [ 1 ].x, outer [ 1 ].y, outer [ 1 ].z, outer [ 3 ].x, outer [ 3 ].y, outer [ 3 ].z, outer [ 2 ].x, outer [ 2 ].y, outer [ 2 ].z, wireframe_p);
        }
    }
  else
    {
    ...
    int which)
{
  ...
  four_tetras(gp, vertex, normal, wireframe_p, (gp->current_depth < 0 ? -gp->current_depth : gp->current_depth), which, &count);
  ...
  if (gp->tick++ >= speed)
    {
      ...
      mi->polygon_count= 0;
      glNewList(gp->gasket0, GL_COMPILE);
      compile_gasket(mi, 0);
      glEndList();
      glNewList(gp->gasket1, GL_COMPILE);
      compile_gasket(mi, 1);
      glEndList();
      glNewList(gp->gasket2, GL_COMPILE);
      compile_gasket(mi, 2);
      glEndList();
      glNewList(gp->gasket3, GL_COMPILE);
      compile_gasket(mi, 3);
      glEndList();

      mi->recursion_depth= (gp->current_depth > 0 ? gp->current_depth : -gp->current_depth);
    }
}
    ...
    ModeInfo *mi)
{
  ...
  make_smooth_colormap(0, 0, 0, gp->colors, &gp->ncolors, False, 0, False);
  gp->ccolor0= 0;
