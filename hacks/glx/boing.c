      ...
      for (x= 0; x < xx; x++)
        {
          ...
          glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (bgp ? bp->ball_color2 : bp->ball_color1));
    ...
    ModeInfo *mi)
{
  ...
  if (bp->ball_y < min)
    bp->ball_y= min, bp->ball_dy= -bp->ball_dy;
  else if (bp->ball_y > max)
    bp->ball_y= max, bp->ball_dy= -bp->ball_dy;

  bp->ball_dz+= bp->ball_ddz;
  bp->ball_z+= bp->ball_dz;
  if (bp->ball_z < min)
    bp->ball_z= min, bp->ball_dz= -bp->ball_dz;
  else if (bp->ball_z > max)
    bp->ball_z= max, bp->ball_dz= -bp->ball_dz;
}
