...
static inline double bednorz_get_kappa(
  int n)
{
  return (n - 1.0) / (2.0 * n);
}
...
/* Compute the Bednorz shape parameter t based on the deformation
   parameter tau. */
static inline double bednorz_get_t(
  double tau)
{
...
/* Compute the Bednorz shape parameter q based on the deformation
   parameter tau. */
static inline double bednorz_get_q(
  double tau)
{
  double abs_tau;
...
/* Compute the Bednorz shape parameter p based on the deformation
   parameter tau. */
static inline double bednorz_get_p(
  double tau)
{
...
/* Compute the Bednorz shape parameter xi based on the deformation
   parameter tau. */
static inline double bednorz_get_xi(
  double tau)
{
  double abs_tau;
...
static inline double bednorz_get_eta(
  double tau,
  double eta_min)
{
  double abs_tau;
...
/* Compute the Bednorz shape parameter alpha based on the deformation
   parameter tau. */
static inline double bednorz_get_alpha(
  double tau)
{
  double xi;
...
static inline double bednorz_get_beta(
  double tau,
  double beta_max)
{
  double xi;
...
static inline double bednorz_get_gamma(
  double alpha,
  double beta)
{
...
/* Compute the Bednorz shape parameter lambda based on the deformation
   parameter tau. */
static inline double bednorz_get_lambda(
  double tau)
{
  double abs_tau;
...
static inline double bednorz_get_eps(
  double tau,
  int n)
{
...
static inline void bednorz_get_p0(
  double phi,
  double theta,
...
static inline void bednorz_get_p1(
  double phi,
  double theta,
  bednorz_shape_par *bsp,
  double y [ 3 ],
...
static void inline bednorz_get_p2(
  double phi,
  double theta,
  bednorz_shape_par *bsp,
  double z [ 3 ],
...
static void inline bednorz_point_normal(
  double phi,
  double theta,
  bednorz_shape_par *bsp,
  float p [ 3 ],
          ...
          for (i= 0; i <= num_theta; i++)
            {
              ...
              if (se->colors [ 0 ] == COLORS_PARALLEL)
                color(se, (2.0f * theta + 3.0f * M_PI_F / 4.0f) * (2.0f / 3.0f), matc, &se->colf [ 0 ][ 4 * o ], &se->colb [ 0 ][ 4 * o ]);
...
/* Draw the Bednorz sphere eversion using OpenGL's fixed
   functionality. */
static int bednorz_sphere_eversion_ff(
  ModeInfo *mi,
  float phi_min,
  float phi_max,
  float theta_min,
  ...
  if (fabsf(tau) <= BEDNORZ_TAU4)
    {
      ...
      else if (fabsf(tau) <= BEDNORZ_TAU2)
        {
...
/* Draw the Bednorz sphere eversion using OpenGL's programmable
   functionality. */
static int bednorz_sphere_eversion_pf(
  ModeInfo *mi,
  float phi_min,
  float phi_max,
  float theta_min,
  ...
  int num_grid)
{
  ...
  if (! se->buffers_initialized)
    {
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, ni * sizeof(GLuint), se->line_indices, GL_STATIC_DRAW);
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
  ...
  glBufferData(GL_ARRAY_BUFFER, 3 * (num_phi + 1) * (num_theta + 1) * sizeof(GLfloat), se->sp, GL_STREAM_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

  glBindBuffer(GL_ARRAY_BUFFER, se->vertex_normal_buffer);
  glBufferData(GL_ARRAY_BUFFER, 3 * (num_phi + 1) * (num_theta + 1) * sizeof(GLfloat), se->sn, GL_STREAM_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);
  ...
  if (se->projection == DISP_PERSPECTIVE)
    ...
    glsl_Orthographic(p_mat, -se->aspect, se->aspect, -1.0f, 1.0f, 0.1f, 10.0f);
  else
    glsl_Orthographic(p_mat, -1.0f, 1.0f, -1.0f / se->aspect, 1.0f / se->aspect, 0.1f, 10.0f);
      ...
      if (se->graticule [ pass ])
        {
          ...
          index_offset= 0;
          glDrawElements(GL_LINES, se->num_lines, GL_UNSIGNED_INT, (const void *) index_offset);
...
void init_sphereeversion_analytic(
  ModeInfo *mi)
{
  sphereeversionstruct *se= &sphereeversion [ MI_SCREEN(mi) ];
...
void display_sphereeversion_analytic(
  ModeInfo *mi)
{
...
#ifdef HAVE_GLSL
  if (se->use_shaders)
    mi->polygon_count= bednorz_sphere_eversion_pf(mi, -M_PI_F, M_PI_F, -M_PI_F / 2.0f, M_PI_F / 2.0f, NUMPH, NUMTH, NUMBDIST, NUMBDIR, NUMGRID);
  else
#endif /* HAVE_GLSL */
    mi->polygon_count= bednorz_sphere_eversion_ff(mi, -M_PI_F, M_PI_F, -M_PI_F / 2.0f, M_PI_F / 2.0f, NUMPH, NUMTH, NUMBDIST, NUMBDIR, NUMGRID);
}
