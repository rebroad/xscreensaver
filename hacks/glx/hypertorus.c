...
ModStruct hypertorus_description=
  {"hypertorus", "init_hypertorus", "draw_hypertorus", NULL, "draw_hypertorus", "change_hypertorus", NULL, &hypertorus_opts, 25000, 1, 1, 1, 1.0, 4, "", "Shows a hypertorus rotating in 4d", 0, NULL};
  ...
  int numv)
{
  ...
  if (! hp->buffers_initialized)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 2 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), hp->uv, GL_STATIC_DRAW);
      ...
      if (! change_colors && colors == COLORS_COLORWHEEL)
        {
          ...
          glBufferData(GL_ARRAY_BUFFER, 4 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), hp->col, GL_STATIC_DRAW);
          glBindBuffer(GL_ARRAY_BUFFER, 0);
        }
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, hp->ni * sizeof(GLuint), hp->indices, GL_STATIC_DRAW);
  ...
  if (projection_3d == DISP_3D_ORTHOGRAPHIC)
    if (hp->aspect >= 1.0)
      glsl_Orthographic(p_mat, -hp->aspect, hp->aspect, -1.0, 1.0, 0.1, 10.0);
    else
      glsl_Orthographic(p_mat, -1.0, 1.0, -1.0 / hp->aspect, 1.0 / hp->aspect, 0.1, 10.0);
  else
  ...
  if (colors == COLORS_COLORWHEEL)
    {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, hp->color_buffer);
      if (change_colors)
        glBufferData(GL_ARRAY_BUFFER, 4 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), hp->col, GL_STREAM_DRAW);
      ...
      for (i= 0; i < hp->ne; i++)
        {
          ...
          glDrawElements(GL_TRIANGLE_STRIP, hp->nt, GL_UNSIGNED_INT, (const void *) index_offset);
        }
    }
  else /* display_mode == DISP_WIREFRAME */
    {
      glLineWidth(1.0f);
      index_offset= 0;
      glDrawElements(GL_LINES, hp->ni, GL_UNSIGNED_INT, (const void *) index_offset);
    }
  ...
  ModeInfo *mi)
{
  ...
  if (! glsl_CompileAndLinkShaders(2, vertex_shader_source, 2, fragment_shader_source, &hp->shader_program))
    return;
...
ENTRYPOINT void reshape_hypertorus(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT Bool hypertorus_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  hypertorusstruct *hp= &hyper [ MI_SCREEN(mi) ];
 ...
 *-----------------------------------------------------------------------------
 */
ENTRYPOINT void draw_hypertorus(
  ModeInfo *mi)
{
...
#ifndef STANDALONE
ENTRYPOINT void change_hypertorus(
  ModeInfo *mi)
{
  hypertorusstruct *hp= &hyper [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_hypertorus(
  ModeInfo *mi)
{
  hypertorusstruct *hp= &hyper [ MI_SCREEN(mi) ];
