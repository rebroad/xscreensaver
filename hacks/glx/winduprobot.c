...
extern const struct gllist
  *robot_arm_half,
  *robot_body_half_outside, *robot_body_half_inside,
...
static const struct gllist *const *all_objs []= {
  &robot_arm_half,
  &robot_body_half_outside,
  &robot_body_half_inside,
  &robot_crank_full,
  &robot_gearbox_half,
  &robot_hand_half,
  &robot_leg_half,
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, bp->user_trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &bp->button_down_p))
    {
      return True;
    }
#ifdef DEBUG
  else if (event->xany.type == KeyPress && debug_p)
    {
      ...
      if (keysym == XK_Right)
        ...
        bp->debug_x= bp->debug_y= bp->debug_z= 0;
      else if (c == '\n' || c == '\r')
    ...
    ModeInfo *mi)
{
  ...
  xi= image_data_to_ximage(mi->dpy, mi->xgwa.visual, chromesphere_png, sizeof(chromesphere_png));
  clear_gl_error();
...
{
  robot_configuration *bp= &bps [ MI_SCREEN(mi) ];
  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, bp->component_colors [ i ]);
  glCallList(bp->dlists [ i ]);
  return (*all_objs [ i ])->points / 3;
}

static int
  draw_transparent_component(
    ModeInfo *mi,
    int i,
    GLfloat alpha)
{
    ...
    const char *label)
{
...

  {
    ...
    draw_bubble_box(mi, w, h, ch * 2, /* corner radius */
      ch * 2.5,                       /* arrow height */
      w / 2 - cw * 8,                 /* arrow x */
      bp->text_bd,
      bp->text_bg);
    ...
    ModeInfo *mi)
{
  ...
  scale= (bp->bubble_tick < duration * fade ? bp->bubble_tick / (duration * fade) : (bp->bubble_tick > duration * (1 - fade) ? 1 - ((bp->bubble_tick - duration * (1 - fade)) / (duration * fade)) : 1));
  ...
  for (i= 0; i < bp->nwalkers; i++)
    {
      ...
      *tag= 0;
      ...
      if (debug_p)
        {
          sprintf(tag, "%.4f, %.4f,  %.4f", bp->debug_x, bp->debug_y, bp->debug_z);
        }
      else
        {
#if 1
