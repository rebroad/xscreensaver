...
ModStruct extrusion_description=
  {"extrusion", "init_extrusion", "draw_extrusion", NULL, "draw_extrusion", "init_extrusion", NULL, &extrusion_opts, 1000, 1, 2, 1, 4, 1.0, "", "OpenGL extrusion", 0, NULL};
#endif
...
BUILTIN:
    ...
    {
      XImage *ximage= file_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), filename);
      if (! ximage)
        goto BUILTIN;
  ...
  if (do_mipmap)
    {
      int status;
      clear_gl_error();
      status= gluBuild2DMipmaps(GL_TEXTURE_2D, 3, width, height, format, GL_UNSIGNED_BYTE, image);
      if (status)
        {
    ...
    ModeInfo *mi)
{
  ...
  gp->rot= make_rotator(spin_speed, spin_speed, spin_speed, 0.2, 0.005, True);
    ...
    ModeInfo *mi)
{
  ...
  get_rotation(gp->rot, &x, &y, &z, ! (gp->button_down_p || gp->button2_down_p));
...

  {
    ...
    get_position(gp->rot, &x, &y, &z, ! (gp->button_down_p || gp->button2_down_p));
