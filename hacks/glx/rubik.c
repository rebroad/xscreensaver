...
ModStruct rubik_description=
  {"rubik", "init_rubik", "draw_rubik", (char *) NULL, "draw_rubik", "change_rubik", "free_rubik", &rubik_opts, 10000, -30, 5, -6, 64, 1.0, "", "Shows an auto-solving Rubik's Cube", 0, NULL};
  ...
  if (plane.face == 1 || /* VERTICAL */
    (plane.face == 2 && (move.face == 1 || move.face == 3)))
    {
      if ((*slice).rotation == CW)
        (*slice).depth= sizeOfRow - 1 - move.position % sizeOfRow;
      else
        (*slice).depth= move.position % sizeOfRow;
    }
  else
    { /* (plane.face == 0 ||  */ /* HORIZONTAL */ /*
   (plane.face == 2 && (move.face == 0 || move.face == 4))) */
      if ((*slice).rotation == CW)
        (*slice).depth= sizeOfColumn - 1 - move.position / sizeOfRow;
      else
        (*slice).depth= move.position / sizeOfRow;
    }
  ...
  switch (slice.face)
    {
      ...
      case TOP_FACE : /* BOTTOM_FACE too */
        ...
        DRAW_CUBIT(mi,
          rp->cubeLoc [ BACK_FACE ][ FIRSTX + MAXSIZEX * FIRSTY ].face,
          NO_FACE,
          rp->cubeLoc [ LEFT_FACE ][ FIRSTZ + MAXSIZEZ * LASTY ].face,
          NO_FACE,
          rp->cubeLoc [ BOTTOM_FACE ][ FIRSTX + MAXSIZEX * LASTZ ].face,
          NO_FACE);
        for (k= 1; k < MAXSIZEZ - 1; k++)
          {
            ...
            DRAW_CUBIT(mi,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ LEFT_FACE ][ k + MAXSIZEZ * LASTY ].face,
              NO_FACE,
              rp->cubeLoc [ BOTTOM_FACE ][ FIRSTX + MAXSIZEX * REVZ(k) ].face,
              NO_FACE);
          }
        glTranslatef(0, 0, S1);
        DRAW_CUBIT(mi,
          NO_FACE,
          rp->cubeLoc [ FRONT_FACE ][ FIRSTX + MAXSIZEX * LASTY ].face,
          rp->cubeLoc [ LEFT_FACE ][ LASTZ + MAXSIZEZ * LASTY ].face,
          NO_FACE,
          rp->cubeLoc [ BOTTOM_FACE ][ FIRSTX + MAXSIZEX * FIRSTZ ].face,
          NO_FACE);
        for (i= 1; i < MAXSIZEX - 1; i++)
          {
            glTranslatef(S1, 0, -SZ);
            DRAW_CUBIT(mi,
              rp->cubeLoc [ BACK_FACE ][ i + MAXSIZEX * FIRSTY ].face,
              NO_FACE,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ BOTTOM_FACE ][ i + MAXSIZEX * LASTZ ].face,
              NO_FACE);
            for (k= 1; k < MAXSIZEZ - 1; k++)
              {
                glTranslatef(0, 0, S1);
                DRAW_CUBIT(mi,
                  ...
                  NO_FACE,
                  NO_FACE,
                  rp->cubeLoc [ BOTTOM_FACE ][ i + MAXSIZEX * REVZ(k) ].face,
                  NO_FACE);
              }
            glTranslatef(0, 0, S1);
            DRAW_CUBIT(mi,
              NO_FACE,
              rp->cubeLoc [ FRONT_FACE ][ i + MAXSIZEX * LASTY ].face,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ BOTTOM_FACE ][ i + MAXSIZEX * FIRSTZ ].face,
              NO_FACE);
          }
        glTranslatef(S1, 0, -SZ);
        DRAW_CUBIT(mi,
          rp->cubeLoc [ BACK_FACE ][ LASTX + MAXSIZEX * FIRSTY ].face,
          NO_FACE,
          NO_FACE,
          rp->cubeLoc [ RIGHT_FACE ][ LASTZ + MAXSIZEZ * LASTY ].face,
          rp->cubeLoc [ BOTTOM_FACE ][ LASTX + MAXSIZEX * LASTZ ].face,
          NO_FACE);
        for (k= 1; k < MAXSIZEZ - 1; k++)
          {
            glTranslatef(0, 0, S1);
            DRAW_CUBIT(mi,
              ...
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ RIGHT_FACE ][ REVZ(k) + MAXSIZEZ * LASTY ].face,
              rp->cubeLoc [ BOTTOM_FACE ][ LASTX + MAXSIZEX * REVZ(k) ].face,
              NO_FACE);
          }
        glTranslatef(0, 0, S1);
        DRAW_CUBIT(mi,
          NO_FACE,
          rp->cubeLoc [ FRONT_FACE ][ LASTX + MAXSIZEX * LASTY ].face,
          NO_FACE,
          rp->cubeLoc [ RIGHT_FACE ][ FIRSTZ + MAXSIZEZ * LASTY ].face,
          rp->cubeLoc [ BOTTOM_FACE ][ LASTX + MAXSIZEX * FIRSTZ ].face,
          NO_FACE);
        glPopMatrix();
        for (j= 1; j < MAXSIZEY - 1; j++)
          {
            ...
            DRAW_CUBIT(mi,
              rp->cubeLoc [ BACK_FACE ][ FIRSTX + MAXSIZEX * j ].face,
              NO_FACE,
              rp->cubeLoc [ LEFT_FACE ][ FIRSTZ + MAXSIZEZ * REVY(j) ].face,
              NO_FACE,
              NO_FACE,
              NO_FACE);
            for (k= 1; k < MAXSIZEZ - 1; k++)
              {
                ...
                DRAW_CUBIT(mi,
                  NO_FACE,
                  NO_FACE,
                  rp->cubeLoc [ LEFT_FACE ][ k + MAXSIZEZ * REVY(j) ].face,
                  NO_FACE,
                  NO_FACE,
                  NO_FACE);
              }
            ...
            DRAW_CUBIT(mi,
              NO_FACE,
              rp->cubeLoc [ FRONT_FACE ][ FIRSTX + MAXSIZEX * REVY(j) ].face,
            ...
            for (i= 1; i < MAXSIZEX - 1; i++)
              {
                glTranslatef(1, 0, -SZ);
                DRAW_CUBIT(mi,
                  rp->cubeLoc [ BACK_FACE ][ i + MAXSIZEX * j ].face,
                  NO_FACE,
                  NO_FACE,
                  NO_FACE,
                  NO_FACE,
                ...
                DRAW_CUBIT(mi,
                  NO_FACE,
                  rp->cubeLoc [ FRONT_FACE ][ i + MAXSIZEX * REVY(j) ].face,
                  NO_FACE,
                  NO_FACE,
                  NO_FACE,
                  NO_FACE);
            ...
            glTranslatef(S1, 0, -SZ);
            DRAW_CUBIT(mi,
              rp->cubeLoc [ BACK_FACE ][ LASTX + MAXSIZEX * j ].face,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ RIGHT_FACE ][ LASTZ + MAXSIZEZ * REVY(j) ].face,
              NO_FACE,
              NO_FACE);
            for (k= 1; k < MAXSIZEZ - 1; k++)
              {
                ...
                DRAW_CUBIT(mi,
                  ...
                  NO_FACE,
                  NO_FACE,
                  rp->cubeLoc [ RIGHT_FACE ][ REVZ(k) + MAXSIZEZ * REVY(j) ].face,
                  NO_FACE,
                  NO_FACE);
              }
            glTranslatef(0, 0, S1);
            DRAW_CUBIT(mi,
              NO_FACE,
              rp->cubeLoc [ FRONT_FACE ][ LASTX + MAXSIZEX * REVY(j) ].face,
              NO_FACE,
              rp->cubeLoc [ RIGHT_FACE ][ FIRSTZ + MAXSIZEZ * REVY(j) ].face,
              NO_FACE,
              NO_FACE);
            glPopMatrix();
          }
        ...
        glTranslatef(-HALFX, HALFY, -HALFZ);
        DRAW_CUBIT(mi,
          rp->cubeLoc [ BACK_FACE ][ FIRSTX + MAXSIZEX * LASTY ].face,
          NO_FACE,
          rp->cubeLoc [ LEFT_FACE ][ FIRSTZ + MAXSIZEZ * FIRSTY ].face,
          NO_FACE,
          NO_FACE,
          rp->cubeLoc [ TOP_FACE ][ FIRSTX + MAXSIZEX * FIRSTZ ].face);
        for (k= 1; k < MAXSIZEZ - 1; k++)
          {
            glTranslatef(0, 0, S1);
            DRAW_CUBIT(mi,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ LEFT_FACE ][ k + MAXSIZEZ * FIRSTY ].face,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ TOP_FACE ][ FIRSTX + MAXSIZEX * k ].face);
          }
        glTranslatef(0, 0, S1);
        DRAW_CUBIT(mi,
          NO_FACE,
          rp->cubeLoc [ FRONT_FACE ][ FIRSTX + MAXSIZEX * FIRSTY ].face,
          rp->cubeLoc [ LEFT_FACE ][ LASTZ + MAXSIZEZ * FIRSTY ].face,
          NO_FACE,
          NO_FACE,
          rp->cubeLoc [ TOP_FACE ][ FIRSTX + MAXSIZEX * LASTZ ].face);
        for (i= 1; i < MAXSIZEX - 1; i++)
          {
            glTranslatef(S1, 0, -SZ);
            DRAW_CUBIT(mi,
              rp->cubeLoc [ BACK_FACE ][ i + MAXSIZEX * LASTY ].face,
              NO_FACE,
              NO_FACE,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ TOP_FACE ][ i + MAXSIZEX * FIRSTZ ].face);
            for (k= 1; k < MAXSIZEZ - 1; k++)
              {
                glTranslatef(0, 0, S1);
                DRAW_CUBIT(mi,
                  ...
                  NO_FACE,
                  NO_FACE,
                  rp->cubeLoc [ TOP_FACE ][ i + MAXSIZEX * k ].face);
              }
            glTranslatef(0, 0, S1);
            DRAW_CUBIT(mi,
              NO_FACE,
              rp->cubeLoc [ FRONT_FACE ][ i + MAXSIZEX * FIRSTY ].face,
              NO_FACE,
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ TOP_FACE ][ i + MAXSIZEX * LASTZ ].face);
          }
        glTranslatef(S1, 0, -SZ);
        DRAW_CUBIT(mi,
          rp->cubeLoc [ BACK_FACE ][ LASTX + MAXSIZEX * LASTY ].face,
          NO_FACE,
          NO_FACE,
          rp->cubeLoc [ RIGHT_FACE ][ LASTZ + MAXSIZEZ * FIRSTY ].face,
          NO_FACE,
          rp->cubeLoc [ TOP_FACE ][ LASTX + MAXSIZEX * FIRSTZ ].face);
        for (k= 1; k < MAXSIZEZ - 1; k++)
          {
            glTranslatef(0, 0, S1);
            DRAW_CUBIT(mi,
              ...
              NO_FACE,
              NO_FACE,
              rp->cubeLoc [ RIGHT_FACE ][ REVZ(k) + MAXSIZEZ * FIRSTY ].face,
              NO_FACE,
              rp->cubeLoc [ TOP_FACE ][ LASTX + MAXSIZEX * k ].face);
          }
        glTranslatef(0, 0, S1);
        DRAW_CUBIT(mi,
          NO_FACE,
          rp->cubeLoc [ FRONT_FACE ][ LASTX + MAXSIZEX * FIRSTY ].face,
          NO_FACE,
          rp->cubeLoc [ RIGHT_FACE ][ FIRSTZ + MAXSIZEZ * FIRSTY ].face,
          NO_FACE,
          rp->cubeLoc [ TOP_FACE ][ LASTX + MAXSIZEX * LASTZ ].face);
        break;
  ...
  if (rp->degreeTurn == 180)
    {
      ...
      slideRC(face, direction, h, sizeOnOppAxis, &newFace, &newDirection, &newH, &rotate, &reverse);
      ...
      for (k= 2; k <= MAXORIENT + 1; k++)
        {
          ...
          if (k != MAXORIENT && k != MAXORIENT + 1)
            readRC(rp, newFace, newDirection, newH, k, (k % 2) ? sizeOnDepthAxis : sizeOnAxis);
          rotateRC(rp, rotate, k - 2, (k % 2) ? sizeOnDepthAxis : sizeOnAxis);
          if (k != MAXORIENT + 1)
            rotateRC(rp, rotate, k - 1, (k % 2) ? sizeOnAxis : sizeOnDepthAxis);
          if (reverse == True)
            {
              reverseRC(rp, k - 2, (k % 2) ? sizeOnDepthAxis : sizeOnAxis);
              if (k != MAXORIENT + 1)
                reverseRC(rp, k - 1, (k % 2) ? sizeOnAxis : sizeOnDepthAxis);
            }
          writeRC(rp, newFace, newDirection, newH, k - 2, (k % 2) ? sizeOnDepthAxis : sizeOnAxis);
          face= newFace;
          direction= newDirection;
          h= newH;
        }
  ...
  for (face= 0; face < MAXFACES; face++)
    {
      ...
      for (position= 0; position < sizeOfRow * sizeOfColumn; position++)
        {
          (void) printf("%d %d  ", rp->cubeLoc [ face ][ position ].face, rp->cubeLoc [ face ][ position ].rotation);
    ...
    Bool opp)
{
  ...
  if (slice1.face == slice2.face &&
    ...
    {
      if (slice1.rotation == slice2.rotation)
        { /* CW or CCW */
          if (! opp)
            return True;
        }
    ...
    ModeInfo *mi)
{
  ...
  if (LRAND() % 2 && ! sizey && ! sizez)
    { /* Make normal (NxNxN) cubes more likely */
  ...
  else
    {
      ...
      if (i < -MINSIZE)
        ...
        i= MINSIZE;
      if (LRAND() % 2 && ! sizez)
        { /* Make more MxNxN more likely than LxMxN */
          MAXSIZEY= MAXSIZEZ= i;
        }
      else
        {
  ...
  for (i= 0; i < rp->storedmoves; i++)
    {
      ...
      do {
          ...
          rp->degreeTurn= (checkFaceSquare(rp,
                            rowToRotate [ move.face ][ move.direction ])) ?
            90 :
            180;
          condition= True;
          if (i > 0)
            { /* avoid immediate undoing moves */
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, rp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &rp->button_down_p))
    {
      return True;
    }
  ...
  if (rp->action == ACTION_SHUFFLE)
    {
      ...
      else if (rp->movement.face == NO_FACE)
        {
