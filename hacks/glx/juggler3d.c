  ...
  for (side= LEFT; side <= RIGHT; side= (Hand) ((int) side + 1))
    {
      ...
      /* Upper arm */
      polys+= tube(a [ 2 ].x - (side == LEFT ? soffx : -soffx), a [ 2 ].y + soffy, 0, a [ 1 ].x, a [ 1 ].y, ARMLENGTH / 2, thickness, 0, slices, True, True, MI_IS_WIREFRAME(mi));

      /* Lower arm */
      polys+= tube(a [ 1 ].x, a [ 1 ].y, ARMLENGTH / 2, a [ 0 ].x, a [ 0 ].y, ARMLENGTH, thickness * 0.8, 0, slices, True, True, MI_IS_WIREFRAME(mi));
  ...
  */
...

  {
    ...
    glPushMatrix();
    {
      ...
      polys+= tube(0, 0, 0, 0, 1.1, 0, 0.64, 0, slices, True, True, MI_IS_WIREFRAME(mi));
      glPopMatrix();
      glScalef(0.9, 0.9, 1);
      ...
      for (i= -1; i <= 1; i+= 2)
        {
          ...
          polys+= tube(0, 0, 0, 0, 3.5, 0, 1, 0, slices, True, True, MI_IS_WIREFRAME(mi));
          glPopMatrix();
          ...
          polys+= tube(0, 0, 0, 0, 4.7, 0, 0.8, 0, slices, True, True, MI_IS_WIREFRAME(mi));
          glPopMatrix();
          ...
          glScalef(0.4, 1, 1);
          polys+= tube(0, 0, 0, 0, 0.6, 0, 1.9, 0, slices * 4, True, True, MI_IS_WIREFRAME(mi));
          glPopMatrix();
  ...
  for (j= 0; j < stacks; j++)
    {
      ...
      glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, ((j == 0 || j == stacks - 1 || j % (stacks / (stripes + 1))) ? color1 : color2));
    ...
    Trace *s)
{
...

  {
    ...
    polys+= striped_unit_sphere(slices, slices, s->divisions, gcolor1, gcolor2, MI_IS_WIREFRAME(mi));
    glPopMatrix();
  }
  return polys;
}
    ...
    Trace *s)
{
...

  {
    ...
    glPushMatrix();
    glScalef(0.5, 1, 0.5);
    polys+= striped_unit_sphere(slices, slices, divs, gcolor2, gcolor1, MI_IS_WIREFRAME(mi));
    glPopMatrix();
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, gcolor2);
    polys+= tube(0, 0, 0, 0, 2, 0, 0.2, 0, slices, True, True, MI_IS_WIREFRAME(mi));
    ...
    Trace *s)
{
  ...
  polys+= tube(0, 0, 0, 0, 10, 0, 1, 0, slices, True, True, MI_IS_WIREFRAME(mi));
  ...
  for (j= -1; j <= 1; j+= 2)
    {
      ...
      for (i= 0; i < slices + (wire_p ? 0 : 1); i++)
        {
          GLfloat th, cth, sth;
          glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (i % (slices / 3) ? gcolor1 : gcolor2));
          th= i * width;
          cth= cos(th);
          sth= sin(th);
    ...
    ModeInfo *mi)
{
  ...
  make_random_colormap(0, 0, 0, mi->colors, &MI_NPIXELS(mi), True, False, 0, False);

  {
    double spin_speed= 0.05;
    double wander_speed= 0.001;
    double spin_accel= 0.05;
    sp->rot= make_rotator(0, spin_speed, 0, spin_accel, wander_speed, False);
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, sp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &sp->button_down_p))
    {
      return True;
    }
  ...
  for (o= sp->objects->next; o != sp->objects; o= o->next)
    {
      if (o->active)
        {
          mi->polygon_count+= ObjectDefs [ o->type ].draw(mi, o->color, o->trace->prev);
          o->active= False;
        }
    }
