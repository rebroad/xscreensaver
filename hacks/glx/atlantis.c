...
ModStruct atlantis_description=
  {"atlantis", "init_atlantis", "draw_atlantis", NULL, "refresh_atlantis", "change_atlantis", "free_atlantis", &atlantis_opts, 1000, NUM_SHARKS, SHARKSPEED, SHARKSIZE, 64, 1.0, "", "Shows moving sharks/whales/dolphin", 0, NULL};
    ...
    ModeInfo *mi)
{
  ...
  ap->texture= image_data_to_ximage(mi->dpy, mi->xgwa.visual, sea_texture_png, sizeof(sea_texture_png));
}
    ...
    ModeInfo *mi)
{
  ...
  if (ap->wire || ! do_texture)
  ...
  else
    {
      ...
      clear_gl_error();
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ap->texture->width, ap->texture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, ap->texture->data);
