    ...
    sp_instance *si)
{
  ...
  if (si->target_count < 0)
    { /* track to current target */
...
static void
  RenderSproingie(
    ...
    sp_instance *si)
{
  GLfloat scale, pointsize, mat_color [ 4 ]= {0.0, 0.0, 0.0, 1.0};
#ifndef HAVE_JWZGLES
    ...
    sp_instance *si)
{
  ...
  LayGround((-g_higher - g_back), (g_higher << 1), (g_back - g_higher), (g_width), (g_height), si);
}

void DisplaySproingies(
  sp_instance *si)
{
  int t;
  GLfloat position []=
