  ...
  if (st) /* might be too early */
    {
      ...
      print_texture_label(st->dpy, data->texfont, xgwa.width, xgwa.height, (data->top_p ? 1 : 2), st->string);
    }
}

void xlockmore_gl_free_fps(
  fps_state *st)
{
  gl_fps_data *data= (gl_fps_data *) st->gl_fps_data;
  if (data)
