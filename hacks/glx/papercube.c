    ...
    enum direction direction)
{
  ...
  if (
    (column == 0 && orientation == Horizontal && direction == Left_down) ||
    (column == (MAP_COLUMNS - 1) && orientation == Horizontal && direction == Right_up) ||
    (row == 0 && orientation == Vertical && direction == Left_down) ||
    (row == (MAP_ROWS - 1) && orientation == Vertical && direction == Right_up))
    return ' ';
  ...
  return map [ 2 * (MAP_ROWS - row - 1) * (MAP_COLUMNS * 2 - 1) + column * 2 + offset ];
}
...
static void
  paint_field_and_neighbours(
    struct papercube *papercube,
    struct field *field,
    const struct edge *entry_edge)
{
  int o;
  int d;
  ...
  if (entry_edge)
    paint_field(field, &papercube->angles [ field->row ][ field->column ], papercube->texture_id);
  else
  ...
  for (o= Horizontal; o < ORIENTATION_COUNT; o++)
    {
      for (d= Left_down; d < DIRECTION_COUNT; d++)
        {
          if (! (entry_edge != NULL && o == entry_edge->orientation && d == entry_edge->direction))
            {
              if ('+' == get_edge_from_map(papercube->map, field->row, field->column, o, d))
                {
                  struct edge edge;
    ...
    struct move_time *move_time)
{
  ...
  big_bang_time= move_time->big_bang.tv_sec + move_time->big_bang.tv_usec / 1000000.0;
  now_time= move_time->now.tv_sec + move_time->now.tv_usec / 1000000.0;
    ...
    double *value)
{
  ...
  if (move->stop <= time)
    ...
    {
      *value= move->start_value + (move->stop_value - move->start_value) * (time - move->start) / (move->stop - move->start);
    ...
    struct papercube *papercube)
{
  ...
  for (i= 0; i < sizeof(papercube->moves.move) / sizeof(papercube->moves.move [ 0 ]); i++)
    {
      ...
      move_stage= get_move_value(&field_move->move,
        papercube->moves.move_time.time,
        &value);
    ...
    struct rgba *color)
{
  ...
  for (x= 1; x < picture->square_count; x++)
    paint_rectangle(picture, x * picture->square_size - half_line_width, 0, picture->line_width, picture->height, color);

  for (y= 1; y < picture->square_count; y++)
    paint_rectangle(picture, 0, y * picture->square_size - half_line_width, picture->width, picture->line_width, color);
}

static void
  paint_picture_border(
    struct picture *picture)
{
  ...
  paint_rectangle(picture, x, y, picture->width, picture->border_width, &black);
  paint_rectangle(picture, x, y, picture->border_width, picture->height, &black);
    ...
    ModeInfo *mi)
{
  ...
  initialize_picture(&picture, papercube->show_grid, &papercube->fg, &papercube->bg);
    ...
    struct moves *moves)
{
  ...
  for (i= 0; i < sizeof(fields) / sizeof(fields [ 0 ]); i++)
    {
      ...
      set_field_move(field_move, time, time + fold * multi, 0.0, angle * multi, fields [ i ], False);
  ...
  set_field_move(field_move, time, time + fold * 2.0f / 3.0f, 30.0, angle, fields [ 13 ], False);

  field_move= &moves->move [ 16 ];

  set_field_move(field_move, time, time + fold * 2.0f / 3.0f, 120.0, angle, fields [ 14 ], True);

  time= field_move->move.stop + pause;

  set_move(&moves->spin, time, time + spin + sun_d, 0.0, (spin + sun_d) * spin_rps * 360);
    ...
    int height)
{
  ...
  if ((double) height / (double) width > PRESERVED_HEIGHT_TO_WIDTH)
    {
      double scale= (double) width * PRESERVED_HEIGHT_TO_WIDTH / (double) height;
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, papercube->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &papercube->button_down_p))
    return True;
