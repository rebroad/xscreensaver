  ...
  for (i= 0; i < o; i+= 3)
    {
      ...
      do_normal(pj [ 0 ], pj [ 1 ], pj [ 2 ], pj [ 0 ], pj [ 1 ], -pj [ 2 ], pk [ 0 ], pk [ 1 ], pk [ 2 ]);
    ...
    ModeInfo *mi)
{
  ...
  if (debug_p)
    fprintf(stderr, "%s: grid  %4d x %-4d               (%.2f)\n", progname, jc->puzzle_width, jc->puzzle_height, ((float) jc->puzzle_width / jc->puzzle_height));
      ...
      for (x= 0; x < jc->puzzle_width; x++)
        {
          ...
          p->polys+= draw_piece(jc, p, resolution_arg, thickness_arg, p->edge [ TOP ], p->edge [ RIGHT ], p->edge [ BOTTOM ], p->edge [ LEFT ], wire);
          glEndList();
        }
    }
}
    ...
    void *closure)
{
  ...
  if (debug_p)
    {
      fprintf(stderr, "%s: image %s\n", progname, (filename ? filename : "(null)"));
      fprintf(stderr, "%s: image %4d x %-4d + %4d + %-4d (%.2f)\n", progname, geometry->width, geometry->height, geometry->x, geometry->y, (float) geometry->width / geometry->height);
      fprintf(stderr, "%s: tex   %4d x %-4d\n", progname, texture_width, texture_height);
      fprintf(stderr, "%s: tex   %4.2f x %4.2f + %4.2f + %4.2f (%.2f)\n", progname, jc->tex_width, jc->tex_height, jc->tex_x, jc->tex_y, (jc->tex_width / jc->tex_height) * (texture_width / texture_height));
    }
...
{
  jigsaw_configuration *jc= &sps [ MI_SCREEN(mi) ];
  load_texture_async(mi->xgwa.screen, mi->window, *jc->glx_context, 0, 0, False, jc->texid, image_loaded_cb, mi);
}
...
#undef RTILT
      ...
      if (debug_p)
        fprintf(stderr, "%s: swapping %2d,%-2d with %2d,%d\n", progname, (int) p0->from.x, (int) p0->from.y, (int) p1->from.x, (int) p1->from.y);
      return;
    }
  ...
  if (! jc->loading_dlist)
    {
      ...
      jc->line_thickness= 1;
      draw_piece(jc, &P, resolution_arg, thickness_arg, OUT, OUT, IN, OUT, True);
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, jc->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &jc->button_down_p))
    {
      return True;
    }
