  ...
  for (i= 0; i < 1000; i++)
    {
      ...
      Bool ok= True;
      s->x= frand(1.0) - 0.5;
      s->y= (early_p && i == 0 ? frand(0.5) + 0.25 : -frand(depth));
    ...
    ModeInfo *mi)
{
  ...
  h2= h1 + 60 + frand(90);
  h3= h2 + 60 + frand(90);
  make_color_loop(0, 0, 0, h1, 0.5 + frand(0.5), 0.8 + frand(0.2), h2, 0.5 + frand(0.5), 0.8 + frand(0.2), h3, 0.5 + frand(0.5), 0.8 + frand(0.2), bp->colors, &bp->ncolors, False, False);
#endif
}
  ...
  for (i= 0; i < SPHINCTER_FRAMES; i++)
    {
      ...
      compute_unit_torus(mi, 0.1 * (i / (double) SPHINCTER_FRAMES), 20, 60);
