...
typedef enum
{ /* must be in the same order as in `all_objs'. */
  OBJ_BOX= 0,
  OBJ_ISO_BASE_A,
  OBJ_ISO_BASE_B,
  OBJ_ISO_DEN,
    ...
    int *outerp)
{
   ...
   */
  if (n [ 1 ] < -0.5)
    *face= 1, *outerp= v [ 1 ] < slack; /* S */
  else if (n [ 2 ] > 0.5)
    *face= 2, *outerp= v [ 2 ] > size - slack; /* U */
  else if (n [ 1 ] > 0.5)
    *face= 3, *outerp= v [ 1 ] > size - slack; /* N */
  else if (n [ 2 ] < -0.5)
    *face= 4, *outerp= v [ 2 ] < slack; /* D */
  else if (n [ 0 ] < -0.5)
    *face= 5, *outerp= v [ 0 ] < slack;        /* W */
  else                                         /* (n[0] >  0.5)*/
    *face= 6, *outerp= v [ 0 ] > size - slack; /* E */
  ...
  for (i= 0; i < countof(p); i++)
    {
      int j= (i + 1) % countof(p);
      /*      if (top_p)*/
      do_normal(z, 0, 0, 0, p [ i ].x, p [ i ].y, 0, p [ j ].x, p [ j ].y);
      /*
            else
              do_normal (z, 0, 0,
                         0, p[j].y, p[j].z,
...
#undef SWAP
      ...
      glBegin(wire ? GL_LINE_LOOP : GL_QUADS);
      do_normal(p [ 0 ][ 0 ], p [ 0 ][ 1 ], p [ 0 ][ 2 ], p [ 1 ][ 0 ], p [ 1 ][ 1 ], p [ 1 ][ 2 ], p [ 2 ][ 0 ], p [ 2 ][ 1 ], p [ 2 ][ 2 ]);
      glTexCoord2fv(t [ 0 ]);
      glVertex3fv(p [ 0 ]);
      glTexCoord2fv(t [ 1 ]);
      glVertex3fv(p [ 1 ]);
    ...
    GLfloat ratio)
{
  ...
  int face;
  static const GLfloat norms [ 6 ][ 3 ]= {{0, -1, 0}, {0, 0, 1}, {0, 1, 0}, {0, 0, -1}, {-1, 0, 0}, {1, 0, 0}};
    ...
    ModeInfo *mi)
{
  ...
  if (lc->type != LAMENT_LID_ZOOM)
    get_rotation(lc->rot, &lc->rotx, &lc->roty, &lc->rotz, ! lc->button_down_p);
#ifdef DEBUG_MODE
  lc->rotx= 0.18;
  lc->roty= 0.22;
  lc->rotx= lc->roty= 0;
  ...
  if (do_texture)
    {
      ...
      lc->texture= image_data_to_ximage(mi->dpy, mi->xgwa.visual, lament512_png, sizeof(lament512_png));
      ...
      for (i= 0; i < countof(lc->texids); i++)
        {
          ...
          clear_gl_error();
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, lc->texture->width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, (lc->texture->data + (lc->texture->bytes_per_line * height * i)));
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, lc->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &lc->button_down_p))
    {
      return True;
    }
