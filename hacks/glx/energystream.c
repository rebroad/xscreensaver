    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, es->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &es->button_down_p))
    return True;
  ...
  for (y= 0; y < TEX_HEIGHT; y++)
    {
      for (x= 0; x < TEX_WIDTH; x++)
        {
          color= 255 - sqrt((x - TEX_WIDTH / 2) * (x - TEX_WIDTH / 2) / COEFF + (y - TEX_HEIGHT / 2) * (y - TEX_HEIGHT / 2) / COEFF);
...
static void render_flare_stream(
  flare_stream *s,
  float cur_time,
  Vector *vx,
  Vector *vy,
  ...
  float m [ 16 ])
{
  ...
  dW= 1.0 / (a * (e * i - f * h) - (b * (d * i - f * g) + c * (e * g - d * h)));
  ...
  m [ 12 ]= (float) ((e * (g * l - i * j) + f * (h * j - g * k) - d * (h * l - i * k)) * dW);
  m [ 13 ]= (float) ((a * (h * l - i * k) + b * (i * j - g * l) + c * (g * k - h * j)) * dW);
  m [ 14 ]= (float) ((b * (d * l - f * j) + c * (e * j - d * k) - a * (e * l - f * k)) * dW);
}
