...
extern const struct gllist
  *teeth_model_jaw_upper_half,
  *teeth_model_jaw_lower_half,
  *teeth_model_teeth_upper_half, *teeth_model_teeth_lower_half;

static const struct gllist *const *all_objs []= {
  &teeth_model_jaw_upper_half,
  &teeth_model_jaw_lower_half,
  &teeth_model_teeth_upper_half,
  &teeth_model_teeth_lower_half,
};
    ...
    ModeInfo *mi)
{
...

  {
    ...
    make_color_ramp(0, 0, 0, h1, s1, v1, h2, s2, v2, b->colors, &b->ncolors, True, False, 0);
  }
    ...
    branch *b)
{
  ...
  b->slices= (slice **) (b->slices ? realloc(b->slices, (b->nslices + 1) * sizeof(*b->slices)) : calloc(b->nslices + 1, sizeof(*b->slices)));
    ...
    ModeInfo *mi)
{
  ...
  bp->branches= (branch **) (bp->branches ? realloc(bp->branches, (bp->nbranches + 1) * sizeof(*bp->branches)) : calloc(bp->nbranches + 1, sizeof(*bp->branches)));
    ...
    ModeInfo *mi)
{
...

  {
    ...
    bp->rot= make_rotator(0, 0, do_spin ? spin_speed : 0, spin_accel, do_wander ? wander_speed : 0, True);
    bp->rot2= make_rotator(0, 0, 0, 0, (do_tilt ? tilt_speed : 0), True);
