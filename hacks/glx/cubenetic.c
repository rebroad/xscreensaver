    ...
    ModeInfo *mi)
{
  ...
  interference(mi);
  clear_gl_error();
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, cc->texture_width, cc->texture_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, cc->texture);
    ...
    ModeInfo *mi)
{
  ...
  S [ 0 ]= S [ 1 ]= S [ 2 ]= 1.0;
  V [ 0 ]= V [ 1 ]= V [ 2 ]= 1.0;
  make_color_loop(0, 0, 0, H [ 0 ], S [ 0 ], V [ 0 ], H [ 1 ], S [ 1 ], V [ 1 ], H [ 2 ], S [ 2 ], V [ 2 ], cc->texture_colors, &cc->ncolors, False, False);

  make_smooth_colormap(0, 0, 0, cc->cube_colors, &cc->ncolors, False, 0, False);
}
