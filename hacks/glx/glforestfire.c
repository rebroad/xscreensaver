...
ModStruct fire_description=
  {"fire", "init_fire", "draw_fire", "release_fire", "draw_fire", "change_fire", (char *) NULL, &fire_opts, 10000, 800, 1, 400, 64, 1.0, "", "Shows a 3D fire-like image", 0, NULL};
#endif /* USE_MODULES */
  ...
  part *p)
{
  ...
  vinit(p->v, vi [ 0 ] * fs->eject_vl / (fs->eject_r / 2), vrnd() * fs->eject_vy + fs->eject_vy / 2, vi [ 2 ] * fs->eject_vl / (fs->eject_r / 2));

  c= partcol1;

  vinit4(p->c [ 0 ], c [ 0 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), c [ 1 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), c [ 2 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), 1.0);
  vinit4(p->c [ 1 ], c [ 0 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), c [ 1 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), c [ 2 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), 1.0);
  vinit4(p->c [ 2 ], c [ 0 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), c [ 1 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), c [ 2 ] * ((1.0 - RIDCOL) + vrnd() * RIDCOL), 1.0);
}

/* initialise new rain particle */
static void setnewrain(
  firestruct *fs,
  rain *r)
{
  r->age= 0.0f;
  ...
  vinit(r->oldpos, fs->min [ 0 ] + (fs->max [ 0 ] - fs->min [ 0 ]) * vrnd(), fs->max [ 1 ] + 0.2f * fs->max [ 1 ] * vrnd(), fs->min [ 2 ] + (fs->max [ 2 ] - fs->min [ 2 ]) * vrnd());
  vequ(r->pos, r->oldpos);
  ...
  if (do_texture)
    {
      ...
      if ((fs->gtexture= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), ground_png, sizeof(ground_png))) == None)
        {
      ...
      clear_gl_error();
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fs->gtexture->width, fs->gtexture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, fs->gtexture->data);
      ...
      if (fs->num_trees)
        {
          ...
          clear_gl_error();
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fs->ttexture->width, fs->ttexture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, fs->ttexture->data);
          check_gl_error("texture");
...
#define SINOID(SCALE, SIZE) \
  ...
  gluLookAt(fs->obs [ 0 ], fs->obs [ 1 ], fs->obs [ 2 ], fs->obs [ 0 ] + fs->dir [ 0 ], fs->obs [ 1 ] + fs->dir [ 1 ], fs->obs [ 2 ] + fs->dir [ 2 ], 0.0, 1.0, 0.0);
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, fs->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &fs->button_down_p))
    return True;
