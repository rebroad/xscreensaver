...
ModStruct cubicgrid_description=
  {"cubicgrid", "init_cubicgrid", "draw_cubicgrid", NULL, "draw_cubicgrid", "change_cubicgrid", NULL, &cubicgrid_opts, 25000, 1, 1, 1, 1.0, 4, "", "Shows a rotating 3D lattice from inside", 0, NULL};
#endif
...
ENTRYPOINT void reshape_cubicgrid(
  ModeInfo *mi,
  int width,
  int height)
{
  ...
  ModeInfo *mi)
{
...

  {
    ...
    cp->rot= make_rotator(spin_speed, spin_speed, spin_speed, spin_accel, 0, True);
    cp->trackball= gltrackball_init(True);
  }
}

ENTRYPOINT void draw_cubicgrid(
  ModeInfo *mi)
{
...
#ifndef STANDALONE
ENTRYPOINT void change_cubicgrid(
  ModeInfo *mi)
{
  cubicgrid_conf *cp= &cubicgrid [ MI_SCREEN(mi) ];
  if (! cp->glx_context) return;
...
ENTRYPOINT void free_cubicgrid(
  ModeInfo *mi)
{
  cubicgrid_conf *cp= &cubicgrid [ MI_SCREEN(mi) ];
  if (! cp->glx_context) return;
