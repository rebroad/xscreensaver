...
static argtype vars []= {
  ...
  {&shader_file0, "shaderChannel0", "ShaderChannel", DEF_SHADER_FILE0, t_String},
  {&shader_file1, "shaderChannel1", "ShaderChannel", DEF_SHADER_FILE1, t_String},
  {&shader_file2, "shaderChannel2", "ShaderChannel", DEF_SHADER_FILE2, t_String},
  {&shader_file3, "shaderChannel3", "ShaderChannel", DEF_SHADER_FILE3, t_String},
  {&shader_file4, "shaderChannel4", "ShaderChannel", DEF_SHADER_FILE4, t_String},
  {&shader_file_common, "shaderCommon", "ShaderChannel", DEF_SHADER_FILE_COMMON, t_String},
};
    ...
    ModeInfo *mi)
{
  ...
  else if (bp->gl_major == 3 && bp->gl_minor >= 2)
    {
  ...
  for (i= 0; i < countof(bp->channels); i++)
    {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, internal_format, MI_WIDTH(mi) * scale, MI_HEIGHT(mi) * scale, 0, format, type, 0);
    ...
    ModeInfo *mi)
{
  ...
  if (! glsl_GetGlAndGlslVersions(&bp->gl_major, &bp->gl_minor, &bp->glsl_major, &bp->glsl_minor, &bp->gl_gles3))
    {
      fprintf(stderr, "%s: GLSL is required\n", progname);
      exit(1);
    }
  ...
  for (i= 0; i < countof(bp->channels); i++)
    {
      ...
      if (! glsl_CompileAndLinkShaders(
            countof(vertex_shader_source),
            vertex_shader_source,
            countof(fragment_shader_source),
            ...
            if (len + res >= size)
              {
                size+= sizeof(buf);
                bp->shader_program [ i ]=
                  (bp->shader_program [ i ] ? realloc(bp->shader_program [ i ], size) : malloc(size));
    ...
    ModeInfo *mi)
{
     ...
     https://www.shadertoy.com/view/MdXyzX */
  {
    ...
    if (automouse_p) /* Hold button forever and drag sinusoidally. */
    ...
    else
      {
        ...
        int rx, ry;
        unsigned int m;
        XQueryPointer(MI_DISPLAY(mi), MI_WINDOW(mi), &r, &c, &rx, &ry, &x, &y, &m);
      }
  ...
  for (i= 0; i < countof(bp->channels); i++)
    {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, bp->vertex_pos_buffer);
      glVertexAttribPointer(bp->channels [ i ].poly_pos, 2, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  glBlitFramebuffer(0, 0, MI_WIDTH(mi) * scale, MI_HEIGHT(mi) * scale, 0, 0, MI_WIDTH(mi), MI_HEIGHT(mi), GL_COLOR_BUFFER_BIT, GL_LINEAR);
