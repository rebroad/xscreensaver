  ...
  providencestruct *mp)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, 3, checkImageWidth, checkImageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, &mp->checkImage [ 0 ][ 0 ]);
}


/* build eye lookup table */
static void build_eye(
  providencestruct *mp)
{
  int i, j;
  double x;
  ...
  providencestruct *mp)
{
  ...
  if (mp->wire)
  ...
  else
    {
      ...
      glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, mp->mono ? MaterialGray5 : MaterialGloryF);
    }
  ...
  providencestruct *mp)
{
  ...
  glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, mp->mono ? MaterialGloryM : MaterialGlory);
  ...
  glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, mp->mono ? MaterialGloryMB : MaterialGloryB);
  glBegin(GL_LINES);
...
/* draw the scene */
static void draw_providence_strip(
  ModeInfo *mi)
{
...
ENTRYPOINT void reshape_providence(
  ModeInfo *mi,
  int width,
  int height)
{
...
/* event handling */
ENTRYPOINT Bool providence_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  providencestruct *mp= &providence [ MI_SCREEN(mi) ];
  ...
  if (gltrackball_event_handler(event, mp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &mp->button_down_p))
    return True;

  return False;
}

ENTRYPOINT void init_providence(
  ModeInfo *mi)
{
  providencestruct *mp;
...
ENTRYPOINT void draw_providence(
  ModeInfo *mi)
{
  providencestruct *mp;
...
#ifndef STANDALONE
ENTRYPOINT void change_providence(
  ModeInfo *mi)
{
  providencestruct *mp= &providence [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_providence(
  ModeInfo *mi)
{
  providencestruct *mp= &providence [ MI_SCREEN(mi) ];
