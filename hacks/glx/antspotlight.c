...
ModStruct antspotlight_description= {
  ...
  1.0,
  "",
  "draws an ant scoping the screen",
  0,
  NULL};
#endif
...
static void draw_antspotlight_strip(
  ModeInfo *mi)
{
...
ENTRYPOINT void reshape_antspotlight(
  ModeInfo *mi,
  int width,
  int height)
{
...
/* event handling */
ENTRYPOINT Bool antspotlight_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  antspotlightstruct *mp= &antspotlight [ MI_SCREEN(mi) ];
  ...
  ModeInfo *modeinfo)
{
  ...
  mp->mipmap_p= True;
  load_texture_async(modeinfo->xgwa.screen, modeinfo->window, *mp->glx_context, 0, 0, mp->mipmap_p, mp->screentexture, image_loaded_cb, mp);
}


ENTRYPOINT void init_antspotlight(
  ModeInfo *mi)
{
...
ENTRYPOINT void draw_antspotlight(
  ModeInfo *mi)
{
  antspotlightstruct *mp;
...
#ifndef STANDALONE
ENTRYPOINT void change_antspotlight(
  ModeInfo *mi)
{
  antspotlightstruct *mp= &antspotlight [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_antspotlight(
  ModeInfo *mi)
{
  antspotlightstruct *mp= &antspotlight [ MI_SCREEN(mi) ];
