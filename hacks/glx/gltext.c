    ...
    ModeInfo *mi)
{
  ...
  const char *tt=
    (text_fmt && *text_fmt && ! ! strcmp(text_fmt, "(default)") ? text_fmt : 0);
  const char *pr=
    (program_str && *program_str && ! ! strcmp(program_str, "(default)") ? program_str : 0);
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, tp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &tp->button_down_p))
    return True;
    ...
    ModeInfo *mi)
{
...

  {
    ...
    tp->rot= make_rotator(tp->spinx ? spin_speed : 0,
      ...
      False);
    tp->rot2= (face_front_p ? make_rotator(0, 0, 0, 0, tilt_speed, True) : 0);
    tp->trackball= gltrackball_init(False);
  }

  tp->ncolors= 255;
  tp->colors= (XColor *) calloc(tp->ncolors, sizeof(XColor));
  make_smooth_colormap(0, 0, 0, tp->colors, &tp->ncolors, False, 0, False);
          ...
          for (j= stroke->num_coords, coord= stroke->coord;
            ...
            j--, coord++)
            {
              ...
              if (j != stroke->num_coords)
                *polysP+= tube(lx, ly, 0, coord->x, coord->y, 0, tube_width, tube_width * 0.15, TUBE_FACES, smooth, False, wire);
              lx= coord->x;
              ly= coord->y;
