    ...
    const void *bb)
{
  ...
  return (a->sortkey == b->sortkey ? 0 : a->sortkey < b->sortkey ? -1 :
                                                                   1);
}
    ...
    ModeInfo *mi)
{
   ...
   */
  {
    ...
    qsort(bp->particles, bp->nparticles, sizeof(*bp->particles), particle_sort_cmp);
  }
          ...
          if (bp->state < QUENCH)
            {
              ...
              memcpy(p->color, fire_colors [ countof(fire_colors) - 1 ], sizeof(p->color));
            }
        }
      else
        {
  ...
  for (i= 0; i < 2; i++)
    {
      ...
      double a2= (bp->state == CLOSE ? 1 - ease(EASE_OUT_BOUNCE, (1 - bp->lid_angle [ i ])) : ease(EASE_OUT_BOUNCE, (bp->lid_angle [ i ])));
