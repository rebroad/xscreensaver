    ...
    ModeInfo *mi)
{
...

  {
    ...
    for (i= 0; i < bp->count; i++)
      {
        plane *p= &bp->planes [ i ];
        p->rot= make_rotator(0, 0, spinp ? spin_speed : 0, spin_accel, wanderp ? wander_speed : 0, True);
        p->rot2= make_rotator(0, 0, 0, 0, scale_speed, True);
        if (! wire)
          {
            clear_gl_error();
            ...
            glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, tex_width, 0, GL_RGBA, GL_UNSIGNED_BYTE, tex_data);
