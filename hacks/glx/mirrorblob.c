...
ModStruct mirrorblob_description=
  {"mirrorblob", "init_mirrorblob", "draw_mirrorblob", NULL, "draw_mirrorblob", "init_mirrorblob", "free_mirrorblob", &mirrorblob_opts, 1000, 1, 2, 1, 4, 1.0, "", "OpenGL mirrorblob", 0, NULL};
#endif
    ...
    void *closure)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (mp->mipmap_p ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
    ...
    int texture_index)
{
...

  {
    ...
    mp->mipmap_p= True;
    load_texture_async(mi->xgwa.screen, mi->window, *mp->glx_context, w, h, mp->mipmap_p, mp->textures [ texture_index ], image_loaded_cb, mp);
  }
}
  ...
  for (i= 0; i < bumps; i++)
    {
      ...
      gp->bump_data [ i ].pos.x= 1.5 * sin(PI * gp->bump_data [ i ].ay) * cos(PI * gp->bump_data [ i ].ax);
      gp->bump_data [ i ].pos.y= 1.5 * cos(PI * gp->bump_data [ i ].ay);
      gp->bump_data [ i ].pos.z= 1.5 * sin(PI * gp->bump_data [ i ].ay) * sin(PI * gp->bump_data [ i ].ax);
  ...
  for (i= 0; i < bumps; i++)
    {
      ...
      gp->bump_data [ i ].vpower+= gp->bump_data [ i ].mpower * (gp->bump_data [ i ].cpower - gp->bump_data [ i ].power);
      gp->bump_data [ i ].vsize+= gp->bump_data [ i ].msize * (gp->bump_data [ i ].csize - gp->bump_data [ i ].size);
      ...
      gp->bump_data [ i ].pos.x= 1.0 * sin(PI * gp->bump_data [ i ].ay) * cos(PI * gp->bump_data [ i ].ax);
      gp->bump_data [ i ].pos.y= 1.0 * cos(PI * gp->bump_data [ i ].ay);
      gp->bump_data [ i ].pos.z= 1.0 * sin(PI * gp->bump_data [ i ].ay) * sin(PI * gp->bump_data [ i ].ax);
    }
          ...
          if (do_texture)
            {
              if (offset_texture)
                ...
                {
                  gp->tex_coords [ index ].x= 0.5 * (1.0 + asin(gp->normals [ index ].x) / (0.5 * PI));
                  gp->tex_coords [ index ].y= -0.5 * (1.0 + asin(gp->normals [ index ].y) / (0.5 * PI));
                }
    ...
    XEvent *event)
{
  ...
  else if (gltrackball_event_handler(event, gp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &gp->button_down))
    {
      return True;
    }
