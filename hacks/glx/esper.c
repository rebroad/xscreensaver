...
#ifdef USE_ASYNC_LOADER
      img->loader= alloc_texture_loader(mi->xgwa.screen, mi->window, *ss->glx_context, 0, 0, False, img->texid);
#else
      load_texture_async(mi->xgwa.screen, mi->window, *ss->glx_context, max, max, False, img->texid, image_loaded_cb, img);
#endif
    }
    ...
    void *closure)
{
  ...
  */
  ...
  if (debug_p)
    fprintf(stderr, "%s: loaded %lu \"%s\" %dx%d\n", progname, img->id, (img->title ? img->title : "(null)"), ow, oh);
DONE:
  ...
  if (target)
    {
      ...
      mi->recursion_depth= (img ? MIN(img->current.w, img->current.h) : 0);
      ...
      if (img->loader)
        {
          if (texture_loader_failed(img->loader))
            abort();
          step_texture_loader(img->loader, end_by_time - double_time(), image_loaded_cb, img);
        }
    }
#endif
      ...
      case IMAGE_ZOOM :
        {
          ...
          double depth= (sp ? MIN(sp->current.w, sp->current.h) : 0);
          if (depth > 20)
  ...
  switch (ss->anim_state)
    {
      ...
      case IMAGE_UNLOAD :
        ...
        if (sp)
          sp->fade_duration= ((prev_state == IMAGE_FORCE_UNLOAD ? 0.2 : 3.0) / speed);
        fadeout_sprites(mi, IMAGE);
...
#define BONK() do {				      \
      ...
      ss->anim_state == BOX_MOVE))
    {
      ...
      BONK();
      max= (2 * (0.5 - MAX(fabs(sp->current.x - 0.5), fabs(sp->current.y - 0.5))) * 0.95);
