...
ModStruct queens_description=
  {"queens", "init_queens", "draw_queens", NULL, "draw_queens", "init_queens", NULL, &queens_opts, 1000, 1, 2, 1, 4, 1.0, "", "Queens", 0, NULL};
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, qs->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &qs->button_down_p))
    {
      return True;
    }
...
static void go(
  Queenscreen *qs)
{
  while (! findSolution(qs, 0, random() % qs->BOARDSIZE));
}
...
static GLfloat findAlpha(
  Queenscreen *qs)
{
  return qs->steps < 128 ? qs->steps / 128.0 : qs->steps < 1024 - 128 ? 1.0 :
                                                                        (1024 - qs->steps) / 128.0;
}

/* draw pieces */
static int drawPieces(
  Queenscreen *qs)
{
  int i, j;
  int polys= 0;
...
ENTRYPOINT void reshape_queens(
  ModeInfo *mi,
  int width,
  int height)
{
