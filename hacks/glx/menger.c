    ...
    int forig)
{
  ...
  if (orig)
    {
      if (wireframe)
        n+= cube(x0, x1, y0, y1, z0, z1, faces & (X0 | X1 | Y0 | Y1), wireframe);
    }
              ...
              for (z= 0; z < 3; z++)
                {
                  if ((x != 1 && y != 1) || (y != 1 && z != 1) || (x != 1 && z != 1))
                    ...
                    {
                      n+= cube(x0 + x * xi, x0 + (x + 1) * xi, y0 + y * yi, y0 + (y + 1) * yi, z0 + z * zi, z0 + (z + 1) * zi, forig & (X0 | X1 | Y0 | Y1), wireframe);
                    }
                }
            }
        }
...
static int
  menger_recurs(
    ...
    int orig)
{
  return menger_recurs_1(level, x0, x1, y0, y1, z0, z1, faces, wireframe, orig, faces);
}
    ...
    int level)
{
  ...
  sp->squares_fp= menger_recurs(level, -1.5, 1.5, -1.5, 1.5, -1.5, 1.5, X0 | X1, wireframe, 1);
  glEndList();

  glDeleteLists(sp->sponge_list1, 1);
  glNewList(sp->sponge_list1, GL_COMPILE);
  sp->squares_fp+= menger_recurs(level, -1.5, 1.5, -1.5, 1.5, -1.5, 1.5, Y0 | Y1, wireframe, 1);
  glEndList();

  glDeleteLists(sp->sponge_list2, 1);
  glNewList(sp->sponge_list2, GL_COMPILE);
  sp->squares_fp+= menger_recurs(level, -1.5, 1.5, -1.5, 1.5, -1.5, 1.5, Z0 | Z1, wireframe, 1);
  glEndList();
}
    ...
    ModeInfo *mi)
{
  ...
  make_smooth_colormap(0, 0, 0, sp->colors, &sp->ncolors, False, 0, False);
  sp->ccolor0= 0;
  ...
  if (sp->draw_tick++ >= speed)
    {
      ...
      build_sponge(sp,
        MI_IS_WIREFRAME(mi),
        (sp->current_depth < 0 ? -sp->current_depth : sp->current_depth));

      mi->polygon_count= sp->squares_fp; /* for FPS display */
      mi->recursion_depth= (sp->current_depth < 0 ? -sp->current_depth : sp->current_depth);
    }
