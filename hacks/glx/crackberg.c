...
static void trile_calc_heights(
  cberg_state *cberg,
  Trile *new)
{
  unsigned int i, j, k, h;

  for (i= 0; i < cberg->epoints - 1; ++i)
    { /* copy in sides */
      TCOORD(i, 0)= new->v [ i ];
...
static void trile_calc_flat_norms(
  cberg_state *cberg,
  Trile *new)
{
  unsigned int x, y;
...
static void trile_calc_smooth_norms(
  cberg_state *cberg,
  Trile *new)
{
  ...
  cur= TCOORD(0, 0);
  SET_sNCOORD(0, 0, down, cur, cur, TCOORD(0, 1), TCOORD(1, 0), cur, cur);
  cur= TCOORD(cberg->epoints - 1, 0);
  SET_sNCOORD(cberg->epoints - 1, 0, down, TCOORD(cberg->epoints - 2, 0), TCOORD(cberg->epoints - 2, 1), cur, cur, cur, cur);
  cur= TCOORD(0, cberg->epoints - 1);
  SET_sNCOORD(0, cberg->epoints - 1, down, cur, cur, cur, cur, TCOORD(1, cberg->epoints - 2), TCOORD(0, cberg->epoints - 2));
  ...
  for (i= 1; i < cberg->epoints - 1; ++i)
    {
      ...
      SET_sNCOORD(cberg->epoints - i - 1, i, down, TCOORD(cberg->epoints - i - 2, i), next, cur, cur, prev, TCOORD(cberg->epoints - i - 1, i - 1));
      prev= cur;
      cur= next;
    }
  ...
  for (i= 1; i < cberg->epoints - 2; ++i)
    {
      ...
      for (j= 1; j < cberg->epoints - i - 1; ++j)
        {
          next= TCOORD(j + 1, i);
          SET_sNCOORD(j, i, down, prev, TCOORD(j - 1, i + 1), TCOORD(j, i + 1), next, TCOORD(j + 1, i - 1), TCOORD(j, i - 1));
          prev= cur;
          cur= next;
        }
    }
...
static inline void trile_light(
  cberg_state *cberg,
  unsigned int x,
  unsigned int y,
  unsigned int which)
  ...
  if (flat)
    {
      if (x)
        ...
        { /* I get mesa errors and bizarre glitches without this!! */
...
static inline void trile_draw_vertex(
  cberg_state *cberg,
  unsigned int ix,
  unsigned int iy,
  unsigned int which,
  ...
  for (y= 0; y < cberg->epoints - 1; ++y)
    {
      ...
      z1= TCOORD(0, y + 1);
      z2= TCOORD(1, y);
      trile_draw_vertex(cberg, 0, y, 0, cornerx, cornery, z0, z1, z2);
      trile_draw_vertex(cberg, 0, y, 1, cornerx + 0.5 * dx, cornery + dy, z1, z0, z2);

      for (x= 1; x < cberg->epoints - 1 - y; ++x)
        {
          trile_draw_vertex(cberg, x, y, 0, cornerx + x * dx, cornery, z2, z1, z0);

          z0= TCOORD(x, y + 1);

          trile_draw_vertex(cberg, x, y, 1, cornerx + (x + 0.5) * dx, cornery + dy, z0, z2, z1);
  ...
  cberg_state *cberg)
{
  ...
  if (cberg->free_head)
    ...
    {
      ++cberg->count;
      if (! (new= calloc(1, sizeof(Trile))) || ! (new->l= (double *) calloc(sizeof(double), cberg->epoints * 3)))
        {
          perror(progname);
          exit(1);
        }
...
static void trile_draw_vanilla(
  Trile *tr)
{
  glCallList(tr->call_list);
}

static void trile_draw(
  Trile *tr,
  void *ignore)
{
...
static void identity_init(
  Trile *tr)
{
  tr->state= TRILE_STABLE;
}

static void identity_free(
  Trile *tr)
{
}

static void identity_draw(
  Trile *tr)
{
  trile_draw_vanilla(tr);
}

static void identity_init_iter(
  Trile *tr,
  cberg_state *cberg)
{
}

static void identity_dying_iter(
  Trile *tr,
  cberg_state *cberg)
{
  tr->state= TRILE_DELETE;
...
static void triles_set_visible(
  cberg_state *cberg,
  Trile **root,
  int x,
  int y)
...
static unsigned int triles_foreach(
  Trile *root,
  void (*f)(Trile *, void *),
  void *data)
{
  ...
  f(root, data);
  return 1 + triles_foreach(root->left, f, data) + triles_foreach(root->right, f, data);
}

static void triles_update_state(
  Trile **root,
  cberg_state *cberg)
{
  ...
  while (process_current)
    {
      ...
      else if ((*root)->state == TRILE_STABLE)
        {
  ...
  double *y4)
{
  ...
  double fhalfwidth= zFar * tan(fovx_2 * M_PI_180) + M_RAD7_4 / cos(fovx_2 * M_PI_180);
  ...
  *y2= y_farcenter - fhalfwidth * sin((cberg->yaw - 90) * M_PI_180);
  ...
  nhalfwidth= zNear * tan(fovx_2 * M_PI_180) + M_RAD7_4 / cos(fovx_2 * M_PI_180);
...
#ifdef DEBUG
  printf(
    "\tnearcenter: (%.3f,%.3f) || nhalfwidth: %.3f\n" "\tp3: (%.3f,%.3f) || p4: (%.3f,%.3f)\n",
    x_nearcenter,
    y_nearcenter,
    nhalfwidth,
    *x3,
...
static void plain_land(
  cberg_state *cberg,
  double z)
{
  glColor3f(pow((z / 0.35), 4), z / 0.35, pow((z / 0.35), 4));
}
static void plain_water(
  cberg_state *cberg,
  double z)
{
...
static void ice_land(
  cberg_state *cberg,
  double z)
{
  glColor3f((0.35 - z) / 0.35, (0.35 - z) / 0.35, 1.0);
}
static void ice_water(
  cberg_state *cberg,
  double z)
{
  glColor3f(0.0, (z + 0.35) * 1.6, 0.8);
...
static void magma_land(
  cberg_state *cberg,
  double z)
{
  glColor3f(z / 0.35, z / 0.2, 0);
}
static void magma_lava(
  cberg_state *cberg,
  double z)
{
...
static const Color *select_color(
  cberg_state *cberg)
{
...
/* simple one for now.. */
static inline double drunken_rando(
  double cur_val,
  double max,
  double width)
{
...
ENTRYPOINT void init_crackberg(
  ModeInfo *mi)
{
  cberg_state *cberg;
...
ENTRYPOINT void reshape_crackberg(
  ModeInfo *mi,
  int w,
  int h)
{
...
ENTRYPOINT Bool crackberg_handle_event(
  ModeInfo *mi,
  XEvent *ev)
{
  cberg_state *cberg= &cbergs [ MI_SCREEN(mi) ];
  ...
  if (ev->xany.type == KeyPress)
    ...
    cberg->button_down_p)
    {
      ...
      if (dx > 0)
        cberg->motion_state|= MOTION_LEFT;
      else if (dx < 0)
        cberg->motion_state|= MOTION_RIGHT;
      else if (dy > 0)
        cberg->motion_state|= MOTION_FORW;
      else if (dy < 0)
...
ENTRYPOINT void draw_crackberg(
  ModeInfo *mi)
{
  cberg_state *cberg= &cbergs [ MI_SCREEN(mi) ];
...
/* uh */
ENTRYPOINT void free_crackberg(
  ModeInfo *mi)
{
  cberg_state *cberg= &cbergs [ MI_SCREEN(mi) ];
