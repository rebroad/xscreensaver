...
{
  GLuint texture_id;
  XImage *image= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), buffer, length);
...
static void initialize_placeholders(
...
static void animate_initial_board(
  klondike_configuration *bp)
{
  int n= 0;
  for (int i= 0; i < 7; i++)
...
// ease in out quartic
static double ease_in_out_quart(
  double x)
{
  if (x < 0.5)
    return 8 * x * x * x * x;
...
static void animate_board_to_deck(
  klondike_configuration *bp)
{
  int n= 0;
  for (int i= 0; i < 7; i++)
