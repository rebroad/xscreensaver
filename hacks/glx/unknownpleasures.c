  ...
  if (bp->orthop)
    {
      ...
      glLoadIdentity();
      gluLookAt(0, 0, magic, 0, 0, 0, 0, 1, 0);
      if (width < height)
        glScalef(1 / h, 1 / h, 1);
...
#ifdef HAVE_COCOA
    else if (MI_WIDTH(mi) > 1280)
      lw= 3; /* WTF */
#endif
    else if (MI_WIDTH(mi) > 1920)
      lw= 3;
    else if (mi->xgwa.width > 640 && mi->xgwa.height > 640)
      lw= 2;
    ...
    ModeInfo *mi)
{
  ...
  bp->mask= XCreateImage(MI_DISPLAY(mi), MI_VISUAL(mi), 32, ZPixmap, 0, 0, bp->resolution, bp->count * image0->height / image0->width * (1.9 / bp->aspect) * 0.75, 32, 0);
  if (! bp->mask) abort();
  bp->mask->data= (char *)
              ...
              for (x2= x * xs; x2 < (x + 1) * xs; x2++)
                {
                  ...
                  unsigned long gray= (a == 0 ? 0 : ((((agbr >> 16) & 0xFF) + ((agbr >> 8) & 0xFF) + ((agbr >> 0) & 0xFF)) / 3));
#if 0
    ...
    XEvent *event)
{
  ...
  else if (gltrackball_event_handler(event, bp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &bp->button_down_p))
    {
      return True;
    }
    ...
    ModeInfo *mi)
{
  ...
  for (j= 0; j < nspikes; j++)
    {
      double off= (bp->mask ? frand(1.0) - 0.5 /* all the way to the edge */
                              :
  ...
  for (i= 0, r= -0.5, p= points; i < bp->resolution; i++, r+= step, p++)
    *p= ((*p / max) * (0.5 + 0.5 * (bp->mask ? 1 : cos1(r * r * M_PI * 14)) * (1 - frand(0.2))));
    ...
    ModeInfo *mi)
{
  ...
  memmove(bp->lines, bp->lines + 2 * bp->frames, sizeof(*bp->lines) * (bp->count - 1) * bp->frames * 2);
  memcpy(bp->lines + (bp->count - 1) * bp->frames * 2, cur, bp->frames * 2 * sizeof(*cur));

  memmove(bp->heights, bp->heights + 1, sizeof(*bp->heights) * (bp->count - 1));
      ...
      for (linesp= 0; linesp <= 1; linesp++)
        {
          ...
          glBegin(linesp ? GL_LINE_STRIP : wire ? GL_LINES :
                                                  GL_QUAD_STRIP);
  ...
  if (! bp->button_down_p)
    {
      ...
      for (i= 0; i < bp->count; i++)
        {
          GLfloat i2= i - ratio;
          GLfloat h= ((i2 < bp->count / 2 ? i2 : (bp->count - 1 - i2)) / dist);
          bp->heights [ i ]= (h > 1 ? 1 : h < 0 ? 0 :
                                                  h);
        }
