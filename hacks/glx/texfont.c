...
static void
  bitmap_to_texture(
    const texture_font_data *tfdata,
    Pixmap p,
    Visual *visual,
    int depth,
    int *wP,
    int *hP)
{
  ...
  if (use_shm)
    {
      image= create_xshm_image(dpy, visual, depth, ZPixmap, 0, &shm_info, ow, oh);
      if (image)
  ...
  if (! image)
    {
       ...
       */
      image= XCreateImage(dpy, visual, depth, ZPixmap, 0, NULL, ow, oh, BitmapPad(dpy), 0);
...
#ifdef HAVE_JWXYZ
            fprintf(stderr, "%c", r >= 0xFF000000 ? '#' : r >= 0x88000000 ? '%' :
                r                                                         ? '.' :
                                                                            ' ');
#else
            fprintf(stderr, "%c", r >= 0xFF0000 ? '#' : r >= 0x880000 ? '%' :
                r                                                     ? '.' :
                                                                        ' ');
#endif
#endif
  ...
  {
    char msg [ 100 ];
    sprintf(msg, "texture font %s (%d x %d)", tfdata->mipmap_p ? "gluBuild2DMipmaps" : "glTexImage2D", w2, h2);
    check_gl_error(msg);
  }
          ...
          if (s != os)
            {
              ...
              XftTextExtentsUtf8(data->dpy, data->xftfont, (FcChar8 *) os, (int) (s - os), &e);
...
#undef MIN
              ...
              if (! cw)
                {
                  ...
                  XGlyphInfo e;
                  XftTextExtentsUtf8(data->dpy, data->xftfont, (FcChar8 *) "m", 1, &e);
                  cw= e.xOff;
                  if (cw <= 0) cw= 1;
                  tabs= cw * 7;
                }
          ...
          if (xftdraw && s != os)
            XftDrawStringUtf8(xftdraw, xftcolor, data->xftfont, draw_x + ox, draw_y + oy + (osub_p ? subscript_offset : 0), (FcChar8 *) os, (int) (s - os));
          if (! *s) break;
          os= s + 1;
          ox= x;
          oy= y;
  ...
  if (data->cache)
    {
      for (prev2= 0, prev= 0, curr= data->cache, next= curr->next;
        curr;
        prev2= prev, prev= curr, curr= next, next= (curr ? curr->next : 0), count++)
        {
    ...
    int *height_ret)
{
  ...
  iterate_texture_string(data, string, -overall.lbearing, overall.ascent, xftdraw, &xftcolor, 0);
  ...
  texture_font_data *data)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, data->mipmap_p ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
    ...
    if (data->use_shaders)
      {
        ...
        glVertexAttribPointer(data->vertex_coord_index, 2, GL_FLOAT, GL_FALSE, 0, 0);

        glEnableVertexAttribArray(data->vertex_tex_index);
        glBindBuffer(GL_ARRAY_BUFFER, data->vertex_tex_buffer);
        glBufferData(GL_ARRAY_BUFFER, sizeof(t), t, GL_STATIC_DRAW);
        glVertexAttribPointer(data->vertex_tex_index, 2, GL_FLOAT, GL_FALSE, 0, 0);
...
#endif /* HAVE_GLSL */
      {
        ...
        glVertex3f(qx0, qy0, 0);
        glTexCoord2f(tx1, ty0);
        glVertex3f(qx1, qy0, 0);
        glTexCoord2f(tx1, ty1);
        glVertex3f(qx1, qy1, 0);
        glTexCoord2f(tx0, ty1);
        glVertex3f(qx0, qy1, 0);
        glEnd();
...
/* Initialize the texture font shaders that are used if the hack prefers
   ...
   hack). */
static void
  initialize_textfont_shaders_glsl(
    texture_font_data *data)
{
  ...
  if (! glsl_CompileAndLinkShaders(2, vertex_shader_source, 2, fragment_shader_source, &data->shader_program))
    {
      data->shaders_initialized= True;
      return;
    }
  ...
  const char *string)
{
...

  {
...
#ifdef HAVE_IPHONE
    {
      ...
      GLfloat scale= ((window_width > window_height ? window_width : window_height) / 768.0);
      if (scale < 1) scale= 1;
    ...
    if (rot > 135 || rot < -135) /* 180 */
      {
        texfont_transrot(data, proj_mat, window_width, window_height, 0, 180, 0, 0, 1);
      }
    else if (rot > 45) /* 90 */
      {
        texfont_transrot(data, proj_mat, window_width, 0, 0, 90, 0, 0, 1);
        swap= window_width;
      ...
      {
        texfont_transrot(data, proj_mat, 0, window_height, 0, -90, 0, 0, 1);
        swap= window_width;
        window_width= window_height;
        window_height= swap;
      }
    ...
    texfont_transrot(data, proj_mat, x, y, 0, 0, 0, 0, 1);
          ...
          if (data->use_shaders)
            {
              ...
              glUniformMatrix4fv(data->proj_mat_index, 1, GL_FALSE, proj_mat_trans);
...
#ifdef HAVE_JWXYZ
char *
  texfont_unicode_character_name(
    texture_font_data *data,
    unsigned long uc)
{
