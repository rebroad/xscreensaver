    ...
    Pixmap pixmap)
{
   ...
   */
  server_ximage= create_xshm_image(dpy, visual, depth, ZPixmap, &shm_info, width, height);
...
typedef struct
{
    ...
    /* Used in async mode
     */
    void (*callback)(const char *filename, XRectangle *geometry, int iw, int ih, int tw, int th, void *closure);
    void *closure;
  ...
  if (mipmap_p)
    {
      ...
      gluBuild2DMipmaps(GL_TEXTURE_2D, 3, ximage->width, ximage->height, format, type, ximage->data);
      err= glGetError();
    }
  else
    {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tex_width, tex_height, 0, format, type, 0);
      err= glGetError();

      /* Now load our non-power-of-2 image data into the existing texture. */
      if (! err)
        {
          glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, ximage->width, ximage->height, GL_RGBA, GL_UNSIGNED_BYTE, ximage->data);
          err= glGetError();
        }
    }
  ...
  if (err)
    {
      ...
      while (glGetError() != GL_NO_ERROR); /* clear any lingering errors */
  ...
  void *closure)
{
  ...
  data->pixmap= XCreatePixmap(dpy, window, data->pix_width, data->pix_height, data->pix_depth);
  load_image_async(screen, window, data->pixmap, load_texture_async_cb, data);
}
    ...
    GLuint texid)
{
  ...
  load_image_async(screen, window, data->pixmap, incremental_load_texture_async_cb, loader);
    ...
    void *closure)
{
...
#else  /* ! REFORMAT_IMAGE_DATA */
  {
    ...
    ximage= XCreateImage(dpy, visual, dd.pix_depth, ZPixmap, 0, 0, dd.pix_width, dd.pix_height, 32, 0);
    ...
    if (! ximage->data ||
      ! XGetSubImage(dpy, dd.pixmap, 0, 0, ximage->width, ximage->height, ~0L, ximage->format, ximage, 0, 0))
      {
 ...
 */
static void
  incremental_load_texture_async_cb(
    Screen *screen,
    Window window,
    Drawable drawable,
    const char *name,
    ...
    void *closure)
{
  ...
  /* Like pixmap_to_gl_ximage(): */
  {
    ...
    loader->ximage= create_xshm_image(dpy, visual, depth, ZPixmap, &loader->shm_info, width, height);
      ...
      if (loader->steps == 1)
        {
          ...
          loader->stripe_height= ((unsigned int) (perf * allowed_seconds /
                                   loader->ximage->width)) /
              8 +
            1;
          if (loader->stripe_height < 1)
...
static unsigned int
  texture_loader_next_stripe_height(
    texture_loader_t *loader)
{
    ...
    double allowed_seconds)
{
  ...
  for (
    ...
    loader->y+= loader->stripe_height, ++iter_count)
    {
      ...
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, loader->y, cvt_patch->width, cvt_patch->height, GL_RGBA, GL_UNSIGNED_BYTE, cvt_patch->data);
    ...
    void *closure)
{
  ...
  callback(name, &geometry, loader->img_width, loader->img_height, loader->tex_width, loader->tex_height, closure);

  free(name);
}

Bool texture_loader_failed(
  texture_loader_t *loader)
{
