...
ModStruct planet_description=
  {"planet", "init_planet", "draw_planet", NULL, "draw_planet", "init_planet", "free_planet", &planet_opts, 1000, 1, 2, 1, 4, 1.0, "", "Animates texture mapped sphere (planet)", 0, NULL};
#endif
...
static void
  setup_xpm_texture(
    ...
    unsigned long size)
{
  XImage *image= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), data, size);
  char buf [ 1024 ];
  clear_gl_error();
    ...
    Bool mercp)
{
  ...
  for (j= 0, y= -0.5, ty= 0; j <= stacks;
    lasty= y, lastty= ty, y+= ys, j++)
    {
      ...
      if (j == 1)
        y+= eqoff; /* don't adjust end caps for equator */
      else if (j == stacks)
        y-= eqoff;
          ...
          for (i= 0, x= 0, lastx= 0; i <= slices; lastx= x, x+= xs, i++)
            {
              ...
              glNormal3f(lx, 0, ly);
              glTexCoord2f(lastx, lastty);
              glVertex3f(lx, ly2, ly);
              glNormal3f(xx, 0, yy);
              glTexCoord2f(x, lastty);
              glVertex3f(xx, ly2, yy);
              glNormal3f(xx, 0, yy);
              glTexCoord2f(x, ty);
              glVertex3f(xx, y2, yy);
              glNormal3f(lx, 0, ly);
    ...
    ModeInfo *mi)
{
  ...
  if (! mode_arg || ! *mode_arg || ! strcasecmp(mode_arg, "random"))
    ...
    {
      fprintf(stderr,
        "%s: mode must be 'globe', 'merecator' or 'equirectangular'," " not '%s'\n",
        progname,
        mode_arg);
      exit(1);
    }
