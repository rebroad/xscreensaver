...
ModStruct antmaze_description=
  {"antmaze", "init_antmaze", "draw_antmaze", NULL, "draw_antmaze", "change_antmaze", NULL, &antmaze_opts, 1000, 1, 1, 1, 4, 1.0, "", "draws some ants", 0, NULL};
  ...
  float radius)
{
  ...
  gluDeleteQuadric(quadObj);
#else
  cone(0, 0, 0, 0, 0, radius * 2, radius, 0, 8, True, True, False);
#endif
  return True;
}
  ...
  antmazestruct *mp)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, 3, checkImageWidth, checkImageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, &mp->checkers [ 0 ][ 0 ]);
...
static Bool draw_antmaze_strip(
  ModeInfo *mi)
{
...
ENTRYPOINT void reshape_antmaze(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT Bool antmaze_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  antmazestruct *mp= &antmaze [ MI_SCREEN(mi) ];
...
#ifndef STANDALONE
ENTRYPOINT void change_antmaze(
  ModeInfo *mi)
{
  antmazestruct *mp= &antmaze [ MI_SCREEN(mi) ];
