...
ModStruct flipflop_description=
  {"flipflop", "init_flipflop", "draw_flipflop", NULL, "draw_flipflop", "init_flipflop", "free_flipflop", &flipflop_opts, 1000, 1, 2, 1, 4, 1.0, "", "Flipflop", 0, NULL};
    ...
    void *closure)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (c->mipmap ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
    ...
    ModeInfo *modeinfo)
{
  ...
  c->got_texture= False;
  c->mipmap= True;
  load_texture_async(modeinfo->xgwa.screen, modeinfo->window, *c->glx_context, 0, 0, c->mipmap, c->texid, image_loaded_cb, c);
}
    ...
    ModeInfo *mi)
{
  ...
  if (c->board_avg_size != 0)
    { /* general size specified by user */
