...
extern void quat_to_angles(float q [ 4 ], float *alpha, float *beta, float *delta);
...
extern void color(sphereeversionstruct *se, float angle, const float mat [ 3 ][ 3 ], float colf [ 4 ], float colb [ 4 ]);
extern void setup_xpm_texture(ModeInfo *mi, const unsigned char *data, unsigned long size);
