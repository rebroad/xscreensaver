...
ModStruct sballs_description=
  {"sballs", "init_sballs", "draw_sballs", NULL, "draw_sballs", "change_sballs", "free_sballs", &sballs_opts,
    /*
    ...
    1.0,
    "",
    "balls spinning like crazy in GL",
    0,
    NULL};
#endif /* USE_MODULES */
  ...
  if (do_texture)
    {
      ...
      sb->btexture= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), sball_bg_png, sizeof(sball_bg_png));
      if (! (sb->btexture))
        {
      ...
      clear_gl_error();
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sb->btexture->width, sb->btexture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, sb->btexture->data);
      ...
      */
      ...
      sb->ftexture= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), sball_png, sizeof(sball_png));
      if (! (sb->ftexture))
        {
      ...
      glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
      clear_gl_error();
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sb->ftexture->width, sb->ftexture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, sb->ftexture->data);
      check_gl_error("texture");
...
static void Reshape(
  ModeInfo *mi)
#else
ENTRYPOINT void reshape_sballs(
  ModeInfo *mi,
  int width,
  int height)
#endif
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, sb->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &sb->button_down_p))
    return True;
