...
static argtype vars []= {
  {&transition_seconds, "transitionDuration", "TransitionDuration", DEF_TRANSITION_DURATION, t_Int},
...
static void image_loaded_cb(const char *filename, XRectangle *geom, int image_width, int image_height, int texture_width, int texture_height, void *closure);
    ...
    ModeInfo *mi)
{
  ...
  if (wire)
    ...
    img->loader= alloc_texture_loader(mi->xgwa.screen, mi->window, *ss->glx_context, 0, 0, mipmap_p, img->texid);
      ...
      if (img->loader)
        {
          if (texture_loader_failed(img->loader))
            abort();
          step_texture_loader(img->loader, allowed_time, image_loaded_cb, img);
          img->steps++;
    ...
    void *closure)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, mipmap_p ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
  ...
  */
  ...
  if (verbose_p)
    fprintf(stderr, "%s: loaded   img %2d: \"%s\" (%d steps)\n", blurb(), img->id, (img->title ? img->title : "(null)"), img->steps + 1);
DONE:
    ...
    Bool want_new_p)
{
  ...
  sp->zoom= (zoom ? 1 + frand((100.0 / zoom) - 1) /* 75% => [1.0 - 1.33] */
                    :
                    1);
  sp->img= img;
    ...
    ModeInfo *mi)
{
  ...
  in= new_sprite(mi, (ss->state == FIRST_IN || ss->state == TRANSITION_OUT || ss->state == TRANSITION_PANZOOM));
  if (! in)
    {
  ...
  switch (ss->state)
    {
          ...
          for (i= 0; i < countof(sprites); i++)
            {
              ...
              sp->to.x= (sp->to.w > vp_w ? -(sp->to.w - vp_w) / 2 : (vp_w - sp->to.w) / 2);
              sp->to.y= (sp->to.h > vp_h ? -(sp->to.h - vp_h) / 2 : (vp_h - sp->to.h) / 2);
              sp->to.r.x= 0;
              sp->to.r.y= 0;
              sp->to.r.z= 0;
              sp->from= sp->to;
                  ...
                  case SPIN :
                    {
                      ...
                      GLfloat spin= (! out_p && out ? -out->to.r.z : (90 * quads * (random() & 1 ? 1 : -1)));
                  ...
                  case FLIP :
                    {
                      ...
                      GLfloat flipx= (! out_p && out ? -out->to.r.x : horiz_p ? 180 * sign :
                                                                                0);
                      GLfloat flipy= (! out_p && out ? -out->to.r.y : horiz_p ? 0 :
                                                                                180 * sign);
                      if (out_p)
                        {
                          sp->to.r.x= flipx;
      ...
      case TRANSITION_PANZOOM :
...

        {
          ...
          in_zoom [ 0 ]= (zoom ? 1 + frand((100.0 / zoom) - 1) /* 75% => [1.0 - 1.33] */
                                 :
                                 1);
          in_zoom [ 1 ]= (zoom ? 1 + frand((100.0 / zoom) - 1) : 1);
          ...
          in->from.x= (in->from.w > vp_w ? -frand(in->from.w - vp_w) : frand(vp_w - in->from.w));
          in->from.y= (in->from.h > vp_h ? -frand(in->from.h - vp_h) : frand(vp_h - in->from.h));
          ...
          if (ss->state == RECENTER)
            {
              in->to.x= (in->to.w > vp_w ? -(in->to.w - vp_w) / 2 : (vp_w - in->to.w) / 2);
              in->to.y= (in->to.h > vp_h ? -(in->to.h - vp_h) / 2 : (vp_h - in->to.h) / 2);
            }
          else
            {
              in->to.x= (in->to.w > vp_w ? -frand(in->to.w - vp_w) : frand(vp_w - in->to.w));
              in->to.y= (in->to.h > vp_h ? -frand(in->to.h - vp_h) : frand(vp_h - in->to.h));
            }
        }
        break;
    ...
    ModeInfo *mi)
{
  ...
  if (launch_p)
    {
      if (verbose_p)
        fprintf(stderr, "%s: %s => %s\n", blurb(), (ostate == LOADING ? "LOADING" : ostate == FIRST_IN ? "FIRST_IN" :
                                                       ...
                                                       ostate == TRANSITION_PANZOOM                    ? "TRANSITION_PANZOOM" :
                                                                                                         "???"),
          (ss->state == LOADING ? "LOADING" : ss->state == FIRST_IN ? "FIRST_IN" :
  ...
  for (i= 0; i < countof(ss->sprites); i++)
    {
      ...
      if (0 && verbose_p && debug_p && ss->redisplay_needed_p)
        fprintf(stderr, "%s: %d: %5.0f %-5.0f  %5.0f %-5.0f  %.1f\n", blurb(), sp->id, sp->current.x, sp->current.y, sp->current.w, sp->current.h, sp->opacity);

      if (sp->state != sp->prev_state)
        {
          if (verbose_p)
            fprintf(stderr,
              "%s: %d: %-4s %-5s %3d frames %2.0f sec %5.1f fps" " (max %.0f fps?)\n",
              blurb(),
              sp->id,
  ...
  if (do_titles)
    {
      ...
      double start= (transition_seconds ? transition_seconds : fade_seconds);

      /* When should the title finish fading out? */
      double end= (transition_seconds && fade_seconds ? image_seconds + fade_seconds : transition_seconds ? image_seconds :
    ...
    ModeInfo *mi)
{
  ...
  if (ss->state == LOADING)
    {
      ...
      print_texture_label(mi->dpy, ss->font_data, mi->xgwa.width, mi->xgwa.height, 0, "Loading...");
    }
  ...
  if (do_titles && ss->title_opacity > 0)
    {
      sprite *sp= (ss->nsprites > 0 ? ss->sprites [ ss->nsprites - 1 ] : 0);
      if (sp && sp->img && sp->img->loaded_p &&
        sp->img->title && *sp->img->title)
        {
          glColor4f(1, 1, 1, ss->title_opacity);
          print_texture_label(mi->dpy, ss->font_data, mi->xgwa.width, mi->xgwa.height, 1, sp->img->title);
        }
    }
    ...
    ModeInfo *mi)
{
  ...
  if (zoom < 1)
    zoom= 1; /* zoom is a positive percentage */
  else if (zoom > 100)
    zoom= 100;
