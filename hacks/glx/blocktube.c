...
static argtype vars []= {
  ...
  {&changetime, "changetime", "Change Time", DEF_CHANGETIME, t_Int},
...
ModStruct blocktube_description=
  {"blocktube", "init_blocktube", "draw_blocktube", (char *) NULL, "draw_blocktube", "init_blocktube", "free_blocktube", &blocktube_opts, 40000, 30, 1, 1, 64, 1.0, "", "A shifting tunnel of reflective blocks", 0, NULL};
#endif /* USE_MODULES */
  ...
  ModeInfo *mi)
{
  ...
  lp->texti= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), blocktube_png, sizeof(blocktube_png));
  if (! lp->texti)
    {
      status= False;
    }
  else
    {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, lp->texti->width, lp->texti->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, lp->texti->data);
...
ENTRYPOINT void init_blocktube(
  ModeInfo *mi)
{
  int loop;
...
ENTRYPOINT void free_blocktube(
  ModeInfo *mi)
{
  blocktube_configuration *lp= &lps [ MI_SCREEN(mi) ];
#if defined(I_HAVE_XPM)
