    ...
    double now)
{
  ...
  for (i= 0; i < histogram_speed; i++)
    {
      int nn= (n == BIT_YES ? 240 : n == BIT_NO ? 17 :
                                                  128);
    ...
    ModeInfo *mi)
{
...

  {
    ...
    int omodel= bp->history [ bp->history_fp > 0 ? bp->history_fp - 1 : countof(bp->history) - 1 ];
    double now= double_time();
    double ratio= 1 - ((bp->last_time + bp->frequency) - now) / bp->frequency;
    if (ratio > 1) ratio= 1;
    mi->polygon_count+= draw_histogram(mi, ratio);

    if (MI_WIDTH(mi) > MI_HEIGHT(mi) * 5)
      { /* wide window: scale up */
        glScalef(8, 8, 8);
      }
