      ...
      for (x= 0; x < bp->grid_w; x++)
        {
          ...
          glTranslatef(-bp->grid_w / 2.0 - off.x + (x * TILE_PIXEL_SIZE),
            -bp->grid_h / 2.0 - off.y - (y * TILE_PIXEL_SIZE) - TILE_PIXEL_SIZE,
            0);
    ...
    ModeInfo *mi)
{
  ...
  const char *dirs[] = { "N", "NNE", "NE", "ENE",
                         ...
                         "W", "WNW", "NW", "NNW" };
#else
  const char *dirs []= {"N", "NbE", "NNE", "NEbN", "NE", "NEbE", "ENE", "EbN", "E", "EbS", "ESE", "SEbE", "SE", "SEbS", "SSE", "SbE", "S", "SbW", "SSW", "SWbS", "SW", "SWbW", "WSW", "WbS", "W", "WbN", "WNW", "NWbW", "NW", "NWbN", "NNW", "NbW"};
#endif
    ...
    for (i= 0; i < bp->grid_w * bp->grid_h; i++)
      {
        ...
        if (t->map.x == x &&
          ...
          ! t->texid)
          {
            ...
            if (strlen(file) == 3) /* HTTP error code */
              {
                ...
                if (verbose_p)
                  fprintf(stderr, "%s: error %s: tile: %s pos: %.4f, %.4f\n", blurb(mi), file, buf, tiley2lat(y, z), tilex2lon(x, z));
                break;
              }
            ...
            sprintf(buf2, "texture %d: %.100s (%dx%d)", t->texid, file, image->width, image->height);
            check_gl_error(buf2);
            XDestroyImage(image);
            if (verbose_p > 3)
              fprintf(stderr, "%s: %s\n", blurb(mi), buf2);
    ...
    ModeInfo *mi)
{
  ...
  bp->oceans= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), oceantiles_12_png, sizeof(oceantiles_12_png));
  if (! bp->oceans) abort();
        ...
        ! strcasecmp(origin_arg, "random-city"))
        {
          if (force_change_p ||
              ...
              now > bp->change_time + bp->duration))
            {
              ...
              if (verbose_p)
                fprintf(stderr, "%s: new %s%s, was %.4f, %.4f\n", blurb(mi), (! strcasecmp(origin_arg, "random-city") ? "city" : "position"), (all_failed_p ? " (all tiles failed)" : bp->ocean_p ? " (ocean)" :
                                                                                                                                                                                                    ""),
                  bp->pos.lat,
                  bp->pos.lon);
            }
  ...
  if (bp->mode == FADE_OUT)
    {
      ...
      if (bp->opacity < 0 && bp->dead_p)
      ...
      else if (bp->opacity < 0)
        {
          ...
          randomize_position(mi);
          if (verbose_p)
            fprintf(stderr, "%s: new position %.4f, %.4f%s\n", blurb(mi), bp->pos.lat, bp->pos.lon, (bp->ocean_p ? " (ocean)" : ""));
        }
    }
  ...
  if (do_titles && bp->mode == RUN)
    {
      ...
      sprintf(buf,
        "%.0f\xC2\xB0 %.0f' %.2f\" %c, " "%.0f\xC2\xB0 %.0f' %.2f\" %c",
        alat,
        (alat - (int) alat) * 60,
      ...
      double alon= bp->pos.lon >= 0 ? bp->pos.lon : -bp->pos.lon;
      sprintf(buf,
        "%.0f\xC2\xB0 %.0f' %c, " "%.0f\xC2\xB0 %.0f' %c",
        alat,
        (alat - (int) alat) * 60,
        (bp->pos.lat >= 0 ? 'N' : 'S'),
        alon,
      ...
      print_texture_label(mi->dpy, bp->font_data, MI_WIDTH(mi), MI_HEIGHT(mi), 1, buf);
    }
  ...
  if (bp->dead_p ||
      ...
      now >= bp->start_time + 5))
    {
      ...
      *s= 0;
      ...
      glColor3f(0.3, 0.3, 0.3);
      print_texture_label(mi->dpy, bp->font_data, MI_WIDTH(mi), MI_HEIGHT(mi), 0, buf);
    }
