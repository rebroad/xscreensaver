...
ModStruct gflux_description=
  {"gflux", "init_gflux", "draw_gflux", NULL, "draw_gflux", "init_gflux", NULL, &gflux_opts, 1000, 1, 2, 1, 4, 1.0, "", "GFlux: an OpenGL gflux", 0, NULL};
#endif
  ...
  ModeInfo *mi)
{
...

  {
    ...
    if (! s || ! *s)
    ...
    else if (! strcasecmp(s, "wire"))
      {
        _draw= wire;
      }
    else if (! strcasecmp(s, "solid"))
      {
        _draw= solid;
      }
    ...
    else if (! strcasecmp(s, "checker"))
      {
        _draw= checker;
      }
    else if (! strcasecmp(s, "grab"))
      {
        _draw= grab;
      }
    else
      {
        fprintf(stderr,
          "%s: mode must be one of: wire, solid, " "light, checker, or grab; not \"%s\"\n",
          progname,
          s);
        exit(1);
      }
...
{
  int size= 4;
  unsigned int data []= {0xFFFFFFFF, 0xAAAAAAAA, 0xFFFFFFFF, 0xAAAAAAAA, 0xAAAAAAAA, 0xFFFFFFFF, 0xAAAAAAAA, 0xFFFFFFFF, 0xFFFFFFFF, 0xAAAAAAAA, 0xFFFFFFFF, 0xAAAAAAAA, 0xAAAAAAAA, 0xFFFFFFFF, 0xAAAAAAAA, 0xFFFFFFFF};
    ...
    void *closure)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (gp->mipmap_p ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
    ...
    gfluxstruct *gp)
{
  ...
  gp->mipmap_p= True;
  load_texture_async(gp->modeinfo->xgwa.screen, gp->modeinfo->window, *gp->glx_context, 0, 0, gp->mipmap_p, gp->texName, image_loaded_cb, gp);
}
  ...
  double a)
{
  ...
  for (i= 0; i < _waves; i++)
    retval+= gp->wa [ i ] * tmp * sin(gp->freq [ i ] * ((x + gp->dispx [ i ]) * (x + gp->dispx [ i ]) + (y + gp->dispy [ i ]) * (y + gp->dispy [ i ]) + a));
  return (retval);
}
