...
ModStruct cube21_description=
  {"cube21", "init_cube21", "draw_cube21", NULL, "draw_cube21", "change_cube21", NULL, &cube21_opts, 25000, 1, 1, 1, 1.0, 4, "", "Shows randomly shuffling Cube 21", 0, NULL};
#endif
  ...
  col_t colors)
{
  ...
  glTexCoord2f(cp->texq, 0.0);
  glVertex3f(cp->posc [ 1 ], 0.0, s);
  glTexCoord2f(cp->texp, 0.0);
  ...
  glVertex3f(cp->posc [ 2 ], cp->posc [ 3 ], s);
  glTexCoord2f(1.0, cp->texp);
  glVertex3f(cp->posc [ 2 ], cp->posc [ 3 ], s1);
  glTexCoord2f(1.0, cp->texq);
  glVertex3f(cp->posc [ 1 ], 0.0, s1);
  mi->polygon_count++;
  ...
  col_t colors)
{
  ...
  glVertex3f(cp->posc [ 4 ], cp->posc [ 5 ], s);
  glTexCoord2f(0.0, 0.0);
  glVertex3f(cp->posc [ 4 ], cp->posc [ 5 ], s);
  glTexCoord2f(0.0, cp->texp);
  ...
  col_t colors)
{
  ...
  glVertex3f(cp->posc [ 4 ], cp->posc [ 5 ], cp->posc [ 0 ]);
  glTexCoord2f(cp->texq, 0.5);
  glVertex3f(-cp->posc [ 5 ], cp->posc [ 4 ], cp->posc [ 0 ]);
  glTexCoord2f(cp->texq, 0.75);
  glVertex3f(-cp->posc [ 5 ], cp->posc [ 4 ], -cp->posc [ 0 ]);
  ...
  void)
{
  ...
  if (strstr(colmode_s, "se") || strstr(colmode_s, "sil"))
    colmode= CUBE21_COLOR_SILVER;
  else if (strstr(colmode_s, "ce") || strstr(colmode_s, "cla"))
    colmode= CUBE21_COLOR_CLASSIC;
  else if (strstr(colmode_s, "2") || strstr(colmode_s, "two"))
    colmode= CUBE21_COLOR_TWORND;
  else if (strstr(colmode_s, "6") || strstr(colmode_s, "six"))
    colmode= CUBE21_COLOR_SIXRND;
  else if (strstr(colmode_s, "1") || strstr(colmode_s, "ran") || strstr(colmode_s, "rnd"))
    colmode= CUBE21_COLOR_RANDOM;
  else
    colmode= CUBE21_COLOR_WHITE;
}

static void init_posc(
  cube21_conf *cp)
{
...
#ifdef MIPMAP
  clear_gl_error();
  status= gluBuild2DMipmaps(GL_TEXTURE_2D, 1, TEX_WIDTH, TEX_HEIGHT, GL_LUMINANCE, GL_UNSIGNED_BYTE, texture);
  if (status)
    {
...
#else
  glTexImage2D(GL_TEXTURE_2D, 0, 1, TEX_WIDTH, TEX_HEIGHT, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, cp->texture);
#endif
...
ENTRYPOINT void reshape_cube21(
  ModeInfo *mi,
  int width,
  int height)
{
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, cp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &cp->button_down_p))
    return True;
