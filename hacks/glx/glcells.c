...
ENTRYPOINT ModeSpecOpt glcells_opts= {countof(opts), opts, countof(vars), vars, NULL};
 ...
 */
static inline int random_interval(
  int min,
  int max)
{
  int n= max - min;
...
/* set to 0 */
static inline void vector_clear(
  Vector *vec)
{
  vec->x= vec->y= vec->z= 0;
}
...
static inline double vector_length(
  Vector *vec)
{
...
/* normalize vector */
static inline void vector_normalize(
  Vector *vec)
{
...
/* crossproduct */
static inline void vector_crossprod(
  Vector *a,
  Vector *b,
  Vector *out)
{
...
static inline int vector_compare(
  Vector *a,
  Vector *b)
{
  ...
  Object *obj)
{
  ...
  /* copy */
  memcpy(ret->vertex, obj->vertex, obj->num_vertex * sizeof(Vector));
  memcpy(ret->triangle, obj->triangle, obj->num_triangle * sizeof(Triangle));
...
*/
static ObjectSmooth *create_ObjectSmooth(
  Object *obj)
{
  int t, v, i;
  Vector *t_normal=
  ...
  int divisions)
{
  ...
  double e;
  int vi [ 30 ]= {0, 7, 1, 1, 7, 2, 2, 8, 3, 3, 8, 4, 4, 6, 5, 5, 6, 0, 0, 6, 7, 2, 7, 8, 4, 8, 6, 6, 8, 7};
  ...
  double fac)
{
  ...
  glDrawElements(GL_TRIANGLES, vertex_array->num_index, GL_UNSIGNED_INT, vertex_array->index);
...
static void create_nucleus_texture(
  State *st)
{
  int x, y;
  int w2= TEX_SIZE / 2;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, TEX_SIZE, TEX_SIZE, 0, GL_RGBA, GL_UNSIGNED_BYTE, st->texture);
}

static void draw_nucleus(
  State *st)
{
      ...
      if (st->cell [ b ].energy > 0)
        {
          ...
          st->cell [ b ].x+= fac * (2.0 - (4.0 * (double) random() / (double) RAND_MAX) + st->cell [ b ].vx);
          st->cell [ b ].y+= fac * (2.0 - (4.0 * (double) random() / (double) RAND_MAX) + st->cell [ b ].vy);
