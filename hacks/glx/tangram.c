...
static void draw_tangram_shape(
  tangram_shape ts)
{
  glPushMatrix();
...
ENTRYPOINT void reshape_tangram(
  ModeInfo *mi,
  int w,
  int h)
{
              ...
              if (all_solved(ls))
                {
                  ...
                  get_solved_puzzle(mi, nls [ small_triangle1 ], nls [ small_triangle2 ], nls [ medium_triangle ], nls [ large_triangle1 ], nls [ large_triangle2 ], nls [ square ], nls [ rhomboid ]);
                  tp->ncolors= 128;
                  tp->colors=
                    (XColor *) calloc(tp->ncolors, sizeof(XColor));

                  make_random_colormap(0, 0, 0, tp->colors, &tp->ncolors, True, False, 0, False);
