...
ModStruct klein_description=
  {"klein", "init_klein", "draw_klein", NULL, "draw_klein", "change_klein", NULL, &klein_opts, 25000, 1, 1, 1, 1.0, 4, "", "Rotate a Klein bottle in 4d or walk on it", 0, NULL};
...
#define KLEIN_BOTTLE_LAWSON        " STR(KLEIN_BOTTLE_LAWSON)
        "            \n\
#define FIGURE_8_RADIUS            " STR(FIGURE_8_RADIUS)
          "f               \n\
#define PINCHED_TORUS_RADIUS       " STR(PINCHED_TORUS_RADIUS)
            "f          \n\
#define RADIUS_INCR                " STR(RADIUS_INCR)
              "f                   \n\
                                                                         \n\
...
static void setup_pinched_torus(
  ModeInfo *mi,
  double umin,
  double umax,
  double vmin,
...
/* Rotate a 4D point x by the rotation matrix mat and project it to the 3D
   ...
   kleinstruct kb. */
static void project_4d_point_to_3d(
  kleinstruct *kb,
  double x [ 4 ],
  float mat [ 4 ][ 4 ],
  float p [ 3 ])
...
static void compute_tangent_space_basis_rotation(
  kleinstruct *kb,
  double x [ 4 ],
  double xu [ 4 ],
  double xv [ 4 ],
...
static void compute_walk_frame_figure8(
  kleinstruct *kb,
  float mat [ 4 ][ 4 ])
{
  float p [ 3 ];
...
/* Compute the current walk frame for the pinched torus Klein bottle, i.e.,
   the coordinate system of the point and direction at which the viewer is
   currently walking on the projective plane. */
static void compute_walk_frame_pinched_torus(
  kleinstruct *kb,
  float mat [ 4 ][ 4 ])
{
  float p [ 3 ];
...
/* Compute the current walk frame for the Lawson Klein bottle, i.e.,
   the coordinate system of the point and direction at which the viewer is
   currently walking on the projective plane. */
static void compute_walk_frame_lawson(
  kleinstruct *kb,
  float mat [ 4 ][ 4 ])
{
  float p [ 3 ];
...
static void set_opengl_state_ff(
  ModeInfo *mi,
  float matc [ 3 ][ 3 ])
{
      ...
      if (kb->colors == COLORS_ONESIDED)
        {
          ...
          if (kb->display_mode == DISP_TRANSPARENT)
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, mat_diff_trans_oneside);
          else
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, mat_diff_oneside);
        }
...
/* Set the OpenGL programmable functionality projection and lighting state
   for displaying a Klein bottle. */
static void set_opengl_state_pf(
  ModeInfo *mi,
  float mat [ 4 ][ 4 ],
  float matc [ 3 ][ 3 ])
{
  ...
  if (kb->projection_3d == DISP_3D_PERSPECTIVE ||
    ...
    glsl_Orthographic(p_mat, -1.0f, 1.0f, -1.0f / kb->aspect, 1.0f / kb->aspect, 0.1f, 10.0f);
      ...
      for (j= 0; j <= NUMV; j++)
        {
          ...
          t= 1.0 / sqrt(kb->pn [ o ][ 0 ] * kb->pn [ o ][ 0 ] + kb->pn [ o ][ 1 ] * kb->pn [ o ][ 1 ] + kb->pn [ o ][ 2 ] * kb->pn [ o ][ 2 ]);
          kb->pn [ o ][ 0 ]*= t;
          kb->pn [ o ][ 1 ]*= t;
          kb->pn [ o ][ 2 ]*= t;
        }
  ...
  double vmax)
{
  ...
  if (! kb->buffers_initialized)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 2 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), kb->uv, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, kb->vertex_t_buffer);
      glBufferData(GL_ARRAY_BUFFER, 2 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), kb->tex, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);
      ...
      if (! kb->change_colors &&
        kb->colors != COLORS_ONESIDED && kb->colors != COLORS_TWOSIDED)
        {
          ...
          glBufferData(GL_ARRAY_BUFFER, 4 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), kb->col, GL_STATIC_DRAW);
          glBindBuffer(GL_ARRAY_BUFFER, 0);
        }
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, kb->ni * sizeof(GLuint), kb->indices, GL_STATIC_DRAW);
  ...
  if (kb->colors != COLORS_ONESIDED && kb->colors != COLORS_TWOSIDED)
    {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, kb->color_buffer);
      if (kb->change_colors)
        glBufferData(GL_ARRAY_BUFFER, 4 * (NUMU + 1) * (NUMV + 1) * sizeof(GLfloat), kb->col, GL_STREAM_DRAW);
  ...
  if (kb->display_mode != DISP_WIREFRAME)
      ...
      for (i= 0; i < kb->ne; i++)
        {
          ...
          glDrawElements(GL_TRIANGLE_STRIP, kb->nt, GL_UNSIGNED_INT, (const GLvoid *) index_offset);
          polys= 2 * kb->ne * NUMV;
        }
    }
  else /* kb->display_mode == DISP_WIREFRAME */
    {
      glLineWidth(1.0f);
      index_offset= 0;
      glDrawElements(GL_LINES, kb->ni, GL_UNSIGNED_INT, (const void *) index_offset);
    }
...
#ifdef HAVE_GLSL
  if (kb->use_shaders)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, TEX_DIMENSION, TEX_DIMENSION, 0, GL_RED, GL_UNSIGNED_BYTE, texture);
  else
#endif /* HAVE_GLSL */
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, TEX_DIMENSION, TEX_DIMENSION, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, texture);
}
  ...
  ModeInfo *mi)
{
  ...
  if (! glsl_CompileAndLinkShaders(2, vertex_shader_source, 2, fragment_shader_source, &kb->shader_program))
    return;
