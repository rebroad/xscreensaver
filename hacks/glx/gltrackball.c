  ...
  int flip_p)
{
  ...
  scale= mx= my= 1000;
  move= (up_p ? floor(scale * (1.0 - (percent / 100.0))) : ceil(scale * (1.0 + (percent / 100.0))));
  if (horizontal_p)
    mx= move;
  else
    my= move;
...
void gltrackball_get_quaternion(
  trackball_state *ts,
  float q [ 4 ])
{
  int i;
