  ...
  int avail)
{
  ...
  sp->src= src;
  sp->dst= dst;
  sp->available= avail & ~((unsigned long) (1 << src)) & ~((unsigned long) (1 << dst));
  /*
...
static Bool solveStackEmpty(
  glhcfg *glhanoi)
{
  return (glhanoi->solveStackIdx < 1);
}

static SubProblem *popMove(
  glhcfg *glhanoi)
{
  SubProblem *sp;
      ...
      if (glhanoi->move == 0)
        {
          ...
          pushMove(glhanoi, glhanoi->numberOfDisks, 0, glhanoi->numberOfPoles - 1, REMOVE_BIT(REMOVE_BIT(ALL_POLES, 0), glhanoi->numberOfPoles - 1));
        }
      ...
      while (! solveStackEmpty(glhanoi))
        {
          ...
          if (sp.nDisks == 1)
          ...
          else
            {
              ...
              pushMove(glhanoi, k, tmp, sp.dst, REMOVE_BIT(ADD_BIT(sp.available, sp.src), tmp));
              pushMove(glhanoi, sp.nDisks - k, sp.src, sp.dst, REMOVE_BIT(sp.available, tmp));
              pushMove(glhanoi, k, sp.src, tmp, REMOVE_BIT(ADD_BIT(sp.available, sp.dst), tmp));
  ...
  glhcfg *glhanoi)
{
  ...
  get_position(glhanoi->the_rotator, NULL, NULL, &radius, ! glhanoi->button_down_p);
  get_rotation(glhanoi->the_rotator, &longitude, &latitude, NULL, ! glhanoi->button_down_p);
  ...
  glTranslatef(0.0f, 0.0f, -(MIN_CAMERA_RADIUS + radius * (MAX_CAMERA_RADIUS - MIN_CAMERA_RADIUS)));
  glRotatef(longitude * 360.0, 0.0f, 1.0f, 0.0f);
  glRotatef(latitude * 180.0, cos(longitude * 2.0 * M_PI), 0.0, sin(longitude * 2.0 * M_PI));
}
...
static int drawTube(
  ...
  GLdouble topRadius,
  GLdouble bottomThickness,
  GLdouble topThickness,
  GLdouble height,
  GLuint nSlice,
  GLuint nLoop)
{
  ...
  for (slice= lastSlice; slice >= 0; --slice)
    {
      ...
      glVertex3f(innerRadius * sinCache [ slice ], y, innerRadius * cosCache [ slice ]);
  ...
  for (loop= 0; loop < nLoop; ++loop)
    {
      GLfloat lowerRadius=
        bottomRadius + (topRadius - bottomRadius) * (float) loop / (nLoop);
      GLfloat upperRadius=
        bottomRadius + (topRadius - bottomRadius) * (float) (loop + 1) / (nLoop);
      ...
      for (slice= 0; slice < nSlice; ++slice)
        {
          ...
          glVertex3f(upperRadius * sinCache [ slice ], upperY, upperRadius * cosCache [ slice ]);
          glVertex3f(lowerRadius * sinCache [ slice ], lowerY, lowerRadius * cosCache [ slice ]);
          polys++;
        }
...
static int drawDisk3D(
  GLdouble inner_radius,
  GLdouble outer_radius,
  GLdouble height)
{
  return drawTube(outer_radius, outer_radius, outer_radius - inner_radius, outer_radius - inner_radius, height, NSLICE, NLOOPS);
}

/* used for drawing base */
static int drawCuboid(
  GLfloat length,
  GLfloat width,
  GLfloat height)
{
  ...
  glhcfg *glhanoi)
{
  ...
  if (glhanoi->trailQSize)
    ...
    {
      glhanoi->trailQ= (TrailPoint *) NULL;
    }
  ...
  double z)
{
  ...
  return lerp(w, lerp(v, lerp(u, grad(glhanoi->p [ AA ], x, y, z),                                           /* AND ADD */
    ...
    lerp(v, lerp(u, grad(glhanoi->p [ AA + 1 ], x, y, z - 1), grad(glhanoi->p [ BA + 1 ], x - 1, y, z - 1)), /* OF CUBE */
      lerp(u, grad(glhanoi->p [ AB + 1 ], x, y - 1, z - 1), grad(glhanoi->p [ BB + 1 ], x - 1, y - 1, z - 1))));
}
...
static GLubyte *makeTexture(
  glhcfg *glhanoi,
  int x_size,
  int y_size,
  int z_size,
  GLuint (*texFunc)(glhcfg *, double, double, double, tex_col_t *),
  tex_col_t *colours)
{
  int i, j, k;
  GLuint *textureData;
...
static tex_col_t *makeMarbleColours(
  void)
{
...
static GLuint makeMarbleTexture(
  glhcfg *glhanoi,
  double x,
  double y,
  double z,
  ...
  glhcfg *glhanoi)
{
  ...
  if ((marbleColours= makeMarbleColours()) == NULL)
    return 1;
  if ((marbleTexture=
          makeTexture(glhanoi, MARBLE_TEXTURE_SIZE, MARBLE_TEXTURE_SIZE, 1, makeMarbleTexture, marbleColours)) == NULL)
    return 1;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, MARBLE_TEXTURE_SIZE, MARBLE_TEXTURE_SIZE, 0, GL_RGBA, GL_UNSIGNED_BYTE, marbleTexture);
  free(marbleTexture);
 ...
 */
ENTRYPOINT void reshape_glhanoi(
  ModeInfo *mi,
  int width,
  int height)
{
  ...
  glLoadIdentity();
  gluPerspective(30.0, 1 / h, 1.0, 2 * MAX_CAMERA_RADIUS);
...
ENTRYPOINT Bool glhanoi_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  glhcfg *glhanoi= &glhanoi_cfg [ MI_SCREEN(mi) ];
  ...
  else if (event->xany.type == MotionNotify && glhanoi_cfg->button_down_p)
    {
      int x_diff, y_diff;
