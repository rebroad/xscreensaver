    ...
    ModeInfo *mi)
{
  ...
  if (! g)
  ...
  else
    {
      ...
      if (debug_p)
        sprintf(label + strlen(label), "\nPolys:  %d\nModel:  %s  (%.2f)\n", g->polygons, (g->size == INVOLUTE_SMALL ? "small" : g->size == INVOLUTE_MEDIUM ? "medium" :
                                                                                                                                                              "large"),
...
AGAIN:
   ...
   */
  {
    ...
    if (pix <= 2.5)
      g->size= INVOLUTE_SMALL;
    else if (pix <= 3.5)
      g->size= INVOLUTE_MEDIUM;
    else if (pix <= 25)
      g->size= INVOLUTE_LARGE;
    else
      g->size= INVOLUTE_HUGE;
  }
  ...
  if (! g)
    {
      ...
      if (verbose_p && debug_placement_p)
        fprintf(stderr,
          "%s: placement: resetting growth zone!  " "failed: %d size, %d pos\n",
          progname,
          pp->debug_size_failures,
          pp->debug_position_failures);
      for (i= pp->ngears - 1; i >= 0; i--)
  ...
  if (verbose_p)
    {
      ...
      fprintf(stderr, " %2d%%", (int) (g->r * 2 * 100 / pp->vp_height));
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, pp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &pp->button_down_p))
    {
      return True;
    }
