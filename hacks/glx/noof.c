  ...
  if (bp->screenshot_texture)
    {
      ...
      glVertex3f(0, 0, 0);
      glTexCoord2f(tw, 0);
      glVertex3f(bp->wd, 0, 0);
      glTexCoord2f(tw, th);
      glVertex3f(bp->wd, bp->ht, 0);
      glTexCoord2f(0, th);
      glVertex3f(0, bp->ht, 0);
      glEnd();
  ...
  if (bp->screenshot_texture) /* Store a screenshot into the texture. */
    {
      ...
      glEnable(GL_TEXTURE_2D);
      glBindTexture(GL_TEXTURE_2D, bp->screenshot_texture);
      glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, MI_WIDTH(mi), MI_HEIGHT(mi));
    ...
    int h)
{
  ...
  if (w <= h)
    {
      ...
      glOrtho(0.0, 1.0, 0.0, 1.0 * (GLfloat) h / (GLfloat) w, -16.0, 4.0);
    }
  else
    {
      bp->wd= (GLfloat) w / (GLfloat) h;
      bp->ht= 1.0;
      glOrtho(0.0, 1.0 * (GLfloat) w / (GLfloat) h, 0.0, 1.0, -16.0, 4.0);
    }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bp->tex_w, bp->tex_h, 0, GL_RGBA, GL_UNSIGNED_BYTE, s);
