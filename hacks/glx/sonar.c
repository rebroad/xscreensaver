...
static XrmOptionDescRec opts []= {
  ...
  THREAD_OPTIONS {"-debug", ".debug", XrmoptionNoArg, "True"},
};
  ...
  for (i= r_steps; i > 0; i--)
    {
      ...
      for (j0= 0; j0 <= th_steps; j0++)
        {
          if (mesh_p &&
            (i < outer_r ? (j0 % th_skip != 0) : (j0 % th_skip2 != 0)))
            continue;
    ...
    sonar_configuration *sp)
{
  ...
  this_sweep= ((cycle_secs - fmod(double_time() - sp->start_time + sp->sweep_offset, cycle_secs)) / cycle_secs * M_PI * 2);
          ...
          if (debug_p > 1)
            {
              ...
              fprintf(stderr,
                "%s: sweep hit: %02d:%02d: %s: (%5.2f %5.2f %5.2f;" " th=[%.2f < %.2f <= %.2f])\n",
                progname,
                (int) (t / 60) % 60,
                (int) t % 60,
                b->name,
  ...
  switch (family)
    {
      case AF_INET :
        ...
        break;
      case AF_INET6 :
        inet_ntop(family, &((struct sockaddr_in6 *) addr)->sin6_addr, buf, sizeof(buf));
        fputs(buf, stream);
        break;
      default :
        abort();
    ...
    ModeInfo *mi)
{
  ...
  if (! ping_arg || ! *ping_arg ||
    ...
    ! ! strcmp(ping_arg, "simulation"))
    {
      ...
      sp->ssd= sonar_init_ping(MI_DISPLAY(mi), &sp->error, &sp->desc, ping_arg, ping_timeout, resolve_p, times_p, debug_p);
    }
  ...
  if (! sp->ssd)
    sp->ssd= sonar_init_simulation(MI_DISPLAY(mi), &sp->error, &sp->desc, team_a_name, team_b_name, team_a_count, team_b_count, debug_p);
  if (! sp->ssd)
    abort();
...

  {
    ...
    sp->query0= async_name_from_addr_start(MI_DISPLAY(mi), (void *) &addr, sizeof(addr));
    assert(sp->query0);
    if (sp->query0)
      {
  ...
  if (sp->query0 && async_name_from_addr_is_done(sp->query0))
    {
      if (! (random() & 3))
      ...
      else
        {
          char *hostname= NULL;
          int errno_error;
          int gai_error= async_name_from_addr_finish(sp->query0, &hostname, &errno_error);
  ...
  if (sp->query1 && async_addr_from_name_is_done(sp->query1))
    {
      if (! (random() & 3))
      ...
      else
        {
          ...
          socklen_t addrlen;
          int errno_error;
          int gai_error= async_addr_from_name_finish(sp->query1, &addr, &addrlen, &errno_error);
