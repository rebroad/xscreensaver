  ...
  for (x= 1; x < countof(coords [ 0 ]); x++)
    {
      ...
      glVertex3f(px0, -py0, -zz);
      glTexCoord2f(px1, py1);
      glVertex3f(px1, -py1, -zz);
      glTexCoord2f(px3, py3);
      glVertex3f(px3, -py3, -zz);
      glTexCoord2f(px2, py2);
      glVertex3f(px2, -py2, -zz);
      polys++;
      glEnd();
  ...
  while (1)
    {
      ...
      if ((top_slice->part == CAP && last_part == 0) ||
        ...
        bp->bottle_poly_count+=
          draw_disc(top_slice->radius, top_slice->elevation, faces, True, wire);
      ...
      if ((top_slice->part == CAP && bot_slice->part == BOTTLE) ||
        ...
        {
          const lamp_geometry *sl= (bot_slice->part == 0 ? top_slice : bot_slice);
          bp->bottle_poly_count+=
      ...
      bp->bottle_poly_count+=
        draw_tube(top_slice->radius, bot_slice->radius, top_slice->elevation, bot_slice->elevation, t0, t1, faces, (top_slice->part == BOTTLE), smooth, wire);
...
static double
  bellrand(
    double extent) /* like frand(), but a bell curve. */
{
  return (((frand(extent) + frand(extent) + frand(extent)) / 3) - (extent / 2));
}
...
DONE:;
  ...
  mi->polygon_count= 0;
  {
    ...
    glScalef(s, s, s);
    marching_cubes(resolution, isolevel, wire, do_smooth, obj_init, obj_compute, obj_free, bp, &mi->polygon_count);
    glPopMatrix();
  }
    ...
    ModeInfo *mi)
{
  ...
  {
    char *s= do_style;
    if (! s || ! *s || ! strcasecmp(s, "classic"))
      {
        bp->style= CLASSIC;
      }
    else if (! strcasecmp(s, "giant"))
      {
        bp->style= GIANT;
      }
    else if (! strcasecmp(s, "cone"))
      {
        bp->style= CONE;
      }
    else if (! strcasecmp(s, "rocket"))
      {
        bp->style= ROCKET;
      }
      ...
      {
        if (random() & 1)
          bp->style= CLASSIC; /* half the time */
        else
          bp->style= (random() % ((int) ROCKET + 1));
      }
    else
      {
        fprintf(stderr,
...

  {
    ...
    bp->rot= make_rotator(spinx ? spin_speed : 0,
      ...
      False);
    bp->rot2= make_rotator(spin_speed, 0, 0, 1, 0.1, False);
    ...
    gltrackball_reset(bp->trackball,
      -0.3 + frand(0.6),
      (bp->style == ROCKET || bp->style == GIANT ? frand(0.2) : -frand(0.6)));
  }
  ...
  bp->nballs= (((MI_COUNT(mi) + 1) * bp->blobs_per_group) + 2);
