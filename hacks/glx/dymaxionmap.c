...
ModStruct planet_description=
  {"planet", "init_planet", "draw_planet", NULL, "draw_planet", "init_planet", "free_planet", &planet_opts, 1000, 1, 2, 1, 4, 1.0, "", "Buckminster Fuller's unwrapped icosahedral globe", 0, NULL};
#endif
    ...
    XImage *image)
{
  ...
  int w= image->width;
  int h= image->height;
  XImage *out= XCreateImage(dpy, visual, image->depth, ZPixmap, 0, 0, w / 2, h / 2, 32, 0);
    ...
    ModeInfo *mi)
{
  ...
  for (i= 0; i < 2; i++)
    {
      ...
      if (! strcmp(s, "BUILTIN_DAY"))
        {
          image= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), earth_png, sizeof(earth_png));
          builtin_p= True;
        }
      else if (! strcmp(s, "BUILTIN_NIGHT"))
        {
          image= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), earth_night_png, sizeof(earth_night_png));
          builtin_p= True;
        }
        ...
        (i == 0 && ! strcmp(s, "")))
        {
          image= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), earth_flat_png, sizeof(earth_flat_png));
          builtin_p= True;
        }
  ...
  if (gp->day && gp->night && ! gp->dusk)
    {
      if (gp->day->width != gp->night->width ||
        ...
        {
          fprintf(stderr,
            "%s: day and night images must be the same size" " (%dx%d vs %dx%d)\n",
            progname,
            gp->day->width,
            gp->day->height,
            gp->night->width,
      ...
      gp->dusk= create_daylight_mask(MI_DISPLAY(mi), MI_VISUAL(mi), gp->day->width, gp->day->height);
    }
   ...
   */
  {
    ...
    gp->cvt= XCreateImage(MI_DISPLAY(mi), MI_VISUAL(mi), 32, ZPixmap, 0, 0, gp->day->width * 2, gp->day->height * 2, 32, 0);
    gp->cvt->data= (char *)
    ...
    ModeInfo *mi)
{
  ...
  xoff= (gp->dusk ? gp->dusk->width * ((double) gp->current_frame / gp->nimages) : 0);
  ...
  if (! gp->night)
  ...
  else
    {
       ...
       */
      blended= XCreateImage(MI_DISPLAY(mi), MI_VISUAL(mi), gp->day->depth, ZPixmap, 0, 0, w, h, 32, 0);
      if (! blended) abort();
...
#undef ADD
   ...
   */
  dymaxion= XCreateImage(MI_DISPLAY(mi), MI_VISUAL(mi), gp->day->depth, ZPixmap, 0, 0, w, h, 32, 0);
    ...
    ModeInfo *mi)
{
  ...
  ground= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), ground_png, sizeof(ground_png));
  clear_gl_error();
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  /* glPixelStorei(GL_UNPACK_ROW_LENGTH, ground->width); */
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ground->width, ground->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, ground->data);
      ...
      for (i= 0; i < nstars / steps; i++)
        {
          ...
          GLfloat z= ((random() & 1) ? frand(1) - 0.5 : (BELLRAND(1) - 0.5) / 12); /* milky way */
          d= sqrt(x * x + y * y + z * z);
          x/= d;
          y/= d;
          z/= d;
...
static void
  do_normal2(
    ...
    XYZ c)
{
  XYZ n= (frontp ? calc_normal(a, b, c) : calc_normal(b, a, c));
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, gp->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &gp->button_down_p))
    {
      return True;
    }
      ...
      if (c == ' ' || c == '\t' || c == '\r' || c == '\n')
        {
          ...
          which_image= strdup(! strcmp(which_image, "BUILTIN_DAY") ? "BUILTIN_FLAT" : "BUILTIN_DAY");
          which_image2= strdup(! strcmp(which_image2, "BUILTIN_NIGHT") ? "BUILTIN_FLAT" : "BUILTIN_NIGHT");
          load_images(mi);
          gp->current_frame= cf;
#if 0
          switch (gp->state) {
    ...
    ModeInfo *mi)
{
  ...
  {
    int size= MI_WIDTH(mi) < MI_HEIGHT(mi) ? MI_WIDTH(mi) : MI_HEIGHT(mi);
      ...
      if (i != prev || ! gp->images [ i ])
        {
          ...
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, gp->images [ i ]->width, gp->images [ i ]->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, gp->images [ i ]->data);
