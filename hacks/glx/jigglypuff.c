...
static inline void vector_copy(
  vector d,
  vector s)
{
  d [ 0 ]= s [ 0 ];
...
static inline void vector_add_to(
  vector v1,
  vector v2)
{
  v1 [ 0 ]+= v2 [ 0 ];
...
static inline void vector_scale(
  vector v,
  coord s)
{
  v [ 0 ]*= s;
...
static inline coord magnitude2(
  vector v)
{
  return v [ 0 ] * v [ 0 ] + v [ 1 ] * v [ 1 ] + v [ 2 ] * v [ 2 ];
}

static inline coord magnitude(
  vector v)
{
...
static inline void normalize_to(
  vector v,
  coord m)
{
...
static solid *tesselated_tetrahedron(
  coord size,
  int iter,
  jigglystruct *js)
{
...
static void clownbarf_colorize(
  solid *s)
{
  face *f= s->faces;
  while (f)
...
static inline void vertex_calcnormal(
  vertex *vtx,
  jigglystruct *js)
{
...
static inline void vertex_render(
  vertex *vtx,
  jigglystruct *js)
{
  glNormal3fv(vtx->n);
  ...
  ModeInfo *mi)
{
  ...
  XImage *img= image_data_to_ximage(mi->dpy, mi->xgwa.visual, jigglymap_png, sizeof(jigglymap_png));
  ...
  jigglystruct *js)
{
  ...
  ;
#endif
  if (js->color_style == COLOR_STYLE_NORMAL || js->color_style == COLOR_STYLE_CYCLE)
    {
  ...
  int subdivs)
{
   ...
   * low complexity. */
  float dist_factor= (subdivs == 6) ? 2 : (subdivs == 5) ? 1 :
                                                           0.5;

  js->stable_distance= ((float) distance / 500.0) * (STABLE_DISTANCE / dist_factor);
...
ENTRYPOINT Bool jigglypuff_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  jigglystruct *js= &jss [ MI_SCREEN(mi) ];

  if (gltrackball_event_handler(event, js->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &js->button_down))
    return True;

  return False;
}

ENTRYPOINT void reshape_jigglypuff(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT void draw_jigglypuff(
  ModeInfo *mi)
{
  jigglystruct *js= &jss [ MI_SCREEN(mi) ];
...
ENTRYPOINT void init_jigglypuff(
  ModeInfo *mi)
{
  jigglystruct *js;
  int subdivs;
...
ENTRYPOINT void free_jigglypuff(
  ModeInfo *mi)
{
  jigglystruct *js= &jss [ MI_SCREEN(mi) ];
