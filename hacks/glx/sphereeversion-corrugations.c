...
typedef void surface_time_function(float u, float v, float t, int num_strips, one_jet_vec *val);


/* ------------------------------------------------------------------------- */


static inline void copy_one_jet(
  const one_jet *x,
  one_jet *res)
{
  res->f= x->f;
...
static inline void set_one_jet(
  float d,
  float du,
  float dv,
  one_jet *res)
...
static inline void add_one_jets(
  const one_jet *x,
  const one_jet *y,
  one_jet *res)
{
...
static inline void sub_one_jets(
  const one_jet *x,
  const one_jet *y,
  one_jet *res)
{
...
static inline void mult_one_jets(
  const one_jet *x,
  const one_jet *y,
  one_jet *res)
{
...
static inline void add_one_jet_float(
  const one_jet *x,
  float d,
  one_jet *res)
{
...
static inline void mult_one_jet_float(
  const one_jet *x,
  float d,
  one_jet *res)
{
...
static inline void neg_one_jet(
  const one_jet *x,
  one_jet *res)
{
  res->f= -x->f;
...
static inline void fmod_one_jet(
  const one_jet *x,
  float d,
  one_jet *res)
{
...
static inline void sin_one_jet(
  const one_jet *x,
  one_jet *res)
{
  float s, c;
...
static inline void cos_one_jet(
  const one_jet *x,
  one_jet *res)
{
  float s, c;
...
static inline void pow_one_jet(
  const one_jet *x,
  float n,
  one_jet *res)
{
...
static inline void annihilate_one_jet(
  const one_jet *x,
  int index,
  one_jet *res)
{
...
static inline void interpolate_one_jets_float(
  const one_jet *v1,
  const one_jet *v2,
...
static inline void copy_two_jet(
  const two_jet *x,
  two_jet *res)
{
  res->f= x->f;
...
static inline void set_two_jet(
  float d,
  float du,
  float dv,
  two_jet *res)
...
static inline void two_jet_to_one_jet(
  const two_jet *x,
  one_jet *res)
{
  res->f= x->f;
...
static inline void add_two_jets(
  const two_jet *x,
  const two_jet *y,
  two_jet *res)
{
...
static inline void sub_two_jets(
  const two_jet *x,
  const two_jet *y,
  two_jet *res)
{
...
static inline void mult_two_jets(
  const two_jet *x,
  const two_jet *y,
  two_jet *res)
{
...
static inline void add_two_jet_float(
  const two_jet *x,
  float d,
  two_jet *res)
{
...
static inline void add_two_jet_float_inplace(
  float d,
  two_jet *res)
{
  res->f+= d;
...
static inline void mult_two_jet_float(
  const two_jet *x,
  float d,
  two_jet *res)
{
...
static inline void neg_two_jet(
  const two_jet *x,
  two_jet *res)
{
  res->f= -x->f;
...
static inline void fmod_two_jet(
  const two_jet *x,
  float d,
  two_jet *res)
{
...
static inline void sin_two_jet(
  const two_jet *x,
  two_jet *res)
{
  float s, c;
...
static inline void cos_two_jet(
  const two_jet *x,
  two_jet *res)
{
  float s, c;
...
static inline void pow_two_jet(
  const two_jet *x,
  float n,
  two_jet *res)
{
...
static inline void differentiate_two_jet(
  const two_jet *x,
  int index,
  one_jet *res)
{
...
static inline void annihilate_two_jet(
  const two_jet *x,
  int index,
  two_jet *res)
{
...
static inline void add_one_jet_vecs(
  const one_jet_vec *v,
  const one_jet_vec *w,
  one_jet_vec *res)
{
...
static inline void mult_one_jet_vec_one_jet(
  const one_jet_vec *v,
  const one_jet *a,
  one_jet_vec *res)
{
...
static inline void annihilate_one_jet_vec(
  const one_jet_vec *v,
  int index,
  one_jet_vec *res)
{
...
static inline void cross_one_jet_vecs(
  const one_jet_vec *v,
  const one_jet_vec *w,
  one_jet_vec *res)
{
...
static inline void dot_one_jet_vecs(
  const one_jet_vec *v,
  const one_jet_vec *w,
  one_jet *res)
{
...
static inline void normalize_one_jet_vec(
  const one_jet_vec *v,
  one_jet_vec *res)
{
  one_jet a, sqrt_a;
...
static inline void rotate_one_jet_vec_z_float(
  const one_jet_vec *v,
  const one_jet *angle,
  one_jet_vec *res)
{
...
static inline void two_jet_vec_to_one_jet_vec(
  const two_jet_vec *v,
  one_jet_vec *res)
{
...
static inline void add_two_jet_vecs(
  const two_jet_vec *v,
  const two_jet_vec *w,
  two_jet_vec *res)
{
...
static inline void mult_two_jet_vec_two_jet(
  const two_jet_vec *v,
  const two_jet *a,
  two_jet_vec *res)
{
...
static inline void mult_two_jet_vec_float(
  const two_jet_vec *v,
  float a,
  two_jet_vec *res)
{
...
static inline void annihilate_two_jet_vec(
  const two_jet_vec *v,
  int index,
  two_jet_vec *res)
{
...
static inline void differentiate_two_jet_vec(
  const two_jet_vec *x,
  int index,
  one_jet_vec *res)
{
...
static inline void rotate_two_jet_vec_z_float(
  const two_jet_vec *v,
  float angle,
  two_jet_vec *res)
{
...
static inline void rotate_two_jet_vec_y_float(
  const two_jet_vec *v,
  float angle,
  two_jet_vec *res)
{
...
static inline void interpolate_two_jet_vecs_two_jet(
  const two_jet_vec *v1,
...
static inline void interpolate_two_jet_vecs_float(
  const two_jet_vec *v1,
  const two_jet_vec *v2,
  float weight,
  two_jet_vec *res)
...
static void figure_eight(
  const one_jet_vec *w,
  const one_jet_vec *h,
  const one_jet_vec *bend,
  const one_jet *form,
  const one_jet *v,
  one_jet_vec *res)
{
...
static inline void push_through(
  float u,
  float v,
  float t,
  int num_strips,
...
static inline void un_corrugate(
  float u,
  float v,
  float t,
  int num_strips,
...
static inline void gen_point_and_normal(
  one_jet_vec *p,
  float *point,
  float *normal)
{
...
static inline void gen_surface(
  surface_time_function *func,
  float umin,
  float umax,
  int ucount,
  float vmin,
  ...
  float v_max)
{
  ...
  if (time >= uncorr_start)
    gen_surface(un_corrugate, u_min, u_max, u_count, v_min, v_max, v_count, (time - uncorr_start) / (1.0f - uncorr_start), points, normals, num_strips);
  else if (time >= unpush_start)
    gen_surface(un_push, u_min, u_max, u_count, v_min, v_max, v_count, (time - unpush_start) / (uncorr_start - unpush_start), points, normals, num_strips);
  else if (time >= twist_start)
    gen_surface(twist, u_min, u_max, u_count, v_min, v_max, v_count, (time - twist_start) / (unpush_start - twist_start), points, normals, num_strips);
  else if (time >= push_start)
    gen_surface(push_through, u_min, u_max, u_count, v_min, v_max, v_count, (time - push_start) / (twist_start - push_start), points, normals, num_strips);
  else if (time >= corr_start)
    gen_surface(corrugate, u_min, u_max, u_count, v_min, v_max, v_count, (time - corr_start) / (push_start - corr_start), points, normals, num_strips);
}
...
static void setup_surface_colors(
  ModeInfo *mi)
{
                  ...
                  for (k= 0; k <= NUM_V; k++)
                    {
                      ...
                      if (se->colors [ 0 ] == COLORS_PARALLEL)
                        color(se, (2.0f * theta + 3.0f * M_PI_F / 4.0f) * (2.0f / 3.0f), matc, &se->colf [ 0 ][ 4 * l ], &se->colb [ 0 ][ 4 * l ]);
                      else
  ...
  ModeInfo *mi)
{
  ...
  generate_geometry(se->sp, se->sn, se->time, NUM_STRIPS, 0.0f, NUM_U, 1.0f, 0.0f, NUM_V, 1.0f);
  ...
  if (se->projection == DISP_PERSPECTIVE)
    gluPerspective(60.0f, se->aspect, -se->offset3d [ 2 ] - 2.0f, -se->offset3d [ 2 ] + 2.0f);
  else if (se->aspect >= 1.0f)
    glOrtho(-1.8f * se->aspect, 1.8f * se->aspect, -1.8f, 1.8f, -se->offset3d [ 2 ] - 2.0f, -se->offset3d [ 2 ] + 2.0f);
  else
    glOrtho(-1.8f, 1.8f, -1.8f / se->aspect, 1.8f / se->aspect, -se->offset3d [ 2 ] - 2.0f, -se->offset3d [ 2 ] + 2.0f);
  ...
  ModeInfo *mi)
{
  ...
  if (! se->buffers_initialized)
    {
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, ni * sizeof(GLuint), se->solid_indices, GL_STATIC_DRAW);
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, ni * sizeof(GLuint), se->parallel_indices, GL_STATIC_DRAW);
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, ni * sizeof(GLuint), se->meridian_indices, GL_STATIC_DRAW);
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(blend_indices), blend_indices, GL_STATIC_DRAW);
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
  ...
  glBufferData(GL_ARRAY_BUFFER, 3 * (NUM_U + 1) * (NUM_V + 1) * sizeof(GLfloat), se->sp, GL_STREAM_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

  glBindBuffer(GL_ARRAY_BUFFER, se->vertex_normal_buffer);
  glBufferData(GL_ARRAY_BUFFER, 3 * (NUM_U + 1) * (NUM_V + 1) * sizeof(GLfloat), se->sn, GL_STREAM_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);

  glsl_Identity(p_mat);
  if (se->projection == DISP_PERSPECTIVE)
    glsl_Perspective(p_mat, 60.0f, se->aspect, -se->offset3d [ 2 ] - 2.0f, -se->offset3d [ 2 ] + 2.0f);
  else if (se->aspect >= 1.0f)
    glsl_Orthographic(p_mat, -1.8f * se->aspect, 1.8f * se->aspect, -1.8f, 1.8f, -se->offset3d [ 2 ] - 2.0f, -se->offset3d [ 2 ] + 2.0f);
  else
    glsl_Orthographic(p_mat, -1.8f, 1.8f, -1.8f / se->aspect, 1.8f / se->aspect, -se->offset3d [ 2 ] - 2.0f, -se->offset3d [ 2 ] + 2.0f);
          ...
          for (strip= 0; strip < NUM_STRIPS; strip+= se->strip_step)
            {
              ...
              glBindBuffer(GL_ARRAY_BUFFER, se->vertex_tex_coord_buffer);
              glVertexAttribPointer(se->poly_vertex_tex_index, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid *) index_offset);
              ...
              if (se->colors [ pass ] != COLORS_TWOSIDED &&
                se->colors [ pass ] != COLORS_EARTH)
                {
                  ...
                  glVertexAttribPointer(se->poly_colorf_index, 4, GL_FLOAT, GL_FALSE, 0, (const GLvoid *) index_offset);

                  glEnableVertexAttribArray(se->poly_colorb_index);
                  glBindBuffer(GL_ARRAY_BUFFER, se->vertex_colorb_buffer [ pass ]);
                  glVertexAttribPointer(se->poly_colorb_index, 4, GL_FLOAT, GL_FALSE, 0, (const GLvoid *) index_offset);
                }
#ifdef VERTEXATTRIBARRAY_WORKAROUND
                  ...
                  for (i= 0; i < se->num_solid_strips; i++)
                    {
                      ...
                      glDrawElements(GL_TRIANGLE_STRIP, se->num_solid_triangles, GL_UNSIGNED_INT, (const GLvoid *) index_offset);
                    }
                }
                  ...
                  for (i= 0; i < se->num_parallel_strips; i++)
                    {
                      ...
                      glDrawElements(GL_TRIANGLE_STRIP, se->num_parallel_triangles, GL_UNSIGNED_INT, (const GLvoid *) index_offset);
                    }
                }
                  ...
                  for (i= 0; i < se->num_meridian_strips; i++)
                    {
                      ...
                      glDrawElements(GL_TRIANGLE_STRIP, se->num_meridian_triangles, GL_UNSIGNED_INT, (const GLvoid *) index_offset);
                    }
                }
            }
        }
  ...
  if (num_passes == 2)
    {
      ...
      index_offset= 0;
      glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, (const void *) index_offset);
...
void init_sphereeversion_corrugations(
  ModeInfo *mi)
{
  sphereeversionstruct *se= &sphereeversion [ MI_SCREEN(mi) ];
...
void display_sphereeversion_corrugations(
  ModeInfo *mi)
{
