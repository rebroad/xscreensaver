...
extern const struct gllist
  *seccam_body,
  *seccam_cap, *seccam_hinge, *seccam_pipe, *seccam_lens;
    ...
    camera *c)
{
  ...
  count+= draw_component(mi, CAMERA_LENS, (c->state == IDLE ? 0 : color));
    ...
    GLfloat ratio)
{
  ...
  pos.z+= sin(M_PI * ratio * p->frequency * 2) * p->amplitude + p->amplitude / 2;
  return pos;
}
    ...
    ModeInfo *mi)
{
  ...
  p->ratio= 0;
  p->speed= ((4 + frand(4) + ((random() % 10) ? 0 : frand(10))) * (random() & 1 ? 1 : -1) * speed_arg);
    ...
    camera *c)
{
  ...
  if (c->state == IDLE &&
    (c->focus_id <= 0 ? ! (random() % (int) MAX(1, (50 / speed_arg))) : ! (random() % (int) MAX(1, (1000 / speed_arg)))))
    {
