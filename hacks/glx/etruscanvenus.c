...
ModStruct etruscanvenus_description=
  {"etruscanvenus", "init_etruscanvenus", "draw_etruscanvenus", NULL, "draw_etruscanvenus", "change_etruscanvenus", "free_etruscanvenus", &etruscanvenus_opts, 25000, 1, 1, 1, 1.0, 4, "", "Rotate a 3d immersion of a Klein bottle in 3d or walk on it", 0, NULL};
                  ...
                  if (ev->colors != COLORS_ONESIDED && ev->colors != COLORS_TWOSIDED)
                    {
                      ...
                      glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, &ev->col [ 4 * o ]);
                    }
  ...
  int numv)
{
  ...
  if (! ev->buffers_initialized)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, 2 * (numu + 1) * (numv + 1) * sizeof(GLfloat), ev->uv, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);

      glBindBuffer(GL_ARRAY_BUFFER, ev->vertex_t_buffer);
      glBufferData(GL_ARRAY_BUFFER, 2 * (numu + 1) * (numv + 1) * sizeof(GLfloat), ev->tex, GL_STATIC_DRAW);
      glBindBuffer(GL_ARRAY_BUFFER, 0);
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, ev->ni * sizeof(GLuint), ev->indices, GL_STATIC_DRAW);
  ...
  if (ev->projection == DISP_PERSPECTIVE || ev->view == VIEW_WALK)
    ...
    glsl_Orthographic(p_mat, -ev->aspect, ev->aspect, -1.0f, 1.0f, 0.1f, 10.0f);
  else
    glsl_Orthographic(p_mat, -1.0f, 1.0f, -1.0f / ev->aspect, 1.0f / ev->aspect, 0.1f, 10.0f);
  ...
  if (ev->colors != COLORS_ONESIDED && ev->colors != COLORS_TWOSIDED)
    {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, ev->color_buffer);
      if (ev->change_colors)
        glBufferData(GL_ARRAY_BUFFER, 4 * (numu + 1) * (numv + 1) * sizeof(GLfloat), ev->col, GL_STREAM_DRAW);
      ...
      for (i= 0; i < ev->ne; i++)
        {
          ...
          glDrawElements(GL_TRIANGLE_STRIP, ev->nt, GL_UNSIGNED_INT, (const GLvoid *) index_offset);
        }
    }
  else /* ev->display_mode == DISP_WIREFRAME */
    {
      glLineWidth(1.0f);
      index_offset= 0;
      glDrawElements(GL_LINES, ev->ni, GL_UNSIGNED_INT, (const void *) index_offset);
    }
  ...
  ModeInfo *mi)
{
  ...
  if (! glsl_CompileAndLinkShaders(2, vertex_shader_source, 2, fragment_shader_source, &ev->shader_program))
    return;
...
static void display_etruscanvenus(
  ModeInfo *mi)
{
  etruscanvenusstruct *ev= &etruscanvenus [ MI_SCREEN(mi) ];
...
#ifdef HAVE_GLSL
  if (ev->use_shaders)
    mi->polygon_count= etruscan_venus_pf(mi, 0.0, 2.0 * M_PI, 0.0, 2.0 * M_PI, NUMU, NUMV);
  else
#endif /* HAVE_GLSL */
    mi->polygon_count= etruscan_venus_ff(mi, 0.0, 2.0 * M_PI, 0.0, 2.0 * M_PI, NUMU, NUMV);
}


ENTRYPOINT void reshape_etruscanvenus(
  ModeInfo *mi,
  int width,
  int height)
{
...
ENTRYPOINT Bool etruscanvenus_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  etruscanvenusstruct *ev= &etruscanvenus [ MI_SCREEN(mi) ];
  ...
  if (event->xany.type == ButtonPress && event->xbutton.button == Button1)
    {
      ...
      gltrackball_start(ev->trackball, event->xbutton.x, event->xbutton.y, MI_WIDTH(mi), MI_HEIGHT(mi));
      return True;
    }
    ...
    {
      gltrackball_track(ev->trackball, event->xmotion.x, event->xmotion.y, MI_WIDTH(mi), MI_HEIGHT(mi));
      return True;
    }
...
ENTRYPOINT void init_etruscanvenus(
  ModeInfo *mi)
{
  etruscanvenusstruct *ev;
 ...
 *-----------------------------------------------------------------------------
 */
ENTRYPOINT void draw_etruscanvenus(
  ModeInfo *mi)
{
...
#ifndef STANDALONE
ENTRYPOINT void change_etruscanvenus(
  ModeInfo *mi)
{
  etruscanvenusstruct *ev= &etruscanvenus [ MI_SCREEN(mi) ];
...
ENTRYPOINT void free_etruscanvenus(
  ModeInfo *mi)
{
  etruscanvenusstruct *ev= &etruscanvenus [ MI_SCREEN(mi) ];
