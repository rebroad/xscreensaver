  ...
  Bool onealpha)
{
  ...
  if (filename)
    teximage= file_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), filename);
  else
    teximage= image_data_to_ximage(MI_DISPLAY(mi), MI_VISUAL(mi), fn, size);
  if (teximage == NULL)
    {
  ...
  if (rescale)
    {
      ...
      if (gluScaleImage(GL_RGBA, teximage->width, teximage->height, GL_UNSIGNED_BYTE, teximage->data, bx, by, GL_UNSIGNED_BYTE, tmpbuf))
