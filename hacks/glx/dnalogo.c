  ...
  while (th + th_inc <= max_th)
    {
      ...
      if (dc->wall_taper > 0)
        {
          ...
          if (th < dc->wall_taper)
            {
              h2= dc->wall_height / 2 * cos(M_PI / 2 * (1 - (th / dc->wall_taper)));
              if (dc->clockwise)
          ...
          else if (th >= max_th - dc->wall_taper)
            {
              if (th + th_inc > max_th) /* edge case: always come to a point */
                ...
                h2= dc->wall_height / 2 * cos(M_PI / 2 * (1 - ((max_th - th) / dc->wall_taper)));
              if (dc->clockwise)
                h2off= dc->wall_height / 2 - h2;
              else
                h2off= h2 - dc->wall_height / 2;
      ...
      glFrontFace(GL_CCW);
      /* glNormal3f(0, 0, 1);*/
      do_normal(x2, y2, z2 + h2 + h2off, x2b, y2b, z2 + h2 + h2off, x1b, y1b, z1 + h1 + h1off);
       ...
       */
      glFrontFace(GL_CCW);
      do_normal(x1, y1, z1 - h1 + h1off, x1b, y1b, z1 - h1 + h1off, x2b, y2b, z2 - h2 + h2off);
      glBegin(wire ? GL_LINE_LOOP : GL_QUADS);
      glNormal3f(0, 0, -1);
      glVertex3f(x1, y1, z1 - h1 + h1off);
      glVertex3f(x1b, y1b, z1 - h1 + h1off);
  ...
  for (i= 0; i < nbars; i++)
    {
      ...
      polys+= tube(x, y, z, -x, -y, z, dc->bar_thickness, 0, facets, True, True, wire);
      z+= z_inc;
      th+= th_inc;
    }
  return polys;
  ...
  for (i= 0; i < 4; i++)
    {
      ...
      glVertex3f(x [ 2 ], y [ 2 ], dc->frame_depth / 2);
      glVertex3f(x [ 3 ], y [ 2 ], 0);
      glVertex3f(x [ 3 ], y [ 2 ], dc->frame_depth / 2);
      glVertex3f(x [ 4 ], y [ 3 ], 0);
      glVertex3f(x [ 4 ], y [ 3 ], dc->frame_depth / 2);
      glVertex3f(x [ 5 ], y [ 2 ], 0);
      glVertex3f(x [ 5 ], y [ 2 ], dc->frame_depth / 2);
      glVertex3f(x [ 6 ], y [ 2 ], 0);
      glVertex3f(x [ 6 ], y [ 2 ], dc->frame_depth / 2);
      polys+= 4;
      glEnd();
...
#define _GLUfuncptr void(*)(void)
  ...
  for (i= (wire ? 0 : 1); i < endpoints; i++)
    {
      ...
      do_normal(p [ 0 ], p [ 1 ], -thick2, p [ 0 ], p [ 1 ], thick2, x, y, thick2);
      if (! wire)
        {
          glVertex3f(x, y, -thick2);
          glVertex3f(x, y, thick2);
  ...
  do_normal(points [ 0 ], points [ 1 ], -thick2, points [ 0 ], points [ 1 ], thick2, x, y, thick2);
  glVertex3f(x, y, -thick2);
  glVertex3f(x, y, thick2);
      ...
      for (i= 0; i < discfaces; i++)
        {
          GLdouble *p2= p + (i * 3);
          if (i > 0)
            do_normal(p2 [ 0 ], p2 [ 1 ], -thick2, p2 [ 0 ], p2 [ 1 ], thick2, p2 [ -3 ], p2 [ -2 ], thick2);
  ...
  for (i= 0; i < topfaces * 2; i++)
    {
      ...
      if (i == 0 || i == (topfaces * 2) - 1)
        ...
        do_normal(p [ -3 ], p [ -2 ], thick2, p [ 0 ], p [ 1 ], thick2, p [ 0 ], p [ 1 ], -thick2);
      glVertex3f(p [ 0 ], p [ 1 ], -thick2);
      glVertex3f(p [ 0 ], p [ 1 ], thick2);
      polys++;
    }
  ...
  for (letter= -1; letter < letters; letter++)
    {
      if (letter == -1) /* Inner starting point */
      ...
      else /* Add arc for this letter */
        {
          ...
          for (th= th0;
            (th0 < th1 ? th <= th1 + facet_th : th >= th1 + facet_th);
            th+= facet_th)
            {
              GLfloat th2= th;
              if (th0 < th1 && th > th1)
      ...
      for (th= 0; th <= M_PI * 2; th+= facet)
        {
          ...
          if (z < 0)
            do_normal(r * x0, r * y0, thick, r * x0, r * y0, -thick, r * x1, r * y1, -thick);
          else
            do_normal(r * x1, r * y1, thick, r * x1, r * y1, -thick, r * x0, r * y0, -thick);
    ...
    GLfloat anim_ratio)
{
  ...
  for (i= 0; i < dc->codeword_nguides; i++)
    {
      GLfloat ratio;
      if (anim_ratio < i * each)
        ratio= 0;
      else if (anim_ratio >= (i + 1) * each)
        ratio= 1;
      else
        ratio= (anim_ratio - i * each) / each;
      ...
      if (dc->codeword_thickness < dc->codeword_line_width * 1.3 &&
        ...
        {
          polys+= tube(0, 0, -dc->codeword_thickness / 2, 0, 0, dc->codeword_thickness / 2, s, 0, segments, True, True, wire);
        }
    ...
    ModeInfo *mi)
{
  ...
  for (i= 0; i < dc->codeword_path_npoints; i++)
    {
      ...
      XYZ p2= (i < dc->codeword_path_npoints - 1 ? dc->codeword_path [ i + 1 ] : dc->codeword_path [ i - 1 ]);
      XYZ p1a, p1b;
      ...
      for (i= k; i < nquads - 2; i+= 2)
        {
          ...
          norms [ i ]= (k == 0 ? calc_normal(p1a, p1b, p2b) : calc_normal(p2a, p2b, p1a));
        }
    }
      ...
      for (i= k; i < nquads; i+= 2)
        {
          ...
          */
          GLfloat angle= vector_angle(n1.x, n1.y, n1.z, n2.x, n2.y, n2.z);
          GLfloat pointy= 0.8;
  ...
  {
    GLfloat size= 0.95;
    GLfloat r= (anim_ratio > size ? (anim_ratio - size) / (1 - size) : 0);
    ...
    ModeInfo *mi)
{
...

  {
    ...
    dc->anim_state= (dc->mode == HELIX_AND_PIZZA ? ((random() & 1) ? HELIX : PIZZA) : dc->mode);
    dc->anim_ratio= 0;
  }
  ...
  if (dc->speed > 0) /* start off with the gasket in motion */
    {
      ...
      dc->gasket_spinnerz.speed= (0.002 * ((random() & 1) ? 1 : -1) * dc->speed);
    }
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, dc->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &dc->button_down_p))
    {
      return True;
    }
  ...
  if (s->spinning_p)
    {
      ...
      if (s->position >= 1.0 || s->position <= 0.0)
        ...
        {
          s->position_eased= (1 - s->easement * spinner_ease((1 - s->position) / s->easement));
        }
      else
        {
    ...
    ModeInfo *mi)
{
  ...
  glRotatef(current_device_rotation(), 0, 0, 1);
  {
...
#endif
      {
        GLfloat p= (dc->frame_spinner.position_eased >= 0 ? dc->frame_spinner.position_eased : -dc->frame_spinner.position_eased);
    ...
    if (! codeword_p)
#endif
      {
        ...
        scale= ((dc->anim_state == PIZZA_IN || dc->anim_state == HELIX_IN) ? dc->anim_ratio : ((dc->anim_state == PIZZA_OUT || dc->anim_state == HELIX_OUT) ? 1.0 - dc->anim_ratio : 1.0));
  ...
  {
    char s [ 1024 ];
    sprintf(s, "a/z, s/x; per = %0.2f pos = %0.2f", dc->persp_off, dc->pos_off);
    glColor3f(1, 1, 1);
    print_texture_label(dpy, dc->label_font, MI_WIDTH(mi), MI_HEIGHT(mi), 1, s);
  }
#endif
