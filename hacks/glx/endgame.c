...
/* build piece texture */
static void make_piece_texture(
  Chesscreen *cs)
{
  int i, j, c;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, 3, checkImageWidth, checkImageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, &cs->checkImage [ 0 ][ 0 ]);
}

/* build board texture (uniform noise in [180,180+50]) */
static void make_board_texture(
  Chesscreen *cs)
{
  int i, j, c;
...
ENTRYPOINT Bool chess_handle_event(
  ModeInfo *mi,
  XEvent *event)
{
  Chesscreen *cs= &qs [ MI_SCREEN(mi) ];

  if (gltrackball_event_handler(event, cs->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &cs->button_down_p))
    {
      return True;
    }
...
static enum color get_piece_color(
  int piece)
{
  if (piece < BKING)
    return WHITE;
  ...
  for (int i= 0; i < CONCURRENT_MOVES; i++)
    {
      ...
      if ((movestate->mpiece == PAWN && abs(movestate->to [ 0 ]) < 0.01) ||
        ...
        {
          glTranslatef(movestate->from [ 1 ] + cs->steps * movestate->dx, 0.0, movestate->from [ 0 ] + cs->steps * movestate->dz);
          glColor4f(shadow ? MaterialShadow [ 0 ] : cs->colors [ movestate->mpiece / 7 ][ 0 ],
            shadow ? MaterialShadow [ 1 ] : cs->colors [ movestate->mpiece / 7 ][ 1 ],
            shadow ? MaterialShadow [ 2 ] : cs->colors [ movestate->mpiece / 7 ][ 2 ],
        ...
        movestate->castling)
        {
          ...
          glTranslatef(movestate->from [ 1 ] +
            ...
            movestate->from [ 0 ] +
              ease_move(cs->steps * movestate->dz +
                  (movestate->color == WHITE ? offset : -offset),
                movestate->to [ 0 ] - movestate->from [ 0 ]));
        }
      else
        {
  ...
  for (int i= 0; i < CONCURRENT_MOVES; i++)
    {
      ...
      if (movestate->en_passant [ 0 ] != -1)
        glTranslatef(movestate->en_passant [ 1 ], 0.0, movestate->en_passant [ 0 ]);
      else
        glTranslatef(movestate->to [ 1 ], 0.0, movestate->to [ 0 ]);

      if (movestate->tpiece % PIECES == KNIGHT)
        glScalef(1.0 + cs->steps / num_steps_f, 1.0, 1.0 + cs->steps / num_steps_f);
      else
        glScalef(1.0,
          1.0 - cs->steps / num_steps_f / 2 > 0.01 ? 1.0 - cs->steps / num_steps_f : 0.01,
          1.0);
      ...
      for (j= 0; j < BOARDSIZE; ++j)
        {
          ...
          glColor4f(cs->colors [ (i + j) % 2 ][ 0 ], cs->colors [ (i + j) % 2 ][ 1 ], cs->colors [ (i + j) % 2 ][ 2 ], 0.65);
      ...
      if (labels)
        {
          ...
          if (MI_WIDTH(mi) >= 500 && MI_HEIGHT(mi) >= 375)
            ...
            f= cs->font3_data;
          glColor3f(0.8, 0.8, 0);
          print_texture_label(mi->dpy, f, mi->xgwa.width, mi->xgwa.height, 1, cs->game_desc);
        }
    }
}

/** reshape handler */
ENTRYPOINT void reshape_chess(
  ModeInfo *mi,
  int width,
  int height)
{
  ...
  switch (cs->stage)
    {
      case CHOSE_GAME :
        ...
        set_description(cs);
        init_moves(cs);
        glLightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, (double) FADE_FACTOR * TICKS_BET_MOVES);
        glLightf(GL_LIGHT1, GL_CONSTANT_ATTENUATION, (double) FADE_FACTOR * TICKS_BET_MOVES);
        if (cs->abort)
          cs->abort= 0;
        break;
      case FADE_IN :
        glLightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, (double) FADE_FACTOR * TICKS_BET_MOVES / cs->ticks);
        glLightf(GL_LIGHT1, GL_CONSTANT_ATTENUATION, (double) FADE_FACTOR * TICKS_BET_MOVES / cs->ticks);
        if (cs->abort)
          {
            cs->abort= 0;
            cs->stage= FADE_OUT;
