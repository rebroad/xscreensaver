...
/* Normalize a vector. */
static inline void glsl_Normalize(
  float v [ 3 ])
{
  float l;
...
GLboolean glsl_GetGlAndGlslVersions(
  GLint *gl_major,
  GLint *gl_minor,
  GLint *glsl_major,
  GLint *glsl_minor,
...
DONE:
...
#ifndef __OPTIMIZE__
  {
    ...
    if (printed_p)
      ...
      fprintf(stderr, "%s: OpenGL %d.%d, GLSL %d.%d GLES3 = %s\n", progname, *gl_major, *gl_minor, *glsl_major, *glsl_minor, *gl_gles3 ? "yes" : "no");
    printed_p= True;
  }
#endif
...
const GLchar *glsl_GetGLSLVersionString(
  void)
{
...
extern GLboolean glsl_IsCoreProfile(
  void)
{
  GLint gl_major, gl_minor, glsl_major, glsl_minor;
  GLboolean gl_gles3;
...
static void print_compile_error(
  GLuint shader)
{
  GLint length_log;
  GLsizei length;
...
GLboolean glsl_CompileAndLinkShaders(
