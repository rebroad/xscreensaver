    ...
    void *closure)
{
  ...
  */
  ...
  if (debug_p)
    fprintf(stderr, "%s:   loaded %4d x %-4d  %4d x %-4d  \"%s\"\n", progname, frame->geom.width, frame->geom.height, frame->tw, frame->th, (frame->title ? frame->title : "(null)"));
    ...
    ModeInfo *mi)
{
  ...
  if (wire)
  ...
  else
    {
      ...
      if (size <= 10) size= 10;
      load_texture_async(mi->xgwa.screen, mi->window, *ss->glx_context, size, size, mipmap_p, frame->texid, image_loaded_cb, ss);
    }
}
  ...
  if (titles_p)
    {
      ...
      title= xft_word_wrap(MI_DISPLAY(mi), texfont_xft(ss->texfont), title, (w * 2) / tscale);
      token= title;
