    ...
    floater *f)
{
  ...
  if (bp->nfloaters == 1)
    f->scale*= 0.5;
  else if (bp->nfloaters <= 3)
    f->scale*= 0.4;
  else if (bp->nfloaters <= 9)
    f->scale*= 0.3;
  else if (bp->nfloaters <= 15)
    f->scale*= 0.2;
  else if (bp->nfloaters <= 25)
    f->scale*= 0.15;
  else if (bp->nfloaters <= 90)
    f->scale*= 0.12;
  else
    f->scale*= 0.07;
  ...
  switch (which)
    {
      ...
      case SCLERA :
        xstart= 0;
        xstop= xstep * (1 - iris_ratio / 2);
        break;
      case IRIS :
        xstart= xstep * (1 - iris_ratio / 2 * 1.2);
        xstop= xstep;
        break;
      case RETINA :
        xstart= xstep * (1 - iris_ratio / 2 * 1.2);
    ...
    ModeInfo *mi)
{
  ...
  if (! wire)
    {
      ...
      xi= image_data_to_ximage(mi->dpy, mi->xgwa.visual, sclera_png, sizeof(sclera_png));
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, xi->width, xi->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, xi->data);
      check_gl_error("texture");
      XDestroyImage(xi);

      xi= image_data_to_ximage(mi->dpy, mi->xgwa.visual, iris_png, sizeof(iris_png));
  ...
  if (! mode_opt || ! *mode_opt || ! strcasecmp(mode_opt, "random"))
    ...
    {
      fprintf(stderr,
        "%s: mode must be bounce, scroll, random, xeyes or beholder," " not \"%s\"\n",
        progname,
        mode_opt);
      exit(1);
    }
  ...
  if (bp->mode == BEHOLDER)
    {
      if (bp->nfloaters <= 20)
        bp->nfloaters= 20; /* This is lame */
      else if (bp->nfloaters <= 80)
        bp->nfloaters= 80;
      else if (bp->nfloaters <= 320)
        bp->nfloaters= 320;
      else
        bp->nfloaters= 1280;
    }
  ...
  for (i= 0; i < bp->nfloaters; i++)
    {
      floater *f= &bp->floaters [ i ];
      f->idx= i;
      f->rot= make_rotator(10.0, 0, 0, 4, 0.05 * speed, True);
      if (bp->nfloaters == 2)
        {
          f->x= 10 * (i ? 1 : -1);
        }
    ...
    component which)
{
  ...
  get_position(f->rot, &x, &y, &z, which == LENS && ! bp->button_down_p);
  ...
  switch (which)
    {
      ...
      case SCLERA :
        if (! wire)
          {
            GLfloat *c= (f->jaundice == 2 ? c2b : f->jaundice == 1 ? c2c :
                                                                     c2);
            glColor4fv(c);
