...
static const atom_data all_atom_data []= {
  {"H", 1.17, 0.40, "#FFFFFF", "#000000", {
                                            0,
                                          }},
  {"C", 1.75, 0.58, "#999999", "#FFFFFF", {
                                            0,
                                          }},
  {"CA", 1.80, 0.60, "#0000FF", "#ADD8E6", {
                                             0,
                                           }},
  {"N", 1.55, 0.52, "#428DC3", "#EE99FF", {
                                            0,
                                          }},
  {"O", 1.40, 0.47, "#FF0000", "#FFB6C1", {
                                            0,
                                          }},
  {"P", 1.28, 0.43, "#9370DB", "#DB7093", {
                                            0,
                                          }},
  {"S", 1.80, 0.60, "#8B8B00", "#FFFF00", {
                                            0,
                                          }},
  {"bond", 0, 0, "#B3B3B3", "#FFFF00", {
                                         0,
                                       }},
  {"*", 1.40, 0.47, "#008B00", "#90EE90", {
                                            0,
                                          }}};
  ...
  if (gl_color [ 3 ] == 0)
    {
      ...
      if (! XParseColor(mi->dpy, mi->xgwa.colormap, string, &xcolor))
        {
          fprintf(stderr, "%s: unparsable color in %s: %s\n", progname, (a ? a->label : d->name), string);
          exit(1);
        }
      ...
      for (i= 0; i < m->nbonds; i++)
        {
          ...
          if (wire)
          ...
          else
            {
              ...
              polys+= tube(from->x, from->y, from->z, to->x, to->y, to->z, thickness, cap_size, faces, smooth, cap_p, wire);
            }
        }
    }
  ...
  while (*s)
    {
      if ((! m->label || ! *m->label) &&
        ...
        {
          int atoms [ 11 ];
          int i= sscanf(s + 8, " %d %d %d %d %d %d %d %d %d %d %d %d ", &atoms [ 0 ], &atoms [ 1 ], &atoms [ 2 ], &atoms [ 3 ], &atoms [ 4 ], &atoms [ 5 ], &atoms [ 6 ], &atoms [ 7 ], &atoms [ 8 ], &atoms [ 9 ], &atoms [ 10 ], &atoms [ 11 ]);
          int j;
          for (j= 1; j < i; j++)
            {
              if (atoms [ j ] > 0)
    ...
    char *f)
{
  if (! strcmp(f, "H[2]Be"))
    strcpy(f, "BeH[2]");
  else if (! strcmp(f, "H[3]B"))
    strcpy(f, "BH[3]");
  else if (! strcmp(f, "H[3]N"))
    strcpy(f, "NH[3]");
  else if (! strcmp(f, "CHN"))
    strcpy(f, "HCN");
  else if (! strcmp(f, "CKN"))
    strcpy(f, "KCN");
  else if (! strcmp(f, "H[4]N[2]"))
    strcpy(f, "N[2]H[4]");
  else if (! strcmp(f, "Cl[3]P"))
    strcpy(f, "PCl[3]");
  else if (! strcmp(f, "Cl[5]P"))
    strcpy(f, "PCl[5]");
}
    ...
    XEvent *event)
{
  ...
  if (gltrackball_event_handler(event, mc->trackball, MI_WIDTH(mi), MI_HEIGHT(mi), &mc->button_down_p))
    {
      return True;
    }
  else
    ...
    ModeInfo *mi)
{
  ...
  mc->no_label_threshold= get_float_resource(mi->dpy, "noLabelThreshold", "NoLabelThreshold");
  mc->wireframe_threshold= get_float_resource(mi->dpy, "wireframeThreshold", "WireframeThreshold");
  mc->mode= 0;
    ...
    ModeInfo *mi)
{
  ...
  if (mc->mode != 0)
    {
      GLfloat s= (mc->mode == 1 ? mc->mode_tick / (80 / speed) : ((80 / speed) - mc->mode_tick + 1) / (80 / speed));
      glScalef(s, s, s);
    }
