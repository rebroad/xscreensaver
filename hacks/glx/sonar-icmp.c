    ...
    Bool resolve_p)
{
  ...
  if (4 == sscanf(sb->name, " %u.%u.%u.%u %c", &ip [ 0 ], &ip [ 1 ], &ip [ 2 ], &ip [ 3 ], &c))
    {
      /* It's an IP address.
       */
    ...
    socklen_t addrlen)
{
  ...
  if (! addr->sa_family)
    ...
    {
#ifdef HAVE_GETADDRINFO
      int gai_error= getnameinfo(sockaddr, addrlen, buf, sizeof(buf), NULL, 0, NI_NUMERICHOST);
      ...
      for (ifa= all; ifa; ifa= ifa->ifa_next)
        {
          ...
          mask= ntohl(((struct sockaddr_in *) ifa->ifa_netmask)
              ->sin_addr.s_addr);
          if (pd->debug_p)
            fprintf(stderr, "%s:     if: %4s: inet = %s /%d 0x%08lx\n", progname, ifa->ifa_name, inet_ntoa(in2), mask_width(mask), mask);
    ...
    const sonar_bogie *b)
{
  ...
  *(socklen_t *) host_id= pb->addrlen;
  ...
  if (sendto(pd->icmpsock, packet, pcktsiz, 0, (struct sockaddr *) &pb->address, sizeof(pb->address)) != pcktsiz)
    {
#if 0
      char buf[BUFSIZ];
  ...
  while (! timer_expired)
    {
      ...
      if (select(pd->icmpsock + 1, &rfds, 0, 0, &tv) > 0)
        {
          result= (int) recvfrom(pd->icmpsock, packet, sizeof(packet), 0, &from, &fromlen);
           ...
           */
          {
            const socklen_t *host_id= (socklen_t *) &packet [ iphdrlen + sizeof(struct ICMP) + sizeof(struct timeval) ];
                          ...
                          ! memcmp(&pb->address, host_id + 1, pb->addrlen))
                          {
                            ...
                            if (pb->lookup_name &&
                              async_name_from_addr_is_done(pb->lookup_name))
                              {
                                ...
                                if (pd->debug_p > 1)
                                  fprintf(stderr, "%s:   %s => %s\n", progname, b->name, host ? host : "<unknown>");
          ...
          if (! new) /* not in targets? */
            {
              ...
              fprintf(stderr,
                "%s: UNEXPECTED PING REPLY! " "%4d bytes, icmp_seq=%-4d from %d.%d.%d.%d\n",
                progname,
                result,
                ICMP_SEQ(icmph),
                a,
      ...
      if (! *sbp)
      ...
      else
        {
          ...
          if (pb->lookup_addr &&
            ...
            {
              if (async_addr_from_name_finish(pb->lookup_addr, &pb->address, &pb->addrlen, NULL))
                {
                  ...
                  if (fallback)
                    {
                      sonar_bogie *new_bogie= bogie_for_host(ssd, fallback, NULL);
                      if (new_bogie)
                        {
                          new_bogie->next= *sbp;
...
#ifdef HAVE_LIBCAP
          *error_ret= strdup(
            "Sonar must be setuid or libcap to ping!\n" "Running simulation instead.");
#else
          *error_ret= strdup(
            "Sonar must be setuid to ping!\n" "Running simulation instead.");
#endif
          return 0;
        }
