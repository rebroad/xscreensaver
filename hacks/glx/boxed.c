  ...
  for (b= 0; b < bman->num_balls; b++)
    {
      ...
      /* boundary check */
      if (bman->balls [ b ].loc.y < bman->balls [ b ].radius)
        { /* ball onder bodem? (bodem @ y=0) */
      ...
      if (! bman->balls [ b ].offside)
        {
          if (bman->balls [ b ].loc.x - bman->balls [ b ].radius < -20.0f)
            { /* x ondergrens */
              if (bman->balls [ b ].loc.y > 41 + bman->balls [ b ].radius)
                {
                  bman->balls [ b ].offside= 1;
                }
              else
                {
          ...
          if (bman->balls [ b ].loc.z - bman->balls [ b ].radius < -20.0f)
            { /* z ondergrens */
              if (bman->balls [ b ].loc.y > 41 + bman->balls [ b ].radius)
                {
                  bman->balls [ b ].offside= 1;
...
static void createtrisfromball(
  triman *tman,
  vectorf *spherev,
  GLint *spherei,
  int ind_num,
    ...
    ModeInfo *mi)
{
  ...
  if (gp->config.decay <= 0.8182)
    gp->config.decay= gp->config.decay / 3;
  else
    gp->config.decay= (gp->config.decay - 0.75) * 4;
...
#else
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 256, 256, 0, GL_RGB, GL_UNSIGNED_BYTE, gp->tex1);
