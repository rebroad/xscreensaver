  ...
  struct state *st)
{
  ...
  m->atX= (random() % (xgwa.width <= m->width ? 1 : xgwa.width - m->width));
  m->atY= (random() % (xgwa.height <= m->width ? 1 : xgwa.height - m->width));

  /* wrap-around by these values; no need to % as we end up doing that
     later anyway */
  m->kX= ((random() % 2) ? (random() % m->width) : 0);
  m->kT= ((random() % 2) ? (random() % m->width) : 0);
  m->kY= ((random() % 2) ? (random() % m->width) : 0);
