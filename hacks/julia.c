...
#else
      jp->cr= 1.5 * (sin(M_PI * (jp->inc / 290.0)) * sin(jp->inc * M_PI / 210.0));
      jp->ci= 1.5 * (cos(M_PI * (jp->inc / 310.0)) * cos(jp->inc * M_PI / 190.0));
    ...
    ModeInfo *mi)
{
  ...
  if (jp->button_down_p && ! jp->cursor && ! jp->cursor)
    {
      ...
      bit= XCreatePixmapFromBitmapData(display, window, "\000", 1, 1, MI_WIN_BLACK_PIXEL(mi), MI_WIN_BLACK_PIXEL(mi), 1);
      jp->cursor= XCreatePixmapCursor(display, bit, bit, &black, &black, 0, 0);
  ...
  if (jp->pixmap == None)
    {
      ...
      XFillRectangle(display, jp->pixmap, bg_gc, 0, 0, jp->circsize, jp->circsize);
      if (jp->circsize < 2)
        XFillRectangle(display, jp->pixmap, fg_gc, 0, 0, jp->scale, jp->scale);
      else
        XFillArc(display, jp->pixmap, fg_gc, 0, 0, jp->circsize, jp->circsize, 0, 23040);
      if (fg_gc != None)
  ...
  if (! jp->stippledGC)
    {
      ...
      if ((jp->stippledGC= XCreateGC(display, window, GCForeground | GCBackground, &gcv)) == None)
        return;
    }
  if (MI_NPIXELS(mi) > 2)
  ...
  for (i= 0; i < jp->nbuffers; ++i)
    if (jp->pointBuffer [ i ])
      memset(jp->pointBuffer [ i ], 0, numpoints * sizeof(**jp->pointBuffer));
    else
    ...
    ModeInfo *mi)
{
  ...
  XFillArc(display, window, gc, old_circle.x - jp->circsize / 2 - 2, old_circle.y - jp->circsize / 2 - 2, jp->circsize + 4, jp->circsize + 4, 0, 360 * 64);
...
#endif /* HAVE_JWXYZ */
  XDrawArc(display, window, jp->stippledGC, new_circle.x - jp->circsize / 2, new_circle.y - jp->circsize / 2, jp->circsize, jp->circsize, 0, 360 * 64);

  if (jp->erase == 1)
    XFillRectangles(display, window, gc, jp->pointBuffer [ jp->buffer ], numpoints);
  jp->inc++;
  if (MI_NPIXELS(mi) > 2)
    {
      ...
      for (i= 0; i < REDRAWSTEP; i++)
        {
          ...
          XFillRectangles(display, window, gc, jp->pointBuffer [ j ], numpoints);
