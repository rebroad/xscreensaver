          ...
          if (pp->ghosts [ ghost ].aistate == hiding)
            {
              ...
              XFillRectangle(display, window, pp->stippledGC, pp->ghosts [ ghost ].cf, pp->ghosts [ ghost ].rf, pp->spritexs, pp->spriteys);
            }
          /* DIE PACMAN... */
          else
            {
...
static void
  draw_dot(
    ...
    int (*arc_func)(Display *display, Drawable d, GC gc, int x, int y, unsigned int width, unsigned int height, int angle1, int angle2))
{
  Display *display= MI_DISPLAY(mi);
  Window window= MI_WINDOW(mi);
  int w, h;
  ...
  width_height(mi, &w, &h);
  (void) arc_func(display, window, pp->stippledGC, x, y, w, h, 0, 23040);
}
...
#endif /* !HAVE_JWXYZ */
  XSetLineAttributes(display, pp->stippledGC, pp->wallwidth, LineSolid, CapRound, JoinMiter);
      ...
      switch (pp->level [ y * LEVWIDTH + x ])
        {
          ...
          case '.' :
            setdotcolor(mi);
            (void) XDrawPoint(display, window, pp->stippledGC, x * pp->xs + pp->xb, y * pp->ys + pp->yb);
            break;
          default :
            setwallcolor(mi);
            (void) XDrawPoint(display, window, pp->stippledGC, x * pp->xs + pp->xb, y * pp->ys + pp->yb);
  ...
  switch (pp->level [ y * LEVWIDTH + x ])
    {
      ...
      case '.' :
        ...
        if (pp->xs < 8 || pp->ys < 8)
          {
            (void) XDrawPoint(display, window, pp->stippledGC, x * pp->xs + pp->xb + pp->xs / 2, y * pp->ys + pp->yb + pp->ys / 2);
            break;
          }
      ...
      case '-' :
        setwallcolor(mi);
        (void) XDrawLine(display, window, pp->stippledGC, (pp->xs * x) + pp->xb, (pp->ys * y) + (pp->ys / 2) + pp->yb, (pp->xs * (x + 1)) + pp->xb, (pp->ys * y) + (pp->ys / 2) + pp->yb);
        break;

      case '|' :
        setwallcolor(mi);
        (void) XDrawLine(display, window, pp->stippledGC, (pp->xs * x) + (pp->xs / 2) + pp->xb, (pp->ys * y) + pp->yb, (pp->xs * x) + (pp->xs / 2) + pp->xb, (pp->ys * (y + 1)) + pp->yb);
        break;

      case '_' :
        setwallcolor(mi);
        (void) XDrawArc(display, window, pp->stippledGC, (pp->xs * x) - (pp->ys / 2) + pp->xb + dx, (pp->ys * y) + (pp->ys / 2) + pp->yb, pp->xs, pp->ys, 0 * 64, 90 * 64);
        break;

      case ',' :
        setwallcolor(mi);
        (void) XDrawArc(display, window, pp->stippledGC, (pp->xs * x) + (pp->ys / 2) + pp->xb, (pp->ys * y) + (pp->ys / 2) + pp->yb, pp->xs, pp->ys, 90 * 64, 90 * 64);
        break;

      case '`' :
        setwallcolor(mi);
        (void) XDrawArc(display, window, pp->stippledGC, (pp->xs * x) + (pp->ys / 2) + pp->xb, (pp->ys * y) - (pp->ys / 2) + pp->yb + dy, pp->xs, pp->ys, 180 * 64, 90 * 64);
        break;

      case '\'' :
        setwallcolor(mi);
        (void) XDrawArc(display, window, pp->stippledGC, (pp->xs * x) - (pp->ys / 2) + pp->xb + dx, (pp->ys * y) - (pp->ys / 2) + pp->yb + dy, pp->xs, pp->ys, 270 * 64, 90 * 64);
        break;
    }
}
...
#define MAX_DEATH_DELAY 20
  ...
  if (pp->pacman.aistate == ps_dieing)
    ...
    {
      pp->pacman.cf= pp->pacman.col * pp->xs + pp->pacman.delta.x * pp->pacman.cfactor + pp->xb + pp->spritedx;
      pp->pacman.rf= pp->pacman.row * pp->ys + pp->pacman.delta.y * pp->pacman.rfactor + pp->yb + pp->spritedy;
    }

  dir= (ABS(pp->pacman.cfactor) * (2 - pp->pacman.cfactor) +
         ABS(pp->pacman.rfactor) * (1 + pp->pacman.rfactor)) %
    4;
  ...
  XSetClipOrigin(display, pp->stippledGC, pp->pacman.oldcf, pp->pacman.oldrf);
  XFillRectangle(display, window, pp->stippledGC, pp->pacman.oldcf, pp->pacman.oldrf, pp->spritexs, pp->spriteys);
  XSetClipMask(display, pp->stippledGC, new_mask);
  XSetClipOrigin(display, pp->stippledGC, pp->pacman.cf, pp->pacman.rf);
  XCopyArea(display, pacman, window, pp->stippledGC, 0, 0, pp->spritexs, pp->spriteys, pp->pacman.cf, pp->pacman.rf);
...
#define MAX_WAG_COUNT 50
  ...
  dir= (ABS(pp->ghosts [ ghost ].cfactor) * (2 - pp->ghosts [ ghost ].cfactor) +
         ABS(pp->ghosts [ ghost ].rfactor) * (1 + pp->ghosts [ ghost ].rfactor)) %
    4;
  ...
  pp->ghosts [ ghost ].cf=
    pp->ghosts [ ghost ].col * pp->xs + pp->ghosts [ ghost ].delta.x * pp->ghosts [ ghost ].cfactor + pp->xb + pp->spritedx;
  pp->ghosts [ ghost ].rf=
    pp->ghosts [ ghost ].row * pp->ys + pp->ghosts [ ghost ].delta.y * pp->ghosts [ ghost ].rfactor + pp->yb + pp->spritedy;

  XSetForeground(display, pp->stippledGC, MI_BLACK_PIXEL(mi));

  XSetClipMask(display, pp->stippledGC, pp->ghostMask);
  XSetClipOrigin(display, pp->stippledGC, pp->ghosts [ ghost ].oldcf, pp->ghosts [ ghost ].oldrf);
  XFillRectangle(display,
    window,
    pp->stippledGC,
  ...
  if (pp->pacman.aistate != ps_dieing)
    {
      drawlevelblock(mi, pp, (unsigned int) pp->ghosts [ ghost ].col, (unsigned int) pp->ghosts [ ghost ].row);



      XSetClipOrigin(display, pp->stippledGC, pp->ghosts [ ghost ].cf, pp->ghosts [ ghost ].rf);

      XCopyArea(display, g_pix, window, pp->stippledGC, 0, 0, pp->spritexs, pp->spriteys, pp->ghosts [ ghost ].cf, pp->ghosts [ ghost ].rf);
    }
  XSetClipMask(display, pp->stippledGC, None);
    ...
    int y)
{
  ...
  for (ghost= 0; ghost < pp->nghosts; ghost++)
    {
      if ((pp->ghosts [ ghost ].cf <= x && x <= pp->ghosts [ ghost ].cf + pp->spritexs) && (pp->ghosts [ ghost ].rf <= y && y <= pp->ghosts [ ghost ].rf + pp->spriteys))
        {
          ret= True;
          break;
        }
    ...
    pacmangamestruct **ps)
{
  ...
  sprites= image_data_to_pixmap(display, window, pacman_png, sizeof(pacman_png), &sw, &sh, &sprites_mask);
  ...
  pp->ghostMask= subpixmap(display, window, sprites_mask, sw, sw, srcy, 1);
  pp->ghostMask= scale_pixmap(&display, gc, pp->ghostMask, pp->spritexs, pp->spriteys);
          ...
          for (k= 0; k < MAXGWAG; k++)
            {
              ...
              pp->ghostPixmap [ i ][ j ][ k ]=
                scale_pixmap(&display, pp->stippledGC, pp->ghostPixmap [ i ][ j ][ k ], pp->spritexs, pp->spriteys);
              /* m++; */
              srcy+= sw;
              if (srcy >= sh) abort();
            }
      ...
      for (j= 0; j < MAXGWAG; j++)
        {
          pp->s_ghostPixmap [ i ][ j ]=
            subpixmap(display, window, sprites, sw, sw, srcy, xgwa.depth);
          /* m++; */
          pp->s_ghostPixmap [ i ][ j ]= scale_pixmap(&display, pp->stippledGC, pp->s_ghostPixmap [ i ][ j ], pp->spritexs, pp->spriteys);
          srcy+= sw;
          if (srcy >= sh) abort();
        }
    }
  ...
  for (i= 0; i < MAXGDIR; i++)
    {
      pp->ghostEyes [ i ]=
        subpixmap(display, window, sprites, sw, sw, srcy, xgwa.depth);
      pp->ghostEyes [ i ]= scale_pixmap(&display, pp->stippledGC, pp->ghostEyes [ i ], pp->spritexs, pp->spriteys);
      srcy+= sw;
      if (srcy >= sh) abort();
    }
      ...
      for (j= 0; j < MAXMOUTH; j++)
        {
          ...
          /* m++; */
          pp->pacmanPixmap [ i ][ j ]= scale_pixmap(&display, pp->stippledGC, pp->pacmanPixmap [ i ][ j ], pp->spritexs, pp->spriteys);
          pp->pacmanMask [ i ][ j ]=
            scale_pixmap(&display, gc, pp->pacmanMask [ i ][ j ], pp->spritexs, pp->spriteys);
          srcy+= sw;
          if (srcy >= sh) abort();
        }
    }
  ...
  for (i= 0; i < PAC_DEATH_FRAMES; i++)
    {
      ...
      pp->pacman_ds [ i ]= scale_pixmap(&display, pp->stippledGC, pp->pacman_ds [ i ], pp->spritexs, pp->spriteys);
      pp->pacman_ds_mask [ i ]=
        scale_pixmap(&display, gc, pp->pacman_ds_mask [ i ], pp->spritexs, pp->spriteys);
      srcy+= sw;
    }
  XFreeGC(*dpy, gc);
  ...
  if (size == 0 ||
    ...
    MINGRIDSIZE * size > (int) pp->height)
    {
      ...
      if (pp->width > pp->height * 5 || /* weird window aspect ratio */
        ...
        scale= 0.8 * (pp->width / pp->height ? pp->width / (double) pp->height : pp->height / (double) pp->width);
      pp->ys= MAX(scale, 1);
      pp->xs= pp->ys;
    }
  else
    {
      if (size < -MINSIZE)
        pp->ys= (short) (NRAND(MIN(-size, MAX(MINSIZE, MIN(pp->width, pp->height) / MINGRIDSIZE)) - MINSIZE + 1) + MINSIZE);
  ...
  if (! pp->stippledGC)
    {
      ...
      if ((pp->stippledGC= XCreateGC(display, window, GCForeground | GCBackground, &gcv)) == None)
        {
          free_pacman(mi);
          return;
        }
