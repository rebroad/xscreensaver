...
static void updateDisplayPixel(
  machine_6502 *machine,
  Bit16 addr)
{
  ...
  m6502_AddrMode adm)
{
  ...
  if (isValue)
    {
      machine->regP=
        setBit(machine->regP, CARRY_FL, bitOn(ptr.value, CARRY_FL));
      ptr.value= ptr.value >> 1;
    ...
    { /* Accumulator */
      machine->regP=
        setBit(machine->regP, CARRY_FL, bitOn(machine->regA, CARRY_FL));
      machine->regA= machine->regA >> 1;
  ...
  if (isValue)
    {
      cf= bitOn(machine->regP, CARRY_FL);
      machine->regP=
        setBit(machine->regP, CARRY_FL, bitOn(ptr.value, NEGATIVE_FL));
      ptr.value= ptr.value << 1;
    ...
    { /* Implied */
      cf= bitOn(machine->regP, CARRY_FL);
      machine->regP=
        setBit(machine->regP, CARRY_FL, bitOn(machine->regA, NEGATIVE_FL));
      machine->regA= machine->regA << 1;
...
/* OPCODES */
static void assignOpCodes(
  m6502_Opcodes *opcodes)
{
...
static BOOL changeParamLabelAddr(
  AsmLine *asmline,
  void *label)
{
  Label *la= label;
  ...
  FILE *output)
{
  ...
  fprintf(output, "\n   NVFBDIZC\nP: %d%d%d%d%d%d%d%d ", bitOn(machine->regP, NEGATIVE_FL), bitOn(machine->regP, OVERFLOW_FL), bitOn(machine->regP, FUTURE_FL), bitOn(machine->regP, BREAK_FL), bitOn(machine->regP, DECIMAL_FL), bitOn(machine->regP, INTERRUPT_FL), bitOn(machine->regP, ZERO_FL), bitOn(machine->regP, CARRY_FL));
