    ...
    Window window)
{
  ...
  if (mono_p)
  ...
  else
    {
      ...
      make_uniform_colormap(xgwa.screen, xgwa.visual, cmap, st->colors, &st->ncolors, True, &writeable, False);
      if (st->ncolors <= 0)
        {
          st->ncolors= 1;
          st->colors [ 0 ].pixel= get_pixel_resource(st->dpy, cmap, "foreground", "Foreground");
    ...
    void *closure)
{
  ...
  if (st->inclear < st->height)
    {
      XFillRectangle(st->dpy, st->window, st->erase_gc, 0, st->inclear * st->scale, (st->width - 1) * st->scale, st->scale);
      memset(&st->fill [ st->inclear * st->width ], 0, sizeof(int) * st->width);
      XFillRectangle(st->dpy, st->window, st->erase_gc, 0, (st->height - st->inclear - 1) * st->scale, (st->width - 1) * st->scale, st->scale);
