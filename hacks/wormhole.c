...
static void destroyColorChanger(
  struct state *st,
  color_changer *ch)
{
  int q;
...
static XColor *getColorShade(
  color_changer *ch)
{
  return ch->shade + ch->min;
}

static void drawWormhole(
  struct state *st,
  wormhole *worm)
{
