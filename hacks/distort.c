    ...
    struct state *st)
{
  ...
  if (st->effect == NULL && st->radius == 0 && st->speed == 0 && st->number == 0 && ! st->blackhole && ! st->vortex && ! st->magnify && ! st->reflect)
    {
   ...
   */
  {
    int max= (st->xgwa.width > st->xgwa.height ? st->xgwa.height : st->xgwa.width) * 2 / (5 * st->number);
    ...
    Window window)
{
  ...
  st->pm= XCreatePixmap(st->dpy, st->window, st->xgwa.width, st->xgwa.height, st->xgwa.depth);

  st->img_loader= load_image_async_simple(0, st->xgwa.screen, st->window, st->pm, 0, 0);
    ...
    struct state *st)
{
  ...
  XCopyArea(st->dpy, st->pm, st->window, st->gc, 0, 0, st->xgwa.width, st->xgwa.height, 0, 0);
  if (st->orig_map) XDestroyImage(st->orig_map);
  st->orig_map= XGetImage(st->dpy, st->pm, 0, 0, st->xgwa.width, st->xgwa.height, ~0L, ZPixmap);
  ...
  st->buffer_map= create_xshm_image(st->dpy, st->xgwa.visual, st->orig_map->depth, ZPixmap, &st->shm_info, 2 * st->radius + st->speed + 2, 2 * st->radius + st->speed + 2);

  if ((st->buffer_map->byte_order == st->orig_map->byte_order) && (st->buffer_map->depth == st->orig_map->depth) && (st->buffer_map->format == ZPixmap) && (st->orig_map->format == ZPixmap) && ! st->slow)
    {
  ...
  for (i= 0; i < 2 * st->radius + st->speed + 2; i++)
    {
      for (j= 0; j < 2 * st->radius + st->speed + 2; j++)
        {
          *(p + i + j * st->buffer_map->bytes_per_line / st->bpp_size)= st->from [ i ][ j ][ 0 ] + st->xgwa.width * st->from [ i ][ j ][ 1 ];
          if (*(p + i + j * st->buffer_map->bytes_per_line / st->bpp_size) < 0 || *(p + i + j * st->buffer_map->bytes_per_line / st->bpp_size) >= st->orig_map->height * st->orig_map->width)
  ...
  int *distort_matrix)
{
  ...
  while (u < (CARD8 *) (dest->data + sizeof(CARD8) * dest->height * dest->bytes_per_line / sizeof(CARD8)))
    *u++= t [ *distort_matrix++ ];
}
  ...
  int *distort_matrix)
{
  ...
  while (u < (CARD16 *) (dest->data + sizeof(CARD16) * dest->height * dest->bytes_per_line / sizeof(CARD16)))
    *u++= t [ *distort_matrix++ ];
}

static void fast_draw_32(
  struct state *st,
  XImage *src,
  XImage *dest,
  int x,
  ...
  int *distort_matrix)
{
  ...
  while (u < (CARD32 *) (dest->data + sizeof(CARD32) * dest->height * dest->bytes_per_line / sizeof(CARD32)))
    *u++= t [ *distort_matrix++ ];
}

static void generic_draw(
  struct state *st,
  XImage *src,
  XImage *dest,
  int x,
  ...
  for (i= 0; i < dest->width; i++)
    {
      for (j= 0; j < dest->height; j++)
        if (st->from [ i ][ j ][ 0 ] + x >= 0 &&
          ...
          XPutPixel(dest, i, j, XGetPixel(src, st->from [ i ][ j ][ 0 ] + x, st->from [ i ][ j ][ 1 ] + y));
    }
}
  ...
  int k)
{
  ...
  put_xshm_image(st->dpy, st->window, st->gc, st->buffer_map, 0, 0, st->xy_coo [ k ].x, st->xy_coo [ k ].y, 2 * st->radius + st->speed + 2, 2 * st->radius + st->speed + 2, &st->shm_info);
}
      ...
      for (j= 0; j < 2 * st->radius + st->speed + 2; j++)
        {
          ...
          if (dist > rsq ||
            ...
            {
              XPutPixel(st->buffer_map, j, i, XGetPixel(st->orig_map, st->xy_coo [ k ].x + j, ny));
            }
          else if (dist == 0)
            {
          ...
          else
            {
              ...
              if (x < 0 || x >= st->xgwa.width ||
                ...
                XPutPixel(st->buffer_map, j, i, XGetPixel(st->orig_map, x, y));
            }
        }
    }

  XPutImage(st->dpy, st->window, st->gc, st->buffer_map, 0, 0, st->xy_coo [ k ].x, st->xy_coo [ k ].y, 2 * st->radius + st->speed + 2, 2 * st->radius + st->speed + 2);
}
  ...
  for (i= 0; i < st->number; i++)
    {
      if (i != k)
        {
          if ((abs(st->xy_coo [ k ].x - st->xy_coo [ i ].x) <= 2 * st->radius + st->speed + 2) && (abs(st->xy_coo [ k ].y - st->xy_coo [ i ].y) <= 2 * st->radius + st->speed + 2))
            {
              st->xy_coo [ k ].x= (random() % (st->xgwa.width - 2 * st->radius));
              st->xy_coo [ k ].y= (random() % (st->xgwa.height - 2 * st->radius));
              i= -1; /* ugly */
  ...
  for (i= 0; i < st->number; i++)
    {
      if ((i != k)
        ...
        && ((st->xy_coo [ k ].x - st->xy_coo [ i ].x) * (st->xy_coo [ k ].x - st->xy_coo [ i ].x) + (st->xy_coo [ k ].y - st->xy_coo [ i ].y) * (st->xy_coo [ k ].y - st->xy_coo [ i ].y) <= 2 * st->radius * 2 * st->radius))
        {
    ...
    unsigned int h)
{
  ...
  if (st->orig_map) /* created in distort_finish_loading, might be early */
    XPutImage(st->dpy, st->window, st->gc, st->orig_map, 0, 0, st->orig_map->width, st->orig_map->height, 0, 0);
}
