          ...
          if (ret >= 0)
            {
              ...
              if (! swr_is_initialized(swr_ctx))
                {
                  av_check(swr_config_frame(swr_ctx, audio_ost->frame, audio_ist->frame));
    ...
    Bool fast_p)
{
  ...
  if (audiofile)
    {
      ...
      av_check(avformat_open_input(&ffst->audio_fmt_ctx, audiofile, NULL, NULL));
      ...
      */
      ...
      av_check(avcodec_open2(ffst->audio_ist.ctx, ffst->audio_ist.codec, NULL));

      add_stream(&ffst->audio_ost, ffst->oc, audio_codec);

      sample_fmts= get_sample_formats(&ffst->audio_ost);
      ffst->audio_ost.ctx->sample_fmt=
        (sample_fmts && sample_fmts [ 0 ] != AV_SAMPLE_FMT_NONE ? sample_fmts [ 0 ] : AV_SAMPLE_FMT_FLTP);
      ...
      if (! sample_rates || sample_rates [ 0 ] == 0)
      ...
      else
        {
          ...
          while (*r)
            {
              if ((*r >= ffst->audio_ist.ctx->sample_rate) ? *r < best : *r > best)
                best= *r;
              ++r;
            }
      ...
      ffst->audio_ost.frame->nb_samples=
        (ffst->audio_ost.ctx->codec->capabilities &
            ...
            ((av_rescale(ffst->audio_ist.ctx->frame_size,
                ...
                ffst->audio_ist.ctx->sample_rate) +
               15) &
              ~(int64_t) 15) :
  ...
  ffst->sws_ctx= sws_getContext(output_width, output_height, (bpp == 4 ? AV_PIX_FMT_BGR32 : AV_PIX_FMT_BGR24), ffst->video_ost.ctx->width, ffst->video_ost.ctx->height, ffst->video_ost.ctx->pix_fmt, SWS_BICUBIC, NULL, NULL, NULL);
  ...
  XImage *img)
{
  ...
  if (ffst->audiofile)
    {
      while (av_compare_ts(ffst->frames_written,
               ...
               ffst->audio_ost.ctx->time_base) > 0)
        {
          ...
          get_audio_frame(ffst->audio_fmt_ctx, &ffst->audio_ist, ffst->audio_pkt, ffst->swr_ctx, &ffst->audio_ost);
  ...
  sws_scale(ffst->sws_ctx, &img_data, &img->bytes_per_line, 0, ffst->video_ost.frame->height, ffst->video_ost.frame->data, ffst->video_ost.frame->linesize);
