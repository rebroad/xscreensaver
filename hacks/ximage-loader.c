  ...
  if (filename)
    {
      ...
      ximage= XGetImage(dpy, pixmap, geom.x, geom.y, geom.width, geom.height, ~0L, ZPixmap);
      if (! ximage) abort();
    ...
    unsigned long data_size)
{
...

  {
    ...
    image= XCreateImage(dpy, visual, 32, ZPixmap, 0, 0, width, height, 32, 0);
    ...
    Pixmap *mask_ret)
{
  ...
  out= XCreateImage(dpy, xgwa.visual, xgwa.depth, ZPixmap, 0, 0, in->width, in->height, 8, 0);
  ...
  if (mask_ret)
    {
      mask= XCreateImage(dpy, xgwa.visual, 1, XYPixmap, 0, 0, in->width, in->height, 8, 0);
...
Pixmap
  image_data_to_pixmap(
    ...
    Pixmap *mask_ret)
{
  return make_pixmap(dpy, window, 0, image_data, data_size, width_ret, height_ret, mask_ret);
}
...
Pixmap
  file_to_pixmap(
    ...
    Pixmap *mask_ret)
{
  return make_pixmap(dpy, window, filename, 0, 0, width_ret, height_ret, mask_ret);
}
