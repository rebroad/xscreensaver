  ...
  struct state *st)
{
  ...
  put_xshm_image(st->dpy, st->window, st->gc, st->pImage, 0, 0, 0, 0, st->iWinWidth, st->iWinHeight, &st->shm_info);
}
  ...
  XWindowAttributes *XWinAttribs)
{
  ...
  st->pImage= create_xshm_image(st->dpy, XWinAttribs->visual, XWinAttribs->depth, ZPixmap, &st->shm_info, XWinAttribs->width, XWinAttribs->height);
