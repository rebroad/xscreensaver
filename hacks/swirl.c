    ...
    SWIRL_P swirl)
{
  ...
  swirl->ximage= create_xshm_image(dpy, swirl->visual, swirl->rdepth, ZPixmap, &swirl->shm_info, swirl->width, swirl->height);
}
    ...
    XColor *values)
{
  ...
  interpolate(swirl, &value, &pixel, (int) ((double) swirl->dcolours * ((double) L1 / (double) L)) + 1, c, c + 1);
  interpolate(swirl, &value, &pixel, (int) ((double) swirl->dcolours * ((double) L2 / (double) L)) + 1, c + 1, c + 2);
  interpolate(swirl, &value, &pixel, (int) ((double) swirl->dcolours * ((double) L3 / (double) L)) + 1, c + 2, c);
    ...
    SWIRL_P swirl)
{
  ...
  swirl->fixed_colourmap= ! setupColormap(mi, &(swirl->colours), &truecolor, &redmask, &greenmask, &bluemask);
      ...
      for (i= 0; i < swirl->colours; i++)
        swirl->rgb_values [ i ].pixel=
          ((rsh > 0 ? (swirl->rgb_values [ i ].red) >> rsh :
                      (swirl->rgb_values [ i ].red) << (-rsh)) &
            redmask) |
          ((gsh > 0 ? (swirl->rgb_values [ i ].green) >> gsh :
                      (swirl->rgb_values [ i ].green) << (-gsh)) &
            greenmask) |
          ((bsh > 0 ? (swirl->rgb_values [ i ].blue) >> bsh :
                      (swirl->rgb_values [ i ].blue) << (-bsh)) &
            bluemask);
    }
  else
    {
      ...
      for (i= 0; i < swirl->colours; i++)
        (void) XAllocColor(display, MI_COLORMAP(mi), &(swirl->rgb_values [ i ]));
    }
}
  ...
  if (! swirl->fixed_colourmap)
    {
      ...
      /* store it */
      XStoreColors(dpy, swirl->cmap, swirl->rgb_values + swirl->current_map * swirl->colours, swirl->colours);
    }
}
#endif /* !STANDALONE */
    ...
    SWIRL_P swirl)
{
  ...
  put_xshm_image(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi), swirl->ximage, x, y, x, y, r, r, &swirl->shm_info);
}
...
#ifdef STANDALONE
          if (mi->writable_p)
            rotate_colors(mi->xgwa.screen, MI_COLORMAP(mi), swirl->rgb_values, swirl->colours, 1);
#else  /* !STANDALONE */
          /* rotate the colours */
          ...
          else
            {
              ...
              if (swirl->start_again == -1)
              ...
              else if (swirl->start_again == 0)
                {
                  ...
                  free_colors(mi->xgwa.screen, MI_COLORMAP(mi), mi->colors, mi->npixels);
                  make_smooth_colormap(mi->xgwa.screen, MI_VISUAL(mi), MI_COLORMAP(mi), mi->colors, &mi->npixels, True, &mi->writable_p, True);
                  swirl->colours= mi->npixels;
#endif /* STANDALONE */
...
#endif /* !STANDALONE */
  if (swirl->ximage != NULL)
    destroy_xshm_image(MI_DISPLAY(mi), swirl->ximage, &swirl->shm_info);
  if (swirl->knots)
    (void) free((void *) swirl->knots);
}
