...
ModStruct loop_description=
  {"loop", "init_loop", "draw_loop", "release_loop", "refresh_loop", "init_loop", "free_loop", &loop_opts, 100000, 5, 1600, -12, 64, 1.0, "", "Shows Langton's self-producing loops", 0, NULL};
  ...
  if (local_neighbors == 6)
    {
      ...
      if (lp->xs == 1 && lp->ys == 1)
        XDrawPoint(MI_DISPLAY(mi), MI_WINDOW(mi), gc, lp->shape.hexagon [ 0 ].x, lp->shape.hexagon [ 0 ].y);
      else
        XFillPolygon(MI_DISPLAY(mi), MI_WINDOW(mi), gc, lp->shape.hexagon, 6, Convex, CoordModePrevious);
    }
  else
    {
      XFillRectangle(MI_DISPLAY(mi), MI_WINDOW(mi), gc, lp->xb + lp->xs * col, lp->yb + lp->ys * row, lp->xs - (lp->xs > 3), lp->ys - (lp->ys > 3));
    }
}
    ...
    int state)
{
  ...
  while (locallist)
    {
      (void) printf("%d x %d, y %d\n", i, locallist->pt.x, locallist->pt.y);
      ...
      while (current)
        {
          ...
          lp->shape.hexagon [ 0 ].y= lp->yb + crow * lp->ys;
          if (lp->xs == 1 && lp->ys == 1)
            XDrawPoint(display, MI_WINDOW(mi), gc, lp->shape.hexagon [ 0 ].x, lp->shape.hexagon [ 0 ].y);
          else
            XFillPolygon(display, MI_WINDOW(mi), gc, lp->shape.hexagon, 6, Convex, CoordModePrevious);
...
#define BLUE 2
  if (lp->bncols <= 3 || lp->bnrows <= 3)
    return;
  a= MIN(lp->bncols - 3, 2 * ((local_neighbors == 6) ? HEX_MINGRIDSIZE : MINGRIDSIZE));
  a= NRAND(a) + (lp->bncols - a) / 2;
  b= MIN(lp->bnrows - 3, 2 * ((local_neighbors == 6) ? HEX_MINGRIDSIZE : MINGRIDSIZE));
  ...
  if (local_neighbors == 6)
    {
            ...
            for (j= 0; j < HEX_ADAM_LOOPY; j++)
              {
                for (i= 0; i < HEX_ADAM_LOOPX; i++)
                  {
                    k= (((lp->bnrows / 2 + (HEX_ADAM_LOOPX + HEX_ADAM_LOOPY) / 2) % 2) ? -(i + j + 1) / 2 : -(i + j) / 2);
      ...
      (void) printf("%d %d %d %d %d\n",
        ...
        hex_self_reproducing_loop [ j ][ i ]);
      /* Draw right away */
      drawcell(mi, start.x + i + ((lp->bnrows / 2 % 2) ? -j / 2 : -(j + 1) / 2) - lp->bx, start.y + j - lp->by, hex_self_reproducing_loop [ j ][ i ]);
#endif
    }
  else
    {
...
#if DEBUG
      /* Draw right away */
      drawcell(mi, start.x + dirx.x * i + diry.x * j - lp->bx, start.y + dirx.y * i + diry.y * j - lp->by, (lp->clockwise) ? self_reproducing_loop [ j ][ ADAM_LOOPX - i - i ] : self_reproducing_loop [ j ][ i ]);
#endif
    }
}
      ...
      if (lp->stippledGC == None)
        {
          ...
          if ((lp->stippledGC= XCreateGC(MI_DISPLAY(mi), window, GCFillStyle, &gcv)) == None)
            {
              free_loop(mi);
              return;
            }
  ...
  if (local_neighbors == 6)
    {
      ...
      if (lp->height < 8)
        lp->height= 8;
      if (size < -MINSIZE)
        lp->ys= NRAND(MIN(-size, MAX(MINSIZE, MIN(lp->width, lp->height) / HEX_MINGRIDSIZE)) - MINSIZE + 1) + MINSIZE;
        ...
        lp->ys= MIN(size, MAX(MINSIZE, MIN(lp->width, lp->height) / HEX_MINGRIDSIZE));
    ...
    {
      if (size < -MINSIZE)
        lp->ys= NRAND(MIN(-size, MAX(MINSIZE, MIN(lp->width, lp->height) / MINGRIDSIZE)) - MINSIZE + 1) + MINSIZE;
      else if (size < MINSIZE)
        if (! size)
        ...
        lp->ys= MIN(size, MAX(MINSIZE, MIN(lp->width, lp->height) / MINGRIDSIZE));
      if (lp->width > 2560 || lp->height > 2560)
        lp->ys*= 3; /* Retina displays */
      lp->xs= lp->ys;
      ...
      for (i= 0; i < REDRAWSTEP; i++)
        {
          if ((*(lp->oldcells + lp->redrawpos)))
            drawcell(mi, lp->redrawpos % lp->bncols - lp->bx, lp->redrawpos / lp->bncols - lp->by, *(lp->oldcells + lp->redrawpos));
