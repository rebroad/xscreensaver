...
{
#ifdef DEBUG
  printf("%d %g\n", r, 10.0 * PI * (double) r * (double) r * (double) r);
#endif /* DEBUG */
  if (st->threed)
  ...
  if (st->simple)
    {
      if ((bb->x < 0) || (bb->x > st->screen_width) ||
        ...
        (bb->radius < st->bubble_min_radius) || (bb->radius > st->bubble_max_radius))
        {
          fprintf(stderr,
  ...
  if (! bb->visible)
    {
      ...
      if (st->simple)
        {
          XDrawArc(st->dpy, st->window, st->draw_gc, (bb->x - bb->radius), (bb->y - bb->radius), bb->radius * 2, bb->radius * 2, 0, 360 * 64);
        }
      else
        {
#ifdef FANCY_BUBBLES
          XSetClipOrigin(st->dpy, st->step_pixmaps [ bb->step ]->draw_gc, (bb->x - bb->radius), (bb->y - bb->radius));

          XCopyArea(st->dpy, st->step_pixmaps [ bb->step ]->ball, st->window, st->step_pixmaps [ bb->step ]->draw_gc, 0, 0, (bb->radius * 2), (bb->radius * 2), (bb->x - bb->radius), (bb->y - bb->radius));
#endif /* FANCY_BUBBLES */
        }
    }
}
  ...
  if (bb->visible)
    {
      ...
      if (st->simple)
        {
          XDrawArc(st->dpy, st->window, st->erase_gc, (bb->x - bb->radius), (bb->y - bb->radius), bb->radius * 2, bb->radius * 2, 0, 360 * 64);
        }
      else
        {
#ifdef FANCY_BUBBLES
          if (! st->broken)
            {
              XSetClipOrigin(st->dpy, st->step_pixmaps [ bb->step ]->erase_gc, (bb->x - bb->radius), (bb->y - bb->radius));

              XFillRectangle(st->dpy, st->window, st->step_pixmaps [ bb->step ]->erase_gc, (bb->x - bb->radius), (bb->y - bb->radius), (bb->radius * 2), (bb->radius * 2));
            }
#endif /* FANCY_BUBBLES */
        }
    }
   ...
   touching. */
{
  ...
  if (null_bubble(bb))
    {
      fprintf(stderr, "NULL pointer 0x%x passed to get_closest_bubble()!", (int) bb);
      exit(1);
    }
#endif /* DEBUG */
...
changes made to either should be propagated onwards! */
{
  ...
  {
    int w, h;
    bl->ball= image_data_to_pixmap(st->dpy, st->window, png_data, data_size, &w, &h, &bl->shape_mask);
    bl->radius= MAX(w, h) / 2;
  ...
  if (st->simple)
    {
      ...
      st->bubble_min_radius= (int) (0.006 * (double) (MIN(st->screen_width, st->screen_height)));
      st->bubble_max_radius= (int) (0.045 * (double) (MIN(st->screen_width, st->screen_height)));
      /* Some trivial values */
