...
#define _DONE_2 DONE_2:
  ...
  if (se->xftfont)
    {
      ...
      XftColorFree(s->dpy, s->xgwa.visual, s->xgwa.colormap, &se->xftcolor_fg);
      XftColorFree(s->dpy, s->xgwa.visual, s->xgwa.colormap, &se->xftcolor_bg);
  ...
  if (s->font_override)
    ...
    sprintf(pattern, "-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s", "*", /* foundry */
      "*",                                                           /* family */
      "*",                                                           /* weight */
      "*",                                                           /* slant */
      "*",                                                           /* swidth */
...
FAIL:
  ...
  if (! se->xftfont)
    se->xftfont= XftFontOpenXlfd(s->dpy, screen_number(s->xgwa.screen), pattern);
  if (! se->xftfont)
    se->xftfont= load_xft_font_retry(s->dpy, screen_number(s->xgwa.screen), pattern);
...
#ifdef DEBUG
    if (s->debug_p)
      fprintf(stderr, "%s: M ratio %.2f (%d %d): %s\n", progname, ratio, rbearing, width, pattern2);
#endif
    ...
    Bool alloc_p)
{
  ...
  XftTextExtentsUtf8(s->dpy, se->xftfont, (FcChar8 *) txt, strlen(txt), &extents);
  ...
  if (alloc_p)
    {
      ...
      xftdraw= XftDrawCreate(s->dpy, w->pixmap, s->xgwa.visual, s->xgwa.colormap);
      ...
      if (s->debug_p)
        {
          ...
          XDrawRectangle(s->dpy, w->pixmap, (se->dark_p ? gc_bg : gc_fg), 0, 0, width - 1, height - 1);
        }
#endif /* DEBUG */
      ...
      for (i= -bw; i <= bw; i++)
        for (j= -bw; j <= bw; j++)
          XftDrawStringUtf8(xftdraw, &se->xftcolor_bg, se->xftfont, -w->lbearing + i, w->ascent + j, (FcChar8 *) txt, strlen(txt));

      /* Draw foreground text */
      XftDrawStringUtf8(xftdraw, &se->xftcolor_fg, se->xftfont, -w->lbearing, w->ascent, (FcChar8 *) txt, strlen(txt));
          ...
          if (w->ascent != height)
            {
              ...
              XDrawLine(s->dpy, w->pixmap, (se->dark_p ? gc_bg : gc_fg), 0, w->ascent, width - 1, w->ascent);
            }

          if (w->lbearing < 0)
            {
              /* left edge of charcell */
              XDrawLine(s->dpy, w->pixmap, (se->dark_p ? gc_bg : gc_fg), -w->lbearing, 0, -w->lbearing, height - 1);
            }

          if (w->rbearing != w->width)
            {
              /* right edge of charcell */
              XDrawLine(s->dpy, w->pixmap, (se->dark_p ? gc_bg : gc_fg), w->width - w->lbearing, 0, w->width - w->lbearing, height - 1);
            }
        }
#endif /* DEBUG */
  ...
  for (i= 0; i < se->nwords; i++)
    {
      ...
      w->nticks= ((100 + ((random() % 140) + (random() % 140) + (random() % 140))) / s->speed);
      if (w->nticks < 2)
        w->nticks= 2;
      w->tick= 0;
    }
    ...
    sentence *se)
{
  ...
  nticks= ((se->words [ 0 ]->start_x - se->words [ 0 ]->target_x) / (s->speed * 7));
    ...
    sentence *se)
{
  ...
  XftColorAllocValue(s->dpy, s->xgwa.visual, s->xgwa.colormap, &fg, &se->xftcolor_fg);
  XftColorAllocValue(s->dpy, s->xgwa.visual, s->xgwa.colormap, &bg, &se->xftcolor_bg);
}
      ...
      if (! se->xftfont) /* Got a word: need a font now */
        {
          ...
          XftTextExtentsUtf8(s->dpy, se->xftfont, (FcChar8 *) " ", 1, &extents);
          space= extents.xOff;
        }
    ...
    word *word)
{
  ...
  XCopyArea(s->dpy, word->pixmap, s->b, se->fg_gc, 0, 0, w, h, x, y);
}
...
#ifdef DEBUG
                      if (s->debug_p)
                        fprintf(stderr, "%s: OUT   %d (x2 = %d%s)\n", progname, se->id, se->words [ 0 ]->x + se->width, rand_p ? " randomly" : "");
#endif /* DEBUG */
                    }
                }
            }
  ...
  if (scale > 2)
    {
      ...
      XDrawRectangle(dpy, p2, gc, margin * scale, margin * scale, width * scale - 1, height * scale - 1);
      for (y= 0; y <= height - 2 * margin; y++)
        XDrawLine(dpy, p2, gc, margin * scale, (y + margin) * scale - 1, (width - margin) * scale, (y + margin) * scale - 1);
      for (x= 0; x <= width - 2 * margin; x++)
        XDrawLine(dpy, p2, gc, (x + margin) * scale - 1, margin * scale, (x + margin) * scale - 1, (height - margin) * scale);
    ...
    state *s)
{
  ...
  xftdraw= XftDrawCreate(s->dpy, dest, s->xgwa.visual, s->xgwa.colormap);
  XftColorAllocName(s->dpy, s->xgwa.visual, s->xgwa.colormap, "white", &xftcolor);
  XftTextExtentsUtf8(s->dpy, s->metrics_xftfont, (FcChar8 *) utxt, strlen(utxt), &extents);


  XTextExtents(s->metrics_font1, txt, strlen(txt), &dir, &ascent, &descent, &overall);
  ...
  XDrawString(s->dpy, dest, gc, s->xgwa.width / 2, s->xgwa.height - 5, fn2, strlen(fn2));
...
#ifdef HAVE_JWXYZ
  {
    ...
    XDrawString(s->dpy, dest, gc, 10, 10 + 2 * (s->metrics_font2->ascent + s->metrics_font2->descent), name, strlen(name));
    free(name);
  }
#endif
      ...
      for (i= 0; i < 2; i++)
        {
          ...
          if (! xft_p && i == 0)
          ...
          else if (xft_p)
            {
              ...
              int dir, ascent, descent;
              XTextExtents16(s->metrics_font1, txt3, 1, &dir, &ascent, &descent, &fake_c);
              cc= fake_c;
            }

          pixw= margin * 2 + cc.rbearing - cc.lbearing;
          pixh= margin * 2 + cc.ascent + cc.descent;
          p= (pixw > 0 && pixh > 0 ? XCreatePixmap(s->dpy, dest, pixw, pixh, s->xgwa.depth) : 0);
          ...
          if (p)
            {
              ...
              if (xft_p && i == 0)
                {
                  XftDraw *xftdraw2= XftDrawCreate(s->dpy, p, s->xgwa.visual, s->xgwa.colormap);
                  XftDrawStringUtf8(xftdraw2, &xftcolor, s->metrics_xftfont, -cc.lbearing + margin, cc.ascent + margin, (FcChar8 *) utxt, strlen(utxt));
                  XftDrawDestroy(xftdraw2);
                }
              else if (xft_p)
                {
                  XDrawString16(s->dpy, p, gc2, -cc.lbearing + margin, cc.ascent + margin, txt3, 1);
                }
              else
                {
                  XDrawString(s->dpy, p, gc2, -cc.lbearing + margin, cc.ascent + margin, txt, strlen(txt));
                }

              {
                unsigned x2, y2;
                XImage *img= XGetImage(s->dpy, p, 0, 0, pixw, pixh, ~0L, ZPixmap);
                XImage *img2;
                ...
                if (i == 1)
                  {
                    ...
                    if (margin > 0)
                      {
                        ...
                        if (ink & 0xFFFFFF)
                          {
                            XSetFont(s->dpy, gc, s->metrics_font2->fid);
                            XDrawString(s->dpy, dest, gc, xoff + 10, 40, "Ink escape!", 11);
                          }
                      }
                    ...
                    if (w && h)
                      {
                        if (check_edge(s->dpy, dest, gc, 120, 60, "left", img, margin, margin, 1, h) |
                          check_edge(s->dpy, dest, gc, 160, 60, "right", img, margin + w - 1, margin, 1, h) |
                          check_edge(s->dpy, dest, gc, 200, 60, "top", img, margin, margin, 0, w) |
                          check_edge(s->dpy, dest, gc, 240, 60, "bottom", img, margin, margin + h - 1, 0, w))
                          {
                            XSetFont(s->dpy, gc, s->metrics_font2->fid);
                            XDrawString(s->dpy, dest, gc, xoff + 10, 60, "Wasted space: ", 14);
                          }
                      }
                  }
                ...
                if (s->debug_metrics_antialiasing_p)
                  {
                    ...
                    img2= XCreateImage(s->dpy, s->xgwa.visual, img->depth, ZPixmap, 0, NULL, img->width, img->height, img->bitmap_pad, 0);
                            ...
                            if ((px & 0xffffff) == 0)
                              {
                                ...
                                if (y2 != pixh - 1)
                                  neighbors|= XGetPixel(img, x2, y2 + 1);
                                XPutPixel(img2, x2, y2, (neighbors & 0xffffff ? cyan : BlackPixelOfScreen(s->xgwa.screen)));
                              }
                            else
                              {
              ...
              XCopyArea(s->dpy, p2, dest, gc, 0, 0, sc * pixw, sc * pixh, xoff + x + sc * (cc.lbearing - margin), y - sc * (cc.ascent + margin));
              XFreePixmap(s->dpy, p);
          ...
          if (i == 0)
            {
...
#endif
              sprintf(txt2, "%s        [XX%sXX]    [%s%s%s%s]", (xft_p ? utxt : txt), (xft_p ? utxt : txt), (xft_p ? utxt : txt), (xft_p ? utxt : txt), (xft_p ? utxt : txt), (xft_p ? utxt : txt));

              if (xft_p)
                XftDrawStringUtf8(xftdraw, &xftcolor, s->metrics_xftfont, xoff + x / 2 + (sc * cc.rbearing / 2) - cc.rbearing / 2 + 40, ascent + 10, (FcChar8 *) txt2, strlen(txt2));
              else
                XDrawString(s->dpy, dest, gc, xoff + x / 2 + (sc * cc.rbearing / 2) - cc.rbearing / 2, ascent + 10, txt2, strlen(txt2));
#ifdef HAVE_JWXYZ
              jwxyz_XSetAntiAliasing(s->dpy, gc, False);
#endif
              XSetFont(s->dpy, gc, s->metrics_font2->fid);
              if (xft_p)
                ...
                {
                  sprintf(txt2, "%c %3ld 0%03lo 0x%02lx%s", (char) s->debug_metrics_p, s->debug_metrics_p, s->debug_metrics_p, s->debug_metrics_p, (txt [ 0 ] < s->metrics_font1->min_char_or_byte2 ? " *" : ""));
                }
              XDrawString(s->dpy, dest, gc, xoff + 10, 20, txt2, strlen(txt2));
            }
          ...
          {
            const char *ss= (j == 0 ? (i == 0 ? "char" : "overall") : (i == 0 ? "utf8" : "16 bit"));
            XSetFont(s->dpy, gc, s->metrics_font2->fid);

            XSetForeground(s->dpy, gc, red);

            sprintf(txt2, "%s ascent %d", ss, ascent);
            XDrawString(s->dpy, dest, gc, xoff + 10, y - sc * ascent - 2, txt2, strlen(txt2));
            XDrawLine(s->dpy, dest, gc, xoff, y - sc * ascent, x3, y - sc * ascent);

            sprintf(txt2, "%s descent %d", ss, descent);
            XDrawString(s->dpy, dest, gc, xoff + 10, y + sc * descent - 2, txt2, strlen(txt2));
            XDrawLine(s->dpy, dest, gc, xoff, y + sc * descent, x3, y + sc * descent);
          }
          ...
          if (cc.ascent != 0)
            XDrawString(s->dpy, dest, gc, x1, y - sc * cc.ascent - 2, txt2, strlen(txt2));
          XDrawLine(s->dpy, dest, gc, x1, y - sc * cc.ascent, x2, y - sc * cc.ascent);

          sprintf(txt2, "descent %d", cc.descent);
          if (cc.descent != 0)
            XDrawString(s->dpy, dest, gc, x1, y + sc * cc.descent - 2, txt2, strlen(txt2));
          XDrawLine(s->dpy, dest, gc, x1, y + sc * cc.descent, x2, y + sc * cc.descent);

          XSetForeground(s->dpy, gc, yellow);
          strcpy(txt2, "baseline");
          XDrawString(s->dpy, dest, gc, x1, y - 2, txt2, strlen(txt2));
          ...
          strcpy(txt2, "origin");
          XDrawString(s->dpy, dest, gc, xoff + x + 2, y + sc * descent + 50, txt2, strlen(txt2));
          XDrawLine(s->dpy, dest, gc, xoff + x, y - sc * (ascent - 10), xoff + x, y + sc * (descent + 10));

          sprintf(txt2, "width %d", cc.width);
          XDrawString(s->dpy, dest, gc, xoff + x + sc * cc.width + 2, y + sc * descent + 60, txt2, strlen(txt2));
          XDrawLine(s->dpy, dest, gc, xoff + x + sc * cc.width, y - sc * (ascent - 10), xoff + x + sc * cc.width, y + sc * (descent + 10));
          ...
          XDrawString(s->dpy, dest, gc, xoff + x + sc * cc.lbearing + 2, y + sc * descent + 30, txt2, strlen(txt2));
          XDrawLine(s->dpy, dest, gc, xoff + x + sc * cc.lbearing, y - sc * ascent, xoff + x + sc * cc.lbearing, y + sc * descent + 20);

          sprintf(txt2, "rbearing %d", cc.rbearing);
          XDrawString(s->dpy, dest, gc, xoff + x + sc * cc.rbearing + 2, y + sc * descent + 40, txt2, strlen(txt2));
          XDrawLine(s->dpy, dest, gc, xoff + x + sc * cc.rbearing, y - sc * ascent, xoff + x + sc * cc.rbearing, y + sc * descent + 40);
  ...
  if (dest != s->window)
    XCopyArea(s->dpy, dest, s->window, s->bg_gc, 0, 0, s->xgwa.width, s->xgwa.height, 0, 0);
    ...
    void *closure)
{
  ...
  if (! s->trails_p)
    XFillRectangle(s->dpy, s->b, s->bg_gc, 0, 0, s->xgwa.width, s->xgwa.height);
  ...
  if (s->debug_p && (s->prev_font_name || s->next_font_name))
    {
      ...
      if (s->prev_font_name)
        XDrawString(s->dpy, s->b, s->label_gc, 10, 10 + s->metrics_font2->ascent, s->prev_font_name, strlen(s->prev_font_name));
      if (s->next_font_name)
        XDrawString(s->dpy, s->b, s->label_gc, 10, 10 + s->metrics_font2->ascent * 2, s->next_font_name, strlen(s->next_font_name));
    }
#endif /* DEBUG */
...
#endif /* HAVE_DOUBLE_BUFFER_EXTENSION */
    if (s->dbuf)
      XCopyArea(s->dpy, s->b, s->window, s->bg_gc, 0, 0, s->xgwa.width, s->xgwa.height, 0, 0);
    ...
    Window window)
{
  ...
  s->debug_metrics_p= (get_boolean_resource(dpy, "debugMetrics", "Debug") ? 199 : 0);
  s->debug_scale= 6;
  ...
  if (s->dbuf)
    {
      ...
      if (! s->b)
        {
          s->ba= XCreatePixmap(s->dpy, s->window, s->xgwa.width, s->xgwa.height, s->xgwa.depth);
          s->b= s->ba;
        }
    }
  else
  ...
  gcv.foreground= get_pixel_resource(s->dpy, s->xgwa.colormap, "background", "Background");
  ...
  if (s->dbuf && s->ba)
    {
      ...
      s->ba= XCreatePixmap(s->dpy, s->window, s->xgwa.width, s->xgwa.height, s->xgwa.depth);
      XFillRectangle(s->dpy, s->ba, s->bg_gc, 0, 0, s->xgwa.width, s->xgwa.height);
      s->b= s->ba;
    }
}
