...
void screenhack_record_anim_gettimeofday(
  struct timeval *tv
...
double
  screenhack_record_anim_double_time(
    void)
{
  struct timeval tv;
#ifdef GETTIMEOFDAY_TWO_ARGS
...
#ifdef USE_GL
  st->img= XCreateImage(dpy, st->xgwa.visual, 24, ZPixmap, 0, 0, st->xgwa.width, st->xgwa.height, 32, 0);
  st->data2= (char *) calloc(st->xgwa.width, st->xgwa.height * 3);

#else  /* !USE_GL */

  st->gc= XCreateGC(dpy, st->window, 0, &gcv);
  st->p= XCreatePixmap(dpy, st->window, st->xgwa.width, st->xgwa.height, st->xgwa.depth);
  st->img= XCreateImage(dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, 0, 0, st->xgwa.width, st->xgwa.height, 8, 0);
#endif /* !USE_GL */
...
#define ST "images/drives-200.mp3"
    ...
    st->ffst= ffmpeg_out_init(st->outfile, soundtrack, st->xgwa.width, st->xgwa.height, 3, False);
  }
  ...
  record_anim_state *st)
{
   ...
   */
  XCopyArea(dpy, st->window, st->p, st->gc, 0, 0, st->xgwa.width, st->xgwa.height, 0, 0);
  XGetSubImage(dpy, st->p, 0, 0, st->xgwa.width, st->xgwa.height, ~0L, ZPixmap, st->img, 0, 0);
...
#undef glReadPixels /* Kludge -- unimplemented in the GLES compat layer */
  ...
  if (st->frame_count != 0)
    glReadPixels(0, 0, st->xgwa.width, st->xgwa.height, GL_BGR, GL_UNSIGNED_BYTE, st->img->data);
  ...
  if (st->frame_count < st->fade_frames)
    fade_frame(st, (unsigned char *) st->img->data, (double) st->frame_count / st->fade_frames);
  else if (st->frame_count >= st->target_frames - st->fade_frames)
    fade_frame(st, (unsigned char *) st->img->data, (double) (st->target_frames - st->frame_count - 1) / st->fade_frames);
    ...
    if (st->title && st->secs_elapsed != (int) elapsed)
      {
        ...
        sprintf(t2,
          ...
          " %d:%02d:%02d at"
          " %d%% speed;"
          " %d:%02d:%02d elapsed," " %d:%02d:%02d remaining",
          st->title,
...
void screenhack_record_anim_free(
  record_anim_state *st)
{
#ifndef USE_GL
  Display *dpy= DisplayOfScreen(st->screen);
...
#define FMT "%-24.24s %d:%02d:%02d  %5.1f fps"
  fprintf(stderr, FMT ", %d frames, %.1f MB\n", st->outfile, (((int) video_dur) / 60 / 60), (((int) video_dur) / 60) % 60, ((int) video_dur) % 60, (double) st->fps, st->frame_count, s.st_size / (float) (1024 * 1024));
  fprintf(stderr, FMT "\n", "elapsed virtual", (((int) virt_elapsed) / 60 / 60), (((int) virt_elapsed) / 60) % 60, ((int) virt_elapsed) % 60, st->frame_count / virt_elapsed);
  fprintf(stderr, FMT "\n", "elapsed actual", (((int) real_elapsed) / 60 / 60), (((int) real_elapsed) / 60) % 60, ((int) real_elapsed) % 60, st->frame_count / real_elapsed);
