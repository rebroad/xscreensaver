    ...
    apple2_sim_t *sim)
{
  ...
  Pixmap m= 0;
  Pixmap p= image_data_to_pixmap(sim->dpy, sim->window, apple2font_png, sizeof(apple2font_png), &pix_w, &pix_h, &m);
  ...
  sim->text_im= XCreateImage(sim->dpy, xgwa.visual, 1, XYBitmap, 0, 0, pix_w, pix_h, 8, 0);
  ...
  for (y= 0; y < pix_h; y++)
    for (x= 0; x < pix_w; x++)
      XPutPixel(sim->text_im, x, y, (XGetPixel(mm, x, y) ? XGetPixel(im, x, y) == black : 0));
    ...
    apple2_sim_t *sim)
{
  ...
  for (i= 0; a2_fixfont [ i ]; i++)
    XPutPixel(sim->text_im, a2_fixfont [ i ] & 0x3ff, (a2_fixfont [ i ] >> 10) & 0xf, (a2_fixfont [ i ] >> 15) & 1);
}
      ...
      for (row= textrow * 8; row < textrow * 8 + 8; row++)
        {
          ...
          if ((sim->st->gr_mode & A2_GR_HIRES) &&
              ...
              for (col= 0; col < 40; col++)
                {
                  ...
                  for (i= 0; i < 7; i++)
                    {
                      pp [ shift + 1 ]= pp [ shift ]= (((b >> i) & 1) ? ANALOGTV_WHITE_LEVEL : ANALOGTV_BLACK_LEVEL);
                      pp+= 2;
                    }
                }
            }
            ...
            (row < 160 || (sim->st->gr_mode & A2_GR_FULL)))
            {
              ...
              for (col= 0; col < 40; col++)
                {
                  unsigned char nib= ((sim->st->textlines [ textrow ][ col ] >> (((row / 4) & 1) * 4)) & 0xf);
                  /* The low or high nybble was shifted out one bit at a time. */
                  for (i= 0; i < 14; i++)
                    {
                      *pp= (((nib >> ((col * 14 + i) & 3)) & 1) ? ANALOGTV_WHITE_LEVEL : ANALOGTV_BLACK_LEVEL);
                      pp++;
                    }
                }
            }
                  ...
                  for (i= 0; i < 7; i++)
                    {
                      unsigned long pix= XGetPixel(sim->text_im,
                        ((c & 0x3f) ^ 0x20) * 7 + i,
                        row % 8);
                      pp [ 1 ]= pp [ 2 ]= ((pix ^ rev) ? ANALOGTV_WHITE_LEVEL : ANALOGTV_BLACK_LEVEL);
                      pp+= 2;
                    }
                }
            }
