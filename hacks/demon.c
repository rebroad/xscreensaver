...
ModStruct demon_description=
  {"demon", "init_demon", "draw_demon", (char *) NULL, "refresh_demon", "init_demon", "free_demon", &demon_opts, 50000, 0, 1000, -7, 64, 1.0, "", "Shows Griffeath's cellular automata", 0, NULL};
    ...
    unsigned char state)
{
  ...
  if (! state)
    ...
    {
      XSetForeground(MI_DISPLAY(mi), MI_GC(mi), MI_PIXEL(mi, (((int) state - 1) * MI_NPIXELS(mi) / (dp->states - 1)) % MI_NPIXELS(mi)));
      gc= MI_GC(mi);
    }
  else
    {
      ...
      XChangeGC(MI_DISPLAY(mi), dp->stippledGC, GCStipple | GCForeground | GCBackground, &gcv);
      gc= dp->stippledGC;
    }
  if (dp->neighbors == 6)
    {
      ...
      if (dp->xs == 1 && dp->ys == 1)
        XDrawPoint(MI_DISPLAY(mi), MI_WINDOW(mi), gc, dp->shape.hexagon [ 0 ].x, dp->shape.hexagon [ 0 ].y);
      else
        XFillPolygon(MI_DISPLAY(mi), MI_WINDOW(mi), gc, dp->shape.hexagon, 6, Convex, CoordModePrevious);
    }
  else if (dp->neighbors == 4 || dp->neighbors == 8)
    {
      XFillRectangle(MI_DISPLAY(mi), MI_WINDOW(mi), gc, dp->xb + dp->xs * col, dp->yb + dp->ys * row, dp->xs - (dp->xs > 3), dp->ys - (dp->ys > 3));
    }
  else
    {                              /* TRI */
      ...
      if (dp->xs <= 3 || dp->ys <= 3)
        {
          XDrawPoint(MI_DISPLAY(mi), MI_WINDOW(mi), gc, ((orient) ? -1 : 1) + dp->shape.triangle [ orient ][ 0 ].x, dp->shape.triangle [ orient ][ 0 ].y);
        }
      else
        {
          if (orient)
            ...
            dp->shape.triangle [ orient ][ 0 ].x-= (dp->xs / 2 - 1);
          XFillPolygon(MI_DISPLAY(mi), MI_WINDOW(mi), gc, dp->shape.triangle [ orient ], 3, Convex, CoordModePrevious);
        }
    }
}
      ...
      if (dp->stippledGC == None)
        {
          ...
          if ((dp->stippledGC= XCreateGC(MI_DISPLAY(mi), window, GCFillStyle, &gcv)) == None)
            {
              free_demon(mi);
              return;
            }
  ...
  if (dp->neighbors == 6)
    {
      ...
      if (dp->height < 8)
        dp->height= 8;
      if (size < -MINSIZE)
        dp->ys= NRAND(MIN(-size, MAX(MINSIZE, MIN(dp->width, dp->height) / MINGRIDSIZE)) - MINSIZE + 1) + MINSIZE;
        ...
        dp->ys= MIN(size, MAX(MINSIZE, MIN(dp->width, dp->height) / MINGRIDSIZE));
      dp->xs= dp->ys;
    ...
    ModeInfo *mi)
{
  ...
  if (dp->state >= dp->states)
    {
      (void) memcpy((char *) dp->newcell, (char *) dp->oldcell, dp->ncols * dp->nrows * sizeof(unsigned char));
      ...
      for (i= 0; i < REDRAWSTEP; i++)
        {
          if (dp->oldcell [ dp->redrawpos ])
            drawcell(mi, dp->redrawpos % dp->ncols, dp->redrawpos / dp->ncols, dp->oldcell [ dp->redrawpos ]);
