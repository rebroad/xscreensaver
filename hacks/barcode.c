...
static void bitmapDrawString5x8(
  Bitmap *b,
  int x,
  int y,
  char *str)
 ...
 * supplement form */
static int upcEanSupplementWidth(
  char *digits)
{
...
static void drawUpcEanSupplementalBars(
  struct state *st,
  Bitmap *upcBitmap,
  char *digits,
  int x,
  ...
  for (i= 0; i < 6; i++)
    {
      ...
      int off= (clockp ? (i < 2    ? 0 :
                             i < 4 ? 4 :
                                     /* extra spacing for clock bars */
                                     8) :
                         0);
      drawUpcEanDigit(upcBitmap,
        x + 3 + i * 7 + off,
 ...
 * specified as '?' */
static void expandToUpcADigits(
  char *compressed,
  char *expanded)
{
  int i;
  ...
  Bitmap *dest)
{
  ...
  if (supplement)
    drawUpcEanSupplementalBars(st, dest, supDigits, width, vstart + 1, dest->height - 4, 1);
  ...
  struct state *st)
{
  ...
  gcv.background= get_pixel_resource(st->dpy, xgwa.colormap, "background", "Background");
  gcv.foreground= get_pixel_resource(st->dpy, xgwa.colormap, "foreground", "Foreground");
  ...
  st->theImage= XCreateImage(st->dpy, st->visual, 1, XYBitmap, 0, st->theBitmap->buf, st->theBitmap->width, st->theBitmap->height, 8, st->theBitmap->widthBytes);
  ...
  struct state *st)
{
  ...
  if (! st->button_down_p)
    sprintf(b->code, "0%02d%02d%02d?:", (st->mode == BC_CLOCK24 ? tm->tm_hour : (tm->tm_hour > 12 ? tm->tm_hour - 12 : (tm->tm_hour == 0 ? 12 : tm->tm_hour))), tm->tm_min, tm->tm_sec);
  else
    sprintf(b->code, "0%02d%02d%02d?:", tm->tm_year % 100, tm->tm_mon + 1, tm->tm_mday);
  ...
  for (i= 0; i < st->barcode_count; i++)
    {
      ...
      XPutImage(st->dpy, st->window, st->theGC, st->theImage, 0, 0, barcode->x, barcode->y, BARCODE_WIDTH * barcode->mag, BARCODE_HEIGHT * barcode->mag);
    }
}
