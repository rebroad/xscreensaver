    ...
    state *st)
{
  ...
  size= ((st->xgwa.width > st->xgwa.height ? st->xgwa.width : st->xgwa.height) / grid_size);
  ...
  for (j= 0; j <= 1; j++)
    {
      ...
      XFillPolygon(st->dpy, st->window, st->gc, points, c->sides, Convex, CoordModeOrigin);
    }
    ...
    state *st)
{
  ...
  if (! st->sides)
    {
      static int defs []= {3, 3, 3, 4, 6, 6, 6, 6, 8, 8, 8};
