    ...
    Flake *flake)
{
  ...
  if (! XParseColor(flake->dpy, flake->XGWA.colormap, flake->Colors.Back, &color))
    {
      fprintf(stderr, "%s: can't parse color %s", progname, flake->Colors.Back);
      return 1;
    }
  iR= color.red >> 8;
  iG= color.green >> 8;
    ...
    Flake *flake)
{
      ...
      if (flake->RandomColors)
        {
          ...
          sprintf(flake->Colors.Back, "#%X%X%X%X%X%X", random() % 16, random() % 16, random() % 16, random() % 16, random() % 16, random() % 16);
        }
  ...
  flake->GCVar=
    XCreateGC(flake->dpy, flake->window, flake->GCFlags, &flake->GCValues);
      ...
      for (j= 0; j < flake->Density; j++)
        {
          ...
          FlakeVar->TrueX=
            (sin(FlakeVar->XOffset +
              FlakeVar->Ticks * (M_PI / 180) * ((double) flake->FallingSpeed / 10))) *
              10 +
            FlakeVar->XPos;
          FlakeVar->Angle=
  ...
  for (i= 1; i <= flake->Arms; i++)
    {
      ...
      XFreeGC(flake->dpy, flake->GCVar);
      flake->GCVar=
        XCreateGC(flake->dpy, flake->DB.b, flake->GCFlags, &flake->GCValues);
      XSetForeground(flake->dpy, flake->GCVar, flake->BordColor);
      XDrawLine(flake->dpy, flake->DB.b, flake->GCVar, XPos, YPos, XPos + x, YPos + y);
    }
  /* draw the flake one arm at a time */
  for (i= 1; i <= flake->Arms; i++)
    {
    ...
    void *closure)
{
  ...
  if (flake->DB.dbuf)
    {
      XCopyArea(flake->dpy, flake->DB.b, flake->window, flake->GCVar, 0, 0, flake->XGWA.width, flake->XGWA.height, 0, 0);
      flake->DB.b=
