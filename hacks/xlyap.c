...
static const PFD Maps [ NUMMAPS ]= {logistic, circle, leftlog, rightlog, doublelog};
static const PFD Derivs [ NUMMAPS ]= {dlogistic, dcircle, dleftlog, drightlog, ddoublelog};
...
#endif
  for (i= 0; i < st->settle; i++)
    {                   /* Here's where we let the thing */
      x= st->map(x, r); /* "settle down". There is usually */
      if (++bindex >= st->maxindex)
        {                /* some initial "noise" in the */
...
static double
  logistic(
    double x,
    double r) /* the familiar logistic map */
{
  return (r * x * (1.0 - x));
}
...
static double
  dlogistic(
    double x,
    double r) /* the derivative of logistic map */
{
  return (r - (2.0 * r * x));
}
...
static double
  circle(
    double x,
    double r) /* sin() hump or sorta like the circle map */
{
  return (r * sin(M_PI * x));
}
...
static double
  dcircle(
    double x,
    double r) /* derivative of the "sin() hump" */
{
...
static double
  dleftlog(
    double x,
    double r) /* derivative of the left skewed logistic */
{
...
static double
  rightlog(
    double x,
    double r) /* right skewed logistic */
{
  return (r * x * x * (1.0 - x));
}
...
static double
  drightlog(
    double x,
    double r) /* derivative of the right skewed logistic */
{
...
static double
  ddoublelog(
    double x,
    double r) /* derivative of the double logistic */
{
  double d;
    ...
    struct state *st)
{
  ...
  make_smooth_colormap(st->screen, st->visual, st->cmap, st->colors, &st->ncolors, True, NULL, True);
  ...
  for (i= 0; i < st->maxcolor; i++)
    {
      ...
      XSetForeground(st->dpy, st->Data_GC [ i ], st->colors [ ((int) ((i / ((float) st->maxcolor)) * st->ncolors)) ].pixel);
    }
}
    ...
    struct state *st)
{
  ...
  if ((st->maxcolor - st->startcolor) <= 0)
    st->startcolor= get_pixel_resource(st->dpy, st->cmap, "background", "Background");
  ...
  st->a_minimums [ 0 ]= st->min_a;
  st->b_minimums [ 0 ]= st->min_b;
  st->a_maximums [ 0 ]= st->max_a;
  st->b_maximums [ 0 ]= st->max_b;
    ...
    XKeyEvent *event)
{
  unsigned char key;
  int i;
  if (XLookupString(event, (char *) &key, sizeof(key), (KeySym *) 0, (XComposeStatus *) 0) > 0)
    {
      ...
      switch (key)
        {
          case '<' :
            st->dwell/= 2;
            if (st->dwell < 1) st->dwell= 1;
            return True;
          case '>' : st->dwell*= 2; return True;
          case '[' :
            st->settle/= 2;
            if (st->settle < 1) st->settle= 1;
            return True;
    ...
    double expo)
{
  ...
  if (tmpexpo > 0)
  ...
  else if (! mono_p)
    {
      ...
      st->sendpoint_index= ((st->sendpoint_index % st->numfreecols) + st->mincolindex);
    }
  else
    {
    ...
    struct state *st)
{
  XClearWindow(st->dpy, st->canvas);
#ifdef BACKING_PIXMAP
  XCopyArea(st->dpy, st->canvas, st->pixmap, st->Data_GC [ 0 ], 0, 0, st->width, st->height, 0, 0);
#endif
  InitBuffer(st);
}
    ...
    int y)
{
  ...
  if (st->Points.npoints [ color ] == MAXPOINTS)
    {
      XDrawPoints(st->dpy, st->canvas, st->Data_GC [ color ], st->Points.data [ color ], st->Points.npoints [ color ], CoordModeOrigin);
#ifdef BACKING_PIXMAP
      XDrawPoints(st->dpy, st->pixmap, st->Data_GC [ color ], st->Points.data [ color ], st->Points.npoints [ color ], CoordModeOrigin);
#endif
...
#ifdef BACKING_PIXMAP
  st->pixmap= XCreatePixmap(st->dpy, window, st->width, st->height, xgwa.depth);
#endif
