...
#else  /* !DO_LOG_TABLES */
  {
    double s= (iaspect == oaspect ? 0.7 : 0.8 * (iaspect > oaspect ? iaspect : oaspect));
    st->ixr*= s;
    st->iyr*= s;
  }
#endif /* !DO_LOG_TABLES */
    ...
    int oi)
{
  ...
  t->zr= (log_zabs2 +
           t->st->log2_table [ (shift >= 0 ? t->zabs2 << shift : t->zabs2 >> -shift) -
             countof(t->st->log2_table) ]) *
      t->st->clog_mul +
    t->st->orat;
      ...
      for (; oi != oimax; oi++)
        {
          ...
          XPutPixel(st->out, cx + or, cy - oi - 1, pixel(t, -t->zi - 0 * N));
          XPutPixel(st->out, cx + oi, cy - or - 1, pixel(t, t->zi - 1 * N));
          XPutPixel(st->out, cx - oi - 1, cy - or - 1, pixel(t, -t->zi - 1 * N));
          XPutPixel(st->out, cx - or - 1, cy - oi - 1, pixel(t, t->zi - 2 * N));
          XPutPixel(st->out, cx - or - 1, cy + oi, pixel(t, -t->zi + 2 * N));
          XPutPixel(st->out, cx - oi - 1, cy + or, pixel(t, t->zi + 1 * N));
          XPutPixel(st->out, cx + oi, cy + or, pixel(t, -t->zi + 1 * N));
          XPutPixel(st->out, cx + or, cy + oi, pixel(t, t->zi + 0 * N));
        }
    }
    ...
    void *closure)
{
      ...
      if (! st->img_loader) /* just finished */
        {
          ...
          st->in= XGetImage(st->dpy, st->pixmap, st->geom.x, st->geom.y, st->geom.width, st->geom.height, ~0L, ZPixmap);

          st->out= create_xshm_image(st->dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, &st->shminfo, (st->xgwa.width + 1) & ~1, (st->xgwa.height + 1) & ~1);
  ...
  if (! st->pixmap ||
      ...
      st->start_time + st->duration < double_time()))
    {
      ...
      st->img_loader= load_image_async_simple(0, st->xgwa.screen, st->window, st->pixmap, 0, &st->geom);
      st->start_time= double_time();
      return st->delay;
    }
  ...
  put_xshm_image(st->dpy, st->window, st->gc, st->out, 0, 0, (st->xgwa.width - st->out->width) / 2, (st->xgwa.height - st->out->height) / 2, st->out->width, st->out->height, &st->shminfo);
  ...
  if (event->type == KeyPress)
    {
      ...
      switch (c)
        {
          case '-' :
          case '_' : st->speed-= 0.1; goto P2; /* Zoom */
          case '=' :
          case '+' : st->speed+= 0.1; goto P2;
          case '<' :
          case ',' : st->speed-= 0.1; goto P2;
          case '>' :
          case '.' : st->speed+= 0.1; goto P2;
