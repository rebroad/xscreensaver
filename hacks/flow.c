...
enum
{ /* Name the parameter indices to make it easier to write
     standard examples */
  C,
  X,
  XX,
  ...
  if (sp->count2 == 0)
    {
      ...
      sp->p2 [ 0 ].y= (sp->yperiod > 0) ?
        balance_rand(sp->range.y) :
        Gauss_Rand(sp->range.y);
  ...
  for (N= 0; N < 5000; N++)
    {
      ...
      if (sp->p2 [ 0 ].x < min.x)
        min.x= sp->p2 [ 0 ].x;
      else if (sp->p2 [ 0 ].x > max.x)
        max.x= sp->p2 [ 0 ].x;
      if (sp->p2 [ 0 ].y < min.y)
        min.y= sp->p2 [ 0 ].y;
      else if (sp->p2 [ 0 ].y > max.y)
        max.y= sp->p2 [ 0 ].y;
      if (sp->p2 [ 0 ].z < min.z)
        min.z= sp->p2 [ 0 ].z;
      else if (sp->p2 [ 0 ].z > max.z)
        max.z= sp->p2 [ 0 ].z;
    ...
    ModeInfo *mi)
{
  ...
  if (dbufp)
    { /* Set up double buffer */
      ...
      sp->buffer= XCreatePixmap(MI_DISPLAY(mi), MI_WINDOW(mi), MI_WIDTH(mi), MI_HEIGHT(mi), MI_DEPTH(mi));
    }
  else
    {
...

  {
    ...
    XSetLineAttributes(MI_DISPLAY(mi), MI_GC(mi), lw, LineSolid, CapNotLast, JoinMiter);
  }
  ...
  if (sp->csegs == NULL)
    {
      allocate(sp->csegs, XSegment, (sp->beecount + BOX_L) * MI_NPIXELS(mi) * sp->taillen);
    ...
    ModeInfo *mi)
{
      ...
      for (i= 0; i < end; i++)
        {
          ...
          double y= Y(i, b) * (sp->yperiod < 0 ? (sp->size / sp->yperiod) : 1) - sp->centre.y;
  ...
  if (dbufp)
    { /* In Double Buffer case, prepare off-screen copy */
      /* For slow systems, this can be the single biggest bottleneck
         in the program.  These systems may be better of not using
         the double buffer. */
      XFillRectangle(MI_DISPLAY(mi), sp->buffer, MI_GC(mi), 0, 0, MI_WIDTH(mi), MI_HEIGHT(mi));
    }
  else
    { /* Otherwise, erase previous segment list directly */
      XDrawSegments(MI_DISPLAY(mi), sp->buffer, MI_GC(mi), sp->old_segs, sp->nold_segs);
    }
          ...
          if (sp->cnsegs [ col ] > 0)
            {
              ...
              /* This is usually the biggest bottleneck on most
                 ...
                 to this.  */
              XDrawSegments(MI_DISPLAY(mi), sp->buffer, MI_GC(mi), sp->csegs + col * segindex, sp->cnsegs [ col ]);
            }
        }
    }
  else
    { /* mono handled seperately since xlockmore uses '1' for
         mono white! */
      XSetForeground(MI_DISPLAY(mi), MI_GC(mi), MI_WHITE_PIXEL(mi));
      XDrawSegments(MI_DISPLAY(mi), sp->buffer, MI_GC(mi), sp->csegs, sp->cnsegs [ 0 ]);
    }
  if (dbufp)
    { /* In Double Buffer case, this updates the screen */
      XCopyArea(MI_DISPLAY(mi), sp->buffer, MI_WINDOW(mi), MI_GC(mi), 0, 0, MI_WIDTH(mi), MI_HEIGHT(mi), 0, 0);
    }
  else
    { /* Otherwise, screen is already updated.  Copy segments
         to erase-list to be erased directly next time. */
      int c= 0;
      for (col= 0; col < MI_NPIXELS(mi) - 1; col++)
        {
          memcpy(sp->old_segs + c, sp->csegs + col * segindex, sp->cnsegs [ col ] * sizeof(XSegment));
          c+= sp->cnsegs [ col ];
        }
      sp->nold_segs= c;
    }

  if (sp->count > 1 && swarm == 0)
    { /* all gone */
      if (MI_IS_VERBOSE(mi))
