    ...
    Window window)
{
  ...
  if (s && ! strcmp(s, "shuffle"))
    {
      st->mode= SHUFFLE;
    }
  else if (s && ! strcmp(s, "up"))
    {
      st->mode= UP;
    }
  else if (s && ! strcmp(s, "left"))
    {
      st->mode= LEFT;
    }
  else if (s && ! strcmp(s, "right"))
    {
      st->mode= RIGHT;
    }
  else if (s && ! strcmp(s, "down"))
    {
      st->mode= DOWN;
    }
  else if (s && ! strcmp(s, "upleft"))
    {
      st->mode= UPLEFT;
    }
  else if (s && ! strcmp(s, "downleft"))
    {
      st->mode= DOWNLEFT;
    }
  else if (s && ! strcmp(s, "upright"))
    {
      st->mode= UPRIGHT;
    }
  else if (s && ! strcmp(s, "downright"))
    {
      st->mode= DOWNRIGHT;
    }
  else if (s && ! strcmp(s, "in"))
    {
      st->mode= IN;
    }
  ...
  else if (s && ! strcmp(s, "melt"))
    {
      st->mode= MELT;
    }
  else if (s && ! strcmp(s, "stretch"))
    {
      st->mode= STRETCH;
    }
  else if (s && ! strcmp(s, "fuzz"))
    {
      st->mode= FUZZ;
    }
  else
  ...
  if (st->img_loader) /* still loading */
    {
      ...
      if (! st->img_loader)
        { /* just finished */
          ...
          if (! st->saved)
            {
              st->saved= XCreatePixmap(st->dpy, st->window, st->sizex, st->sizey, st->xgwa.depth);
              st->saved_w= st->sizex;
              st->saved_h= st->sizey;
            }
          XCopyArea(st->dpy, st->window, st->saved, st->gc, 0, 0, st->sizex, st->sizey, 0, 0);
        }
      return st->delay;
    }
...
#define nrnd(x) ((x) ? random() % (x) : x)
  ...
  else if (st->mode == FUZZ)
    { /* By Vince Levey <vincel@vincel.org>;
      ...
      if (st->mode == IN || st->mode == OUT)
        {
          ...
          if (st->mode == IN)
            {
              if (x > cx && y > cy)
                st->current_bias= upleft_bias;
              else if (x < cx && y > cy)
                st->current_bias= upright_bias;
              else if (x < cx && y < cy)
                st->current_bias= downright_bias;
              else /* (x > cx && y < cy)*/
                st->current_bias= downleft_bias;
            }
          else
            {
              if (x > cx && y > cy)
  ...
  if (st->mode == STRETCH)
    XCopyArea(st->dpy, st->window, st->window, st->gc, 0, st->sizey - top - off * 2, st->sizex, top + off, 0, st->sizey - top - off);
  else
    XCopyArea(st->dpy, st->window, st->window, st->gc, left, top, width, height, toleft, totop);
    ...
    unsigned int h)
{
  ...
  XCopyArea(st->dpy, st->saved, st->window, st->gc, 0, 0, st->saved_w, st->saved_h, ((int) w - st->saved_w) / 2, ((int) h - st->saved_h) / 2);
  st->sizex= w;
  st->sizey= h;
}
