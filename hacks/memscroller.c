    ...
    Window window)
{
     ...
     they will be used implicitly by the random image bits. */
  {
    int ncolors= 255;
    XColor colors [ 256 ];
    make_random_colormap(st->xgwa.screen, st->xgwa.visual, st->xgwa.colormap, colors, &ncolors, True, True, 0, False);
  }
    ...
    for (i= 0; i < nfonts; i++)
      {
        ...
        if (fontname && *fontname)
          st->fonts [ i ]=
            load_xft_font_retry(dpy, screen_number(st->xgwa.screen), fontname);
  ...
  gcv.foreground= get_pixel_resource(st->dpy, st->xgwa.colormap, "textColor", "Foreground");
  st->text_gc= XCreateGC(st->dpy, st->window, GCForeground | GCBackground, &gcv);

  gcv.foreground= get_pixel_resource(st->dpy, st->xgwa.colormap, "foreground", "Foreground");
  st->draw_gc= XCreateGC(st->dpy, st->window, GCForeground | GCBackground | GCLineWidth, &gcv);
  s= get_string_resource(st->dpy, "foreground", "Foreground");
  if (! s) s= strdup("white");
  XftColorAllocName(st->dpy, st->xgwa.visual, st->xgwa.colormap, s, &st->xft_fg);
  free(s);
  st->xftdraw= XftDrawCreate(dpy, window, st->xgwa.visual, st->xgwa.colormap);
  ...
  for (i= 0; i < st->nscrollers; i++)
    {
      ...
      sc->image= create_xshm_image(st->dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, &st->shm_info, 1, max_height);
      ...
      if (i == 0)
        {
          ...
          sc->rect.width= (((int) (st->xgwa.width * 0.8) / sc->rez) * sc->rez);
          sc->rect.height= (((int) (st->xgwa.height * 0.3) / sc->rez) * sc->rez);
      ...
      else
        {
          ...
          sc->rect.y= (sc0->rect.y + sc0->rect.height + st->border + (st->border + 2) * 7);
          sc->rect.width= sc0->rect.width;
          sc->rect.height= (((int) (st->xgwa.height * 0.1) / sc->rez) * sc->rez);
        }

      XDrawRectangle(st->dpy, st->window, st->draw_gc, sc->rect.x - st->border * 2, sc->rect.y - st->border * 2, sc->rect.width + st->border * 4, sc->rect.height + st->border * 4);
    }
}
  ...
  for (i= 0; i < countof(st->fonts); i++)
    {
      ...
      sprintf(buf, fmt, 0);
      XftTextExtentsUtf8(st->dpy, st->fonts [ i ], (FcChar8 *) buf, strlen(buf), &overall);
      ...
      if (y + h + 10 <= bot && x > -10)
        {
          XFillRectangle(st->dpy, st->window, st->erase_gc, x - w - 1, y - 1, w * 3 + 2, h + 2);
          XftDrawStringUtf8(st->xftdraw, &st->xft_fg, st->fonts [ i ], x, y + st->fonts [ i ]->ascent, (FcChar8 *) buf, strlen(buf));
          break;
        }
    }
}
  ...
  for (i= 0; i < st->nscrollers; i++)
    {
      ...
      XCopyArea(st->dpy, st->window, st->window, st->draw_gc, sc->rect.x + sc->speed, sc->rect.y, sc->rect.width - sc->speed, sc->rect.height, sc->rect.x, sc->rect.y);
      ...
      for (j= 0; j < sc->speed; j++)
        put_xshm_image(st->dpy, st->window, st->draw_gc, sc->image, 0, 0, sc->rect.x + sc->rect.width - sc->image->width - j, sc->rect.y, sc->rect.width, sc->rect.height, &st->shm_info);
    }
