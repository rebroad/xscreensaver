    ...
    struct bsod_state *bst)
{
  ...
  if (! bst->queue) abort();
  memset(bst->queue + bst->queue_size, 0, (n - bst->queue_size) * sizeof(*bst->queue));
  bst->queue_size= n;
}
      ...
      while (*start)
        {
          ...
          XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) start, end - start, &ov);
    ...
    struct bsod_state *bst)
{
  ...
  if (! bst->scroll_p ||
  ...
  else
    {
      ...
      XCopyArea(bst->dpy, bst->window, bst->window, bst->gc, bst->left_margin + bst->xoff, bst->top_margin + bst->yoff + lh, w, h - lh, bst->left_margin + bst->xoff, bst->top_margin + bst->yoff);
      XClearArea(bst->dpy, bst->window, bst->left_margin + bst->xoff, bst->top_margin + bst->yoff + h - lh, w, lh, False);
    }
}
    ...
    char c)
{
  if (! c)
  ...
  else if (bst->crop_p && bst->y >= (bst->xgwa.height - bst->bottom_margin - bst->yoff))
    {
      ...
      if (bst->macx_eol_kludge)
        {
          ...
          XFillRectangle(bst->dpy, bst->window, bst->gc, bst->x, bst->y - bst->font->ascent, ov.width, (bst->font->ascent + bst->font->descent) * 2);
  ...
  else
    {
      ...
      if ((bst->wrap_p || bst->word_wrap_p) &&
        bst->x + ov.xOff > bst->xgwa.width - bst->right_margin - bst->xoff)
        {
          ...
          if (bst->word_wrap_p && *bst->word_buf)
            {
              ...
              XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) bst->word_buf, L, &ov2);
            }

          if (L) /* Erase the truncated wrapped word */
            {
              XSetForeground(bst->dpy, bst->gc, bst->bg);
              XFillRectangle(bst->dpy, bst->window, bst->gc, bst->last_nonwhite, bst->y - bst->font->ascent, ov2.width, bst->font->ascent + bst->font->descent);
              XSetForeground(bst->dpy, bst->gc, bst->fg);
            }
          ...
          if (L) /* Draw wrapped partial word on the next line, no delay */
            {
              XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) bst->word_buf, L, &ov);
              XSetForeground(bst->dpy, bst->gc, bst->bg);
              XFillRectangle(bst->dpy, bst->window, bst->gc, bst->x, bst->y - bst->font->ascent, ov.xOff, bst->font->ascent + bst->font->descent);
              XftDrawStringUtf8(bst->xftdraw, &bst->xft_fg, bst->font, bst->x, bst->y, (FcChar8 *) bst->word_buf, L);
              bst->x+= ov2.width;
              bst->last_nonwhite= bst->x;
            }
        }
      ...
      XFillRectangle(bst->dpy, bst->window, bst->gc, bst->x, bst->y - bst->font->ascent, ov.xOff, bst->font->ascent + bst->font->descent);
      XSetForeground(bst->dpy, bst->gc, bst->fg);
      XftDrawStringUtf8(bst->xftdraw, &bst->xft_fg, bst->font, bst->x, bst->y, (FcChar8 *) &c, 1);
          ...
          if (! bst->queue [ bst->pos ].arg3) /* "done once" */
            {
              ...
              if (type == CENTER_FULL ||
                ...
                {
                  XSetForeground(bst->dpy, bst->gc, bst->bg);
                  XFillRectangle(bst->dpy, bst->window, bst->gc, 0, bst->y - bst->font->ascent, bst->xgwa.width, bst->font->ascent + bst->font->descent);
                  XSetForeground(bst->dpy, bst->gc, bst->fg);
                }
            }
      ...
      case IMG :
        {
          ...
          bst->img_loader= load_image_async_simple(0, bst->xgwa.screen, bst->window, bst->window, 0, 0);
          bst->pos++;
          return 0;
        }
      case FONT :
          ...
          if (type == CURSOR_BLOCK)
            {
              ...
              XSetBackground(bst->dpy, bst->gc, bst->bg);
              XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) "n", 1, &ov);
              XFillRectangle(bst->dpy, bst->window, bst->gc, bst->x, bst->y - bst->font->ascent, ov.width, bst->font->ascent + bst->font->descent);
              draw_char(bst, ' ');
            }
          else
            {
...
#undef RES2
  ...
  bst->xftdraw= XftDrawCreate(dpy, bst->window, bst->xgwa.visual, bst->xgwa.colormap);

  sprintf(buf1, "%.100s.foreground", name);
  sprintf(buf2, "%.100s.Foreground", class);
  bst->fg= gcv.foreground= get_pixel_resource(dpy, bst->xgwa.colormap, buf1, buf2);
  set_xft_color(bst->dpy, bst->xgwa.colormap, &bst->xft_fg, bst->fg);

  sprintf(buf1, "%.100s.background", name);
  sprintf(buf2, "%.100s.Background", class);
  bst->bg= gcv.background= get_pixel_resource(dpy, bst->xgwa.colormap, buf1, buf2);
    ...
    int pix_h)
{
  ...
  XImage *i1= XGetImage(dpy, pixmap, 0, 0, pix_w, pix_h, ~0L, (depth == 1 ? XYPixmap : ZPixmap));
  XImage *i2= XCreateImage(dpy, visual, depth, (depth == 1 ? XYPixmap : ZPixmap), 0, 0, pix_w * 2, pix_h * 2, 8, 0);
  XGCValues gcv;
    ...
    Window window)
{
  ...
  unsigned long fg2= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware.foreground2", "vmware.foreground");
  BSOD_COLOR(bst, fg2, bg);
  BSOD_TEXT(bst, LEFT, "VMware ESX Server [Releasebuild-98103]\n");
    ...
    Window window)
{
  ...
  unsigned long psod_bg= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm-psod.background", "vmware-arm-psod.background");
  unsigned long term_bg= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm-term.background", "vmware-arm-term.background");
  unsigned long term_fg2= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm-term.foreground2", "vmware-arm-term.foreground2");
  unsigned long term_fg3= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm-term.foreground3", "vmware-arm-term.foreground3");
  unsigned long dbg_bg= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm-dbg.background", "vmware-arm-dbg.background");
  unsigned long dbg_fg= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm-dbg.foreground", "vmware-arm-dbg.foreground");
  unsigned long fg2= get_pixel_resource(dpy, bst->xgwa.colormap, "vmware-arm.foreground2", "vmware-arm.foreground2");
  ...
  if (msg_row > 11)
    {
      ...
      BSOD_TEXT(bst, LEFT,
        ...
        " eslxiscsi elxnet ena esx base esx-dvfilter-generic-fastpath" " esx-ui esx-update i40en i40iwn iavmd igbn iser$");
    }
  ...
  BSOD_TEXT(bst, LEFT, "VMware ESX 7.0.0 [Releasebuild-19076756 aarch64]\n");
  ...
  BSOD_RECT(bst, True, 0, bst->font->ascent + bst->font->descent / 2, bst->xgwa.width, bst->font->ascent);
  ...
  switch (random() % 6)
    {
      ...
      case 5 : /* Windows 8 */
        ...
        BSOD_PAUSE(bst, 4000000);
        BSOD_TEXT(bst, LEFT, "Initializing disk for for crash dump ...\n");
        break;
      default :
        abort();
    }
    ...
    Window window)
{
  ...
  unsigned long fg= bst->fg;
  unsigned long bg= bst->bg;
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "windowslh.background2", "WindowsLH.Background");
  ...
  BSOD_TEXT(bst, CENTER,
    ...
    "or computer\n" "manufacturer.\n");
  BSOD_MOVETO(bst, bst->left_margin + bst->xoff, bst->xgwa.height - bst->yoff - bst->font->descent);
  BSOD_COLOR(bst, bg, bg2);
    ...
    Window window)
{
  ...
  const char *line3=
    "If you'd like to restart and try again, choose "
    ...
    "option is right for you, contact someone you " "trust to help with this.";
  ...
  XftTextExtentsUtf8(bst->dpy, bst->fontB, (FcChar8 *) line2, strlen(line2), &ov);
  ...
  XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) line5, strlen(line5), &ov);
  ...
  BSOD_RECT(bst, False, x2, y - line_height, ov.xOff, line_height + bst->fontA->descent * 2);

  XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) line4, strlen(line4), &ov);
  x3= x2 - ov.xOff - line_height;

  BSOD_TRUNCATE(bst);
  BSOD_MOVETO(bst, x3, y);
  BSOD_TEXT(bst, LEFT, line4);
  BSOD_RECT(bst, False, x3, y - line_height, ov.xOff, line_height + bst->fontA->descent * 2);
    ...
    Window window)
{
  ...
  const char *const *lines= (clownp ? lines2 : honorp ? lines3 :
                                                        lines1);
  ...
  pixmap= XCreatePixmapFromBitmapData(dpy, window, (char *) qr_bits, qs, qs, bst->fg, bst->bg, bst->xgwa.depth);
  {
    int n= 2;
    ...
    for (i= 0; i < n; i++)
      {
        pixmap= double_pixmap(dpy, bst->xgwa.visual, bst->xgwa.depth, pixmap, qs, qs);
        qs*= 2;
      }
  }
  bst->pixmap= pixmap;
  ...
  for (i= 0; i < countof(lines1); i++)
    {
      ...
      line= xft_word_wrap(dpy, font, lines [ i ], bst->xgwa.width - left - right);
      XftTextExtentsUtf8_multi(dpy, font, (FcChar8 *) line, strlen(line), &ov);
    ...
    Window window)
{
  ...
  unsigned long fg= bst->fg;
  unsigned long bg= bst->bg;
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "windowssb.background2", "WindowsSB.Background");
  int line_height= bst->fontA->ascent + bst->fontA->descent;
...
static struct bsod_state *
  windows_ransomware(
    ...
    Window window)
{
  struct bsod_state *bst= make_bsod_state(dpy, window, "ransomware", "Ransomware");
  char buf [ 1024 ];

  int pix_w= 0, pix_h= 0;
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, ransomware_png, sizeof(ransomware_png), &pix_w, &pix_h, &mask);
  int i, n= 0;
  ...
  const char *const excuse_quips []= {
    ...
    "you did not apply the security update for bugs NSA keys secret from " "Microsoft in your Windows(R) operating system",
  ...
  */
  ...
  unsigned long bg= bst->bg;
  /* ransom note */
  unsigned long fg2= get_pixel_resource(dpy, bst->xgwa.colormap, "ransomware.foreground2", "Ransomware.Foreground");
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "ransomware.background2", "Ransomware.Background");
  /* buttons */
  unsigned long fg3= get_pixel_resource(dpy, bst->xgwa.colormap, "ransomware.foreground3", "Ransomware.Foreground");
  unsigned long bg3= get_pixel_resource(dpy, bst->xgwa.colormap, "ransomware.background3", "Ransomware.Background");
  /* links */
  unsigned long link= get_pixel_resource(dpy, bst->xgwa.colormap, "ransomware.link", "Ransomware.Foreground");
  /* headers */
  unsigned long theader= get_pixel_resource(dpy, bst->xgwa.colormap, "ransomware.timerheader", "Ransomware.Foreground");
  ...
  for (i= 0; i < n; i++)
    {
      pixmap= double_pixmap(dpy, bst->xgwa.visual, bst->xgwa.depth, pixmap, pix_w, pix_h);
  ...
  right_column_width= MIN(line_height * 40,
    MAX(line_height * 8,
      bst->xgwa.width - left_column_width - margin * 2));
  top_height= line_height * 2.5;
  bottom_height= line_height * 6;
  right_column_height= MIN(line_height * 36,
    bst->xgwa.height - bottom_height - top_height - line_height);
  ...
  if ((bst->xgwa.width / 4) * 3 > bst->xgwa.height)
  ...
  else if (right_column_width < line_height * 30)
    {
      ...
      right_column_height= (bst->xgwa.height - bottom_height - top_height - line_height);
    }
  ...
  if (pixmap)
    {
      bst->pixmap= pixmap;
      bst->mask= mask;
      BSOD_PIXMAP(bst, 0, 0, pix_w, pix_h, x + (left_column_width - pix_w) / 2, y);
    }

  /* Setup deadlines */
  strftime(stage1_deadline_str, sizeof(stage1_deadline_str), "%m/%d/%Y %H:%M:%S", localtime(&stage1_deadline));
  strftime(stage2_deadline_str, sizeof(stage1_deadline_str), "%m/%d/%Y %H:%M:%S", localtime(&stage2_deadline));
  ...
  BSOD_MOVETO(bst, x + left_column_width + margin, y + bst->fontA->ascent);
  BSOD_COLOR(bst, fg, bg);
  BSOD_WORD_WRAP(bst);
  ...
  if (right_column_height > 425)
    {
      BSOD_MOVETO(bst, x, y + right_column_height + top_height + bottom_height - line_height1 * 5);
  ...
  BSOD_RECT(bst, True, x + left_column_width + margin, y + top_height, right_column_width, right_column_height);
  BSOD_MOVETO(bst,
  ...
  BSOD_RECT(bst, True, x + left_column_width + margin, y + top_height + right_column_height, bst->xgwa.width, bst->xgwa.height);
  BSOD_RECT(bst, True, x + left_column_width + margin + right_column_width, y + top_height, bst->xgwa.width, bst->xgwa.height);
    ...
    Window window)
{
  ...
  switch (which)
    {
      case 0 :
        ...
        BSOD_FONT(bst, 0);
        BSOD_TEXT(bst, LEFT, "Plug in the USB drive that has the BitLocker key\n");
        break;
      ...
      case 1 :
        ...
        BSOD_TEXT(bst, LEFT,
          ...
          "https://www.jwz.org/xscreensaver/ from another PC " "or mobile device.");
        bottom= (
      ...
      case 2 :
        BSOD_TEXT(bst, LEFT, "BitLocker\n\n");
        BSOD_FONT(bst, 0);
        BSOD_TEXT(bst, LEFT, "Enter the PIN to unlock this drive\n");
        BSOD_INVERT(bst);
        BSOD_TRUNCATE(bst);
        BSOD_TEXT(bst, LEFT,
          "                                                " "                                                ");
      ...
      case 3 :
        BSOD_TEXT(bst, LEFT, "BitLocker recovery\n\n");
        BSOD_FONT(bst, 0);
        BSOD_TEXT(bst, LEFT, "Enter the recovery key for this drive\n");
        BSOD_INVERT(bst);
        BSOD_TRUNCATE(bst);
        BSOD_TEXT(bst, LEFT,
          "                                                " "                                                ");
          ...
          "Use the number keys or function keys F1-F10 " "(use F10 for 0).\n");
        {
          ...
          char k [ 200 ];
          sprintf(k,
            "Recovery key ID (to identify your key): " "%08X-%04X-%04X-%04X%08X\n\n",
            random() & 0xFFFFFFFF,
            random() & 0xFFFF,
            random() & 0xFFFF,
            random() & 0xFFFF,
      ...
      case 4 :
        ...
        BSOD_TEXT(bst, LEFT,
          ...
          "disc or USB device), contact your system administrator or " "PC manufacturer.");
        bottom= (
  ...
  BSOD_FONT(bst, 0);
  BSOD_MOVETO(bst, 0, bst->xgwa.height - top - (bst->fontB->ascent + bst->fontB->descent) * 1);
...
static struct bsod_state *
  sparc_linux(
    ...
    Window window)
{
  struct bsod_state *bst= make_bsod_state(dpy, window, "sparclinux", "SparcLinux");
  bst->scroll_p= True;
  bst->y= bst->xgwa.height - bst->yoff - bst->font->ascent - bst->font->descent;
    ...
    Window window)
{
  ...
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "amiga.background2", "Amiga.Background");

  bst->yoff= 0;
  bst->top_margin= bst->bottom_margin= 0;

  pixmap= image_data_to_pixmap(dpy, window, amiga_png, sizeof(amiga_png), &pix_w, &pix_h, &mask);
    ...
    Window window)
{
  ...
  int i, x, y;
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, atari_png, sizeof(atari_png), &pix_w, &pix_h, &mask);
    ...
    Window window)
{
  ...
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, mac_png, sizeof(mac_png), &pix_w, &pix_h, &mask);
  int offset= pix_h * 4;
    ...
    Window window)
{
  ...
  const char *bottom= (
    ...
    "     +00886    40843765     *+$0400           "
    "                                | 4A1F\n"
    "     +00888    40843718     $0004(A7),([0,A7[)" "                  ; 04E8D0AE    | 66B8");
  __extension__ const char *body= (
  ...
  unsigned long fg= bst->fg;
  unsigned long bg= bst->bg;
  unsigned long bc= get_pixel_resource(dpy, bst->xgwa.colormap, "macsbug.borderColor", "MacsBug.BorderColor");
  ...
  BSOD_RECT(bst, True, xoff + col_right, yoff + row_top, page_right - col_right, 1);
  BSOD_RECT(bst, True, xoff + col_right, yoff + row_bottom, page_right - col_right, 1);
  ...
  BSOD_RECT(bst, False, xoff + col_right + (char_width / 2) + 2, yoff + row_bottom + 2, 0, page_bottom - row_bottom - 4);
    ...
    Window window)
{
  ...
  int x, y;
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, macbomb_png, sizeof(macbomb_png), &pix_w, &pix_h, &mask);
    ...
    Window window)
{
  ...
  unsigned long fg2= get_pixel_resource(dpy, bst->xgwa.colormap, "macx.textForeground", "MacX.TextForeground");
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "macx.textBackground", "MacX.TextBackground");
  XClearWindow(dpy, window);
  {
    Pixmap pixmap= 0;
    Pixmap mask= 0;
    int x, y, pix_w, pix_h;
    pixmap= image_data_to_pixmap(dpy, window, hmac_png, sizeof(hmac_png), &pix_w, &pix_h, &mask);
    ...
    Bool v10_3_p)
{
  ...
  if (v10_3_p)
    pixmap= image_data_to_pixmap(dpy, window, osx_10_3_png, sizeof(osx_10_3_png), &pix_w, &pix_h, &mask);
  else
    pixmap= image_data_to_pixmap(dpy, window, osx_10_2_png, sizeof(osx_10_2_png), &pix_w, &pix_h, &mask);
  if (! pixmap) abort();
  if (! mask) abort();
    ...
    Window window)
{
  ...
  fg= bst->fg;
  bg= bst->bg;
  bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "macx.background", "Mac.Background");
  if (L < 0) L= 0;
  if (T < 0) T= 0;
  ...
  BSOD_TEXT(bst, LEFT,
    ...
    "IOBlockStorageDriver/ST96812AS Media/IOGUIDPartitionScheme/" "Customer02\n");
  BSOD_PAUSE(bst, 1000000);
  BSOD_TEXT(bst, LEFT,
    ...
    Window window)
{
  ...
  unsigned long fg= get_pixel_resource(dpy, bst->xgwa.colormap, "macinstall.foreground", "Mac.Foreground");
  unsigned long bg= get_pixel_resource(dpy, bst->xgwa.colormap, "macinstall.background", "Mac.Background");
  unsigned long fg2= get_pixel_resource(dpy, bst->xgwa.colormap, "macinstall.barForeground", "Mac.Foreground");
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "macinstall.barBackground", "Mac.Background");
  char buf [ 1024 ];
  int lh= bst->font->ascent + bst->font->descent;
  int i, min;
  double pct;
  ...
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, apple_png, sizeof(apple_png), &pix_w, &pix_h, &mask);
    ...
    Window window)
{
  ...
  __extension__ static const char *const os2_panics []=
    ...
    Window window)
{
  ...
  int char_width;
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, sun_png, sizeof(sun_png), &pix_w, &pix_h, &mask);
  XGlyphInfo ov;
  XftTextExtentsUtf8(bst->dpy, bst->font, (FcChar8 *) "n", 1, &ov);
  char_width= ov.xOff;
  ...
  BSOD_INVERT(bst);
  BSOD_RECT(bst, True, bst->left_margin, bst->top_margin, bst->xgwa.width - bst->left_margin - bst->right_margin, bst->xgwa.height - bst->top_margin - bst->bottom_margin);
  BSOD_INVERT(bst);
  ...
  BSOD_TEXT(bst, LEFT, "\b: 2803 pages dumped, compression ratio 2.88, dump succeeded\n\033");
  BSOD_PAUSE(bst, 2000000);
  ...
  BSOD_TEXT(bst, LEFT,
    "\r"
    "                                                          \r" "Boot device: /sbus/le@0,c00000   File and args:\n\033");
  ...
  BSOD_PAUSE(bst, 1000000);
  BSOD_TEXT(bst, LEFT,
    "\r External loopback test -- succeeded.\n" "ok ");
  ...
  BSOD_TEXT(bst, LEFT, "  \n");
  BSOD_CHAR_DELAY(bst, 0);
  BSOD_TEXT(bst, LEFT, "\rBoot device: /sbus/esp@0,800000/sd@6,0:c    File and args:\n\033");
  BSOD_PAUSE(bst, 1000000);
  BSOD_TEXT(bst, LEFT,
    "\rroot on   fstype 4.3\n" "Boot: vmunix\n\033");
  ...
  BSOD_TEXT(bst, LEFT, "\b+");
  BSOD_CHAR_DELAY(bst, 0);
  BSOD_TEXT(bst, LEFT, "2218504+28056 bytes\n\033");
  BSOD_PAUSE(bst, 1000000);
      ...
      for (i= 0; i < 3; i++)
        {
          BSOD_PAUSE(bst, 3000000);
          if (i > 0)
            BSOD_TEXT(bst, LEFT, "\r        polled command timeout\n");
          BSOD_TEXT(bst, LEFT,
    ...
    Window window)
{
  ...
  const char *const linux_panic []= {
    " kernel: Unable to handle kernel paging request at virtual " "address 0000f0ad\n",
    " kernel:  printing eip:\n",
    " kernel: c01becd7\n",
    " kernel: *pde = 00000000\n",
    " kernel: Oops: 0000\n",
    ...
    "",
    "",
    " kernel: Call Trace:    [<c01c1e5b>] [<c01c1fbf>] [<c01bef93>] " "[<c01bf02b>] [<c0134c4f>]\n",
    "",
    "",
    "",
  ...
  if (i > 0)
    {
      ...
      sprintf(buf,
        "Warning: Group %d's copy of the group descriptors " "has a bad block (%d).\n",
        g,
        b);
      BSOD_TEXT(bst, LEFT, buf);
      ...
      while (--i > 0)
        {
          ...
          sprintf(buf,
            ...
            "from filesystem resulted in short read) while doing " "inode scan.  Ignore error<y>?",
            b);
          BSOD_TEXT(bst, LEFT, buf);
          BSOD_PAUSE(bst, 10000);
  ...
  if (0 == (random() % 10))
    {
      ...
      while (--i > 0)
        {
          BSOD_TEXT(bst, LEFT,
            "Could not allocate 256 block(s) for inode table: " "No space left on device\n");
          BSOD_PAUSE(bst, 1000);
        }
  ...
  while (--i > 0)
    {
      ...
      int o= random() % 0xFFF;
      sprintf(buf,
        "Directory inode %d, block 0, offset %d: "
        "directory corrupted\n" "Salvage<y>? ",
        n,
        o);
      BSOD_TEXT(bst, LEFT, buf);
    ...
    Window window)
{
  ...
  {
    snprintf(ss, 1024, linux_panic [ 1 ].string, release, sysname, gccversion, version);
    ...
    Window window)
{
  ...
  __extension__ const char *const lines []= {
    ...
    Window window)
{
  ...
  bst->scroll_p= True;
  bst->wrap_p= True;
  bst->y= bst->xgwa.height - bst->bottom_margin - bst->yoff - bst->font->ascent;
      ...
      for (x= 0; x < nvs->cols; x++)
        {
          ...
          XCopyPlane(bst->dpy, nvs->bits [ cell->bit ], bst->window, bst->gc, 0, 0, nvs->cellw, nvs->cellh, x * nvs->cellw, y * nvs->cellh, 1L);
        }
    }

  nvs->tick++;
  if ((random() % 5) == 0) /* change the display */
    nvspatter(nvs->grid, nvs->rows, nvs->cols, nvs->ncolors, countof(nvs->bits), False);
  ...
  for (i= 0; i < countof(nvs->bits); i++)
    {
      ...
      XFillRectangle(dpy, nvs->bits [ i ], nvs->gc1, 0, 0, nvs->cellw, nvs->cellh);
      XSetForeground(dpy, nvs->gc1, 1);

      if ((random() % 40) != 0)
        for (j= 0; j < ((nvs->cellw * nvs->cellh) / 16); j++)
          XFillRectangle(dpy, nvs->bits [ i ], nvs->gc1, (random() % (nvs->cellw - 2)) & ~1, (random() % (nvs->cellh - 2)) & ~1, 2, 2);
    }

  /* Randomize the grid
   */
  nvspatter(nvs->grid, nvs->rows, nvs->cols, nvs->ncolors, countof(nvs->bits), True);
  for (i= 0; i < 20; i++)
    nvspatter(nvs->grid, nvs->rows, nvs->cols, nvs->ncolors, countof(nvs->bits), False);
...
static void a2controller_crash(
  apple2_sim_t *sim,
  int *stepno,
  ...
  switch (*stepno)
    {
        ...
        else
          {
            ...
            sprintf(sim->printing_buf,
              ...
              apple2_basic_errors [ random() %
                (sizeof(apple2_basic_errors) / sizeof(char *)) ],
      ...
      case 170 :
        if (random() % 50 == 0)
          ...
          {
            sprintf(sim->printing_buf, "\n?%s\n]", apple2_dos_errors [ random() % (sizeof(apple2_dos_errors) / sizeof(char *)) ]);
            sim->printing= sim->printing_buf;
          }
        *stepno= 999;
        *next_actiontime+= 1.0;
    ...
    struct bsod_state *bst)
{
  ...
  if (! sim)
    {
      sim= apple2_start(bst->dpy, bst->window, 9999999, a2controller_ransomware);
      bst->closure= sim;
    }
    ...
    struct bsod_state *bst)
{
  ...
  if (! sim)
    {
      sim= apple2_start(bst->dpy, bst->window, 9999999, a2controller_encom);
      bst->closure= sim;
    }
    ...
    Window window)
{
  ...
  float scale= 0.48;
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, atm_png, sizeof(atm_png), &pix_w, &pix_h, &mask);
  ...
  if (i > 0)
    {
      int j;
      XSetForeground(dpy, bst->gc, get_pixel_resource(dpy, bst->xgwa.colormap, "atm.background", "ATM.Background"));
    ...
    Window window)
{
  ...
  float scale= 0.15;
  Pixmap mask= 0;
  Pixmap pixmap= image_data_to_pixmap(dpy, window, dvd_png, sizeof(dvd_png), &pix_w, &pix_h, &mask);
  int i= 0;
  int x, y, dx, dy;
  int steps= 10000;
  int speed= 1;
...
static struct bsod_state *
  nintendo(
    ...
    Window window)
{
  struct bsod_state *bst= make_bsod_state(dpy, window, "nintendo", "Nintendo");
  unsigned long bg= get_pixel_resource(dpy, bst->xgwa.colormap, "nintendo.background", "Nintendo.Background");
  unsigned long bg2= get_pixel_resource(dpy, bst->xgwa.colormap, "nintendo.background2", "Nintendo.Background");
  unsigned long fg= get_pixel_resource(dpy, bst->xgwa.colormap, "nintendo.foreground", "Nintendo.Foreground");
  int line_height= bst->font->ascent + bst->font->descent;
  XGlyphInfo ov;
  int char_width;
  ...
  BSOD_COLOR(bst, bg2, bg);
  BSOD_RECT(bst, True, left2, top2 - line_height * 2, bst->xgwa.width - left2 * 2, bst->xgwa.height - top2 * 2 + line_height * 2);
    ...
    Window window)
{
  ...
  unsigned long bg= get_pixel_resource(dpy, bst->xgwa.colormap, "android.background", "Android.Background");
  unsigned long fg= get_pixel_resource(dpy, bst->xgwa.colormap, "android.foreground", "Android.Foreground");
  unsigned long c1= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color1", "Android.Foreground");
  unsigned long c2= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color2", "Android.Foreground");
  unsigned long c3= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color3", "Android.Foreground");
  unsigned long c4= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color4", "Android.Foreground");
  unsigned long c5= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color5", "Android.Foreground");
  unsigned long c6= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color6", "Android.Foreground");
  unsigned long c7= get_pixel_resource(dpy, bst->xgwa.colormap, "android.color7", "Android.Foreground");
  ...
  pixmap= image_data_to_pixmap(dpy, window, android_png, sizeof(android_png), &pix_w, &pix_h, &mask);
  if (! pixmap) abort();
  {
    int i, n= 0;
    if (bst->xgwa.width > 2560 || bst->xgwa.height > 2560)
  ...
  while (1)
    {
      ...
      if (state <= countof(lines0) + countof(lines1) + countof(lines2))
        {
          ...
          BSOD_MOVETO(bst, bst->left_margin + bst->xoff, bst->top_margin + bst->yoff + line_height);
      ...
      if (state == countof(lines0) ||
        ...
        state == countof(lines0) + countof(lines1) + countof(lines2))
        {
          ...
          BSOD_TEXT(bst, LEFT, "FASTBOOT\n");
          BSOD_COLOR(bst, c6, bg);
          BSOD_TEXT(bst, LEFT, "RECOVERY\n");
          BSOD_COLOR(bst, c7, bg);
          BSOD_TEXT(bst, LEFT, "FACTORY RESET\n");
          BSOD_COLOR(bst, c3, bg);
          BSOD_TEXT(bst, LEFT, "SIMLOCK\n");
          BSOD_COLOR(bst, bg, c3);
          BSOD_TEXT(bst, LEFT, "HBOOT USB\n");
          BSOD_COLOR(bst, fg, bg);
          BSOD_TEXT(bst, LEFT, "IMAGE CRC\n");
          BSOD_COLOR(bst, c3, bg);
          BSOD_TEXT(bst, LEFT, "SHOW BARCODE\n");
          BSOD_PAUSE(bst, 3000000);
        }
    ...
    Window window)
{
  ...
  if (which)
    {
      pixmap= image_data_to_pixmap(dpy, window, gnome2_png, sizeof(gnome2_png), &pix_w, &pix_h, &mask);
      fg= get_pixel_resource(dpy, bst->xgwa.colormap, "gnome.foreground2", "Gnome.Foreground");
      bg= get_pixel_resource(dpy, bst->xgwa.colormap, "gnome.background2", "Gnome.Background");
    }
  else
    {
      pixmap= image_data_to_pixmap(dpy, window, gnome1_png, sizeof(gnome1_png), &pix_w, &pix_h, &mask);
      fg= get_pixel_resource(dpy, bst->xgwa.colormap, "gnome.foreground", "Gnome.Foreground");
      bg= get_pixel_resource(dpy, bst->xgwa.colormap, "gnome.background", "Gnome.Background");
    }
  ...
  BSOD_FONT(bst, 1);
  BSOD_TEXT(bst, CENTER, "A problem has occurred and the system can't recover.\n");
...
DELAY_NOW:
  ...
  if (! dst->bst && time_left > 0) /* run completed; wait out the delay */
  ...
  else /* launch a new sub-mode */
    {
      ...
      if (dst->debug_p)
        fprintf(stderr, "%s: %s: launch\n", progname, all_modes [ dst->which ].name);
      ...
      if (dst->bst)
        {
          if (dst->debug_p)
            fprintf(stderr, "%s: %s: queue size: %d (%d)\n", progname, dst->name, dst->bst->pos, dst->bst->queue_size);
