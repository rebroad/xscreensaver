...
#ifndef HAVE_JWXYZ
  XSelectInput(dpy, window, PointerMotionMask | st->xgwa.your_event_mask);
#endif
    ...
    struct state *st)
{
   ...
   */
  if (! st->pix)
    st->pix= XCreatePixmap(st->dpy, st->window, st->xgwa.width, st->xgwa.height, st->xgwa.depth);

  XCopyArea(st->dpy, st->window, st->pix, st->gc, 0, 0, st->xgwa.width, st->xgwa.height, 0, 0);
  ...
  if (st->specular.s)
    {
      ...
      XFillArc(st->dpy, st->pix, st->gc, st->specular.x - s / 2, st->specular.y - s / 2, s, s, 0, 360 * 64);
    }

#ifdef DEBUG
  in= st->tcimg;
#else
  in= XGetImage(st->dpy, st->pix, 0, 0, st->xgwa.width, st->xgwa.height, ~0L, ZPixmap);
  /* Could actually use st->tv->image here, except we don't have the
     subrectangle being used (overall_top, usewidth, etc.) */
#endif

  out= XCreateImage(st->dpy, st->xgwa.visual, st->xgwa.depth, ZPixmap, 0, NULL, st->w, st->h, 8, 0);
            ...
            for (ox= 0; ox < out->width; ox++)
              {
                ...
                unsigned long p= (ix >= 0 && ix < in->width &&
                      iy >= 0 && iy < in->height ?
                    ((uint32_t *) (in->data + in->bytes_per_line * iy)) [ ix ] :
                    black);
#ifdef HAVE_JWXYZ
