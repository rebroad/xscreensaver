...
static int sgi_gamma_fade(XtAppContext, Display *, Window *wins, int count, double secs, Bool out_p);
#endif
#ifdef HAVE_XF86VMODE_GAMMA
static int xf86_gamma_fade(XtAppContext, Display *, Window *wins, int count, double secs, Bool out_p);
#endif
#ifdef HAVE_RANDR_12
static int randr_gamma_fade(XtAppContext, Display *, Window *wins, int count, double secs, Bool out_p);
#endif
static int colormap_fade(XtAppContext, Display *, Window *wins, int count, double secs, Bool out_p, Bool from_desktop_p);
static int xshm_fade(XtAppContext, Display *, Window *wins, int count, double secs, Bool out_p, Bool from_desktop_p, fade_state *);
      ...
      if (verbose_p > 1)
        {
          ...
          fprintf(stderr, "%s: user input %d %d\n", blurb(), event.xany.type, (re ? re->evtype : -1));
        }
  ...
  if (has_writable_cells(DefaultScreenOfDisplay(dpy),
        DefaultVisual(dpy, 0)))
    {
      /* Do it the old-fashioned way, which only really worked on
         8-bit displays. */
      status= colormap_fade(app, dpy, saver_windows, nwindows, seconds, out_p, from_desktop_p);
      if (status == 0 || status == 1)
        return status; /* faded, possibly canceled */
    }

  /* Else do it the hard way, by hacking a screenshot. */
  status= xshm_fade(app, dpy, saver_windows, nwindows, seconds, out_p, from_desktop_p, state);
  ...
  for (i= 0; i < nscreens; i++)
    {
      ...
      Colormap cmap= (from_desktop_p || ! out_p ? DefaultColormapOfScreen(sc) : window_cmaps [ i ]);
      for (j= 0; j < ncolors; j++)
...
static void sgi_whack_gamma(Display *dpy, int screen, struct screen_sgi_gamma_info *info, float ratio);
    ...
    Bool out_p)
{
  ...
  for (screen= 0; screen < nscreens; screen++)
    {
      if (! XSGIvcQueryGammaMap(dpy, screen, info [ screen ].gamma_map, &info [ screen ].gamma_size, &info [ screen ].gamma_precision, &info [ screen ].alpha_p))
        goto FAIL;

      if (! XSGIvcQueryGammaColors(dpy, screen, info [ screen ].gamma_map, XSGIVC_COMPONENT_RED, &info [ screen ].nred, &info [ screen ].red1))
        goto FAIL;
      if (! XSGIvcQueryGammaColors(dpy, screen, info [ screen ].gamma_map, XSGIVC_COMPONENT_GREEN, &info [ screen ].ngreen, &info [ screen ].green1))
        goto FAIL;
      if (! XSGIvcQueryGammaColors(dpy, screen, info [ screen ].gamma_map, XSGIVC_COMPONENT_BLUE, &info [ screen ].nblue, &info [ screen ].blue1))
        goto FAIL;
...
static void
  sgi_whack_gamma(
    Display *dpy,
    int screen,
    struct screen_sgi_gamma_info *info,
    float ratio)
{
  int k;
  ...
  XSGIvcStoreGammaColors16(dpy, screen, info->gamma_map, info->nred, XSGIVC_MComponentRed, info->red2);
  XSGIvcStoreGammaColors16(dpy, screen, info->gamma_map, info->ngreen, XSGIVC_MComponentGreen, info->green2);
  XSGIvcStoreGammaColors16(dpy, screen, info->gamma_map, info->nblue, XSGIVC_MComponentBlue, info->blue2);
  XSync(dpy, False);
}
...
static Bool xf86_whack_gamma(Display *dpy, int screen, xf86_gamma_info *ginfo, float ratio);
  ...
  for (screen= 0; screen < nscreens; screen++)
    {
      if (ext_ok == 1) /* only have gamma parameter, not ramps. */
      ...
      else if (ext_ok == 2) /* have ramps */
        {
          ...
          if (! XF86VidModeGetGammaRamp(dpy, screen, info [ screen ].size, info [ screen ].r, info [ screen ].g, info [ screen ].b))
            goto FAIL;
        }
    ...
    int *minP)
{
  ...
  return (error_handler_hit_p ? False : result);
}
...
static void randr_whack_gamma(Display *dpy, int screen, randr_gamma_info *ginfo, float ratio);
...
#else
static int xshm_whack(Display *, XShmSegmentInfo *, xshm_fade_info *, float ratio);
#endif
    ...
    Bool verbose_p)
{
  ...
  if (external_p)
  ...
  else
    {
      ...
      XCopyArea(dpy, xgwa.root, pixmap, gc, xgwa.x, xgwa.y, xgwa.width, xgwa.height, 0, 0);
      XFreeGC(dpy, gc);
    }
...

  {
    ...
    if (verbose_p || ! pixmap || *late)
      fprintf(stderr,
        "%s: %s screenshot 0x%lx %dx%d" " for window 0x%lx%s\n",
        blurb(),
  ...
  switch ((i= glGetError()))
    {
      ...
      default :
        e= buf;
        sprintf(buf, "unknown error %d", (int) i);
        break;
    }
  ...
  for (screen= 0; screen < nwindows; screen++)
    {
...
#ifdef USE_GL
      info [ screen ].src=
        XCreateImage(dpy, visual, 32, ZPixmap, 0, NULL, xgwa.width, xgwa.height, 32, 0);
      ...
      while (glGetError() != GL_NO_ERROR); /* Flush */

#else  /* !USE_GL */
      info [ screen ].src=
        create_xshm_image(dpy, visual, xgwa.depth, ZPixmap, &shm_info, xgwa.width, xgwa.height);
      if (! info [ screen ].src) goto FAIL;
      ...
      if (! out_p)
      ...
      else
        {
          info [ screen ].screenshot=
            xshm_screenshot_grab(dpy, saver_windows [ screen ], from_desktop_p, verbose_p);
      ...
      info [ screen ].window=
        XCreateWindow(dpy, root, xgwa.x, xgwa.y, xgwa.width, xgwa.height, xgwa.border_width, xgwa.depth, InputOutput, visual, attrmask, &attrs);
      ...
      XGetSubImage(dpy, info [ screen ].screenshot, 0, 0, xgwa.width, xgwa.height, ~0L, ZPixmap, info [ screen ].src, 0, 0);
...

      {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, info [ screen ].src->width, info [ screen ].src->height, GL_RGBA, GL_UNSIGNED_BYTE, info [ screen ].src->data);
      ...
      if (! get_xshm_image(dpy, info [ screen ].screenshot, info [ screen ].src, 0, 0, ~0L, &shm_info))
        goto FAIL;
      ...
      if (out_p)
        {
          ...
          XSetWindowBackgroundPixmap(dpy, info [ screen ].window, info [ screen ].screenshot);
        }
      else
        {
    ...
    float ratio)
{
  ...
  put_xshm_image(dpy, info->window, info->gc, info->intermediate, 0, 0, 0, 0, info->intermediate->width, info->intermediate->height, shm_info);
  XSync(dpy, False);
  return 0;
}
