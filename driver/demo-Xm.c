    ...
    int center)
{
  ...
  ac= 0;
  XtSetArg(av [ ac ], XmNnumColumns, 1);
  ac++;
  XtSetArg(av [ ac ], XmNorientation, XmVERTICAL);
  ac++;
  XtSetArg(av [ ac ], XmNpacking, XmPACK_COLUMN);
  ac++;
  XtSetArg(av [ ac ], XmNrowColumnType, XmWORK_AREA);
  ac++;
  XtSetArg(av [ ac ], XmNspacing, 0);
  ac++;
  ...
  while (head)
    {
      ...
      ac++;
      XtSetArg(av [ ac ], XmNmarginHeight, 0);
      ac++;
    ...
    Widget widget)
{
   ...
   */
  if (! strcasecmp(visual, ""))
    visual= "";
  else if (! strcasecmp(visual, "any"))
    visual= "";
  else if (! strcasecmp(visual, "default"))
    visual= "Default";
  else if (! strcasecmp(visual, "default-n"))
    visual= "Default-N";
  else if (! strcasecmp(visual, "default-i"))
    visual= "Default-I";
  else if (! strcasecmp(visual, "best"))
    visual= "Best";
  else if (! strcasecmp(visual, "mono"))
    visual= "Mono";
  else if (! strcasecmp(visual, "monochrome"))
    visual= "Mono";
  else if (! strcasecmp(visual, "gray"))
    visual= "Gray";
  else if (! strcasecmp(visual, "grey"))
    visual= "Gray";
  else if (! strcasecmp(visual, "color"))
    visual= "Color";
  else if (! strcasecmp(visual, "gl"))
    visual= "GL";
  else if (! strcasecmp(visual, "staticgray"))
    visual= "StaticGray";
  else if (! strcasecmp(visual, "staticcolor"))
    visual= "StaticColor";
  else if (! strcasecmp(visual, "truecolor"))
    visual= "TrueColor";
  else if (! strcasecmp(visual, "grayscale"))
    visual= "GrayScale";
  else if (! strcasecmp(visual, "greyscale"))
    visual= "GrayScale";
  else if (! strcasecmp(visual, "pseudocolor"))
    visual= "PseudoColor";
  else if (! strcasecmp(visual, "directcolor"))
    visual= "DirectColor";
    ...
    prefs_pair *pair)
{
  ...
  for (h= hacks; *h; h++)
    {
      char *pretty_name= (h [ 0 ]->name ? strdup(h [ 0 ]->name) : make_hack_name(XtDisplay(toplevel), h [ 0 ]->command));
...
static void
  sensitize_demo_widgets(
    ...
    Bool sensitive_p)
{
  const char *names []= {"cmdLabel", "cmdText", "enabled", "visLabel", "combo", "demo", "man"};
  int i;
    ...
    Widget toplevel)
{
  ...
  result= XpmCreatePixmapFromData(dpy, window, up_arrow_xpm, &up_pixmap, 0 /* mask */, &xpmattrs);
  ...
  result= XpmCreatePixmapFromData(dpy, window, down_arrow_xpm, &down_pixmap, 0 /* mask */, &xpmattrs);
    ...
    screenhack *hack)
{
  ...
  char *pretty_name= (hack->name ? strdup(hack->name) : make_hack_name(dpy, hack->command));
  ...
  if (doc_string)
  ...
  else
    {
      ...
      doc_string= strdup(
        ...
        "of this screen saver mode displayed here, along with graphical " "configuration options.");
    }
    ...
    prefs_pair *pair)
{
  ...
  char *pretty_name= (hack ? (hack->name ? strdup(hack->name) : make_hack_name(XtDisplay(toplevel), hack->command)) : 0);
...
static void
  sanity_check_resources(
    Widget toplevel)
{
  const char *names []= {"demoTab", "optionsTab", "cmdLabel", "visLabel", "enabled", "demo", "man", "timeoutLabel", "cycleLabel", "fadeSecondsLabel", "lockLabel", "passwdLabel"};
  int i;
  for (i= 0; i < countof(names); i++)
    {
      Widget w= name_to_widget(toplevel, names [ i ]);
...
static void
  the_network_is_not_the_computer(
    Widget parent)
{
  ...
  /* Make a buffer that's big enough for a number of copies of all the
     strings, plus some. */
  msg= (char *) malloc(10 * ((rversion ? strlen(rversion) : 0) + (ruser ? strlen(ruser) : 0) + (rhost ? strlen(rhost) : 0) + strlen(lhost) + strlen(luser) + strlen(d) + 1024));
  *msg= 0;
  ...
  char **argv)
{
  ...
  toplevel_shell= XtAppInitialize(&app, progclass, 0, 0, &argc, argv, defaults, 0, 0);
