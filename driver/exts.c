  ...
  saver_info *si)
{
...
#ifdef HAVE_XSHM_EXTENSION
      True,
      (Status (*)(Display *, int *, int *)) XShmQueryVersion /* 4 args */
#else
      False,
      0
#endif
  ...
  fprintf(stderr, "%s: vendor is %s, %d\n", blurb(), ServerVendor(si->dpy), VendorRelease(si->dpy));
  ...
  for (i= 0; i < si->nscreens; i++)
    {
      ...
      if (colormapped_depths)
        {
          fprintf(stderr, "%s: screen %d colormapped depths:", blurb(), ssi->real_screen_number);
          for (j= 0; j < 32; j++)
      ...
      if (non_mapped_depths)
        {
          fprintf(stderr, "%s: screen %d non-colormapped depths:", blurb(), ssi->real_screen_number);
          for (j= 0; j < 32; j++)
