    ...
    xmlNodePtr node)
{
  ...
  else if (! strcmp(name, "fullcommand"))
    {
      p->type= COMMAND;
    }
  else if (! strcmp(name, "_description"))
    {
      p->type= DESCRIPTION;
    }
  else if (! strcmp(name, "fakepreview"))
    {
      p->type= FAKEPREVIEW;
    }
  else if (! strcmp(name, "fake"))
    {
      p->type= FAKE;
    }
  ...
  if (p->type == SPINBUTTON)
    {
      ...
      if (! type || ! strcmp(type, "spinbutton"))
        {
          p->type= SPINBUTTON;
        }
  ...
  if (convert && ! strcmp(convert, "invert"))
    p->convert= INVERT;
  else if (convert && ! strcmp(convert, "ratio"))
    p->convert= RATIO;
  else
    p->convert= NONE;
  ...
  for (; node; node= node->next)
    {
      ...
      if (! strcmp(name, "hgroup") ||
        ! strcmp(name, "vgroup"))
        {
          GtkWidget *box= gtk_box_new(*name == 'h' ? GTK_ORIENTATION_HORIZONTAL : GTK_ORIENTATION_VERTICAL,
            0);
          GList *list2;
          gtk_widget_show(box);
          gtk_box_pack_start(GTK_BOX(parent), box, FALSE, FALSE, 0);

          list2= make_parameters_1(filename, node->xmlChildrenNode, box, changed_cb, changed_data);
          if (list2)
          ...
          if (p)
            {
              ...
              make_parameter_widget(filename, p, parent, changed_cb, changed_data);
            }
        }
    }
  return list;
    ...
    gpointer changed_data)
{
  for (; node; node= node->next)
    if (node->type == XML_ELEMENT_NODE &&
      ...
      return make_parameters_1(filename, node->xmlChildrenNode, parent, changed_cb, changed_data);
  return 0;
}
...
static gfloat
  ratio_to_range(
    ...
    gfloat ratio)
{
  return (ratio > 0.5 ? (1 + (2 * (ratio - 0.5) * (high - 1))) : (low + (2 * ratio * (1 - low))));
}
...
static gfloat
  range_to_ratio(
    ...
    gfloat value)
{
  return (value > 1 ? ((value - 1) / (2 * (high - 1))) + 0.5 : ((value - low) / (2 * (1 - low))));
}
    ...
    parameter *p)
{
  ...
  float value= (p->convert == INVERT ? invert_range(low, high, p->value) : p->value);
  ...
  return GTK_ADJUSTMENT(gtk_adjustment_new(value, low, high + page_size, si, pi, page_size));
}


static void
  set_widget_min_width(
    GtkWidget *w,
    int width)
{
      ...
      case STRING :
        {
          ...
          g_signal_connect(p->widget, "changed", G_CALLBACK(changed_cb), changed_data);
          break;
        }
      case FILENAME :
        {
          ...
          g_signal_connect(button, "clicked", G_CALLBACK(browse_button_cb), (gpointer) entry);
      ...
      case SLIDER :
        {
          ...
          p->widget= scale;
          g_signal_connect(adj, "value-changed", G_CALLBACK(changed_cb), changed_data);
          break;
        }
      case SPINBUTTON :
        {
      ...
      case BOOLEAN :
        {
           ...
           */
          gtk_box_pack_start(GTK_BOX(parent), p->widget, FALSE, FALSE, 4);
          g_signal_connect(p->widget, "clicked", G_CALLBACK(changed_cb), changed_data);
          break;
        }
      case SELECT :
        {
    ...
    gpointer user_data)
{
  ...
  if (file_chooser(win, entry, &file, _("Select file."), debug_p, FALSE, FALSE))
    {
      if (debug_p)
  ...
  switch (p->type)
    {
      ...
      case SPINBUTTON :
        ...
        {
          GtkAdjustment *adj=
            (p->type == SLIDER ? gtk_range_get_adjustment(GTK_RANGE(p->widget)) : gtk_spin_button_get_adjustment(GTK_SPIN_BUTTON(p->widget)));
          char buf [ 255 ];
          float value;
          switch (p->convert)
            {
              case INVERT :
                value= invert_range(GET_ADJ_LOWER(adj), GET_ADJ_UPPER(adj), GET_ADJ_VALUE(adj)) - 1;
                break;
              case RATIO :
      ...
      case BOOLEAN :
        if (! p->widget) return 0;
        {
          ...
          const char *s= (gtk_toggle_button_get_active(b) ? (char *) p->arg_set : (char *) p->arg_unset);
          if (s)
            return strdup(s);
          else
            return 0;
  ...
  for (rest= tokens; rest; rest= rest->next)
    {
      ...
      if (option && option [ 0 ] != '-' && option [ 0 ] != '+')
      ...
      else
        {
          ...
          parse_command_line_into_parameters_1(filename, parms, option, value, 0);
          if (value) free(value);
          free(option);
        }
    }
...
static gboolean
  parse_command_line_into_parameters_1(
    const char *filename,
    GList *parms,
    const char *option,
    const char *value,
  ...
  if (f)
    {
      ...
      parms= make_parameters(file, doc->xmlRootNode, vbox0, changed_cb, changed_data);
...
void set_configurator_command_line(
  conf_data *data,
  const char *full_command_line)
{
  char *prog;
