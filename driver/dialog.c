    ...
    Bool clear_p)
{
  ...
  switch (line->type)
    {
      case LABEL :
        ...
        if (*line->text)
          text= text2= xft_word_wrap(ws->dpy, line->font, line->text, right - left);
        break;
      ...
      case TEXT_RO :
         ...
         */
        XftTextExtentsUtf8_multi(ws->dpy, line->font, (FcChar8 *) text, strlen(text), &overall);
      ...
      case TEXT_RO :
        {
          ...
          tleft= left;
          draw_shaded_rectangle(ws->dpy, d, left, y, right - left, h, ws->shadow_width, (in_p ? ws->shadow_bottom : ws->shadow_top), (in_p ? ws->shadow_top : ws->shadow_bottom));
   ...
   */
  nlines= XftTextExtentsUtf8_multi(ws->dpy, line->font, (FcChar8 *) text, strlen(text), &overall);
  ...
  XftDrawStringUtf8_multi(ws->xftdraw,
    (line->button && line->button->disabled_p ? &line->fg2 : &line->fg),
    line->font,
    left + xoff2,
    y + ypad + yoff2 + line->font->ascent,
    (FcChar8 *) text,
    strlen(text),
    (align == LEFT ? 1 : align == CENTER ? 0 :
                                           -1));
#ifdef DEBUG_METRICS
  {
    GC gc;
  ...
  if (line->i_beam)
    draw_i_beam(ws->dpy, d, ws->foreground, left + xoff2 + overall.width, y + ypad + yoff2, line->font->ascent + line->font->descent);
  ...
  switch ((int) forked)
    {
      ...
      case 0 :
        ...
        sprintf(buf, "%s: pid %lu: couldn't exec %s", blurb(), (unsigned long) getpid(), argv [ 0 ]);
        perror(buf);
      ...
      default : /* parent fork */
        if (verbose_p)
          {
            int i;
            fprintf(stderr, "%s: pid %lu: launched", blurb(), (unsigned long) forked);
            for (i= 0; i < argc; i++)
...
{
  resource_keys(ws, &name, &rclass);
  return get_pixel_resource(ws->dpy, DefaultColormapOfScreen(ws->screen), (char *) name, (char *) rclass);
}
  ...
  for (screen= 0; screen < nscreens; screen++)
    {
      ...
      int win_x, win_y;
      unsigned int mask;
      int status= XQueryPointer(dpy, RootWindow(dpy, screen), &pointer_root, &pointer_child, &root_x, &root_y, &win_x, &win_y, &mask);
      if (status != None)
        {
    ...
    window_state *ws)
{
  ...
  XGrabPointer(dpy, root, True, (ButtonPressMask | ButtonReleaseMask | EnterWindowMask | LeaveWindowMask | PointerMotionMask | PointerMotionHintMask | Button1MotionMask | Button2MotionMask | Button3MotionMask | Button4MotionMask | Button5MotionMask | ButtonMotionMask), GrabModeAsync, GrabModeAsync, root, None, CurrentTime);
}
  ...
  if (! ws->xkb_opcode)
    {
      ...
      if (! XkbSelectEvents(ws->dpy, XkbUseCoreKbd, XkbMapNotifyMask | XkbStateNotifyMask, XkbMapNotifyMask | XkbStateNotifyMask))
        {
          if (verbose_p)
    ...
    int h)
{
  ...
  if (verbose_p)
    fprintf(stderr, "%s: %s input method\n", blurb(), ws->ic ? "attached" : "failed to attach");
    ...
    int splash_p)
{
  ...
  get_xft_color(ws, &ws->xft_text_foreground, "text.foreground", "Text.Foreground");
  get_xft_color(ws, &ws->xft_error_foreground, "error.foreground", "Error.Foreground");
  get_xft_color(ws, &ws->xft_button_foreground, "button.foreground", "Button.Foreground");
  dim_xft_color(ws, &ws->xft_button_foreground, ws->background, &ws->xft_button_disabled);
  ...
  /* Load the logo pixmap, based on font size */
  {
    ...
    ws->logo_pixmap= xscreensaver_logo(ws->screen, visual, root, ws->cmap, ws->background, &ws->logo_pixels, &ws->logo_npixels, &ws->logo_clipmask, logo_size);
    if (! ws->logo_pixmap) abort();
    XGetGeometry(dpy, ws->logo_pixmap, &root, &x, &y, &ws->logo_width, &ws->logo_height, &bw, &d);
  }
     ...
     window always be on top. */
  {
    ...
    XSelectInput(ws->dpy, root, xgwa.your_event_mask | SubstructureNotifyMask);
  }
    ...
    Window w)
{
  ...
  if (XGetClassHint(dpy, w, &ch))
    {
      fprintf(stderr, "0x%lx \"%s\", \"%s\"\n", (unsigned long) w, ch.res_class, ch.res_name);
      XFree(ch.res_class);
      XFree(ch.res_name);
    }
...
#define MIN_COLUMNS 22   /* Set window width based on headingFont ascent. */
  ...
  text_left= (thermo_x + ws->internal_padding +
    (ws->thermo_width ? ws->thermo_width + ws->shadow_width * 3 : 0));
...
#define BLANK_LINE                    \
      ...
      switch (ws->msgs [ j ].type)
        {
          ...
          case AUTH_MSGTYPE_ERROR :
            ...
            lines [ i ].font= (ws->msgs [ j ].type == AUTH_MSGTYPE_ERROR ? ws->error_font : ws->body_font);
            lines [ i ].fg= (ws->msgs [ j ].type == AUTH_MSGTYPE_ERROR ? ws->xft_error_foreground : ws->xft_foreground);
            lines [ i ].fg2= lines [ i ].fg;
            lines [ i ].bg= ws->background;
            lines [ i ].type= LABEL;
            lines [ i ].align= CENTER;
          ...
          case AUTH_MSGTYPE_PROMPT_ECHO :
            ...
            lines [ i ].text= (ws->auth_state == AUTH_FINISHED  ? _("Checking...") :
                ws->msgs [ j ].type == AUTH_MSGTYPE_PROMPT_ECHO ? ws->plaintext_passwd /* Hopefully UTF-8 */
                :
  ...
  lines [ i ].text= 0;
  y= draw_dialog_lines(ws, dbuf, lines, text_left, text_right, ws->border_width + ws->internal_padding + ws->shadow_width);
  window_height= y;
  ...
  lines [ i ].text= 0;
  y= draw_dialog_lines(ws, dbuf, lines, text_left, text_right, window_height - ws->internal_padding - ext_border - ws->shadow_width - (ws->button_font->ascent * 2));
      ...
      if (ws->auth_state != AUTH_NOTIFY)
        {
          ...
          XFillRectangle(dpy, dbuf, gc, thermo_x + ws->shadow_width, ext_border + ws->shadow_width, thermo_w, thermo_h2);
          if (thermo_h3 > 0)
            {
              XSetForeground(dpy, gc, ws->thermo_background);
              XFillRectangle(dpy, dbuf, gc, thermo_x + ws->shadow_width, ext_border + ws->shadow_width, thermo_w, thermo_h3);
            }
        }

      draw_shaded_rectangle(dpy, dbuf, thermo_x, ext_border, ws->thermo_width + ws->shadow_width * 2, window_height - ext_border * 2, ws->shadow_width, ws->shadow_bottom, ws->shadow_top);
    }
   ...
   */
  {
    ...
    XCopyArea(dpy, ws->logo_pixmap, dbuf, gc, 0, 0, ws->logo_width, ws->logo_height, x + xoff, y + yoff);
    XSetClipMask(dpy, gc, 0);
    draw_shaded_rectangle(dpy, dbuf, x, y, logo_frame_width, bot, ws->shadow_width, ws->shadow_bottom, ws->shadow_top);
  }

  /* The window's shadow */
  draw_shaded_rectangle(dpy, dbuf, ws->border_width, ws->border_width, window_width - ws->border_width * 2, window_height - ws->border_width * 2, ws->shadow_width, ws->shadow_top, ws->shadow_bottom);

  /* The window's border */
  draw_shaded_rectangle(dpy, dbuf, 0, 0, window_width, window_height, ws->border_width, ws->border_color, ws->border_color);
...
#else
        if (verbose_p)
          fprintf(stderr, "%s: re-creating window: %s\n", blurb(), size_changed_p ? "size changed" : "occluded");
  ...
  XCopyArea(dpy, dbuf, ws->window, gc, 0, 0, window_width, window_height, 0, 0);
  XSync(dpy, False);
  XFreeGC(dpy, gc);
    ...
    window_state *ws)
{
  ...
  memset(ws->plaintext_passwd_char_size, 0, sizeof(ws->plaintext_passwd_char_size));
  ...
  XftColorFree(ws->dpy, DefaultVisualOfScreen(ws->screen), DefaultColormapOfScreen(ws->screen), &ws->xft_foreground);
  XftColorFree(ws->dpy, DefaultVisualOfScreen(ws->screen), DefaultColormapOfScreen(ws->screen), &ws->xft_button_foreground);
  XftColorFree(ws->dpy, DefaultVisualOfScreen(ws->screen), DefaultColormapOfScreen(ws->screen), &ws->xft_text_foreground);
  XftColorFree(ws->dpy, DefaultVisualOfScreen(ws->screen), DefaultColormapOfScreen(ws->screen), &ws->xft_error_foreground);
 ...
 */
static void
  persistent_auth_status_failure(
    window_state *ws,
    Bool increment_p,
    Bool clear_p,
    int *count_ret,
    time_t *time_ret)
{
  ...
  /* Read the old property so that we can increment it.
    ...
    1: 32 bit time_t	1: 64 bit time_t hi
                2: 64 bit time_t lo
   */
  if (XGetWindowProperty(dpy, w, prop, 0, 999, False, XA_INTEGER, &type, &format, &nitems, &bytesafter, &dataP) == Success && type == XA_INTEGER && nitems >= 3 && dataP)
    {
  ...
  if (clear_p)
  ...
  else if (increment_p)
    {
      ...
      XChangeProperty(dpy, w, prop, XA_INTEGER, 32, PropModeReplace, (unsigned char *) vv, 3);
      if (verbose_p)
    ...
    Bool filter_p)
{
   ...
   */
  int decoded_size= XLookupString(event, (char *) decoded, sizeof(decoded), &keysym, &ws->compose_status);
  ...
  if (ws->ic && event->type == KeyPress)
    {
      ...
      Status s= 0;
      KeySym keysym2= 0;
      int size2= Xutf8LookupString(ws->ic, (XKeyPressedEvent *) event, decoded2, sizeof(decoded2) - 1, &keysym2, &s);
      decoded2 [ size2 ]= 0;
...
void xscreensaver_auth_test_mode(
  void)
{
    ...
    ws->auth_state == AUTH_NOTIFY)
    {
       ...
       */
      {
        ...
        if (verbose_p || debug_p)
          print_xinput_event(ws->dpy, &xev, ws->ic, (filtered_p ? " (filtered)" : ""));
        if (swap_p)
          {
 ...
 */
void xscreensaver_auth_finished(
  void *closure,
  ...
  if (authenticated_p)
    {
      ...
      persistent_auth_status_failure(ws, False, True, &unlock_failures, &failure_time);
    }
  else
    {
      /* Increment the failure count. */
      persistent_auth_status_failure(ws, True, False, &unlock_failures, &failure_time);
    }
      ...
      if (sec < warning_slack)
        {
          if (verbose_p)
            fprintf(stderr,
              "%s: ignoring recent unlock failures:" " %d within %d sec\n",
              blurb(),
              unlock_failures,
              warning_slack);
          goto END;
        ...
        {
          sprintf(ago, _("%d days ago"), days);
        }
      else if (hours > 1)
        {
          sprintf(ago, _("%d hours ago"), hours);
        }
      else if (min > 1)
        {
          sprintf(ago, _("%d minutes ago"), min);
        }
      else
        {
          sprintf(ago, _("just now"));
        }
...
void xscreensaver_splash(
  void *closure,
  Bool disable_settings_p)
{
  Widget root_widget= (Widget) closure;
