...
static void merge_system_screenhacks(Display *, saver_preferences *, screenhack **system_list, int count);
  ...
  Bool verbose_p)
{
  ...
  if (stat(name, &st) == 0)
    {
      ...
      if (fchmod(fileno(out), mode) != 0)
        {
          char *buf= (char *) malloc(1024 + strlen(name));
          sprintf(buf, "%s: error fchmodding \"%s\" to 0%o", blurb(), tmp_name, (unsigned int) mode);
          perror(buf);
          free(buf);
          goto END;
        }
...

  {
    ...
    char *whoami= (pw && pw->pw_name && *pw->pw_name ? pw->pw_name : "<unknown>");
...
#define CHECK(X) else if (!strcmp(pr, X))
      ...
      CHECK("grabDesktopImages")
      type= pref_bool,
      b= p->grab_desktop_p;
      CHECK("grabVideoFrames")
      type= pref_bool,
      b= p->grab_video_p;
      CHECK("chooseRandomImages")
      type= pref_bool,
      b= p->random_image_p;
      CHECK("imageDirectory")
      type= pref_str,
      s= p->image_directory;
      ...
      type= pref_str,
      s= programs;
      CHECK("pointerHysteresis")
      type= pref_int,
      i= p->pointer_hysteresis;
      CHECK("font")
      continue; /* don't save */
      CHECK("overlayStderr")
      continue; /* don't save */
      CHECK("overlayTextBackground")
      continue; /* don't save */
      CHECK("overlayTextForeground")
      continue; /* don't save */
      CHECK("bourneShell")
      continue; /* don't save */
      CHECK("authWarningSlack")
      type= pref_int,
      i= p->auth_warning_slack;
      else
      {
  ...
  saver_preferences *p)
{
  ...
  p->new_login_command= get_string_resource(dpy, "newLoginCommand", "Command");
  p->dialog_theme= get_string_resource(dpy, "dialogTheme", "String");
  p->settings_geom= get_string_resource(dpy, "settingsGeom", "String");
  p->auth_warning_slack= get_integer_resource(dpy, "authWarningSlack", "Integer");
...

  {
    ...
    if (s && ! strcasecmp(s, "one"))
      p->mode= ONE_HACK;
    else if (s && ! strcasecmp(s, "blank"))
      p->mode= BLANK_ONLY;
    else if (s && ! strcasecmp(s, "off"))
      p->mode= DONT_BLANK;
    else if (s && ! strcasecmp(s, "random-same"))
...

  {
    ...
    if (s && ! strcasecmp(s, "url"))
      p->tmode= TEXT_URL;
    else if (s && ! strcasecmp(s, "literal"))
      p->tmode= TEXT_LITERAL;
    else if (s && ! strcasecmp(s, "file"))
      p->tmode= TEXT_FILE;
    else if (s && ! strcasecmp(s, "program"))
  ...
  if (system_default_screenhack_count) /* note: first_time is also true */
    {
      merge_system_screenhacks(dpy, p, system_default_screenhacks, system_default_screenhack_count);
 ...
 */
static void
  merge_system_screenhacks(
    Display *dpy,
    saver_preferences *p,
    screenhack **system_list,
    int system_count)
{
          ...
          if (made_space == 0)
            {
              ...
              p->screenhacks= (screenhack **)
                realloc(p->screenhacks,
                  (p->screenhacks_count + made_space + 1) * sizeof(*p->screenhacks));
    ...
    Bool wrap_p)
{
  ...
  size= (2 * (strlen(hack->command) + (hack->visual ? strlen(hack->visual) : 0) + (hack->name ? strlen(hack->name) : 0) + tab));
  h2= (char *) malloc(size);
  out= h2;
