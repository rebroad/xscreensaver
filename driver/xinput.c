  ...
  int *opcode_ret)
{
  ...
  if (XISelectEvents(dpy, RootWindow(dpy, 0), evmasks, countof(evmasks)) != Success)
    {
      ...
      for (i= 0; i < ndevs; i++)
        {
          ...
          fprintf(stderr, "%s:   device %2d/%d: %s: %s\n", blurb(), d->deviceid, d->attachment, (d->use == XIMasterPointer ? "MP" : d->use == XIMasterKeyboard ? "MK" :
      ...
      case XI_RawMotion :
        {
          ...
          for (i= 0; i < ScreenCount(dpy); i++) /* query on correct screen */
            if (XQueryPointer(dpy, RootWindow(dpy, i), &root_ret, &subw, &root_x, &root_y, &win_x, &win_y, &mods))
              break;
        }
      default : break;
    }
  ...
  switch (evtype)
    {
      ...
      case XI_RawButtonPress :
      case XI_RawButtonRelease :
        out->xbutton.type= (evtype == XI_RawButtonPress ? ButtonPress : ButtonRelease);
      ...
      case XI_RawTouchEnd :
        out->xbutton.type= (evtype == XI_RawTouchBegin ? ButtonPress : ButtonRelease);
        out->xbutton.display= in->display;
        out->xbutton.root= in->root;
        out->xbutton.window= subw;
        out->xbutton.subwindow= subw;
  ...
  if (debug_p) /* Passwords show up in plaintext! */
    {
      ...
      if (ic && xkey->type == KeyPress)
        {
          ...
          Status s= 0;
          KeySym keysym2= 0;
          n= Xutf8LookupString(ic, (XKeyPressedEvent *) xkey, c2, sizeof(c2) - 1, &keysym2, &s);
          c2 [ n ]= 0;
      ...
      *mods= 0;
      ...
      fprintf(stderr, "%s: %s    0x%02X %s %s \"%s\"%s\n", blurb(), (x11_p ? (xkey->type == KeyPress ? "X11 KeyPress    " : "X11 KeyRelease  ") : (xkey->type == KeyPress ? "XI_RawKeyPress  " : "XI_RawKeyRelease")), xkey->keycode, mods, ks, c, (desc ? desc : ""));
    }
  else /* Log only that the KeyPress happened. */
    {
      fprintf(stderr, "%s: X11 Key%s\n", blurb(), (xkey->type == KeyPress ? "Press  " : "Release"));
    }
}


void print_xinput_event(
  Display *dpy,
  XEvent *xev,
  XIC ic,
  const char *desc)
  ...
  switch (xev->xany.type)
    {
      ...
      case ButtonPress :
      case ButtonRelease :
        fprintf(stderr, "%s: X11 Button%s   %d %d\n", blurb(), (xev->xany.type == ButtonPress ? "Press  " : "Release"), xev->xbutton.button, xev->xbutton.state);
        break;

      case MotionNotify :
        fprintf(stderr,
          "%s: X11 MotionNotify   %4d, %-4d" "                   %s\n",
          blurb(),
          xev->xmotion.x_root,
          xev->xmotion.y_root,
          (desc ? desc : ""));
  ...
  switch (re->evtype)
    {
      ...
      case XI_RawKeyRelease :
        if (debug_p)
          ...
          {
            fprintf(stderr, "%s: XI RawKey%s\n", blurb(), (re->evtype == XI_RawKeyPress ? "Press  " : "Release"));
          }
        break;
      ...
      case XI_RawButtonRelease :
        {
          ...
          fprintf(stderr, "%s: XI _RawButton%s %4d, %-4d\n", blurb(), (re->evtype == XI_RawButtonPress ? "Press  " : "Release"), root_x, root_y);
        }
        break;
