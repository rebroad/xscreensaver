    ...
    for (i= 0; i < sizeof(menus) / sizeof(*menus); i++)
      {
        ...
        if (menus [ i ][ 0 ] == '-')
        ...
        else
          {
            ...
            XtSetArg(av [ ac ], XmNsubMenuId, menu);
            ac++;
    ...
    Widget parent)
{
  ...
  ac= 0;
  XtSetArg(av [ ac ], XmNchildType, XmFRAME_TITLE_CHILD);
  ac++;
  ...
  ac= 0;
  XtSetArg(av [ ac ], XmNchildType, XmFRAME_WORKAREA_CHILD);
  ac++;
...
#ifdef HAVE_XMCOMBOBOX
  {
    Widget list;
    ac= 0;
    XtSetArg(av [ ac ], XmNcomboBoxType, XmDROP_DOWN_COMBO_BOX);
    ac++;
...
#else  /* !HAVE_XMCOMBOBOX */
  {
    ...
    for (i= 0; visual_menu [ i ]; i++)
      {
        XmString xs= XmStringCreate((char *) visual_menu [ i ],
          XmSTRING_DEFAULT_CHARSET);
        ac= 0;
        XtSetArg(av [ ac ], XmNlabelString, xs);
        ac++;
    ...
    ac= 0;
    XtSetArg(av [ ac ], XmNsubMenuId, popup_menu);
    ac++;
    ...
    Widget parent)
{
  ...
  ac= 0;
  XtSetArg(av [ ac ], XmNdialogType, XmDIALOG_PROMPT);
  ac++;

  ac= 0;
  XtSetArg(av [ ac ], XmNtopAttachment, XmATTACH_FORM);
  ac++;
  XtSetArg(av [ ac ], XmNbottomAttachment, XmATTACH_FORM);
  ac++;
  XtSetArg(av [ ac ], XmNleftAttachment, XmATTACH_FORM);
  ac++;
  XtSetArg(av [ ac ], XmNrightAttachment, XmATTACH_FORM);
  ac++;
  ...
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_END);
  ac++;
  timeout_label= XmCreateLabelGadget(preferences_form, "timeoutLabel", av, ac);
  ac= 0;
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_END);
  ac++;
  cycle_label= XmCreateLabelGadget(preferences_form, "cycleLabel", av, ac);
  ac= 0;
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_END);
  ac++;
  ...
  lock_timeout_text= XmCreateTextField(preferences_form, "lockText", av, ac);
  passwd_timeout_text= XmCreateTextField(preferences_form, "passwdText", av, ac);
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_BEGINNING);
  ac++;
  ...
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_BEGINNING);
  ac++;
  fade_toggle= XmCreateToggleButtonGadget(preferences_form, "fadeToggle", av, ac);
  ac= 0;
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_BEGINNING);
  ac++;
  unfade_toggle= XmCreateToggleButtonGadget(preferences_form, "unfadeToggle", av, ac);
  ac= 0;
  XtSetArg(av [ ac ], XmNalignment, XmALIGNMENT_BEGINNING);
  ac++;
  lock_toggle= XmCreateToggleButtonGadget(preferences_form, "lockToggle", av, ac);
  ac= 0;
