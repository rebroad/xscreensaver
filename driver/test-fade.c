  ...
  char **argv)
{
...

  {
    ...
    Window daemon_window=
      XCreateWindow(dpy, RootWindow(dpy, 0), 0, 0, 1, 1, 0, DefaultDepth(dpy, 0), InputOutput, DefaultVisual(dpy, 0), attrmask, &attrs);
  ...
  if (! XQueryExtension(dpy, XF86_VIDMODE_NAME, &op, &event, &error))
  ...
  else
    {
      ...
      fprintf(stderr, "%s: no support for display's " XF86_VIDMODE_NAME " extension\n", blurb());
...
#else  /* !HAVE_RANDR */
      fprintf(stderr, "%s: no support for display's " RANDR_NAME " extension\n", blurb());
#endif /* !HAVE_RANDR */
    }

  logo= xscreensaver_logo(screen, visual, root, DefaultColormap(dpy, 0), WhitePixel(dpy, 0), &logo_pixels, &logo_npixels, &logo_clipmask, True);
        ...
        for (x= 0; x < 2; x++)
          {
            ...
            windows [ nwindows ]=
              XCreateWindow(dpy, root, 200 + x * win_width * 1.5, 200 + y * win_height * 1.5, win_width, win_height, 0, DefaultDepth(dpy, 0), InputOutput, visual, attrmask, &attrs);
  ...
  while (1)
    {
      ...
      fflush(stderr);
      fade_screens(app, dpy, windows, nwindows, seconds, True, /* out_p */
        True,                                                  /* from_desktop_p */
        &state);
      ...
      fflush(stderr);
      fade_screens(app, dpy, windows, nwindows, seconds * ratio, True, /* out_p */
        False,                                                         /* from_desktop_p */
        &state);
      fade_screens(app, dpy, windows, nwindows, seconds * ratio, False, /* out_p */
        False,                                                          /* from_desktop_p */
        &state);
      XSync(dpy, False);
