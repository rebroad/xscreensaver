...
void sync_server_dpms_settings_1(
  Display *dpy,
  ...
  struct saver_preferences *p)
{
  ...
  if (change_count > 3)
    {
      fprintf(stderr,
        "%s: WARNING: some other program keeps changing" " the DPMS settings. That's bad.\n",
        blurb());
      change_count= 0;
    }
}
  ...
  saver_info *si)
{
  ...
  if (! DPMSQueryExtension(si->dpy, &event_number, &error_number))
  ...
  else
    {
      ...
      if (verbose_p > 1)
        fprintf(stderr, "%s: DPMSInfo = %s %s\n", blurb(), (state == DPMSModeOn ? "DPMSModeOn" : state == DPMSModeStandby ? "DPMSModeStandby" :
