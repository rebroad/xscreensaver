  ...
  for (i= 0; i < si->nscreens; i++)
    {
      ...
      fprintf(stderr, "%s: screen %d/%d: 0x%x, 0x%x\n", blurb(), ssi->real_screen_number, ssi->number, (unsigned int) RootWindowOfScreen(si->screens [ i ].screen), (unsigned int) si->screens [ i ].screensaver_window);
    }
    ...
    saver_info *si)
{
  ...
  toplevel_shell= XtAppInitialize(&si->app, progclass, &options, 0, &ac, av, defaults, 0, 0);
  ...
  if (daemon_window)
    {
      ...
      XSelectInput(si->dpy, daemon_window, xgwa.your_event_mask | PropertyChangeMask);

      /* For tracking changes to XA_SCREENSAVER_STATUS on the root. */
      XGetWindowAttributes(si->dpy, root, &xgwa);
      XSelectInput(si->dpy, root, xgwa.your_event_mask | PropertyChangeMask);
    }
  else
    {
      if (p->verbose_p)
      ...
      daemon_window= XCreateWindow(si->dpy, RootWindow(si->dpy, 0), 0, 0, 1, 1, 0, DefaultDepth(si->dpy, 0), InputOutput, DefaultVisual(si->dpy, 0), attrmask, &attrs);
      XStoreName(si->dpy, daemon_window, "XScreenSaver GFX");
      XSetClassHint(si->dpy, daemon_window, &class_hints);
      XChangeProperty(si->dpy, daemon_window, XA_WM_COMMAND, XA_STRING, 8, PropModeReplace, (unsigned char *) progname, strlen(progname));
      XChangeProperty(si->dpy, daemon_window, XA_SCREENSAVER_VERSION, XA_STRING, 8, PropModeReplace, (unsigned char *) version_number, strlen(version_number));
      XChangeProperty(si->dpy, daemon_window, XA_SCREENSAVER_ID, XA_STRING, 8, PropModeReplace, (unsigned char *) id, strlen(id));
      free(id);
  ...
  if (XRRQueryExtension(si->dpy,
        ...
        &si->randr_error_number))
    {
      ...
      for (i= 0; i < nscreens; i++)
        ...
        XRRSelectInput(si->dpy, RootWindow(si->dpy, i), RRScreenChangeNotifyMask);
    ...
    saver_info *si)
{
  ...
  if (XGetWindowProperty(si->dpy, w, XA_SCREENSAVER_STATUS, 0, 999, False, XA_INTEGER, &type, &format, &nitems, &bytesafter, &dataP) == Success && type == XA_INTEGER && nitems >= 3 && dataP)
    {
...
CYCLE:
  ...
  else if (si->all_clientmessages_p)
    {
      ...
      clientmessage_response(dpy, xev, False, "xscreensaver daemon not running");
    }
  else
    {
    ...
    Bool init_p)
{
  ...
  if (si->wayland_dpy)
    {
      ...
      XtAppAddInput(si->app, fd, (XtPointer) (XtInputReadMask | XtInputExceptMask), wayland_dpy_cb, (XtPointer) si);
  ...
  si->activity_time= (si->selection_mode == 0 ? si->blank_time - p->timeout / 1000 : si->blank_time);
          ...
          if (p->verbose_p)
            {
              ...
              fprintf(stderr, "%s: %d: screen change event: %s\n", blurb(), screen, (changed_p ? "new layout:" : "layout unchanged"));
              if (changed_p)
                describe_monitor_layout(si->monitor_layout);
            }
