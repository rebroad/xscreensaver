    ...
    enum zwlr_output_power_v1_mode mode)
{
  ...
  out->wlr_mode= mode;
  if (verbose_p > 2)
    fprintf(stderr, "%s: wayland: dpms: output %s is %s\n", blurb(), out->name, (out->wlr_mode == ZWLR_OUTPUT_POWER_V1_MODE_ON ? "on" : "off"));
}
    ...
    uint32_t mode)
{
  ...
  out->kde_mode= mode;
  if (verbose_p > 2)
    fprintf(stderr, "%s: wayland: dpms: output %s is %s\n", blurb(), out->name, (out->kde_mode == ORG_KDE_KWIN_DPMS_MODE_ON ? "on" : out->kde_mode == ORG_KDE_KWIN_DPMS_MODE_STANDBY ? "standby" :
  ...
  if (! strcmp(iface, wl_output_interface.name))
    {
      ...
      out->wl_output= wl_registry_bind(reg, name, &wl_output_interface, version);
  ...
  else if (! strcmp(iface, zwlr_output_power_manager_v1_interface.name))
    {
      if (verbose_p)
        fprintf(stderr, "%s: wayland: dpms: found: %s\n", blurb(), iface);
      state->wlr_mgr=
        wl_registry_bind(reg, name, &zwlr_output_power_manager_v1_interface, version);
    }
  else if (! strcmp(iface, org_kde_kwin_dpms_manager_interface.name))
    {
      if (verbose_p)
        fprintf(stderr, "%s: wayland: dpms: found: %s\n", blurb(), iface);
      state->kde_mgr=
        wl_registry_bind(reg, name, &org_kde_kwin_dpms_manager_interface, version);
    }
}
 ...
 */
Bool wayland_monitor_powered_on_p(
  wayland_dpms *state)
{
  Bool off_p= False;
  Bool failed_p= False;
