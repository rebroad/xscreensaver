    ...
    void)
{
  ...
  sprintf(buf, "%s: %02d:%02d:%02d.%03lu", progname, tm->tm_hour, tm->tm_min, tm->tm_sec, (unsigned long) (tv.tv_usec / 1000));
  return buf;
}
  ...
  switch (col->type)
    {
      ...
      case TTIME : sprintf(out, "%*.3f", col->width, (double) (*((unsigned int *) val)) / 1000.0); break;
      default : abort(); break;
    }
}
      ...
      case ESTATE :
        {
          ...
          if (i & ~(ShiftMask | LockMask | ControlMask | Mod1Mask | Mod2Mask | Mod3Mask | Mod4Mask | Mod5Mask | Button1Mask | Button2Mask | Button3Mask | Button4Mask | Button5Mask))
            {
              err= "BAD MODS";
              break;
            }
  ...
  switch (xev->xany.type)
    {
      ...
      case KeyRelease :
        {
          ...
          fields [ ETYPE ]= (void *) (xev->xany.type == KeyPress ? "KeyPress" : "KeyRelease");
      ...
      case ButtonRelease :
        {
          fields [ ETYPE ]= (void *) (xev->xany.type == ButtonPress ? "ButtonPress" : "ButtonRelease");
  ...
  if (grab_kbd_p || grab_mouse_p)
    {
      ...
      if (grab_kbd_p)
        {
          status= XGrabKeyboard(dpy, w, True, (mouse_sync_p ? GrabModeSync : GrabModeAsync), (kbd_sync_p ? GrabModeSync : GrabModeAsync), CurrentTime);
          if (status == GrabSuccess)
            {
              fprintf(stdout, "%s: grabbed keyboard (%s, %s)\n", blurb(), (mouse_sync_p ? "sync" : "async"), (kbd_sync_p ? "sync" : "async"));
            }
          else
            {
              fprintf(stdout, "%s: failed to grab keyboard (%s, %s): %s\n", blurb(), (mouse_sync_p ? "sync" : "async"), (kbd_sync_p ? "sync" : "async"), grab_string(status));
              exit(1);
            }
        }
      ...
      if (grab_mouse_p)
        {
          status= XGrabPointer(dpy, w, True, (ButtonPressMask | ButtonReleaseMask | EnterWindowMask | LeaveWindowMask | PointerMotionMask | PointerMotionHintMask | Button1MotionMask | Button2MotionMask | Button3MotionMask | Button4MotionMask | Button5MotionMask | ButtonMotionMask), (mouse_sync_p ? GrabModeSync : GrabModeAsync), (kbd_sync_p ? GrabModeSync : GrabModeAsync), w, cursor, CurrentTime);
          if (status == GrabSuccess)
            {
