    ...
    gid_t gid)
{
  ...
  p= getpwuid(uid);
  g= getgrgid(gid);
  sprintf(buf, "%.100s/%.100s (%ld/%ld)", (p && p->pw_name ? p->pw_name : "???"), (g && g->gr_name ? g->gr_name : "???"), (long) uid, (long) gid);
  return buf;
}
    ...
    gid_t gid)
{
  ...
  if (uid_errno == 0 && gid_errno == 0 && sgs_errno == 0)
    {
      if (verbose_p)
        fprintf(stderr, "%s: changed uid/gid to %.100s/%.100s (%ld/%ld)\n", blurb(), (p && p->pw_name ? p->pw_name : "???"), (g && g->gr_name ? g->gr_name : "???"), (long) uid, (long) gid);
      return 0;
    }
  else
    {
      ...
      if (sgs_errno)
        {
          sprintf(buf, "%s: couldn't setgroups to %.100s (%ld)", blurb(), (g && g->gr_name ? g->gr_name : "???"), (long) gid);
          if (sgs_errno == -1)
            {
              ...
              for (i= 0; i < n; i++)
                {
                  ...
                  if (g && g->gr_name)
                    fprintf(stderr, "%s", g->gr_name);
                  else
                    fprintf(stderr, "%ld", (long) groups [ i ]);
                }
              fprintf(stderr, "]\n");
            }
        }
      ...
      if (gid_errno)
        {
          sprintf(buf, "%s: couldn't set gid to %.100s (%ld)", blurb(), (g && g->gr_name ? g->gr_name : "???"), (long) gid);
          if (gid_errno == -1)
            {
              fprintf(stderr, "%s: unknown error\n", buf);
            }
      ...
      if (uid_errno)
        {
          sprintf(buf, "%s: couldn't set uid to %.100s (%ld)", blurb(), (p && p->pw_name ? p->pw_name : "???"), (long) uid);
          if (uid_errno == -1)
            {
              fprintf(stderr, "%s: unknown error\n", buf);
            }
  ...
  if (! p ||
    ...
    ! strcmp(p->pw_name, "games"))
    {
      fprintf(stderr,
        ...
        blurb(),
        (p && p->pw_name && *p->pw_name ? p->pw_name : "<unknown>"));
      exit(1);
    }
