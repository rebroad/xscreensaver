    ...
    char **errP)
{
      ...
      for (k= 0; k < res->noutput; k++, j++)
        {
          ...
          m->screen= screen;
          m->id= (i * 1000) + j;
          m->desc= (rroi && rroi->name ? strdup(rroi->name) : strdup(buf));
  ...
  /* Work around more fucking brain damage. */
  {
    ...
    if (! ok)
      {
        *errP= append(*errP,
          (i == 0 ? "WARNING: RANDR reports no screens" : "WARNING: RANDR says all screens are 0x0"));
        free_monitors(monitors);
        monitors= 0;
      }
  }
 ...
 */
static monitor **
  randr_versus_xinerama_fight(
    Display *dpy,
    monitor **randr_monitors,
    char **errP)
{
  ...
  if (randr_count == xinerama_count)
    ...
    sprintf(buf, "RANDR screens: %d, Xinerama: %d", randr_count, xinerama_count);
  ...
  while (token)
    {
      ...
      m->id= nscreens;
      m->screen= screen;
      if (4 != sscanf(token, "%dx%d+%d+%d%c", &m->width, &m->height, &m->x, &m->y, &c))
        abort();
      m->width-= 2;
      m->height-= 2;
      monitors [ nscreens++ ]= m;
      ...
      for (i= 0; i < count; i++)
        {
          ...
          fprintf(stderr, "%s:     %3d/%d: %dx%d+%d+%d", blurb(), m->id, screen_number(m->screen), m->width, m->height, m->x, m->y);
              ...
              switch (m->sanity)
                {
                  ...
                  case S_OVERLAP :
                    ...
                    break;
                  case S_OFFSCREEN :
                    fprintf(stderr, "off screen (%dx%d)\n", WidthOfScreen(e->screen), HeightOfScreen(e->screen));
                    break;
                  case S_DISABLED :
