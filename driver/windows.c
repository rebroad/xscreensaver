  ...
  store_saver_status(saver_info * si)
{
  ...
  XChangeProperty(si->dpy, w, XA_SCREENSAVER_STATUS, XA_INTEGER, 32, PropModeReplace, (unsigned char *) status, nitems);
  XUngrabServer(dpy);
  XSync(dpy, False);
  ...
  if (si->prefs.debug_p && si->prefs.verbose_p)
    {
      int i;
      fprintf(stderr, "%s: wrote status property: 0x%lx: ", blurb(), (unsigned long) w);
      for (i= 0; i < nitems; i++)
        {
          if (i > 0) fprintf(stderr, ", ");
          if (i == 0 || i == 2)
            fprintf(stderr, "%s", (status [ i ] == XA_LOCK ? "LOCK" : status [ i ] == XA_BLANK ? "BLANK" :
    ...
    saver_screen_info *ssi)
{
  ...
  if (! p->verbose_p || printed_visual_info)
  ...
  else if (ssi->current_visual == DefaultVisualOfScreen(ssi->screen))
    {
      ...
      describe_visual(stderr, ssi->screen, ssi->current_visual, install_cmap_p);
    }
  else
    {
      ...
      describe_visual(stderr, ssi->screen, DefaultVisualOfScreen(ssi->screen), ssi->install_cmap_p);
    }
  ...
  if (ssi->screensaver_window)
    {
      ...
      if (! (XConfigureWindow(si->dpy, ssi->screensaver_window, changesmask, &changes) &&
            XChangeWindowAttributes(si->dpy, ssi->screensaver_window, attrmask, &attrs)))
        {
  ...
  if (! ssi->screensaver_window)
    {
      ssi->screensaver_window=
        XCreateWindow(si->dpy, RootWindowOfScreen(ssi->screen), ssi->x, ssi->y, ssi->width, ssi->height, 0, ssi->current_depth, InputOutput, ssi->current_visual, attrmask, &attrs);
      xscreensaver_set_wm_atoms(si->dpy, ssi->screensaver_window, ssi->width, ssi->height, 0);
  ...
  if (! ssi->cursor)
    {
      Pixmap bit=
        XCreatePixmapFromBitmapData(si->dpy, ssi->screensaver_window, "\000", 1, 1, BlackPixelOfScreen(ssi->screen), BlackPixelOfScreen(ssi->screen), 1);
      ssi->cursor= XCreatePixmapCursor(si->dpy, bit, bit, &black, &black, 0, 0);
...
void initialize_screensaver_window(
  saver_info *si)
{
  int i;
  for (i= 0; i < si->nscreens; i++)
 ...
 */
void resize_screensaver_window(
  saver_info *si)
{
  saver_preferences *p= &si->prefs;
  int i;
  ...
  saver_info *si)
{
  ...
  if (p->fade_p &&
    ...
    ! si->emergency_p)
    {
      ...
      user_active_p= fade_screens(si->app, si->dpy, current_windows, si->nscreens, p->fade_seconds / 1000.0, True, /* out_p */
        True,                                                                                                      /* from_desktop_p */
        &si->fade_state);
  ...
  if (unfade_p)
    {
      ...
      interrupted_p= fade_screens(si->app, si->dpy, current_windows, si->nscreens, seconds * ratio, True, /* out_p */
      ...
      if (! interrupted_p)
        interrupted_p= fade_screens(si->app, si->dpy, current_windows, si->nscreens, seconds * (1 - ratio), False, /* out_p */
          False,                                                                                                   /* from_desktop_p */
          &si->fade_state);
      free(current_windows);

      if (p->verbose_p)
        fprintf(stderr, "%s: unfading done%s\n", blurb(), (interrupted_p ? " (interrupted)" : ""));
    }
  else
    {
      for (i= 0; i < si->nscreens; i++)
          ...
          if (ssi->cmap)
            {
              ...
              /* avoid technicolor */
              XSetWindowBackground(si->dpy, ssi->screensaver_window, BlackPixelOfScreen(ssi->screen));
              XClearWindow(si->dpy, ssi->screensaver_window);
...
static Visual *
  get_screen_gl_visual(
    saver_info *si,
    int real_screen_number)
{
  ...
  if (si->ssi_count < count)
    {
      ...
      memset(si->screens + si->ssi_count, 0, sizeof(*si->screens) * (count - si->ssi_count));
      si->ssi_count= count;
    }
  ...
  const char *error)
{
  ...
  window= ssi->error_dialog=
    XCreateWindow(si->dpy, RootWindowOfScreen(ssi->screen), 0, 0, 1, 1, 0, ssi->current_depth, InputOutput, visual, attrmask, &attrs);
  ...
  xscreensaver_set_wm_atoms(si->dpy, window, changes.width, changes.height, ssi->screensaver_window);
  XMapRaised(si->dpy, window);
  XClearWindow(si->dpy, window);

  XDrawRectangle(si->dpy, window, gc, gcv.line_width / 2, gcv.line_width / 2, changes.width - gcv.line_width, changes.height - gcv.line_width);
...
#define WHEN(TT) do { \
  ...
  si->watchdog_id= XtAppAddTimeOut(si->app,
    ...
    (XtPointer) si);
  if (p->verbose_p > 1)
    fprintf(stderr, "%s: watchdog in %d:%02d:%02d\n", blurb(), (int) (when - now) / (60 * 60), (int) ((when - now) % (60 * 60)) / 60, (int) (when - now) % 60);
}
