...
static void describe_dead_child(saver_info *, pid_t, int wait_status, struct rusage);
  ...
  void)
{
  ...
  for (job= jobs; job; job= job->next)
    fprintf(stderr, "  %5ld: %2d: (%s) %s %s\n", (long) job->pid, job->screen, (job->status == job_running ? "running" : job->status == job_stopped ? "stopped" :
                                                                                   job->status == job_killed                                        ? " killed" :
                                                                                   job->status == job_dead                                          ? "   dead" :
                                                                                                                                                      "    ???"),
      (job->killed ? timestring(job->killed) : job->launched ? timestring(job->launched) :
                                                               "??:??:??"),
      job->name);
  fprintf(stderr, "\n");
}
  ...
  for (prev= 0, job= jobs, next= (job ? job->next : 0);
    ...
    prev= job, job= next, next= (job ? job->next : 0))
    {
      if (job->status == job_dead)
        ...
        {
          fprintf(stderr,
            "%s: WARNING: pid %ld (%s) sent SIGTERM %ld seconds ago" " and did not die!\n",
            blurb(),
            (long) job->pid,
            job->name,
            (long) (now - job->killed));
    ...
    int signal)
{
  ...
  if (p->verbose_p)
    fprintf(stderr, "%s: %d: %s pid %lu (%s)\n", blurb(), job->screen, (job->status == job_killed ? "killing" : job->status == job_stopped ? "suspending" :
                                                                                                                                             "resuming"),
  ...
  if (p->verbose_p && status < 0)
    {
      if (errno == ESRCH)
        ...
        {
          char buf [ 1024 ];
          sprintf(buf, "%s: %d: couldn't kill child process %lu (%s)", blurb(), job->screen, (unsigned long) job->pid, job->name);
          perror(buf);
        }
    }
    ...
    RETSIGTYPE (*handler)(int))
{
  ...
  if (((long) signal(sig, handler)) == -1L)
    ...
    {
      char buf [ 255 ];
      sprintf(buf, "%s: couldn't catch signal %s", blurb(), signal_name(sig));
      perror(buf);
      abort();
    }
}
  ...
  if (WIFEXITED(wait_status))
    {
      ...
      if (p->verbose_p)
        fprintf(stderr,
          "%s: %d: child pid %lu (%s) exited abnormally" " with status %d\n",
          blurb(),
          screen_no,
          (unsigned long) kid,
          name,
  ...
  else if (WIFSIGNALED(wait_status))
    {
      ...
      if (job &&
        ...
        WTERMSIG(wait_status) == SIGTERM)
        {
          ...
          if (p->verbose_p)
            fprintf(stderr,
              "%s: %d: child pid %lu (%s)" " exited normally with %s\n",
              blurb(),
              screen_no,
              (unsigned long) kid,
              name,
      ...
      else
        {
          ...
          if (p->verbose_p)
            fprintf(stderr,
              "%s: %d: child pid %lu (%s)" " unexpectedly terminated with %s\n",
              blurb(),
              screen_no,
              (unsigned long) kid,
              name,
    ...
    {
      if (p->verbose_p)
        fprintf(stderr, "%s: child pid %lu (%s) stopped with %s\n", blurb(), (unsigned long) kid, name, signal_name(WSTOPSIG(wait_status)));
      if (job)
  ...
  else
    {
      ...
      fprintf(stderr, "%s: child pid %lu (%s) died in a mysterious way!", blurb(), (unsigned long) kid, name);
      if (job)
        job->status= job_dead;
    }
    ...
    XtIntervalId *id)
{
  ...
  if (ssi->pid)
    {
      fprintf(stderr, "%s: %d: %ld: born to ill\n", blurb(), ssi->number, (unsigned long) ssi->pid);
      kill(ssi->pid, SIGILL);
    }
}
#endif /* ABORT_TESTER */
  ...
  switch ((int) (forked= fork()))
    {
      ...
      default : /* parent */
        ...
        if (p->verbose_p)
          fprintf(stderr,
            "%s: %d: forked \"%s\" in pid %lu" " on window 0x%lx\n",
            blurb(),
            (ssi ? ssi->number : 0),
            command,
...
#ifdef ABORT_TESTER
  if (forked)
    XtAppAddTimeOut(si->app, 1000 * (5 + (5 * ssi->number)), abort_debug_timer, (XtPointer) ssi);
#endif
    ...
    screenhack *hack)
{
  ...
  if (! selected && (p->verbose_p || si->demoing_p))
    fprintf(stderr,
      (si->demoing_p ? "%s: warning, no \"%s\" visual for \"%s\"\n" : "%s: no \"%s\" visual; skipping \"%s\"\n"),
      blurb(),
  ...
  if (! monitor_powered_on_p(si))
    {
      if (si->prefs.verbose_p)
        fprintf(stderr,
          "%s: %d: X says monitor has powered down; " "not launching a hack\n",
          blurb(),
          ssi->number);
      ssi->current_hack= -1;
      ...
      if (p->screenhacks_count < 1)
      ...
      else if (si->selection_mode == -2)
        {
          ...
          if (ssi->current_hack < 0)
            ...
            new_hack= ((ssi->current_hack + p->screenhacks_count - 1) % p->screenhacks_count);
        }
      ...
      else /* (p->mode == RANDOM_HACKS) */
        {
          ...
          while ((new_hack= random() % p->screenhacks_count) == ssi->current_hack);
        }
     ...
     use it regardless.
      ...
      if (! si->best_gl_visuals ||
        ! si->best_gl_visuals [ ssi->real_screen_number ])
        {
           ...
           */
          screenhack_obituary(ssi, "", "No GL visuals: the xscreensaver-gl* packages are required.");
          goto DONE;
        }
      ...
      XChangeProperty(si->dpy, ssi->screensaver_window, XA_WM_COMMAND, XA_STRING, 8, PropModeReplace, (unsigned char *) hack->command, strlen(hack->command));
      XChangeProperty(si->dpy, ssi->screensaver_window, XA_NET_WM_PID, XA_CARDINAL, 32, PropModeReplace, (unsigned char *) &ssi->pid, 1);
    }
  ...
  if (! si->demoing_p && p->cycle)
    {
      ...
      if (p->verbose_p)
        fprintf(stderr, "%s: %d: next cycle in %d:%02d:%02d at %s\n", blurb(), ssi->number, (int) (how_long / 1000) / (60 * 60), (int) ((how_long / 1000) % (60 * 60)) / 60, (int) (how_long / 1000) % 60, timestring(ssi->cycle_at));
    }
}


void kill_screenhack(
  saver_screen_info *ssi)
{
  saver_info *si= ssi->global;
  if (ssi->pid)
...
Bool any_screenhacks_running_p(
  saver_info *si)
{
    ...
    Screen *screen)
{
  ...
  if (! v)
    ...
    fprintf(stderr, "%s: %d: GL visual is 0x%X%s\n", blurb(), screen_number(screen), (unsigned int) XVisualIDFromVisual(v), (v == DefaultVisualOfScreen(screen) ? " (default)" : ""));
  return v;
}
          ...
          *buf= 0;
          ...
          if (result == 0)
          ...
          else
            {
              ...
              if (si->prefs.verbose_p)
                fprintf(stderr, "%s: %d: %s: GL visual is 0x%X%s\n", blurb(), screen_number(screen), av [ 0 ], result, (v == DefaultVisualOfScreen(screen) ? " (default)" : ""));
              return v;
            }
        }
    }
