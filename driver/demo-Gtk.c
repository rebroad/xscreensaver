...
{
  GtkWidget *dialog=
    (button_type == D_NONE ? gtk_dialog_new_with_buttons(title, win, GTK_DIALOG_DESTROY_WITH_PARENT, _("_OK"), D_NONE, NULL) : gtk_dialog_new_with_buttons(title, win, GTK_DIALOG_DESTROY_WITH_PARENT, (button_type == D_LAUNCH ? _("Launch") : button_type == D_GNOME ? _("Kill") :
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(warning_dialog_cb), win);
    ...
    int arg)
{
  ...
  if (s->debug_p)
    fprintf(stderr, "%s: command: %s %d\n", blurb(), XGetAtomName(s->dpy, command), arg);
    ...
    Bool report_errors_p)
{
  ...
  status= xscreensaver_command(s->dpy, XA_DEMO, hack_number + 1, FALSE, &err);
  ...
  switch ((int) forked)
    {
      ...
      default : /* parent fork */
        ...
        if (s->debug_p)
          {
            int i;
            fprintf(stderr, "%s: pid %lu: forked:", blurb(), (unsigned long) forked);
            for (i= 0; i < argc; i++)
              if (strchr(argv [ i ], ' '))
    ...
    gpointer user_data)
{
  ...
  if (! p->help_url || ! *p->help_url)
    {
      warning_dialog(GTK_WINDOW(win), _("Error"), _("No Help URL has been specified.\n"));
      return;
    }

  help_command= (char *) malloc(strlen(p->load_url_command) +
    (strlen(p->help_url) * 5) + 20);
  sprintf(help_command, p->load_url_command, p->help_url, p->help_url, p->help_url, p->help_url, p->help_url);
    ...
    saver_preferences *p)
{
  ...
  if (! write_init_file(s->dpy, p, s->short_version, FALSE))
  ...
  else
    {
      ...
      if (! f || ! *f)
        {
          warning_dialog(GTK_WINDOW(win), _("Error"), _("Couldn't determine init file name!\n"));
        }
      else
        {
    ...
    gpointer data)
{
  ...
  gtk_tree_model_get(model, iter, COL_ENABLED, &checked, -1);

  if (flush_changes(closure->s, closure->i, checked, 0, 0))
...
#define TEXT(PREF, WIDGET) \
...

  {
    ...
    Bool od= s->debug_p;
    s->debug_p= FALSE;
    CHECKBOX(v, text_host_radio);
    if (v) p2->tmode= TEXT_DATE;
    CHECKBOX(v, text_radio);
    if (v) p2->tmode= TEXT_LITERAL;
    CHECKBOX(v, text_file_radio);
    if (v) p2->tmode= TEXT_FILE;
    CHECKBOX(v, text_program_radio);
    if (v) p2->tmode= TEXT_PROGRAM;
    CHECKBOX(v, text_url_radio);
    if (v) p2->tmode= TEXT_URL;
    s->debug_p= od;
  }
...
#undef TEXT
  ...
  if (p2->mode == ONE_HACK)
    {
      int list_elt= selected_list_element(s);
      p2->selected_hack= (list_elt >= 0 ? s->list_elt_to_hack_number [ list_elt ] : -1);
    }
    ...
    gpointer user_data)
{
  ...
  if (s->debug_p)
    {
      if (s->flushing_p)
        fprintf(stderr, "%s:   (pref changed: %s)\n", blurb(), gtk_widget_get_name(widget));
      else
        fprintf(stderr, "%s: pref changed: %s\n", blurb(), gtk_widget_get_name(widget));
    }
    ...
    gpointer data)
{
  ...
  gtk_tree_model_get(model, &iter, COL_ENABLED, &active, -1);

  gtk_list_store_set(GTK_LIST_STORE(model), &iter, COL_ENABLED, ! active, -1);
 ...
 */
static void
  validate_image_directory_quick(
    state *s)
{
  saver_preferences *p= &s->prefs;
  char *warn= 0;
...
static void
  validate_image_directory_cancel_cb(
    GtkDialog *dialog,
    gint response_id,
    gpointer user_data)
{
    ...
    const char *path)
{
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(validate_image_directory_cancel_cb), &closed_p);
  g_signal_connect(dialog, "close", G_CALLBACK(validate_image_directory_cancel_cb), &closed_p);
 ...
 */
G_MODULE_EXPORT gboolean
  image_text_pref_changed_event_cb(
    GtkWidget *widget,
    GdkEvent *event,
    gpointer user_data)
{
    ...
    gboolean program_p)
{
  ...
  GtkWidget *dialog=
    gtk_file_chooser_dialog_new(title, parent, (dir_p ? GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER : GTK_FILE_CHOOSER_ACTION_OPEN), _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Select"), GTK_RESPONSE_ACCEPT, NULL);
    ...
    gpointer user_data)
{
  ...
  settings_switch_page_cb(GTK_NOTEBOOK(dialog->opt_notebook), NULL, 0, s->dialog);
     ...
     which already ran at startup. */
  {
    ...
    if (! old || ! *old ||
      4 != sscanf(old, " %d , %d %d , %d %c", &win_x, &win_y, &dialog_x, &dialog_y, &dummy))
    ...
    state *s)
{
  ...
  if (XGetWindowProperty(s->dpy, RootWindow(s->dpy, 0), /* always screen #0 */
        ...
        &bytesafter,
        &dataP) == Success &&
    type == XA_INTEGER && nitems >= 4 && dataP)
    {
  ...
  if (! model)
    {
      ...
      gtk_tree_view_insert_column_with_attributes(list, COL_ENABLED, _("Use"), ren, "active", COL_ENABLED, NULL);

      g_signal_connect(ren, "toggled", G_CALLBACK(list_checkbox_cb), s);

      ren= gtk_cell_renderer_text_new();
      gtk_tree_view_insert_column_with_attributes(list, COL_NAME, _("Screen Saver"), ren, "markup", COL_NAME, NULL);

      g_signal_connect_after(list, "row_activated", G_CALLBACK(list_activated_cb), s);

      selection= gtk_tree_view_get_selection(list);
      g_signal_connect(selection, "changed", G_CALLBACK(list_select_changed_cb), s);
    }
  ...
  for (i= 0; i < s->list_count; i++)
    {
      ...
      pretty_name= (hack->name ? strdup(hack->name) : make_hack_name(s->dpy, hack->command));
      ...
      if (! available_p)
        {
          ...
          sprintf(buf, "<span foreground=\"#%02X%02X%02X\">%s</span>", (unsigned int) (0xFF * fg.red), (unsigned int) (0xFF * fg.green), (unsigned int) (0xFF * fg.blue), pretty_name);
          free(pretty_name);
          pretty_name= buf;
        }

      gtk_list_store_append(model, &iter);
      gtk_list_store_set(model, &iter, COL_ENABLED, hack->enabled_p, COL_NAME, pretty_name, -1);
      free(pretty_name);
    }
}
    ...
    if (cbox)
      {
        ...
        while ((name= strtok_r(token, ",", &last)))
          {
            ...
            while (*name == ' ' || *name == '\t' || *name == '\n')
              name++;
            L= strlen(name);
            while (L && (name [ L - 1 ] == ' ' || name [ L - 1 ] == '\t' || name [ L - 1 ] == '\n'))
              name [ --L ]= 0;
        ...
        if (! signal_connected_p)
          {
            g_signal_connect(G_OBJECT(cbox), "changed", G_CALLBACK(pref_changed_cb), (gpointer) win);
...
#define SENSITIZE(NAME, SENSITIVEP) \
  ...
  SENSITIZE(fade_label, ((p->fade_p || p->unfade_p) && s->grabbing_supported_p));
  SENSITIZE(fade_spinbutton, ((p->fade_p || p->unfade_p) && s->grabbing_supported_p));
    ...
    Bool force_p)
{
  ...
  gtk_widget_set_sensitive(win->lock_menuitem, (running_p && s->locking_supported_p));
  gtk_widget_set_sensitive(win->kill_menuitem, running_p);

  gtk_menu_item_set_label(GTK_MENU_ITEM(win->restart_menuitem),
    (running_p ? _("Restart Daemon") : _("Launch Daemon")));
}
...
#undef TRY
  ...
  if (p->mode == BLANK_ONLY)
    ...
    {
      int hack_number= (list_elt >= 0 && list_elt < s->list_count ? s->list_elt_to_hack_number [ list_elt ] : -1);
      hack= (hack_number >= 0 ? p->screenhacks [ hack_number ] : 0);

      pretty_name= (hack ? (hack->name ? strdup(hack->name) : make_hack_name(s->dpy, hack->command)) : 0);
  ...
  {
    char title [ 255 ];
    sprintf(title, _("%s: %.100s Settings"), progclass, (pretty_name ? pretty_name : "???"));
  ...
  if (vis)
    gtk_entry_set_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN(vis))),
      (hack ? (hack->visual && *hack->visual ? hack->visual : _("Any")) : ""));
    ...
    state *s)
{
  ...
  for (i= 0; i < p->screenhacks_count; i++)
    {
      screenhack *hack= p->screenhacks [ i ];
      char *name= (hack->name && *hack->name ? strdup(hack->name) : make_hack_name(s->dpy, hack->command));
    ...
    state *s)
{
  XScreenSaverWindow *win= XSCREENSAVER_WINDOW(s->window);
  int list_elt= selected_list_element(s);
  int hack_number= (list_elt >= 0 ? s->list_elt_to_hack_number [ list_elt ] : -1);
  Bool available_p= (hack_number >= 0 ? s->hacks_available_p [ hack_number ] : TRUE);
  ...
  gtk_notebook_set_current_page(GTK_NOTEBOOK(notebook),
    (! s->running_preview_error_p ? 0 : /* ok */
        ...
        ! available_p ? 2 :
                        /* hack not installed */
        s->backend == WAYLAND_BACKEND ? 4 :
                                        /* no previews without X11 */
                                        1)); /* preview failed */
}
    ...
    state *s)
{
  ...
  if (s->dpy &&
    ...
    gtk_widget_get_realized(pr))
    {
      ...
      if (s->debug_p && oid != id)
        fprintf(stderr, "%s: window id 0x%X -> 0x%X\n", blurb(), (unsigned int) oid, (unsigned int) id);
    }
}
  ...
  if (! gvisual2)
    {
      ...
      if (s->debug_p)
        fprintf(stderr,
          "%s: couldn't convert X Visual 0x%lx to a GdkVisual;" " winging it.\n",
          blurb(),
          (xvisual ? (unsigned long) xvisual->visualid : 0L));
    }

  if (s->debug_p)
    fprintf(stderr, "%s: using %s visual 0x%lx\n", blurb(), (gvisual1 == gvisual2 ? "default" : "non-default"), (xvisual ? (unsigned long) xvisual->visualid : 0L));
      ...
      if (status < 0)
        {
          if (errno == ESRCH)
            ...
            {
              char buf [ 1024 ];
              sprintf(buf, "%s: couldn't kill pid %lu (%s)", blurb(), (unsigned long) s->running_preview_pid, ss);
              perror(buf);
            }
        }
      else
    ...
    state *s)
{
  ...
  id= (window && s->backend != WAYLAND_BACKEND ? gdk_x11_window_get_xid(window) : 0);
  if (id == 0)
    {
  ...
  else
    {
       ...
       */
      strcpy(new_cmd, cmd);
      sprintf(new_cmd + strlen(new_cmd), " --window-id 0x%X", (unsigned int) id);
    }
  ...
  XChangeProperty(s->dpy, id, XA_WM_COMMAND, XA_STRING, 8, PropModeReplace, (unsigned char *) new_cmd, strlen(new_cmd));
  XChangeProperty(s->dpy, id, XA_NET_WM_PID, XA_CARDINAL, 32, PropModeReplace, (unsigned char *) &forked, 1);
  ...
  if (s->running_preview_error_p || /* already dead */
  ...
  else
    {
      ...
      if (s->debug_p && s->running_preview_error_p)
        {
          char *ss= subproc_pretty_name(s);
          fprintf(stderr, "%s: timer: pid %lu (%s) is %s\n", blurb(), (unsigned long) s->running_preview_pid, ss, (s->running_preview_error_p ? "dead" : "alive"));
          free(ss);
        }
      ...
      for (i= 0; i < nkids; i++)
        {
          ...
          unsigned long nitems, bytesafter;
          unsigned char *name;
          if (XGetWindowProperty(dpy, kids [ i ], XA_WM_COMMAND, 0, 128, FALSE, XA_STRING, &type, &format, &nitems, &bytesafter, &name) == Success && type != None && (! strcmp((char *) name, "gnome-screensaver") || ! strcmp((char *) name, "mate-screensaver") || ! strcmp((char *) name, "cinnamon-screensaver") || ! strcmp((char *) name, "xfce4-screensaver") || ! strcmp((char *) name, "light-locker")))
            {
              gnome_window= kids [ i ];
              if (name_ret)
    ...
    gpointer data)
{
  ...
  *msg= 0;
  ...
  if (kde_screensaver_active_p())
    warning_dialog_1(s->window, _("Warning"), _("The KDE screen saver daemon appears to be running.\n" "It must be stopped for XScreenSaver to work properly.\n" "\n" "Stop the KDE screen saver daemon now?\n"), D_KDE);

  if (! s->gl_visual)
    warning_dialog(s->window, _("Error"), _("No GL visuals: the xscreensaver-gl* packages are required."));
  ...
  if (s->backend != X11_BACKEND)
    {
      ...
      if (s->have_wayland_idle_p)
      ...
      else if (s->have_wayland_p)
        {
          ...
          warning_dialog(s->window, _("Warning"), _("Wayland error: idle detection is impossible. " "The XScreenSaver daemon will not work.\n"));
        }
      else if (getenv("WAYLAND_DISPLAY") || getenv("WAYLAND_SOCKET"))
        {
          /* Running under Wayland, but unable to connect. */
          warning_dialog(s->window, _("Warning"), _("Unable to connect to the Wayland server. " "The XScreenSaver daemon will not work.\n"));
        }
      else
        {
          if (verbose_p)
      ...
      warning_dialog(s->window, _("Warning"), _("Not compiled with support for Wayland. " "The XScreenSaver daemon will not work.\n"));
#endif /* !HAVE_WAYLAND */
    }
    ...
    gpointer user_data)
{
  ...
  *str= 0;
  ...
  if (cmd)
    ...
    {
      warning_dialog(s->window, _("Error"), _("no `manualCommand' resource set."));
    }
    ...
    state *s)
{
    ...
    if (hack && dialog)
      {
        ...
        if (! cmd_line) abort();
        s->cdata= load_configurator(cmd_line, dialog_change_cb, s, s->debug_p);
        ...
        if (s->cdata && s->cdata->year)
          {
            ...
            char *pretty_name= (hack->name ? strdup(hack->name) : make_hack_name(s->dpy, hack->command));
            ...
            gtk_label_set_use_markup(/* Must be after set_label */
              GTK_LABEL(gtk_frame_get_label_widget(frame1)),
              TRUE);
            gtk_label_set_use_markup(
              GTK_LABEL(gtk_frame_get_label_widget(frame2)),
              TRUE);
            free(pretty_name);
          }
      }
  ...
  doc_string= (s->cdata && s->cdata->description && *s->cdata->description ? _(s->cdata->description) : 0);
  doc_string= hreffify(doc_string);
  if (doc)
    {
      GtkWidget *w= dialog->dialog_vbox;
      gtk_label_set_text(doc, (doc_string ? doc_string : _("No description available.")));
  ...
  /* Also set the documentation on the main window, below the preview. */
  {
    ...
    gtk_label_set_text(doc2,
      (s2 ? _(s2) : (p->mode == BLANK_ONLY || p->mode == DONT_BLANK) ? "" :
    ...
    int *y)
{
  ...
  if (XGetWindowProperty(dpy, parent, swm_vroot, 0, 0, FALSE, AnyPropertyType, &type, &format, &nitems, &bytesafter, (unsigned char **) &data) == Success && type != None)
    return;
    ...
    gpointer data)
{
  ...
  state *s= &win->state;
  save_window_position(s, GTK_WINDOW(win), event->configure.x, event->configure.y, FALSE);
  return FALSE;
}


static int
  delayed_scroll_kludge(
    gpointer data)
{
  state *s= (state *) data;
  XScreenSaverWindow *win= XSCREENSAVER_WINDOW(s->window);
    ...
    gpointer user_data)
{
  ...
  if (s->debug_p)
    fprintf(stderr, "%s: GDK backend is %s\n", blurb(), (s->backend == X11_BACKEND ? "X11" : s->backend == XWAYLAND_BACKEND ? "XWayland" :
  ...
  if (s->dpy)
    {
      ...
      XtAppCreateShell(progname, progclass, applicationShellWidgetClass, s->dpy, 0, 0);
  ...
  if (getenv("WAYLAND_DISPLAY") || getenv("WAYLAND_SOCKET"))
    {
      ...
      else if (desk && (strcasestr(desk, "KDE") || strcasestr(desk, "plasma")))
        {
          s->grabbing_supported_p= False;
          s->grab_why=
...
static void
  xscreensaver_window_init(
    XScreenSaverWindow *win)
{
  gtk_widget_init_template(GTK_WIDGET(win));
  g_signal_connect(win, "destroy", G_CALLBACK(xscreensaver_window_destroy), win);
  g_signal_connect(win, "realize", G_CALLBACK(xscreensaver_window_realize), win);
  g_signal_connect(win, "configure-event", G_CALLBACK(xscreensaver_window_resize_cb), win);
  g_signal_connect(win->preview, "configure-event", G_CALLBACK(preview_resize_cb), win);
}


static void
  xscreensaver_window_class_init(
    XScreenSaverWindowClass *class)
{
    ...
    gpointer data)
{
  ...
  XScreenSaverWindow *win= dialog->main;
  state *s= &win->state;
  save_window_position(s, GTK_WINDOW(dialog), event->configure.x, event->configure.y, TRUE);
  return FALSE;
}
...
static void
  xscreensaver_dialog_init(
    XScreenSaverDialog *win)
{
  gtk_widget_init_template(GTK_WIDGET(win));
  g_signal_connect(win, "destroy", G_CALLBACK(xscreensaver_dialog_destroy), win);
  g_signal_connect(win, "realize", G_CALLBACK(xscreensaver_dialog_realize), win);
  g_signal_connect(win, "configure-event", G_CALLBACK(xscreensaver_dialog_resize_cb), win);
  g_signal_connect(win, "delete-event", G_CALLBACK(xscreensaver_dialog_delete_cb), win);
}


static void
  xscreensaver_dialog_class_init(
    XScreenSaverDialogClass *class)
{
  gtk_widget_class_set_template_from_resource(GTK_WIDGET_CLASS(class),
...
static void
  xscreensaver_app_class_init(
    XScreenSaverAppClass *class)
{
    ...
    void)
{
  ...
  g_application_add_main_option(G_APPLICATION(app), "version", 'v', G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE, "Print the version number", NULL);
  g_application_add_main_option(G_APPLICATION(app), "debug", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE, "Print diagnostics to stderr", NULL);
