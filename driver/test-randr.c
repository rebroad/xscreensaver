      ...
      for (j= 0; j < res->noutput; j++)
        {
          ...
          fprintf(stderr, "%s:   Output %d: %s: %s (%d)\n", blurb(), j, rroi->name, (rroi->connection == RR_Connected ? "connected" : rroi->connection == RR_Disconnected ? "disconnected" :
          ...
          for (k= 0; k < rroi->ncrtc; k++)
            {
              ...
              fprintf(stderr, "%s:   %c CRTC %d (%d): %dx%d+%d+%d\n", blurb(), (rroi->crtc == rroi->crtcs [ k ] ? '+' : ' '), k, (int) rroi->crtcs [ k ], crtci->width, crtci->height, crtci->x, crtci->y);
      ...
      if (error_handler_hit_p)
      ...
      else if (rrc)
        {
          ...
          /* Rotations */
          {
            ...
            if (! (available & (RR_Rotate_0 | RR_Rotate_90 | RR_Rotate_180 | RR_Rotate_270)))
              fprintf(stderr, " none");
            fprintf(stderr, "\n");
            ...
            if (! (current & (RR_Rotate_0 | RR_Rotate_90 | RR_Rotate_180 | RR_Rotate_270)))
              fprintf(stderr, " none");
            fprintf(stderr, "\n");
          ...
          if (event.type == event_number + RRScreenChangeNotify)
            {
              ...
              fprintf(stderr, "%s: screen %d: old size: \t%d x %d\n", blurb(), screen, DisplayWidth(dpy, screen), DisplayHeight(dpy, screen));
              fprintf(stderr, "%s: screen %d: old root: \t%d x %d\t0x%lx\n", blurb(), screen, xgwa [ screen ].width, xgwa [ screen ].height, (unsigned long) w [ screen ]);
