    ...
    char **error_ret)
{
  ...
  if (! window)
    {
      sprintf(err, "no screensaver is running on display %s", DisplayString(dpy));
  ...
  if (command == XA_SCREENSAVER_STATUS ||
    command == XA_SCREENSAVER_VERSION)
    {
      ...
      if (! v || ! *v)
        {
          sprintf(err, "version property not set on window 0x%x?", (unsigned int) window);
          if (error_ret)
            *error_ret= strdup(err);
          else
            fprintf(stderr, "%s: %s\n", progname, err);
      ...
      if (! hint.res_class)
        {
          sprintf(err, "class hints not set on window 0x%x?", (unsigned int) window);
          if (error_ret)
            *error_ret= strdup(err);
          else
            fprintf(stderr, "%s: %s\n", progname, err);
    ...
    char **error_ret)
{
  ...
  while (! (got_event= XCheckIfEvent(dpy, &event, &xscreensaver_command_event_p, 0)) &&
    sleep_count++ < 10)
    {
  ...
  if (! got_event)
  ...
  else
    {
      ...
      error_handler_hit_p= False;
      old_handler= XSetErrorHandler(ignore_all_errors_ehandler);
      st2= XGetWindowProperty(dpy, window, XA_SCREENSAVER_RESPONSE, 0, 1024, True, AnyPropertyType, &type, &format, &nitems, &bytesafter, &msg);
      XSync(dpy, False);
      XSetErrorHandler(old_handler);
 ...
 */
static int
  xscreensaver_command_wait_for_blank(
    Display *dpy,
    Bool verbose_p,
    char **error_ret)
{
          ...
          if (verbose_p > 1)
            {
              ...
              int i;
              fprintf(stderr, "%s: read status property: 0x%lx: %s", progname, (unsigned long) w, (status [ 0 ] == XA_BLANK ? "BLANK" : status [ 0 ] == XA_LOCK ? "LOCK" :
  ...
  char **error_ret)
{
  ...
  if (status == 0)
    status= xscreensaver_command_response(dpy, w, verbose_p, (command == XA_EXIT), error_ret);
...
void server_xscreensaver_version(
  Display *dpy,
  char **version_ret,
  char **user_ret,
  char **host_ret)
{
  ...
  if (version_ret)
    {
      unsigned char *v= 0;
      XGetWindowProperty(dpy, window, XA_SCREENSAVER_VERSION, 0, 100, False, XA_STRING, &type, &format, &nitems, &bytesafter, &v);
      if (v)
        {
  ...
  if (user_ret || host_ret)
    {
      ...
      XGetWindowProperty(dpy, window, XA_SCREENSAVER_ID, 0, 512, False, XA_STRING, &type, &format, &nitems, &bytesafter, &id);
      if (id && *id)
        {
