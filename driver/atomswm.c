 ...
 */
void xscreensaver_set_wm_atoms(
  Display *dpy,
  Window window,
  int width,
  int height,
  Window for_window)
{
  ...
  XChangeProperty(dpy, window, XA_WM_COMMAND, XA_STRING, 8, PropModeReplace, (unsigned char *) class_hints.res_name, strlen(class_hints.res_name));

#ifdef HAVE_UNAME
  if (! uname(&uts))
    XChangeProperty(dpy, window, XA_WM_CLIENT_MACHINE, XA_STRING, 8, PropModeReplace, (unsigned char *) uts.nodename, strlen(uts.nodename));
#endif
  ...
  XChangeProperty(dpy, window, XA_NET_WM_BYPASS_COMPOSITOR, XA_CARDINAL, 32, PropModeReplace, (unsigned char *) vl, i);
  ...
  XChangeProperty(dpy, window, XA_NET_WM_STATE, XA_ATOM, 32, PropModeReplace, (unsigned char *) va, i);
  ...
  XChangeProperty(dpy, window, XA_NET_WM_WINDOW_TYPE, XA_ATOM, 32, PropModeReplace, (unsigned char *) va, i);

  if (for_window) /* This is the error dialog for a saver window */
    {
      va [ 0 ]= for_window;
      /* _WM_TRANSIENT_FOR = screensaver_window */
      XChangeProperty(dpy, window, XA_WM_TRANSIENT_FOR, XA_WINDOW, 32, PropModeReplace, (unsigned char *) va, 1);
    }
}
