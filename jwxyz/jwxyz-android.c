      ...
      if (rh->gl_fbo_p)
        {
          ...
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, to_pow2(p->frame.width), to_pow2(p->frame.height), 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
      ...
      else
        {
          ...
          p->egl_surface= eglCreatePbufferSurface(rh->egl_display, rh->egl_config, attribs);
  ...
  if (rh->egl_p)
    {
      ...
      Assert(eglMakeCurrent(rh->egl_display, rh->egl_surface, rh->egl_surface, rh->egl_ctx),
...
#define ST EGL_STENCIL_SIZE
  ...
  *egl_config= 0;
      ...
      for (i= 0; i < countof(templates); i++)
        {
          ...
          nconfig= -1;
          if (eglChooseConfig(egl_display, attrs, egl_config, 1, &nconfig) && nconfig == 1)
            break;
        }
...
#if 1
  {
    ...
    eglGetConfigAttrib(egl_display, *egl_config, EGL_TRANSPARENT_RED_VALUE, &tr);
    eglGetConfigAttrib(egl_display, *egl_config, EGL_TRANSPARENT_GREEN_VALUE, &tg);
    eglGetConfigAttrib(egl_display, *egl_config, EGL_TRANSPARENT_BLUE_VALUE, &tb);
    for (i= 0; i < countof(fields); i++)
      {
        ...
        if (fields [ i ].i == EGL_RED_SIZE)
          ...
          {
            strcpy(s, (v == EGL_NONE ? "none" : v == EGL_SLOW_CONFIG ? "slow" :
          ...
          {
            strcpy(s, (v == EGL_RGB_BUFFER ? "RGB" : v == EGL_LUMINANCE_BUFFER ? "luminance" :
                                                                                 "???"));
          }
  ...
  for (pass= 0; pass < iter; pass++)
    {
      ...
      *egl_context= eglCreateContext(egl_display, egl_config, EGL_NO_CONTEXT, attrs);
...
#undef ya_rand_init // This is the one and only place it is allowed
  ...
  jmethodID m= (*env)->GetMethodID(env, c, "put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;");
  if (! m) abort();
  if ((*env)->ExceptionOccurred(env)) abort();
  ...
  Log("init: %s @ %dx%d: using JWXYZ_%s", progname, w, h, rh->jwxyz_gl_p ? "GL" : "IMAGE");
  ...
  if (rh->egl_p)
    {
      ...
      get_egl_context_android(rh->window, rh->egl_display, rh->egl_config, &rh->egl_ctx);

      ANativeWindow *native_window=
        ANativeWindow_fromSurface(env, jni_surface);

      rh->egl_surface= eglCreateWindowSurface(rh->egl_display, rh->egl_config, native_window, NULL);
  ...
  else
    {
      ...
      int result= ANativeWindow_setBuffersGeometry(rh->native_window, w, h, WINDOW_FORMAT_RGBX_8888);
      if (result < 0)
        {
  ...
  if (rh->egl_p)
    {
      if (rh->jwxyz_gl_p && rh->frontbuffer_p)
        jwxyz_gl_copy_area(rh->dpy, wnd, &rh->frontbuffer, rh->copy_gc, 0, 0, wnd->frame.width, wnd->frame.height, 0, 0);
  ...
  else
    {
      ...
      if (result)
      ...
      else
        {
          ...
          jwxyz_blit(wnd->image_data, jwxyz_image_pitch(wnd), 0, 0, buffer.bits, buffer.stride * 4, 0, 0, MIN(wnd->frame.width, buffer.width), MIN(wnd->frame.height, buffer.height));
...
JNIEXPORT void JNICALL
  Java_org_jwz_xscreensaver_jwxyz_nativeInit(
    JNIEnv *env,
    jobject thiz,
    jstring jhack,
    jobject defaults,
    ...
    jobject jni_surface)
{
  ...
  if (! classRefCount)
    {
      ...
      runningHackField= (*env)->GetFieldID(env, classjwxyz, "nativeRunningHackPtr", "J");
      if ((*env)->ExceptionOccurred(env)) abort();

      jclass classIterable=
        acquireClass(env, "java/lang/Iterable", &globalRefIterable);
      iterableIterator= (*env)->GetMethodID(env, classIterable, "iterator", "()Ljava/util/Iterator;");
      if ((*env)->ExceptionOccurred(env)) abort();

      jclass classIterator=
        acquireClass(env, "java/util/Iterator", &globalRefIterator);
      iteratorHasNext= (*env)->GetMethodID(env, classIterator, "hasNext", "()Z");
      iteratorNext= (*env)->GetMethodID(env, classIterator, "next", "()Ljava/lang/Object;");
      if ((*env)->ExceptionOccurred(env)) abort();

      jclass classMapEntry=
        acquireClass(env, "java/util/Map$Entry", &globalRefMapEntry);
      entryGetKey= (*env)->GetMethodID(env, classMapEntry, "getKey", "()Ljava/lang/Object;");
      entryGetValue= (*env)->GetMethodID(env, classMapEntry, "getValue", "()Ljava/lang/Object;");
      if ((*env)->ExceptionOccurred(env)) abort();
    }
  ...
  doinit(thiz, rh, env, &function_table [ chosen ], defaults, w, h, jni_surface);
...
JNIEXPORT void JNICALL
  Java_org_jwz_xscreensaver_jwxyz_nativeResize(
    JNIEnv *env,
    jobject thiz,
    jint w,
    jint h,
    jdouble rot)
{
  ...
  if (rh->initted_p)
    rh->xsft->reshape_cb(rh->dpy, rh->window, rh->closure, wnd->frame.width, wnd->frame.height);
...
JNIEXPORT jlong JNICALL
  Java_org_jwz_xscreensaver_jwxyz_nativeRender(
    JNIEnv *env,
    jobject thiz)
{
  pthread_mutex_lock(&mutg);
...
JNIEXPORT void JNICALL
  Java_org_jwz_xscreensaver_jwxyz_nativeDone(
    JNIEnv *env,
    jobject thiz)
{
  pthread_mutex_lock(&mutg);
  ...
  if (rh->egl_p)
    {
      ...
      eglMakeCurrent(rh->egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
    ...
    XEvent *e)
{
  ...
  return (rh->xsft->event_cb ? rh->xsft->event_cb(rh->dpy, rh->window, rh->closure, e) : 0);
}
...
JNIEXPORT void JNICALL
  Java_org_jwz_xscreensaver_jwxyz_sendButtonEvent(
    JNIEnv *env,
    jobject thiz,
    int x,
    int y,
...
JNIEXPORT void JNICALL
  Java_org_jwz_xscreensaver_jwxyz_sendMotionEvent(
    JNIEnv *env,
    jobject thiz,
    int x,
    int y)
...
JNIEXPORT void JNICALL
  Java_org_jwz_xscreensaver_jwxyz_sendKeyEvent(
    JNIEnv *env,
    jobject thiz,
    jboolean down_p,
    int code,
    ...
    Drawable d)
{
  ...
  if (d->texture)
    glFramebufferTexture2DOES(GL_FRAMEBUFFER_OES, GL_COLOR_ATTACHMENT0_OES, GL_TEXTURE_2D, d->texture, 0);
}


void jwxyz_bind_drawable(
  Display *dpy,
  Window w,
  Drawable d)
{
  ...
  int dst_y)
{
  ...
  if (rh->gl_fbo_p && src->texture && src != dst)
    {
      ...
      jwxyz_gl_draw_image(dpy, gc, GL_TEXTURE_2D, to_pow2(src->frame.width), to_pow2(src->frame.height), src_x, src->frame.height - src_y - height, jwxyz_drawable_depth(src), width, height, dst_x, dst_y, False);
      return;
    }
  ...
  jwxyz_gl_copy_area_read_tex_image(dpy, src->frame.height, src_x, src_y, width, height, dst_x, dst_y);

  if (rh->gl_fbo_p)
    bind_drawable_fbo(rh, dst);
  finish_bind_drawable(dpy, dst);

  jwxyz_gl_copy_area_write_tex_image(dpy, gc, src_x, src_y, jwxyz_drawable_depth(src), width, height, dst_x, dst_y);

#else
  // Kumppa: 0.17 FPS
  jwxyz_gl_copy_area_read_pixels(dpy, src, dst, gc, src_x, src_y, width, height, dst_x, dst_y);
#endif
  jwxyz_assert_gl();
}


void jwxyz_assert_drawable(
  Window main_window,
  Drawable d)
{
    ...
    char *name)
{
  ...
  jmethodID m= (*env)->GetMethodID(env, c, "getStringResource", "(Ljava/lang/String;)Ljava/lang/String;");
  if ((*env)->ExceptionOccurred(env)) abort();

  jstring jvalue= (m ? (*env)->CallObjectMethod(env, obj, m, jstr) : NULL);
  (*env)->DeleteLocalRef(env, c);
    ...
    const char *url)
{
  ...
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, "loadURL", "(Ljava/lang/String;)Ljava/nio/ByteBuffer;");
  if ((*env)->ExceptionOccurred(env)) abort();
  jobject buf= (m ? (*env)->CallObjectMethod(env, obj, m, jstr) : NULL);
  (*env)->DeleteLocalRef(env, c);
  (*env)->DeleteLocalRef(env, jstr);
    ...
    int *descent_ret)
{
  ...
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, "loadFont", "(IILjava/lang/String;IF)[Ljava/lang/Object;");
  if ((*env)->ExceptionOccurred(env)) abort();

  jobjectArray array= (m ? (*env)->CallObjectMethod(env, obj, m, (jint) mask_jwxyz, (jint) traits_jwxyz, jname, (jint) font_name_type, (jfloat) size) : NULL);
  ...
  char **pixmap_ret)
{
  ...
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, "renderText", "(Landroid/graphics/Paint;Ljava/lang/String;ZZ)Ljava/nio/ByteBuffer;");
  if ((*env)->ExceptionOccurred(env)) abort();
  jobject buf=
    (m ? (*env)->CallObjectMethod(env, obj, m, (jobject) native_font, jstr, (pixmap_ret ? JNI_TRUE : JNI_FALSE), antialias_p) : NULL);
  (*env)->DeleteLocalRef(env, c);
  (*env)->DeleteLocalRef(env, jstr);
  free(s2);

  if ((*env)->ExceptionOccurred(env)) abort();
  unsigned char *bits= (unsigned char *) (buf ? (*env)->GetDirectBufferAddress(env, buf) : 0);
  if (bits)
    {
      int i= 0;
      int L= (*env)->GetDirectBufferCapacity(env, buf);
      if (L < 10) abort();
      cs->lbearing= (bits [ i ] << 8) | (bits [ i + 1 ] & 0xFF);
      i+= 2;
      cs->rbearing= (bits [ i ] << 8) | (bits [ i + 1 ] & 0xFF);
      i+= 2;
      cs->width= (bits [ i ] << 8) | (bits [ i + 1 ] & 0xFF);
      i+= 2;
      cs->ascent= (bits [ i ] << 8) | (bits [ i + 1 ] & 0xFF);
      i+= 2;
      cs->descent= (bits [ i ] << 8) | (bits [ i + 1 ] & 0xFF);
      i+= 2;
    ...
    GC gc)
{
  ...
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, (grab_p ? "getScreenshot" : "checkThenLoadRandomImage"), "(IIZ)[Ljava/lang/Object;");
  if ((*env)->ExceptionOccurred(env)) abort();
  jobjectArray img_name= (m ? (*env)->CallObjectMethod(env, obj, m, drawable->frame.width, drawable->frame.height, (rotate_p ? JNI_TRUE : JNI_FALSE)) : NULL);
  if ((*env)->ExceptionOccurred(env)) abort();
  (*env)->DeleteLocalRef(env, c);
  ...
  XImage *img= XCreateImage(dpy, NULL, visual_depth(NULL, NULL), ZPixmap, 0, NULL, bmp_info.width, bmp_info.height, 0, bmp_info.stride);

  AndroidBitmap_lockPixels(env, jbitmap, (void **) &img->data);

  XPutImage(dpy, drawable, gc, img, 0, 0, (drawable->frame.width - bmp_info.width) / 2, (drawable->frame.height - bmp_info.height) / 2, bmp_info.width, bmp_info.height);
    ...
    unsigned long data_size)
{
  ...
  jobject obj= rh->jobject;
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, "decodePNG", "([B)Landroid/graphics/Bitmap;");
  if ((*env)->ExceptionOccurred(env)) abort();
  jbyteArray jdata= (*env)->NewByteArray(env, data_size);
  (*env)->SetByteArrayRegion(env, jdata, 0, data_size, (const jbyte *) png_data);
  jobject jbitmap= (m ? (*env)->CallObjectMethod(env, obj, m, jdata) : NULL);
  if ((*env)->ExceptionOccurred(env)) abort();
  (*env)->DeleteLocalRef(env, c);
  ...
  XImage *img= XCreateImage(dpy, NULL, 32, ZPixmap, 0, NULL, bmp_info.width, bmp_info.height, 8, bmp_info.stride);
  char *bits= 0;
    ...
    const char *dir)
{
  ...
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, "listAssetFiles", "(Ljava/lang/String;)[Ljava/lang/String;");
  if ((*env)->ExceptionOccurred(env)) abort();

  jobjectArray array= (m ? (*env)->CallObjectMethod(env, obj, m, jstr) : NULL);
  (*env)->DeleteLocalRef(env, c);
  (*env)->DeleteLocalRef(env, jstr);
    ...
    const char *file)
{
  ...
  jclass c= (*env)->GetObjectClass(env, obj);
  jmethodID m= (*env)->GetMethodID(env, c, "readAssetFile", "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");
  if ((*env)->ExceptionOccurred(env)) abort();

  jobject body= (m ? (*env)->CallObjectMethod(env, obj, m, jstr1, jstr2) : NULL);
  (*env)->DeleteLocalRef(env, c);
