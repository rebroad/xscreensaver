...
extern void jwxyz_render_text(Display *, void *native_font, const char *str, size_t len, Bool utf8_p, Bool antialias_p, XCharStruct *cs_ret, char **pixmap_ret);
...
/* jwxyz-common.c */
extern void jwxyz_validate_pixel(Display *dpy, unsigned long pixel, unsigned int depth, Bool alpha_allowed_p);
extern Bool jwxyz_dumb_drawing_mode(Display *dpy, Drawable d, GC gc, int x, int y, unsigned width, unsigned height);
extern void jwxyz_blit(const void *src_data, ptrdiff_t src_pitch, unsigned src_x, unsigned src_y, void *dst_data, ptrdiff_t dst_pitch, unsigned dst_x, unsigned dst_y, unsigned width, unsigned height);
extern void jwxyz_fill_rect(Display *, Drawable, GC, int x, int y, unsigned int width, unsigned int height, unsigned long pixel);
extern void jwxyz_gcv_defaults(Display *dpy, XGCValues *gcv, int depth);
extern int jwxyz_draw_string(Display *dpy, Drawable d, GC gc, int x, int y, const char *str, size_t len, int utf8_p);
...
extern void set_color(Display *dpy, CGContextRef cgc, unsigned long argb, unsigned int depth, Bool alpha_allowed_p, Bool fill_p);
extern void push_gc(Drawable d, GC gc);
extern void push_color_gc(Display *dpy, Drawable d, GC gc, unsigned long color, Bool antialias_p, Bool fill_p);
extern CGPoint map_point(Drawable d, int x, int y);
extern void jwxyz_quartz_copy_area(Display *dpy, Drawable src, Drawable dst, GC gc, int src_x, int src_y, unsigned int width, unsigned int height, int dst_x, int dst_y);
...
extern void jwxyz_set_matrices(Display *dpy, unsigned width, unsigned height, Bool screen_p);
extern void jwxyz_gl_flush(Display *dpy);
extern void jwxyz_gl_set_gc(Display *dpy, GC gc);
extern void jwxyz_gl_copy_area(Display *dpy, Drawable src, Drawable dst, GC gc, int src_x, int src_y, unsigned int width, unsigned int height, int dst_x, int dst_y);
...
extern void jwxyz_gl_copy_area_read_tex_image(Display *dpy,
  ...
  int dst_x,
  int dst_y);
extern void jwxyz_gl_copy_area_write_tex_image(Display *dpy, GC gc, int src_x, int src_y, int src_depth, unsigned int width, unsigned int height, int dst_x, int dst_y);

extern void jwxyz_gl_draw_image(Display *dpy, GC gc, GLenum gl_texture_target, unsigned int tex_w, unsigned int tex_h, int src_x, int src_y, int src_depth, unsigned int width, unsigned int height, int dst_x, int dst_y, Bool flip_y);

/* glReadPixels followed by glTexImage2D. This is terrible, so only use this
   if nothing else works.
 */
extern void jwxyz_gl_copy_area_read_pixels(Display *dpy, Drawable src, Drawable dst, GC gc, int src_x, int src_y, unsigned int width, unsigned int height, int dst_x, int dst_y);
