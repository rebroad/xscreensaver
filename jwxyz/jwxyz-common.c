  ...
  if (gcv->function == GXset || gcv->function == GXclear)
    {
      ...
      jwxyz_fill_rect(dpy, d, 0, x, y, width, height, (gcv->function == GXset ? (depth == 1 ? 1 : WhitePixel(dpy, 0)) : (depth == 1 ? 0 : BlackPixel(dpy, 0))));
      return True;
    }
...
#undef CLIP
  ...
  if (width0 <= 0 || height0 <= 0)
    ...
    {
      VTBL->copy_area(dpy, src, dst, gc, src_x, src_y, width0, height0, dst_x, dst_y);
    }
  ...
  if (clipped && dst == XRootWindow(dpy, 0))
    {
      ...
      gcv->function= GXcopy;
      VTBL->fill_rects(dpy, dst, gc, rects, rects_end - rects, *VTBL->window_background(dpy));
  ...
  unsigned long plane)
{
  ...
  // not to white/black.
  return XCopyArea(dpy, src, dest, gc, src_x, src_y, width, height, dest_x, dest_y);
}
...
int XFillRectangle(
  ...
  unsigned int height)
{
  jwxyz_fill_rect(dpy, d, gc, x, y, width, height, VTBL->gc_gcv(gc)->foreground);
  return 0;
}
...
int XDrawArc(
  ...
  int angle2)
{
  return VTBL->draw_arc(dpy, d, gc, x, y, width, height, angle1, angle2, False);
}
...
int XFillArc(
  ...
  int angle2)
{
  return VTBL->draw_arc(dpy, d, gc, x, y, width, height, angle1, angle2, True);
}
  ...
  int narcs)
{
  int i;
  for (i= 0; i < narcs; i++)
    VTBL->draw_arc(dpy, d, gc, arcs [ i ].x, arcs [ i ].y, arcs [ i ].width, arcs [ i ].height, arcs [ i ].angle1, arcs [ i ].angle2, False);
  return 0;
}
  ...
  int narcs)
{
  int i;
  for (i= 0; i < narcs; i++)
    VTBL->draw_arc(dpy, d, gc, arcs [ i ].x, arcs [ i ].y, arcs [ i ].width, arcs [ i ].height, arcs [ i ].angle1, arcs [ i ].angle2, True);
  return 0;
}

void jwxyz_gcv_defaults(
  Display *dpy,
  XGCValues *gcv,
  int depth)
{
  ...
  XGCValues *from)
{
  ...
  Assert((! (mask & (GCLineStyle | GCPlaneMask | GCFillStyle | GCTile | GCStipple | GCTileStipXOrigin | GCTileStipYOrigin | GCGraphicsExposures | GCDashOffset | GCDashList | GCArcMode))),
...
static unsigned long
  ximage_getpixel_8(
    ...
    int y)
{
  return ((unsigned long) *((uint8_t *) ximage->data +
...
static unsigned long
  ximage_getpixel_32(
    ...
    int y)
{
  return ((unsigned long) *((uint32_t *) ximage->data +
    ...
    int format)
{
  ...
  XImage *image= XCreateImage(dpy, 0, depth, format, 0, 0, width, height, 0, 0);
  image->data= (char *) malloc(height * image->bytes_per_line);

  return XGetSubImage(dpy, d, x, y, width, height, plane_mask, format, image, 0, 0);
}
    ...
    unsigned int depth)
{
  ...
  XImage *image= XCreateImage(dpy, 0, 1, XYPixmap, 0, (char *) data, w, h, 0, 0);
  XGCValues gcv;
  gcv.foreground= fg;
  gcv.background= bg;
  ...
  while ((name2= strtok_r(token, ",", &lasts)))
    {
      ...
      fid->native_font= jwxyz_load_native_font(XRootWindow(dpy, 0), 0, 0, name2, strlen(name2) - strlen(spc), JWXYZ_FONT_FACE, size, NULL, &fid->ascent, &fid->descent);
      if (fid->native_font)
        {
...
#define UNSPEC   (L == 0 || (L == 1 && *s == '*'))
  if (CMP("6x10"))
    {
      size= 8, require|= JWXYZ_STYLE_MONOSPACE;
    }
  else if (CMP("6x10bold"))
    {
      size= 8, require|= JWXYZ_STYLE_MONOSPACE | JWXYZ_STYLE_BOLD;
    }
  else if (CMP("fixed"))
    {
      size= 12, require|= JWXYZ_STYLE_MONOSPACE;
    }
  else if (CMP("9x15"))
    {
      size= 12, require|= JWXYZ_STYLE_MONOSPACE;
    }
  else if (CMP("9x15bold"))
    {
      size= 12, require|= JWXYZ_STYLE_MONOSPACE | JWXYZ_STYLE_BOLD;
    }
  else if (CMP("vga"))
    {
      size= 12, require|= JWXYZ_STYLE_MONOSPACE;
  ...
  if (fid->native_font)
    {
      ...
      asprintf(&fid->xa_font, "-*-%s-%s-%c-*-*-%u-%u-%u-%u-%c-0-iso10646-1", family_name_ptr, (require & JWXYZ_STYLE_BOLD) ? "bold" : "medium", (require & JWXYZ_STYLE_ITALIC) ? 'o' : 'r', (unsigned) (dpi * size / 72.27 + 0.5), (unsigned) (size * 10 + 0.5), dpi, dpi, (require & JWXYZ_STYLE_MONOSPACE) ? 'm' : 'p');
    }
  ...
  XCharStruct *cs)
{
...

  {
    Font ff= f->fid;
    Display *dpy= ff->dpy;
    jwxyz_render_text(dpy, ff->native_font, utf8, strlen(utf8), True, False, cs, 0);
  }
  ...
  XRectangle *overall_logical_return)
{
  ...
  jwxyz_render_text(f->dpy, f->native_font, str, len, True, False, &cs, NULL);
  ...
  int utf8_p)
{
  ...
  char *data= 0;
  jwxyz_render_text(dpy, jwxyz_native_font(ff), str, len, utf8_p, gcv->antialias_p, &cs, &data);
  ...
  XImage *img= XCreateImage(dpy, VTBL->visual(dpy), 32, ZPixmap, 0, data, w, h, 0, 0);
...
#define ROTL(x, rot) (((x) << ((rot) & 31)) | ((x) >> (32 - ((rot) & 31))))
...

  {
    ...
    XPutImage(dpy, d, gc, img, 0, 0, x + cs.lbearing, y - cs.ascent, w, h);
  ...
  int len)
{
  int ascent, descent, dir;
  XCharStruct cs;
  XTextExtents(&VTBL->gc_gcv(gc)->font->metrics, str, len, &dir, &ascent, &descent, &cs);
  jwxyz_fill_rect(dpy, d, gc, x + MIN(0, cs.lbearing), y - MAX(0, ascent),
    ...
    MAX(MAX(0, cs.rbearing) - MIN(0, cs.lbearing), cs.width) + 1,
