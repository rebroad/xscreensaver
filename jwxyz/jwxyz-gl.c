    ...
    const void *data)
{
  ...
  if (*tex_w == w && *tex_h == h)
    {
      glTexImage2D(dpy->gl_texture_target, 0, internalformat, *tex_w, *tex_h, 0, format, type, data);
    }
  else
    {
      // TODO: Sampling the last row might be a problem if src_x != 0.
      glTexImage2D(dpy->gl_texture_target, 0, internalformat, *tex_w, *tex_h, 0, format, type, NULL);
      glTexSubImage2D(dpy->gl_texture_target, 0, 0, 0, w, h, format, type, data);
    }
}
    ...
    Window w)
{
...

  {
    ...
    if (sscanf((const char *) version_str, "%u.%u", &version.major, &version.minor) < 2)
      {
        version.major= 1;
        version.minor= 1;
      }
...
#ifndef HAVE_JWZGLES
  d->gl_texture_npot_p= gluCheckExtension((const GLubyte *) "GL_ARB_texture_rectangle",
    extensions);
  d->gl_texture_target= d->gl_texture_npot_p ?
    GL_TEXTURE_RECTANGLE_EXT :
    GL_TEXTURE_2D;
#else
  d->gl_texture_npot_p= jwzgles_gluCheckExtension((const GLubyte *) "GL_APPLE_texture_2D_limited_npot", extensions) ||
    jwzgles_gluCheckExtension((const GLubyte *) "GL_OES_texture_npot", extensions) ||
    ...
    unsigned long pixel)
{
  ...
  if (dpy->queue_size > dpy->queue_capacity)
    {
      ...
      uint32_t *new_color= realloc(
        dpy->queue_color,
        sizeof(*dpy->queue_color) * dpy->queue_capacity);
      ...
      GLshort *new_vertex= realloc(
        dpy->queue_vertex,
        sizeof(GLfloat) * 2 * dpy->queue_capacity);
  ...
  jwxyz_validate_pixel(dpy, pixel, jwxyz_drawable_depth(d), gc ? gc->gcv.alpha_allowed_p : False);
  ...
  void *result= (char *) dpy->queue_vertex + old_size * 2 * (mode == GL_TRIANGLE_STRIP ? sizeof(GLfloat) : sizeof(GLshort));
  if (prepend2)
    {
...
static void
  finish_triangle_strip(
    Display *dpy,
    GLfloat *enqueue_result)
{
  ...
  if (clip_mask)
    {
      ...
      glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, alpha_allowed_p ? GL_MODULATE : GL_REPLACE);

      glMatrixMode(GL_TEXTURE);
      glLoadIdentity();

      unsigned
        tex_w= gc->clip_mask_width + 2,
        tex_h= gc->clip_mask_height + 2;
      tex_size(dpy, &tex_w, &tex_h);
...
static void
  clear_texture(
    Display *dpy)
{
  glTexImage2D(dpy->gl_texture_target, 0, texture_internalformat(dpy), 0, 0, 0, dpy->pixel_format, gl_pixel_type(dpy), NULL);
}
  ...
  if (dpy->queue_mode == GL_LINES && dpy->queue_line_cap)
    {
      ...
      glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(GLubyte) * 8, dpy->queue_color);
      vertex_pointer(dpy, GL_SHORT, sizeof(GLshort) * 4, (GLshort *) dpy->queue_vertex + 2);
...
void jwxyz_gl_copy_area_read_tex_image(
  Display *dpy,
  unsigned src_height,
  int src_x,
  int src_y,
  ...
  int dst_y)
{
  ...
  if (tex_w == width && tex_h == height)
    {
      glCopyTexImage2D(dpy->gl_texture_target, 0, internalformat, src_x, src_height - src_y - height, width, height, 0);
    }
  else
    {
      glTexImage2D(dpy->gl_texture_target, 0, internalformat, tex_w, tex_h, 0, dpy->pixel_format, gl_pixel_type(dpy), NULL);
      glCopyTexSubImage2D(dpy->gl_texture_target, 0, 0, 0, src_x, src_height - src_y - height, width, height);
    }
}

void jwxyz_gl_copy_area_write_tex_image(
  Display *dpy,
  GC gc,
  int src_x,
  int src_y,
  ...
  int dst_y)
{
  ...
  jwxyz_gl_draw_image(dpy, gc, dpy->gl_texture_target, tex_w, tex_h, 0, 0, src_depth, width, height, dst_x, dst_y, False);

  clear_texture(dpy);
}


void jwxyz_gl_draw_image(
  Display *dpy,
  GC gc,
  GLenum gl_texture_target,
  unsigned int tex_w,
  ...
  Bool flip_y)
{
  if (! gc || src_depth == gc->depth)
  ...
  else
    {
      ...
      GLfloat rgba [ 4 ];
      query_color(dpy, gc->gcv.foreground, gc->depth, gc->gcv.alpha_allowed_p, rgba);
    ...
    XSegment *segments)
{
  ...
  GLfloat *coords= enqueue(dpy, d, gc, GL_TRIANGLE_STRIP, 4, gc->gcv.foreground);
  coords [ 0 ]= x3;
  coords [ 1 ]= y3;
  coords [ 2 ]= x4;
  coords [ 3 ]= y4;
    ...
    Bool fill_p)
{
  ...
  if (fill_p)
    {
      GLfloat *data= enqueue(dpy, d, gc, GL_TRIANGLE_STRIP, segments + 5, gc->gcv.foreground);
      ...
      while (s)
        {
          ...
          arc_xy(data_ptr + 2, cx, cy, w2, h2, (segment1 - s) * segment_angle);
          data_ptr+= 4;
        }
      ...
      for (s= 0; s != ds2; ++s)
        {
          ...
          arc_xy(data_ptr + 2, cx, cy, w2, h2, (segment2 - s - 1) * segment_angle);
          data_ptr+= 4;
        }
  ...
  else
    {
      ...
      GLfloat *data= enqueue(dpy, d, gc, GL_TRIANGLE_STRIP, 2 * segments + 4, gc->gcv.foreground);
      GLfloat *data_ptr= data;
      ...
      for (unsigned s= 0; s != segments; ++s)
        {
          arc_xy2(data_ptr, cx, cy, w2, h2, (segment0 + s) * segment_angle, gglw);
          data_ptr+= 4;
        }
    ...
    unsigned int h)
{
  ...
  tex_image(dpy, tex_internalformat, &tex_w, &tex_h, tex_format, tex_type, tex_data);

  if (bpp == 1)
    free(tex_data);

  jwxyz_gl_draw_image(dpy, gc, dpy->gl_texture_target, tex_w, tex_h, 0, 0, bpp, w, h, dest_x, dest_y, True);
#else
  ...
  if (dest_image->depth == visual_depth(NULL, NULL))
    {
      ...
      glReadPixels(x, jwxyz_frame(d)->height - (y + height), width, height, dpy->pixel_format, gl_pixel_type(dpy), dest_data);
  ...
  else
    {
      ...
      glReadPixels(x, jwxyz_frame(d)->height - (y + height), width, height, GL_RGBA, GL_UNSIGNED_BYTE, rgba_image);

      Assert(! (dest_x % 8), "XGetSubImage: dest_x not byte-aligned");
      uint8_t *top=
        (uint8_t *) dest_image->data + dest_image->bytes_per_line * dest_y + dest_x / 8;
#if 0
    {
      char s[10240];
    ...
    Pixmap m)
{
  ...
  glReadPixels(0, 0, frame->width, frame->height, GL_RGBA, GL_UNSIGNED_BYTE, rgba);
  ...
  tex_image(dpy, GL_ALPHA, &tex_w, &tex_h, GL_ALPHA, GL_UNSIGNED_BYTE, alpha0);
...
Bool is_three_point_loop(
  linked_point *head)
{
  return head->x == head->next->next->next->x && head->y == head->next->next->next->y;
}


void traverse_points_list(
  Display *dpy,
  linked_point *root)
{
  linked_point *head;
